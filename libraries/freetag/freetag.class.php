...
class freetag {
	...
	/**
	 * @access private
	 * @param string The prefix of freetag database ottocrat_tables.
	 */
	var $_table_prefix = 'ottocrat_';
	/**
	 * @access private
	...
	/**
	 * @access private
	 * @param string Whether to prevent multiple ottocrat_users from tagging the same object. By default, set to block (ala Upcoming.org)
	 */
	 ...
	 * - ottocrat_table_prefix: If you wish to create multiple Freetag databases on the same database, you can put a prefix in front of the ottocrat_table names and pass separate prefixes to the constructor. [default: '']
	 ...
	 * - block_multiuser_tag_on_object: Set to 0 in order to allow individual ottocrat_users to all tag the same object with the same tag. Default is 1 to only allow one occurence of a tag per object. [default: 1]
...
/*
	 ...
	 * tagArray parameter. Use this to provide tag combo services to your ottocrat_users.
	 *
	...
	function safe_tag($tagger_id, $object_id, $tag, $module) {
		...
		// Dynamically switch between allowing duplication between ottocrat_users on the constructor param 'block_multiuser_tag_on_object'.
		...
		if(!$rs->EOF) {
			...
			// Add new tag! 
			$tag_id = $adb->getUniqueId('ottocrat_freetags');
	 ...
	 * @param string The raw string form of the tag to delete. See above for ottocrat_notes.
	 *
	 * @return string Returns the tag in normalized form.
	 */ 
	...
	function get_tag_cloud_html($module="",$tagger_id = NULL,$obj_id= NULL,$num_tags = 100, $min_font_size = 10, $max_font_size = 20, $font_units = 'px', $span_class = '', $tag_page_url = '/tag/') {
		...
		if($module != 'All') {
			foreach ($tag_list[0] as $tag => $qty) {
				...
				$cloud_span[] = '<span id="tag_'.$tag_list[1][$tag].'" class="' . $span_class . '" onMouseOver=$("tagspan_'.$tag_list[1][$tag].'").style.display="inline"; onMouseOut=$("tagspan_'.$tag_list[1][$tag].'").style.display="none";><a class="tagit" href="'.Ottocrat_Request:: encryptLink("index.php?module=Home&action=UnifiedSearch&search_module=".$module."&search_tag=tag_search&query_string=". urlencode($tag)) . '" style="font-size: '. $size . $font_units . '">' . htmlspecialchars(stripslashes($tag)) . '</a><span class="'. $span_class .'" id="tagspan_'.$tag_list[1][$tag].'" style="display:none;cursor:pointer;" onClick="DeleteTag('.$tag_list[1][$tag].','.$obj_id.');"><img src="' . ottocrat_imageurl('del_tag.gif', $theme) . '"></span></span>';

			}
		} else {
			foreach($tag_list[0] as $tag => $qty) {
				$size = $min_font_size + ($qty - $min_qty) * $step;
				$cloud_span[] = '<span class="' . $span_class . '"><a class="tagit" href="'.Ottocrat_Request:: encryptLink("index.php?module=Home&action=UnifiedSearch&search_module=".$module."&search_tag=tag_search&query_string=". urlencode($tag)) . '" style="font-size: '. $size . $font_units . '">' . htmlspecialchars(stripslashes($tag)) . '</a></span>';
			}
		}
		$cloud_html = join("\n ", $cloud_span);
		return $cloud_html;
	 ...
	 * Confusing? Think of it like e-commerce's "Other ottocrat_users who bought this also bought," 
