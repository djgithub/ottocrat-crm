...
<div class="quickWidgetContainer accordion">
	...
	{foreach item=SIDEBARWIDGET key=index from=$QUICK_LINKS['SIDEBARWIDGET']}
		<div class="quickWidget">
			<div class="accordion-heading accordion-toggle quickWidgetHeader" data-target="#{$MODULE}_sideBar_{Ottocrat_Util_Helper::replaceSpaceWithUnderScores($SIDEBARWIDGET->getLabel())}"
			...
			<div class="widgetContainer accordion-body collapse" id="{$MODULE}_sideBar_{Ottocrat_Util_Helper::replaceSpaceWithUnderScores($SIDEBARWIDGET->getLabel())}" data-url="{$SIDEBARWIDGET->getUrl()}">
			</div>
		</div>
	{/foreach}
</div>
