...
Ottocrat_Edit_Js("Calendar_Edit_Js",{
	...
	registerTimeStartChangeEvent : function(container) {
		container.on('changeTime','input[name="time_start"]',function(e) {
			...
			var result = Ottocrat_Time_Validator_Js.invokeValidation(strtTimeElement);
			if(result != true){
				return;
			}
			...
			var date = Ottocrat_Helper_Js.getDateInstance(dateTime,dateFormat);
			...
			var endDateString = app.getDateInOttocratFormat(dateFormat,endDateInstance);
			if(timeFormat == 24){
        ...
        container.find('[name="date_start"]').on('change',function(e) {
            ...
            var result = Ottocrat_Date_Validator_Js.invokeValidation(startDateElement);
            if(result != true){
				return;
			}
    ...
    registerEndDateTimeChangeLogger : function(container) {
        container.find('[name="time_end"]').on('changeTime',function(e) {
            ...
            var result = Ottocrat_Time_Validator_Js.invokeValidation(timeElement);
			if(result != true){
				return;
			}
        ...
        container.find('[name="due_date"]').on('change',function(e) {
            ...
            var result = Ottocrat_Date_Validator_Js.invokeValidation(dueDateElement);
            if(result != true){
				return;
			}
            jQuery('[name="userChangedEndDateTime"]').val('1');
    ...
    registerRelatedContactSpecificEvents : function() {
		...
		form.find('[name="contact_id"]').on(Ottocrat_Edit_Js.preReferencePopUpOpenEvent,function(e){
            var form = thisInstance.getForm();
            var parentIdElement  = form.find('[name="parent_id"]');
            ...
            if(popupReferenceModule == 'Leads' && parentIdElement.val().length > 0) {
                e.preventDefault();
                Ottocrat_Helper_Js.showPnotify(app.vtranslate('LBL_CANT_SELECT_CONTACT_FROM_LEADS'));
            }
        })
        ...
        form.find('[name="contact_id"]').on(Ottocrat_Edit_Js.refrenceMultiSelectionEvent,function(e,result){
