...
{if $FIELD_NAME eq 'defaulteventstatus'}
    {assign var=EVENT_MODULE value=Ottocrat_Module_Model::getInstance('Events')}
...
{/if}
    <select class="chzn-select {if $OCCUPY_COMPLETE_WIDTH} row-fluid {/if}" name="{$FIELD_NAME}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
        ...
        {if $FIELD_MODEL->get('name') eq 'defaulteventstatus' || $FIELD_MODEL->get('name') eq 'defaultactivitytype' }<option value="{vtranslate('LBL_SELECT_OPTION','Ottocrat')}">{vtranslate('LBL_SELECT_OPTION','Ottocrat')}</option>{/if}
        {foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
            ...
            {else}
                {assign var=OPTION_VALUE value=Ottocrat_Util_Helper::toSafeHTML($PICKLIST_NAME)}
            {/if}
