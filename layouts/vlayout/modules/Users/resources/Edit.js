...
Ottocrat_Edit_Js("Users_Edit_Js",{},{
		...
		form.on(Ottocrat_Edit_Js.recordPreSave, function(e, data) {
			...
			if(record == ''){
				if(newPassword != confirmPassword){
					Ottocrat_Helper_Js.showPnotify(app.vtranslate('JS_REENTER_PASSWORDS'));
					e.preventDefault();
				}
					...
					thisInstance.checkDuplicateUser(userName).then(
						function(data){
							if(data.result) {
								...
								Ottocrat_Helper_Js.showPnotify(app.vtranslate('JS_USER_EXISTS'));
							}
						}, 
						function (data, error){
							thisInstance.duplicateCheckCache[userName] = data.result;
