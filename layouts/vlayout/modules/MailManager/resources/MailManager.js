...
Ottocrat_Header_Js.extend('MailManager_QuickCreate_Js', {
...
if (typeof(MailManager) == 'undefined') {
	...
	// Legacy classes used
	if (typeof OttocratJS_DialogBox == 'undefined') {
		OttocratJS_DialogBox = {
			block: function() { },
			unblock: function() { }
		}
	}
		...
		*/
		...
		mail_open: function(folder, msgno){
			...
			jQuery('#_mailrow_' + msgno).addClass('mm_normal');
					...
					if (!resultJSON['ui']) {
						Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_Failed_To_Open_Mail'));
						return;
					}
		...
		mail_associate_actions : function(form) {
			...
			// No record is selected for linking?
			if (selected == false) {
				Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_PLEASE_SELECT_ATLEAST_ONE_RECORD'));
				MailManager.resetLinkToDropDown();
				return false;
			}
			...
			var progressIndicatorElement = jQuery.progressIndicator({
				...
				+"&docid="+ encodeURIComponent(docid)).then(function(responseJSON) {
					...
					if(responseJSON.result.success == true) {
						...
						Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_ATTACHMENT_NOT_DELETED'));
					}
				}
				);
		},
		...
		mail_reply: function(all){
			...
			Ottocrat_Index_Js.showComposeEmailPopup(params, function(win){
		...
		mail_reply_send: function(form){
			...
			if (form.to.value == '') {
				Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_Recepient_Cannot_Be_Empty'));
				return false;
			}
			if (form.subject.value == '') {
				Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_Subject_Cannot_Be_Empty'));
				return false;
			}
			var bodyval = $('_mail_replyfrm_body_').value.trim();
			if (bodyval == '<br />' || bodyval == '') {
				Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_Body_Cannot_Be_Empty'));
				return false;
			}
			...
			MailManager.Request(baseurl, params, function(transport) {
				...
				if (responseJSON['success']) {
					...
					Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_Failed_To_Send_Mail') +
		...
		save_draft: function(form){
			...
			MailManager.Request(baseurl, params, function(transport) {
				...
				if (responseJSON['success']) {
					...
					Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_Failed_To_Save_Mail'));
				}
			});
		},
		...
		checkUploadCount : function() {
			...
			if(MailManagerCurrentUploadCount >= MailManager.MailManagerUploadLimit) {
				Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_FILEUPLOAD_LIMIT_EXCEEDED'));
				return false;
			}
			return true;
		},
		...
		AjaxDuplicateValidate : function (module, fieldname, form) {
			...
			function execute() {
				...
				function validate() {
					if(fieldvalue == '') {
						Ottocrat_Helper_Js.showPnotify(app.vtranslate('JSLBL_ACCOUNTNAME_CANNOT_EMPTY'));
