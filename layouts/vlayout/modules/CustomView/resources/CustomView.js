...
var Ottocrat_CustomView_Js = {
	...
	loadFilterView : function(url) {
		...
		AppConnector.request(url).then(
			function(data){
				...
				Ottocrat_CustomView_Js.registerEvents();
				Ottocrat_CustomView_Js.advanceFilterInstance = Ottocrat_AdvanceFilter_Js.getInstance(jQuery('.filterContainer',contents));
			},
			function(error,err){
	...
	getContentsContainer : function() {
		if(Ottocrat_CustomView_Js.contentsCotainer == false) {
			Ottocrat_CustomView_Js.contentsCotainer = jQuery('div.contentsDiv');
		}
		return Ottocrat_CustomView_Js.contentsCotainer;
	},

	getColumnListSelect2Element : function() {
		return Ottocrat_CustomView_Js.columnListSelect2Element;
	},
	...
	getColumnSelectElement : function() {
		if(Ottocrat_CustomView_Js.columnSelectElement == false) {
			Ottocrat_CustomView_Js.columnSelectElement = jQuery('#viewColumnsSelect');
		}
		return Ottocrat_CustomView_Js.columnSelectElement;
	},
	...
	getSelectedColumnsList : function() {
		if(Ottocrat_CustomView_Js.selectedColumnsList == false) {
			Ottocrat_CustomView_Js.selectedColumnsList = jQuery('#selectedColumnsList');
		}
		return Ottocrat_CustomView_Js.selectedColumnsList;
	},
	...
	makeColumnListSortable : function() {
		var select2Element = Ottocrat_CustomView_Js.getColumnListSelect2Element();
		...
		chozenChoiceElement.sortable({
                ...
                start: function() { Ottocrat_CustomView_Js.getSelectedColumnsList().select2("onSortStart"); },
                update: function() { Ottocrat_CustomView_Js.getSelectedColumnsList().select2("onSortEnd"); }
            });
	},
	...
	getSelectedColumns : function() {
		var columnListSelectElement = Ottocrat_CustomView_Js.getColumnSelectElement();
		var select2Element = Ottocrat_CustomView_Js.getColumnListSelect2Element();
	...
	arrangeSelectChoicesInOrder : function() {
		var contentsContainer = Ottocrat_CustomView_Js.getContentsContainer();
		var chosenElement = Ottocrat_CustomView_Js.getColumnListSelect2Element();
		var choicesContainer = chosenElement.find('ul.select2-choices');
		var choicesList = choicesContainer.find('li.select2-search-choice');
		var coulmnListSelectElement = Ottocrat_CustomView_Js.getColumnSelectElement();
	...
	saveAndViewFilter : function(){
		Ottocrat_CustomView_Js.saveFilter().then(
			function(response){
				if (response.success) {
					...
					Ottocrat_Helper_Js.showPnotify(params);
				}
			},
			function(error) {
	...
	registerSelect2ElementForColumnsSelection : function() {
		var selectElement = Ottocrat_CustomView_Js.getColumnSelectElement();
		app.changeSelectElementView(selectElement, 'select2', {maximumSelectionSize: 12,dropdownCss : {'z-index' : 0}});
	},

	registerEvents: function(){
		Ottocrat_CustomView_Js.registerSelect2ElementForColumnsSelection();
		var contentsContainer = Ottocrat_CustomView_Js.getContentsContainer();
		jQuery('.stndrdFilterDateSelect').datepicker();
		jQuery('.chzn-select').chosen();

		var select2Element = app.getSelect2ElementFromSelect(Ottocrat_CustomView_Js.getColumnSelectElement());
		Ottocrat_CustomView_Js.columnListSelect2Element = select2Element;

		//To arrange the chosen choices in the order that is selected
		Ottocrat_CustomView_Js.arrangeSelectChoicesInOrder();

		jQuery("#standardDateFilter").change(function(){
			Ottocrat_CustomView_Js.loadDateFilterValues();
		});

		Ottocrat_CustomView_Js.makeColumnListSortable();

		jQuery("#CustomView").submit(function(e) {
			var selectElement = Ottocrat_CustomView_Js.getColumnSelectElement();
			var select2Element = app.getSelect2ElementFromSelect(selectElement);
			var result = Ottocrat_MultiSelect_Validator_Js.invokeValidation(selectElement);
			if(result != true){
			...
			if(result == true){
				...
				var advfilterlist = Ottocrat_CustomView_Js.advanceFilterInstance.getValues();
				jQuery('#advfilterlist').val(JSON.stringify(advfilterlist));
				jQuery('input[name="columnslist"]', contentsContainer).val(JSON.stringify(Ottocrat_CustomView_Js.getSelectedColumns()));
				Ottocrat_CustomView_Js.saveAndViewFilter();
				return false;
			} else {
