...
{if $FILE_LOCATION_TYPE_FIELD eq NULL}
    {assign var=DOCUMENTS_MODULE_MODEL value=Ottocrat_Module_Model::getInstance('Documents')}
...
<div class="fileUploadContainer">
	{if $IS_EXTERNAL_LOCATION_TYPE}
		<input type="text" class="input-large {if $FIELD_MODEL->isNameField()}nameField{/if}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required,{/if}funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}"
	...
	{else}
		<input type="file" class="input-large {if $FIELD_MODEL->isNameField()}nameField{/if}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required,{/if}funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}"
