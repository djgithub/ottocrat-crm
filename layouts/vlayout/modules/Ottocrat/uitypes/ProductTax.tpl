...
{foreach item=tax key=count from=$TAXCLASS_DETAILS}
	...
	<td class="fieldValue">
		<input type="text" class="detailedViewTextBox {if $show_value eq "hidden"} hide {else} show {/if}" name="{$tax.taxname}" value="{$tax.percentage}" data-validation-engine="validate[funcCall[Ottocrat_PositiveNumber_Validator_Js.invokeValidation]]" />
	</td>
