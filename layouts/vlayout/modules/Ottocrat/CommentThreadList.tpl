					...
					<div class="span11 commentorInfo">
						...
						<div class="inner">
							...
							<span class="pull-right">
								<p class="muted"><small title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString($COMMENT->getCommentedTime())}" class="commentModifiedTime">{Ottocrat_Util_Helper::formatDateDiffInStrings($COMMENT->getCommentedTime())}</small></p>
							</span>
				...
				<div class="row-fluid">
					...
					{if $COMMENT->getCommentedTime() neq $COMMENT->getModifiedTime()}
						<span class="{if empty($REASON_TO_EDIT)}row-fluid{else} span6{/if}">
							<span class="pull-right">
								<p class="muted"><small title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString($COMMENT->getModifiedTime())}">{Ottocrat_Util_Helper::formatDateDiffInStrings($COMMENT->getModifiedTime())}</small></p>
							</span>
						</span>
					{/if}
