...
<div  class="summaryWidgetContainer">
	<div class="widget_header row-fluid">
		...
		<span class="span4"><button class="btn pull-right addButton createActivity" data-url={Ottocrat_Request::encryptLink("sourceModule={$RECORD->getModuleName()}&sourceRecord={$RECORD->getId()}&relationOperation=true")} type="button"><strong>{vtranslate('LBL_ADD',$MODULE_NAME)}</strong></button></span>
	</div>
		...
		{if count($ACTIVITIES) neq '0'}
			{foreach item=RECORD key=KEY from=$ACTIVITIES}
				...
				<div class="activityEntries">
					...
					<div class="row-fluid">
						<span class="span6"><strong title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString("$START_DATE $START_TIME")}">{Ottocrat_Util_Helper::formatDateIntoStrings($START_DATE, $START_TIME)}</strong></span>
