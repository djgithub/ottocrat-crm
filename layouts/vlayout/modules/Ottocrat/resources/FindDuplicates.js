 ...
 * All Rights Reserved.
 ************************************************************************************/
Ottocrat_List_Js('Ottocrat_FindDuplicates_Js',{

	massDeleteRecords : function(url) {
		var listInstance = new Ottocrat_FindDuplicates_Js();
		Ottocrat_List_Js.massDeleteRecords(url,listInstance);
	}
},{
	...
	registerMergeRecordEvent : function(cb) {
		...
		jQuery('input[name="merge"]').on('click', function(e) {
			...
			if(mergeRecordsCheckBoxes.length < 2) {
				Ottocrat_Helper_Js.showMessage({text: app.vtranslate('JS_SELECT_ATLEAST_TWO_RECORD_FOR_MERGING')});
				return false;
			} else {
				var count = 0;
				var records = [];
				...
				mergeRecordsCheckBoxes.each(function(key, obj) {
					...
					if(ele.data('group') != groupName) {
						Ottocrat_Helper_Js.showMessage({text: app.vtranslate('JS_SELECT_RECORDS_TO_MERGE_FROM_SAME_GROUP')});
						stop = true;
						return false;
					}
				...
				if(count > 3) {
					Ottocrat_Helper_Js.showMessage({text: app.vtranslate('JS_ALLOWED_TO_SELECT_MAX_OF_THREE_RECORDS')});
					return false;
				}
