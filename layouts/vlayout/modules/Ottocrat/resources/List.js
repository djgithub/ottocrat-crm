...
jQuery.Class("Ottocrat_List_Js",{

	listInstance : false,

	getRelatedModulesContainer : false,

	massEditPreSave : 'Ottocrat.MassEdit.PreSave',



   getInstance: function(){
		if(Ottocrat_List_Js.listInstance == false){
			...
			if(parentModule == 'Settings'){
				...
				var fallbackClassName = parentModule+"_Ottocrat_List_Js";
				if(typeof window[fallbackClassName] == 'undefined') {
					fallbackClassName = "Ottocrat_List_Js";
				}
			} else {
				moduleClassName = module+"_List_Js";
				fallbackClassName = "Ottocrat_List_Js";
			...
			Ottocrat_List_Js.listInstance = instance;
			return instance;
		}
		return Ottocrat_List_Js.listInstance;
	},
		/*
	...
	triggerSendEmail : function(massActionUrl, module, params){
		var listInstance = Ottocrat_List_Js.getInstance();
		...
		if(validationResult != true){
			...
			Ottocrat_Index_Js.showComposeEmailPopup(actionParams);
		} else {
	...
	triggerSendSms : function(massActionUrl, module){
		...
		if(validationResult != true){
			...
			Ottocrat_Helper_Js.checkServerConfig(module).then(function(data){
				progressIndicatorElement.progressIndicator({'mode' : 'hide'});
				if(data == true){
					Ottocrat_List_Js.triggerMassAction(massActionUrl);
				} else {
	...
	massDeleteRecords : function(url,instance) {
		...
		if(validationResult != true){
			...
			Ottocrat_Helper_Js.showConfirmationBox({'message' : message}).then(
				function(e) {

					var deleteURL = url+'&viewname='+cvId+'&selected_ids='+selectedIds+'&excluded_ids='+excludedIds;
                    var listViewInstance = Ottocrat_List_Js.getInstance();
	...
	triggerMassEdit : function(massEditUrl) {
		...
		Ottocrat_List_Js.triggerMassAction(massEditUrl, function(container){
			...
			var editInstance = Ottocrat_Edit_Js.getInstance();
	...
	triggerExportAction :function(exportActionUrl){
		...
		exportActionUrl=jQuery.ajax({	type: "POST",
			url:encAjaxUrl,
			data: {"queryStr": exportActionUrl},
	...
	showDuplicateSearchForm : function(url) {
		...
		app.showModalWindow("", url, function() {
			progressIndicatorElement.progressIndicator({'mode' : 'hide'});
			Ottocrat_List_Js.registerDuplicateSearchButtonEvent();
		});
	},
		...
		AppConnector.requestPjax(urlParams).then(
			function(data){
				...
				thisInstance.calculatePages().then(function(data){
					...
					Ottocrat_Helper_Js.showHorizontalTopScrollBar();
	...
	massActionSave : function(form, isMassEdit){
		...
		if(isMassEdit) {
			...
			if(fieldsChanged == false){
				Ottocrat_Helper_Js.showPnotify(app.vtranslate('NONE_OF_THE_FIELD_VALUES_ARE_CHANGED_IN_MASS_EDIT'));
				form.find('[name="saveButton"]').removeAttr('disabled');
				aDeferred.reject();
				return aDeferred.promise();
			}
			//on submit form trigger the massEditPreSave event
			var massEditPreSaveEvent = jQuery.Event(Ottocrat_List_Js.massEditPreSave);
	...
	registerRecordAccessCheckEvent : function(form) {
		...
		form.on(Ottocrat_List_Js.massEditPreSave, function(e) {
				...
				if(assignedToSelectElement.data('recordaccessconfirmation') == true) {
					...
					if(assignedToSelectElement.data('recordaccessconfirmationprogress') != true) {
						...
						if(recordAccess == false) {
							...
							Ottocrat_Helper_Js.showConfirmationBox({
								'message' : message
							}).then(
								function(e) {
	...
	registerPageNavigationEvents : function(){
		...
		jQuery('#listViewPageJump').on('click',function(e){
            if(typeof Ottocrat_WholeNumberGreaterThanZero_Validator_Js.invokeValidation(jQuery('#pageToJump'))!= 'undefined') {
		...
		jQuery('#listViewPageJumpDropDown').on('click','li',function(e){
			...
			if(e.which == 13){
				...
				var response = Ottocrat_WholeNumberGreaterThanZero_Validator_Js.invokeValidation(element);
	...
	*/
	...
	registerCreateFilterClickEvent : function(){
		...
		jQuery('#createFilter').on('click',function(event){
			...
			Ottocrat_CustomView_Js.loadFilterView(createUrl);
		});
	},
	...
	registerEditFilterClickEvent : function(){
		...
		if(listViewFilterBlock != false){
			listViewFilterBlock.on('mouseup','li i.editFilter',function(event){
				...
				Ottocrat_CustomView_Js.loadFilterView(editUrl);
	...
	registerDeleteRecordClickEvent: function(){
		...
		listViewContentDiv.on('click','.deleteRecordButton',function(e){
			...
			Ottocrat_List_Js.deleteRecord(recordId);
			e.stopPropagation();
		});
	},
	/*
	...
	registerReferenceFieldsForValidation : function(form){
		...
		form.find('.sourceField').on(Ottocrat_Edit_Js.referenceSelectionEvent,function(e,params){
			var element = jQuery(e.currentTarget);
		...
		form.find('.clearReferenceSelection').on(Ottocrat_Edit_Js.referenceDeSelectionEvent,function(e){
...
/* start of functions for cloudTagwidget*/
	...
	registerPostTagCloudWidgetLoad : function() {
		var thisInstance = this;
		app.getContentsContainer().on('Ottocrat.Widget.Load.LBL_TAG_CLOUD',function(e,data){
