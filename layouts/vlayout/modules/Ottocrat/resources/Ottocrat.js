...
var Ottocrat_Index_Js = {
	...
	registerWidgetsEvents : function() {
		...
		widgets.on({
				shown: function(e) {
					...
					Ottocrat_Index_Js.loadWidgets(widgetContainer);
	...
	loadWidgets : function(widgetContainer, open) {
		...
		AppConnector.request(listViewWidgetParams).then(
			...
			if(typeof open == 'undefined') open = true;
				...
				jQuery('.bodyContents').trigger('Ottocrat.Widget.Load.'+label,jQuery(widgetContainer));
			}
		);
	},
	...
	showComposeEmailPopup : function(params, cb){
		...
		Ottocrat_Helper_Js.checkServerConfig(currentModule).then(function(data){
			if(data == true){
	...
	registerActivityReminder : function() {
		...
		if(activityReminder != '') {
			...
			if((currentTime + activityReminder) > nextActivityReminderCheck) {
				Ottocrat_Index_Js.requestReminder();
				setTimeout('Ottocrat_Index_Js.requestReminder()', activityReminder);
		...
		AppConnector.request(url).then(function(data){
			if(data.success && data.result) {
				for(i=0; i< data.result.length; i++) {
					...
					Ottocrat_Index_Js.showReminderPopup(record);
				}
			}
		});
	},
	...
	showReminderPopup : function(record) {
		...
		var notify = Ottocrat_Helper_Js.showPnotify(params);
	...
	registerResizeEvent: function(){
		...
		$(window).bind('resizeEnd', function() {
			Ottocrat_Index_Js.adjustTopMenuBarItems();
		});
	},
	/**
	...
	registerEvents : function(){
		Ottocrat_Index_Js.registerWidgetsEvents();
		Ottocrat_Index_Js.loadWidgetsOnLoad();
		Ottocrat_Index_Js.registerActivityReminder();
		Ottocrat_Index_Js.adjustTopMenuBarItems();
		Ottocrat_Index_Js.registerPostAjaxEvents();
		Ottocrat_Index_Js.changeSkin();
		Ottocrat_Index_Js.registerShowHideLeftPanelEvent();
		Ottocrat_Index_Js.registerResizeEvent();
	},

	registerPostAjaxEvents: function() {
		Ottocrat_Index_Js.registerTooltipEvents();
	}
}
...
jQuery(document).ready(function() {
	Ottocrat_Index_Js.registerEvents();
