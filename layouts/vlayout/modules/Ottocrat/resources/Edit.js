...
jQuery.Class("Ottocrat_Edit_Js",{

	//Event that will triggered when reference field is selected
	referenceSelectionEvent : 'Ottocrat.Reference.Selection',

	//Event that will triggered when reference field is selected
	referenceDeSelectionEvent : 'Ottocrat.Reference.DeSelection',

	//Event that will triggered before saving the record
	recordPreSave : 'Ottocrat.Record.PreSave',

	refrenceMultiSelectionEvent : 'Ottocrat.MultiReference.Selection',

	preReferencePopUpOpenEvent : 'Ottocrat.Referece.Popup.Pre',

	editInstance : false,

    postReferenceSelectionEvent: 'Ottocrat.PostReference.Selection',
	...
	getInstanceByModuleName : function(moduleName){
		...
		if(parentModule == 'Settings'){
			...
			var fallbackClassName = parentModule+"_Ottocrat_Edit_Js";
			if(typeof window[fallbackClassName] == 'undefined') {
				fallbackClassName = "Ottocrat_Edit_Js";
			}
		} else {
			moduleClassName = moduleName+"_Edit_Js";
			fallbackClassName = "Ottocrat_Edit_Js";
	...
	getInstance: function(){
		if(Ottocrat_Edit_Js.editInstance == false){
			var instance = Ottocrat_Edit_Js.getInstanceByModuleName();
			Ottocrat_Edit_Js.editInstance = instance;
			return instance;
		}
		return Ottocrat_Edit_Js.editInstance;
	}
	...
	openPopUp : function(e){
		...
		var prePopupOpenEvent = jQuery.Event(Ottocrat_Edit_Js.preReferencePopUpOpenEvent);
		...
		popupInstance.show(params,function(data){
			...
			if(isMultiple) {
                    sourceFieldElement.trigger(Ottocrat_Edit_Js.refrenceMultiSelectionEvent,{'data':dataList});
			}
                sourceFieldElement.trigger(Ottocrat_Edit_Js.postReferenceSelectionEvent,{'data':responseData});
		});
	},
	...
	setReferenceFieldValue : function(container, params) {
		...
		fieldElement.trigger(Ottocrat_Edit_Js.referenceSelectionEvent, {'source_module' : popupReferenceModule, 'record' : id, 'selectedName' : selectedName});
	...
	registerAutoCompleteFields : function(container) {
		...
		container.find('input.autoComplete').autocomplete({
			...
			'select' : function(event, ui ){
                ...
                fieldElement.trigger(Ottocrat_Edit_Js.postReferenceSelectionEvent,{'data':selectedItemData});
			},
	...
	registerClearReferenceSelectionEvent : function(container) {
		container.find('.clearReferenceSelection').on('click', function(e){
			...
			element.trigger(Ottocrat_Edit_Js.referenceDeSelectionEvent);
			e.preventDefault();
		})
	},
	...
	registerEventForImageDelete : function(){
		...
		formElement.find('.imageDelete').on('click',function(e){
			...
			if(exisitingImages.length < 1 && mandatoryStatus){
				...
				imageUploadElement.attr('data-validation-engine','validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]');
		...
		editViewForm.submit(function(e){
			...
			if(typeof editViewForm.data('submit') != "undefined") {
				...
				if(editViewForm.validationEngine('validate')) {
					...
					var recordPreSaveEvent = jQuery.Event(Ottocrat_Edit_Js.recordPreSave);
	...
	registerRecordAccessCheckEvent : function(form) {
		...
		form.on(Ottocrat_Edit_Js.recordPreSave, function(e, data) {
