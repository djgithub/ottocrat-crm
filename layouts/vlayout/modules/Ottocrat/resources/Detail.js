...
jQuery.Class("Ottocrat_Detail_Js",{

    detailInstance : false,

	getInstance: function(){
        if( Ottocrat_Detail_Js.detailInstance == false ){
            var module = app.getModuleName();
            var view = app.getViewName();
            var moduleClassName = module+"_"+view+"_Js";
            var fallbackClassName = Ottocrat_Detail_Js;
            if(typeof window[moduleClassName] != 'undefined'){
                var instance = new window[moduleClassName]();
            }else{
                var instance = new fallbackClassName();
            }
            Ottocrat_Detail_Js.detailInstance = instance;
        }
        return Ottocrat_Detail_Js.detailInstance;
	},
	...
	triggerSendEmail : function(detailActionUrl, module){
        Ottocrat_Helper_Js.checkServerConfig(module).then(function(data){
			if(data == true){
                var currentInstance = Ottocrat_Detail_Js.getInstance();
                ...
                Ottocrat_Index_Js.showComposeEmailPopup(params);
			} else {
    ...
    triggerDetailViewAction : function(detailActionUrl, callBackFunction){
		var detailInstance = Ottocrat_Detail_Js.getInstance();
    ...
    triggerSendSms : function(detailActionUrl, module) {
        Ottocrat_Helper_Js.checkServerConfig(module).then(function(data){
			if(data == true){
                Ottocrat_Detail_Js.triggerDetailViewAction(detailActionUrl);
			} else {
    ...
    deleteRecord : function(deleteRecordActionUrl) {
		...
		Ottocrat_Helper_Js.showConfirmationBox({'message' : message}).then(function(data) {
				AppConnector.request(deleteRecordActionUrl+'&ajaxDelete=true').then(
				function(data){
					if(data.success == true){
						window.location.href = data.result;
					}else{
						Ottocrat_Helper_Js.showPnotify(data.error.message);
					}
				});
			},
			function(error, err){
	...
	fieldUpdatedEvent : 'Ottocrat.Field.Updated',
	widgetPostLoad : 'Ottocrat.Widget.PostLoad',
	...
	fieldPreSave : 'Ottocrat.Field.PreSave',
	...
	loadRelatedList : function(pageNumber){
		var relatedListInstance = new Ottocrat_RelatedList_Js(this.getRecordId(), app.getModuleName(), this.getSelectedTab(), this.getRelatedModuleName());
	...
	registerEventForRelatedListPagination : function(){
		...
		detailContentsHolder.on('click','#relatedListNextPageButton',function(e){
			...
			var relatedController = new Ottocrat_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
	...
	registerEventForRelatedList : function(){
		...
		detailContentsHolder.on('click', 'a.relationDelete', function(e){
			e.stopImmediatePropagation();
			var element = jQuery(e.currentTarget);
			var instance = Ottocrat_Detail_Js.getInstance();
	...
	ajaxEditHandling : function(currentTdElement) {
			...
			var saveHandler = function(e) {
                ...
                var fieldInfo = Ottocrat_Field_Js.getInstance(fieldElement.data('fieldinfo'));
	...
	registerEventForActivityWidget : function(){
		...
		jQuery('.createActivity').on('click', function(e){
			...
			if(quickCreateNode.length <= 0) {
				Ottocrat_Helper_Js.showPnotify(app.vtranslate('JS_NO_CREATE_OR_NOT_QUICK_CREATE_ENABLED'))
			}
			...
			var callbackFunction = function() {
				...
				if(previousValue == ajaxEditNewValue) {
					...
					currentTarget.show();
                                        ...
                                        //If validation fails  
                                        if(errorExists) {  
                                                Ottocrat_Helper_Js.addClickOutSideEvent(currentDiv, callbackFunction);   
                                                                return;   
                                        }
	...
	addRelationBetweenRecords : function(relatedModule, relatedModuleRecordId){
		...
		var thisInstance = this;
		var selectedTabElement = thisInstance.getSelectedTab();
		var relatedController = new Ottocrat_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModule);
		...
		detailContentsHolder.on('click','[name="emailsEditView"]',function(e){
			...
			Ottocrat_Helper_Js.checkServerConfig(module).then(function(data){
				if(data == true){
					...
					Ottocrat_Helper_Js.showPnotify(app.vtranslate('JS_EMAIL_SERVER_CONFIGURATION'));
				}
			})
		})
	},
	...
	registerEvents : function(){
		...
		detailContentsHolder.on('click', '.relatedPopup', function(e){
			var editViewObj = new Ottocrat_Edit_Js();
