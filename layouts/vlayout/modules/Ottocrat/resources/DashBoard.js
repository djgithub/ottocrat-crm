...
jQuery.Class("Ottocrat_DashBoard_Js", {
	gridster : false,
	...
	addWidget : function(element, url) {
		...
		Ottocrat_DashBoard_Js.gridster.add_widget(widgetContainer, width, height);
		Ottocrat_DashBoard_Js.currentInstance.loadWidget(widgetContainer);
	},
	...
	init : function() {
		Ottocrat_DashBoard_Js.currentInstance = this;
	},
	...
	getWidgetInstance : function(widgetContainer) {
		...
		if(!(id in this.instancesCache)) {
			...
			this.instancesCache[id] = Ottocrat_Widget_Js.getInstance(widgetContainer, widgetName);
		}
	...
	registerGridster : function() {
		...
		Ottocrat_DashBoard_Js.gridster = this.getContainer().gridster({
			widget_margins: [7, 7],
	...
	savePositions: function(widgets) {
		...
		AppConnector.request({module: 'Ottocrat', action: 'SaveWidgetPositions', 'positionsmap': widgetRowColPositions}).then(function(data){
		});
	},
		...
		if(mode == 'open') {
			AppConnector.request(urlParams).then(
				function(data){
					...
					widgetContainer.trigger(Ottocrat_Widget_Js.widgetPostLoadEvent);
				},
				function(){
				}
				);
	...
	gridsterStop : function() {
		...
		var gridster = Ottocrat_DashBoard_Js.gridster;
							...
							function(response) {
								if (response.success) {
									...
									if (jQuery.inArray(widgetName, nonReversableWidgets) == -1) {
										var data = '<li><a onclick="Ottocrat_DashBoard_Js.addWidget(this, \''+response.result.url+'\')" href="javascript:void(0);"';
	...
	registerFilterInitiater : function() {
		...
		container.on('click', 'a[name="dfilter"]', function(e) {
			...
			Ottocrat_Helper_Js.addClickOutSideEvent(dashboardWidgetHeader, callbackFunction);
