		...
		{if $MODELNAME == 'ModTracker_Record_Model'}
			...
			{if $PROCEED}
				<div class="row-fluid">
					...
					<div class="span11">
					<p class="pull-right muted" style="padding-right:5px;"><small title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString("$TIME")}">{Ottocrat_Util_Helper::formatDateDiffInStrings("$TIME")}</small></p>
							...
							{if $INDEX lt 2}
								...
								<div class='font-x-small'>
									...
									{if $FIELD->get('prevalue') neq '' && $FIELD->get('postvalue') neq '' && !($FIELD->getFieldInstance()->getFieldDataType() eq 'reference' && ($FIELD->get('postvalue') eq '0' || $FIELD->get('prevalue') eq '0'))}
										&nbsp;{vtranslate('LBL_FROM')} <b>{Ottocrat_Util_Helper::toOttocrat6SafeHTML($FIELD->getDisplayValue(decode_html($FIELD->get('prevalue'))))}</b>
									{else if $FIELD->get('postvalue') eq '' || ($FIELD->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELD->get('postvalue') eq '0')}
	                                    &nbsp; <b> {vtranslate('LBL_DELETED')} </b> ( <del>{Ottocrat_Util_Helper::toOttocrat6SafeHTML($FIELD->getDisplayValue(decode_html($FIELD->get('prevalue'))))}</del> )
	                                {else}
	                                ...
	                                {if $FIELD->get('postvalue') neq '' && !($FIELD->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELD->get('postvalue') eq '0')}
										{vtranslate('LBL_TO')} <b>{Ottocrat_Util_Helper::toOttocrat6SafeHTML($FIELD->getDisplayValue(decode_html($FIELD->get('postvalue'))))}</b>
	                                {/if}    
								</div>
								{/if}
							{else}
			...
			<div class="row-fluid">
				...
				<div class="span11">
					...
					<p class="pull-right muted" style="padding-right:5px;"><small title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString("$COMMENT_TIME")}">{Ottocrat_Util_Helper::formatDateDiffInStrings("$COMMENT_TIME")}</small></p>
					<div>
