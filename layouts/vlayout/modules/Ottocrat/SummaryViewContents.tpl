			...
			<tr class="summaryViewEntries">
				...
				<td class="fieldValue" style="width:65%">
                    <div class="row-fluid">
                        ...
                        {if $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Ottocrat_Field_Model::REFERENCE_TYPE) && $IS_AJAX_ENABLED && $FIELD_MODEL->isAjaxEditable() eq 'true' && $FIELD_MODEL->get('uitype') neq 69}
...
<div class="row-fluid">
	<div class="span4 toggleViewByMode">
		...
		{*assign var="FULL_MODE_URL" value={$RECORD->getDetailViewUrl()
		|cat:'&mode=showDetailViewByMode&requestMode=full'} *}
	...
	<div class="span8">
		<div class="pull-right">
			<div>
				<p>
					<small>
						{vtranslate('LBL_CREATED_ON',$MODULE_NAME)} {Ottocrat_Util_Helper::formatDateTimeIntoDayString($RECORD->get('createdtime'))}
					</small>
				</p>
			</div>
			<div>
				<p>
					<small>
						{vtranslate('LBL_MODIFIED_ON',$MODULE_NAME)} {Ottocrat_Util_Helper::formatDateTimeIntoDayString($RECORD->get('modifiedtime'))}
					</small>
				</p>
			</div>
		</div>
