	...
	<div class="singleComment">
					...
					<div class="span11 commentorInfo">
						...
						<div class="inner">
							...
							<span class="pull-right">
								<p class="muted"><small title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString($COMMENT->getCommentedTime())}">{Ottocrat_Util_Helper::formatDateDiffInStrings($COMMENT->getCommentedTime())}</small></p>
							</span>
		...
		<div class="row-fluid commentActionsContainer">
				...
				<div class="row-fluid">
					...
					{if $COMMENT->getCommentedTime() neq $COMMENT->getModifiedTime()}
						<span class="{if empty($REASON_TO_EDIT)}row-fluid{else} span6{/if}">
							<span class="pull-right">
								<p class="muted"><small><em>{vtranslate('LBL_MODIFIED',$MODULE_NAME)}</em></small>&nbsp;<small title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString($COMMENT->getModifiedTime())}" class="commentModifiedTime">{Ottocrat_Util_Helper::formatDateDiffInStrings($COMMENT->getModifiedTime())}</small></p>
							</span>
						</span>
					{/if}
				</div>
			...
			<div class="row-fluid commentActionsDiv">
				{assign var=COMMENTS_MODULE_MODEL value = Ottocrat_Module_Model::getInstance('ModComments')}
