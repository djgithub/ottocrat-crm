		...
		<div class="contents">
			{if $totalCount eq 100}
				<div class='alert alert-block'>
					...
					{if $SEARCH_MODULE}
						{vtranslate('LBL_GLOBAL_SEARCH_MAX_MESSAGE_FOR_MODULE', 'Ottocrat')}
					{else}
						{vtranslate('LBL_GLOBAL_SEARCH_MAX_MESSAGE', 'Ottocrat')}
					{/if}
				</div>
			{/if}
			...
			{foreach item=recordObject from=$searchRecords name=globalSearch}
				...
				<li id="{$ID}">
					<a target="_blank" id="{$ID}_link" class="cursorPointer" {if stripos($DETAILVIEW_URL, 'javascript:')===0} 
						...
						<span id="{$ID}_time" class="pull-right" title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString($recordObject->get('createdtime'))}">{Ottocrat_Util_Helper::formatDateDiffInStrings($recordObject->get('createdtime'))}</span>
					</a>
				</li>
			{foreachelse}
