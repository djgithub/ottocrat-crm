			...
			<div class="related span2 marginLeftZero">
				<div class="">
					<ul class="nav nav-stacked nav-pills">
						...
						{assign var="LURL" value="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}"}
						<li class="{if $RELATED_LINK->getLabel()==$SELECTED_TAB_LABEL}active{/if}" data-url={Ottocrat_Request::encryptLink("{htmlspecialchars_decode($LURL)}")} data-label-key="{$RELATED_LINK->getLabel()}" data-link-key="{$RELATED_LINK->get('linkKey')}" >
						...
						<li class="{if $RELATED_LINK->getLabel()==$SELECTED_TAB_LABEL}active{/if}" data-url={Ottocrat_Request::encryptLink("{htmlspecialchars_decode($RELATED_LINK->getUrl())}&tab_label={$RELATED_LINK->getLabel()}")} data-label-key="{$RELATED_LINK->getLabel()}" >
