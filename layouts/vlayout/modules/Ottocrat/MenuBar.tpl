						...
						<li class="dropdown" id="moreMenu">
							...
							<div class="dropdown-menu moreMenus" {if ($NUMBER_OF_PARENT_TABS <= 2) && ($NUMBER_OF_PARENT_TABS != 0)}style="width: 30em;"{elseif $NUMBER_OF_PARENT_TABS == 0}style="width: 10em;"{/if}>
								...
								{if $USER_MODEL->isAdminUser()}
									<div class="row-fluid">
										<a id="menubar_item_moduleManager" href="index.php?biUwMFQlQ0FiJUVFJUUyJTg4JUNDJUJFJTNEayVEMCVEMSUxMyVCQSUzRSUyRiVDRlElNDBlJUQ5JUYyJUI1anolRTYlRUIlOEMlMDUlREIlRTMlMjglOTUlMDclQjglMDMlMUVPJUIxJUJEJUVBJUZBJUE5JTgwJUU3JUNGJUVEMiVGNGklMDhMJTk4JUZDJTFEJTlEJTdEeSU1QyVENiU4OCU4Ng==" class="pull-right">{vtranslate('LBL_CUSTOMIZE_MAIN_MENU',$MODULE)}</a>
									</div>
									<!--div class="row-fluid">
						...
						{*<span class="dropdown span settingIcons">
							...
							<ul class="dropdown-menu themeMenuContainer">
								<div id="themeContainer">
									{assign var=COUNTER value=0}
									{assign var=THEMES_LIST value=Ottocrat_Theme::getAllSkins()}
										...
										{foreach key=index item=obj from=$childLinks}
											{if $obj->getLabel() eq NULL}
												...
												{else}
													...
													{/if}
												<li>
														<a target="{$obj->target}" id="menubar_item_right_{Ottocrat_Util_Helper::replaceSpaceWithUnderScores($label)}" {if $label=='Switch to old look'}switchLook{/if} href="{$href}" {$onclick}>{vtranslate($label,$MODULE)}</a>
												</li>
											{/if}
										{/foreach}
									</ul>
