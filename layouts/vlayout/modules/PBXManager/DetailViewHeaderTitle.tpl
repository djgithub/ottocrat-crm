    ...
    <span class="span2">
     <div style="position:relative;display:inline;">
        {if $RECORD->get('customer') and $RECORD->get('customertype') eq 'Contacts'}
            {assign var=MODULE_INSTANCE value=Ottocrat_Record_Model::getInstanceById($RECORD->get('customer'),$RECORD->get('customertype'))}
            ...
            <span class="recordLabel pushDown" title="{$RECORD->getName()}">
                ...
                {if $FIELD_MODEL->getPermissions()}
                    ...
                    {assign var=CALLER_INFO value=PBXManager_Record_Model::lookUpRelatedWithNumber($RECORD->get('customernumber'))}
                        {if $CALLER_INFO.id}
                            {assign var=MODULEMODEL value=Ottocrat_Module_Model::getInstance($RECORD->get('customertype'))}
                            {assign var=FIELDMODEL value=Ottocrat_Field_Model::getInstance($CALLER_INFO.fieldname,$MODULEMODEL)}
