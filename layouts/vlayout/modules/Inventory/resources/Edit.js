...
Ottocrat_Edit_Js("Inventory_Edit_Js",{
		...
		var taxDiv = '<div class="taxUI validCheck hide" id="tax_div'+rowNumber+'">'+
		   ...
		   if(!jQuery.isEmptyObject(taxObj)){
			   for(var taxName in taxObj){
				   ...
				   taxDiv += '<tr>'+
					'<td>'+
					'<input type="text" name="'+taxName+'_percentage'+rowNumber+'" data-validation-engine="validate[funcCall[Ottocrat_PositiveNumber_Validator_Js.invokeValidation]]" id="'+taxName+'_percentage'+rowNumber+'" value="'+taxInfo.percentage+'" class="smallInputBox taxPercentage">&nbsp;%'+
					'</td>'+
	...
	registerReferenceSelectionEvent : function(container) {
		...
		jQuery('input[name="contact_id"]', container).on(Ottocrat_Edit_Js.referenceSelectionEvent, function(e, data){
			...
			function(data){
				...
				if(typeof addressMap != "undefined"){
					...
					if(noAddress){
						...
						Ottocrat_Helper_Js.showPnotify(app.vtranslate(errorMsg));
					} else{
		...
		jQuery('[name="copyAddressFromRight"],[name="copyAddressFromLeft"]').change(function(){
			...
			if(elementClass == "accountAddress"){
				...
				if(recordRelativeAccountId == "" || recordRelativeAccountId == "0"){
					Ottocrat_Helper_Js.showPnotify(app.vtranslate('JS_PLEASE_SELECT_AN_ACCOUNT_TO_COPY_ADDRESS'));
				} else {
				...
				if(recordRelativeContactId == "" || recordRelativeContactId == "0"){
					Ottocrat_Helper_Js.showPnotify(app.vtranslate('JS_PLEASE_SELECT_AN_CONTACT_TO_COPY_ADDRESS'));
				} else {
