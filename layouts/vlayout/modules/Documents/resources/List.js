...
Ottocrat_List_Js("Documents_List_Js", {
		...
		AppConnector.request(params).then(
			function(data) {
				...
				var callBackFunction = function(data){
					...
					Ottocrat_List_Js.getInstance().folderSubmit().then(function(data){
						app.hideModalWindow();
						if(data.success){
							var result = data.result;
							if(result.success){
								var info = result.info;
								Ottocrat_List_Js.getInstance().updateCustomFilter(info);
								var  params = {
			...
			listViewFilterBlock.on('mouseup','li i.deleteFilter',function(event){
				...
				if(liElement.hasClass('folderOption')){
					if(liElement.find('.deleteFilter').hasClass('dull')){
						Ottocrat_Helper_Js.showPnotify(app.vtranslate('JS_FOLDER_IS_NOT_EMPTY'));
						return;
					} else {
...
Ottocrat_Base_Validator_Js("Ottocrat_FolderName_Validator_Js",{
