...
var Settings_Picklist_Js = {
	...
	registerModuleChangeEvent : function() {
		jQuery('#pickListModules').on('change',function(e){
            ...
            if(selectedModule.length <= 0) {
                Settings_Ottocrat_Index_Js.showMessage({'type': 'error','text':app.vtranslate('JS_PLEASE_SELECT_MODULE')});
                return;
            }
			var params = {
	...
	registerAssignValueToRoleSaveEvent : function(data) {
		jQuery('#assignValueToRoleForm').on('submit',function(e) {
			...
			var assignValueResult = Ottocrat_MultiSelect_Validator_Js.invokeValidation(assignValuesSelectElement);
			...
			var result = Ottocrat_MultiSelect_Validator_Js.invokeValidation(rolesSelectElement);
			if(result != true){
			...
			AppConnector.request(params).then(function(data) {
				if(typeof data.result != 'undefined') {
					app.hideModalWindow();
					Settings_Ottocrat_Index_Js.showMessage({text:app.vtranslate('JS_VALUE_ASSIGNED_SUCCESSFULLY'),type : 'success'})
				}
			});
			e.preventDefault();
		});
		...
		jQuery('#saveOrder').on('click',function(e) {
			...
			AppConnector.request(params).then(function(data) {
				if(typeof data.result != 'undefined') {
					...
					progressIndicatorElement.progressIndicator({mode : 'hide'});
					Settings_Ottocrat_Index_Js.showMessage({text:app.vtranslate('JS_LIST_UPDATED_SUCCESSFULLY'),type : 'success'})
				}
			});
		});
	},
	...
	registerDeleteItemEvent : function() {
		...
		jQuery('#deleteItem').on('click',function(e){
			...
			if(pickListValues.length == selectedListItem.length) {
				Settings_Ottocrat_Index_Js.showMessage({text:app.vtranslate('JS_YOU_CANNOT_DELETE_ALL_THE_VALUES'),type : 'error'})
				return;
			}
			var params = {
				module : app.getModuleName(),
		...
		jQuery('#saveSequence').on('click',function(e) {
			...
			AppConnector.request(params).then(function(data) {
				if(typeof data.result != 'undefined') {
					...
					progressIndicatorElement.progressIndicator({mode : 'hide'});
					Settings_Ottocrat_Index_Js.showMessage({text:app.vtranslate('JS_SEQUENCE_UPDATED_SUCCESSFULLY'),type : 'success'})
				}
			});
		});
	},
...
Ottocrat_Base_Validator_Js("Ottocrat_FieldLabel_Validator_Js",{
	...
	invokeValidation: function(field, rules, i, options){
		var instance = new Ottocrat_FieldLabel_Validator_Js();
