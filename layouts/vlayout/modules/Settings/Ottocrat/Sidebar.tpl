...
<div class="sidebarTitleBlock">
	<h3 class="titlePadding themeTextColor unSelectedQuickLink cursorPointer"><a href="index.php?module=Ottocrat&parent=Settings&view=Index">{vtranslate('LBL_SETTINGS', $QUALIFIED_MODULE)}</a></h3>
</div>
<!--div>
...
<div class="quickWidgetContainer accordion" id="settingsQuickWidgetContainer">
	{foreach item=MENU from=$SETTINGS_MENUS}
		<div class="quickWidget">
			<div class="accordion-heading accordion-toggle quickWidgetHeader" data-parent="#settingsQuickWidgetContainer" data-target="#Settings_sideBar_{Ottocrat_Util_Helper::replaceSpaceWithUnderScores($MENU->getLabel())}"
			...
			<div  style="border-bottom: 1px solid black;" class="widgetContainer accordion-body {if $SELECTED_MENU->get('blockid') eq $MENU->get('blockid')  && !empty($SELECTED_FIELDID)} in {/if} collapse" id="Settings_sideBar_{Ottocrat_Util_Helper::replaceSpaceWithUnderScores($MENU->getLabel())}">
