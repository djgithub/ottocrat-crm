						...
						<span><label class="checkbox" style="padding-left: 25px; padding-top: 5px;"><input type="hidden" name="defaultvalue" value="" />
								...
								{if $FIELD_MODEL->isDefaultValueOptionDisabled() neq "true"}
									...
									{elseif $FIELD_MODEL->getFieldDataType() eq "multipicklist"}
										...
										<select multiple class="span2" name="fieldDefaultValue" {if !$FIELD_MODEL->hasDefaultValue()} disabled="" {/if} data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]"  data-fieldinfo='{Ottocrat_Util_Helper::toSafeHTML(ZEND_JSON::encode($FIELD_INFO))}'>
									...
									{elseif $FIELD_MODEL->getFieldDataType() eq "date"}
										<div class="input-append date">
											...
											<input type="text" class="input-medium" name="fieldDefaultValue" data-toregister="date" {if !$FIELD_MODEL->hasDefaultValue()} disabled="" {/if} data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]"  data-date-format="{$USER_MODEL->get('date_format')}" data-fieldinfo='{ZEND_JSON::encode($FIELD_INFO)}'
									...
									{elseif $FIELD_MODEL->getFieldDataType() eq "percentage"}
										<div class="input-append">
											<input type="number" class="input-medium" name="fieldDefaultValue" {if !$FIELD_MODEL->hasDefaultValue()} disabled="" {/if} data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" 
									...
									{elseif $FIELD_MODEL->getFieldDataType() eq "currency"}
										<div class="input-prepend">
											...
											<input type="text" class="input-medium" name="fieldDefaultValue" {if !$FIELD_MODEL->hasDefaultValue()} disabled="" {/if} data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" 
									...
									{else}
										<input type="text" {if !$FIELD_MODEL->hasDefaultValue()} disabled="" {/if} data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" class="input-medium" name="fieldDefaultValue" value="{$FIELD_MODEL->get('defaultvalue')}" data-fieldinfo='{ZEND_JSON::encode($FIELD_INFO)}'/>
									{/if}
								{/if}
								</div>
							</span></div>
