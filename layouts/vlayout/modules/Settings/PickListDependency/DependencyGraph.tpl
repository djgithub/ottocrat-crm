...
{strip}
    ...
    {/foreach}
    <input type="hidden" class="allSourceValues" value='{Ottocrat_Util_Helper::toSafeHTML(ZEND_JSON::encode($SOURCE_PICKLIST_VALUES))}' />
        ...
        <div class="span10 marginLeftZero dependencyMapping">
            <table class="table-bordered table-condensed themeTableColor pickListDependencyTable">
                <thead><tr class="blockHeader">
                        {foreach item=SOURCE_PICKLIST_VALUE from=$SOURCE_PICKLIST_VALUES}
                            <th data-source-value="{Ottocrat_Util_Helper::toSafeHTML($SOURCE_PICKLIST_VALUE)}" style="
            ...
            <tbody>
                {foreach key=TARGET_INDEX item=TARGET_VALUE from=$TARGET_PICKLIST_VALUES name=targetValuesLoop}
                    <tr>
                        {foreach item=SOURCE_PICKLIST_VALUE from=$SOURCE_PICKLIST_VALUES}
                            {assign var=targetValues value=$MAPPED_TARGET_PICKLIST_VALUES[Ottocrat_Util_Helper::toSafeHTML($SOURCE_PICKLIST_VALUE)]}
                            ...
                            {/if}
                            <td	data-source-value='{Ottocrat_Util_Helper::toSafeHTML($SOURCE_PICKLIST_VALUE)}' data-target-value='{Ottocrat_Util_Helper::toSafeHTML($TARGET_VALUE)}'
                    ...
                    <td>
                        <div class="control-group">
                            <div class="controls row-fluid">
                                <label class="checkbox"><input type="checkbox" class="sourceValue {Ottocrat_Util_Helper::toSafeHTML($SOURCE_VALUE)}"
                                                               data-source-value="{Ottocrat_Util_Helper::toSafeHTML($SOURCE_VALUE)}" value="{Ottocrat_Util_Helper::toSafeHTML($SOURCE_VALUE)}" 
