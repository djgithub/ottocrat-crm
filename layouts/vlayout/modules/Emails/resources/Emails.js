...
//		return aDeferred.promise();
	...
	//configured ckeditor toolbar for ottocrat
	var Ottocrat_ckeditor_toolbar = 
	[
    ...
    CKEDITOR.replace( 'description',
	{
		...
		toolbar : Ottocrat_ckeditor_toolbar
	});
	...
	getComposeEmailForm : function(form){
		...
		AppConnector.request(massSendEmailUrl).then(
			function(data) {
				...
				Ottocrat_Emails_Js.loadCkEditor();
				Ottocrat_Emails_Js.registerEvents();
			},
			function(error,err){
	...
	registerComposeEmailStep1Events : function(){
		jQuery('.SendEmailFormStep1').on('submit',"#SendEmailFormStep1",function(e){
			...
			var result = Ottocrat_MultiSelect_Validator_Js.invokeValidation(chosenElement);
			if(result == true){
				Ottocrat_Emails_Js.getComposeEmailForm(form);
			} else {
	...
	registerEvents : function(){
		...
		composeEmailContainer.on('submit','#SendEmailFormStep2',function(e){
			var form = jQuery(e.currentTarget);
			Ottocrat_Emails_Js.SendEmailStep2(form);
			e.preventDefault();
		});
