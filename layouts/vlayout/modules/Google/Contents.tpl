...
<div id = 'sync_message' class='row-fluid' >
    <div class='padding10 span12'>     
        ...
        {if $STATE eq 'home'}
            {if $SYNCTIME}
            <p class="muted" id='synctime'><small title="{Ottocrat_Util_Helper::formatDateTimeIntoDayString($SYNCTIME)}">{vtranslate('LBL_SYNCRONIZED',$MODULE_NAME)} : {Ottocrat_Util_Helper::formatDateDiffInStrings($SYNCTIME)}</small></p>
            {else}
       ...
       {/if}
        <div class='row-fluid'>
            ...
            <button data-url={Ottocrat_Request::encryptLink("index.php?module=Google&view=List&operation=sync&sourcemodule={$SOURCEMODULE}")}  id="sync_button" class="btn btn-success span9" ><b>{vtranslate('LBL_SYNC_BUTTON',$MODULE_NAME)}</b></button>
        ...
        <div class='row-fluid {if !$FIRSTTIME}hide {/if}' id="removeSyncBlock">
            <span class='span0'>&nbsp;</span>
            <button id="remove_sync" class="btn btn-danger span9"  data-url={Ottocrat_Request::encryptLink("index.php?module=Google&view=List&operation=removeSync&sourcemodule={$SOURCEMODULE}")}><b>{vtranslate('LBL_REMOVE_SYNC',$MODULE_NAME)}</b></button>
            <span class="span0">
