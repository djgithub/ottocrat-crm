...
jQuery.Class("Contact",{
                        ...
                        AppConnector.request(url).then(
                           function(data) {
                               ...
                               if(jQuery('#norefresh').length == 0){
                                listInstance  =  Ottocrat_List_Js.getInstance();
    ...
    packFieldmappingsForSubmit : function(container) {
        ...
        jQuery.each(rows,function(index,row) {
            var tr = jQuery(row);
            var ottocrat_field_name = tr.find('.ottocrat_field_name').not('.select2-container').val();
            ...
            var map = {};
            map['ottocrat_field_name'] = ottocrat_field_name;
    ...
    validateFieldMappings : function(container) {
        ...
        var customMapElements = jQuery('select.ottocrat_field_name');
        ...
        container.find('button#save_syncsetting').on('click',function() {
            thisInstance.validateFieldMappings(container).then(function() {
                ...
                AppConnector.request(serializedFormData).then(function(data) {
                    ...
                    app.hideModalWindow();
                    Ottocrat_Helper_Js.showMessage({text: app.vtranslate('JS_SAVED_SUCCESSFULLY')});
                });
            }, function(inputs) {
    ...
    registerAddCustomFieldMappingEvent : function(container) {
        ...
        jQuery('.addCustomFieldMapping',container).on('click',function(e) {
            ...
            var ottocratFields = currentSelectionElement.data('ottocratfields');
            
            var ottocratFieldSelectElement = '<select class="ottocrat_field_name" style="width:200px" data-category="'+selectionType+'">';
            if(!Object.keys(ottocratFields).length) {
                alert(app.vtranslate('JS_SUITABLE_OTTOCRAT_FIELD_NOT_AVAILABLE_FOR_MAPPING'));
                return;
            }
            
            var customMapElements = jQuery('select.ottocrat_field_name[data-category="'+selectionType+'"]');
            var mappedCustomFields = [];
            jQuery.each(customMapElements,function(i,elem) {
                    mappedCustomFields.push(jQuery(elem).val());
            });
            var numberOfOptions = 0;
            jQuery.each(ottocratFields,function(fieldname,fieldLabel) {
                if(jQuery.inArray(fieldname,mappedCustomFields) === -1) {
                    numberOfOptions++;
                    var option = '<option value="'+fieldname+'">'+fieldLabel+'</option>';
                    ottocratFieldSelectElement += option;
                }
            });
            ...
            ottocratFieldSelectElement += '</select>';
            ...
            if(selectionType != 'custom') {
                ...
                googleTypeSelectElement += '</select>\n\
                                 &nbsp;&nbsp;<input type="text" class="google-custom-label" style="visibility:hidden;width:190px;" data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" />';
            } else {
                googleTypeSelectElement = '<input type="hidden" class="google_field_name" value="'+ googleFields[selectionType]['name'] +'" />';
                googleTypeSelectElement += '<input type="hidden" class="google-type" value="'+selectionType+'" />';
                googleTypeSelectElement += '<input type="text" class="google-custom-label" style="width:190px;" data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" />';
            }
            var tabRow = '<tr>\n\
                            <td>' + ottocratFieldSelectElement + '</td>\n\
            ...
            thisInstance.registerOttocratFieldSelectOnChangeEvent(container,lastRow.find('select.ottocrat_field_name'));
            thisInstance.registerGoogleTypeChangeEvent(container,lastRow.find('select.google-type'));
            lastRow.find('select.ottocrat_field_name').trigger('change');
    ...
    registerDeleteCustomFieldMappingEvent : function(container) {
        jQuery('.deleteCustomMapping',container).on('click',function() {
            ...
            var currentCategory = currentRow.find('select.ottocrat_field_name').data('category');
            currentRow.remove();
            jQuery('select.ottocrat_field_name[data-category="'+currentCategory+'"]').trigger('change');
    ...
    removeOptionFromSelectList : function(selectElement,optionValue,category) {
        ...
        var categorisedSelectElements = jQuery('select.ottocrat_field_name[data-category="'+category+'"]');
    ...
    registerOttocratFieldSelectOnChangeEvent : function(container,selectElement) {
        var thisInstance = this;
        if(typeof selectElement === 'undefined') {
            selectElement = jQuery('select.ottocrat_field_name',container);   
        }
        selectElement.on('change', function(e){
            var element = jQuery(e.currentTarget);
            var category = element.data('category');
            
            var allCategorizedSelects = jQuery('select.ottocrat_field_name[data-category="'+category+'"]');
            var selectedValues = [];
            ...
            jQuery.each(allCategorizedSelects, function(i, selectElement){
                if(e.currentTarget !== selectElement || allCategorizedSelects.length == 1) {
                    var allCategoryFieldLabelValues = jQuery('li.addCustomFieldMapping[data-type="'+category+'"]').data('ottocratfields');
    ...
    registerPostSettingRenderEvents : function(container) {
        ...
        this.registerOttocratFieldSelectOnChangeEvent(container);
        this.registerGoogleTypeChangeEvent(container);

        jQuery('select.ottocrat_field_name',container).trigger('change');
