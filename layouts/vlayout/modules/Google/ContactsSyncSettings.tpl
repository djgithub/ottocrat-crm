        ...
        <div class="modal-body">
            ...
            <div class="btn-group pull-right">
                ...
                <ul class="dropdown-menu dropdown-menu-left" role="menu">
                    <li class="addCustomFieldMapping" data-type="email" data-ottocratfields='{Zend_Json::encode($OTTOCRAT_EMAIL_FIELDS)}'><a>{vtranslate('LBL_EMAIL',$MODULENAME)}</a></li>
                    <li class="addCustomFieldMapping" data-type="phone" data-ottocratfields='{Zend_Json::encode($OTTOCRAT_PHONE_FIELDS)}'><a>{vtranslate('LBL_PHONE',$MODULENAME)}</a></li>
                    <li class="addCustomFieldMapping" data-type="url" data-ottocratfields='{Zend_Json::encode($OTTOCRAT_URL_FIELDS)}'><a>{vtranslate('LBL_URL',$MODULENAME)}</a></li>
                    <li class="divider"></li>
                    <li class="addCustomFieldMapping" data-type="custom" data-ottocratfields='{Zend_Json::encode($OTTOCRAT_OTHER_FIELDS)}'><a>{vtranslate('LBL_CUSTOM',$MODULENAME)}</a></li>
                </ul>
            </div>
            <div class="row-fluid">
                <div class="control-group">
                    ...
                    <div class="controls">
                        <select class="select2 stretched" name="sync_direction" style="width:250px;">
                            ...
                            <option value="10" {if $SYNC_DIRECTION eq '10'}selected{/if}>{vtranslate('LBL_ONLY_SYNC_FROM_GOOGLE_TO_OTTOCRAT',$MODULENAME)}</option> 
                            <option value="01" {if $SYNC_DIRECTION eq '01'}selected{/if}>{vtranslate('LBL_ONLY_SYNC_FROM_OTTOCRAT_TO_GOOGLE',$MODULENAME)}</option>
                        </select>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="hidden" class="ottocrat_field_name" value="{$FLDNAME}" />
                            </td>
                            <td>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="text" class="google-custom-label" style="visibility:{if $FIELD_MAPPING[$FLDNAME]['google_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                       ...
                                       data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]" />
                            </td>
                        </tr>
                        <tr>
                        ...
                        {foreach key=OTTOCRAT_FIELD_NAME item=CUSTOM_FIELD_MAP from=$CUSTOM_FIELD_MAPPING}
                            <tr>
                                <td>
                                    {if $CUSTOM_FIELD_MAP['google_field_name'] eq 'gd:email'}
                                        <select class="select2 stretched ottocrat_field_name" style="width:200px;" data-category="email">
                                            {foreach key=EMAIL_FIELD_NAME item=EMAIL_FIELD_LABEL from=$OTTOCRAT_EMAIL_FIELDS}
                                                <option value="{$EMAIL_FIELD_NAME}" {if $OTTOCRAT_FIELD_NAME eq $EMAIL_FIELD_NAME}selected{/if}>{vtranslate($EMAIL_FIELD_LABEL,$SOURCE_MODULE)}</option>
                                            {/foreach}
                                        </select>
                                    {else if $CUSTOM_FIELD_MAP['google_field_name'] eq 'gd:phoneNumber'}
                                        <select class="select2 stretched ottocrat_field_name" style="width:200px;" data-category="phone">
                                            {foreach key=PHONE_FIELD_NAME item=PHONE_FIELD_LABEL from=$OTTOCRAT_PHONE_FIELDS}
                                                <option value="{$PHONE_FIELD_NAME}" {if $OTTOCRAT_FIELD_NAME eq $PHONE_FIELD_NAME}selected{/if}>{vtranslate($PHONE_FIELD_LABEL,$SOURCE_MODULE)}</option>
                                            {/foreach}
                                        </select>
                                    {else if $CUSTOM_FIELD_MAP['google_field_name'] eq 'gContact:userDefinedField'}
                                        <select class="select2 stretched ottocrat_field_name" style="width:200px;" data-category="custom">
                                            {foreach key=OTHER_FIELD_NAME item=OTHER_FIELD_LABEL from=$OTTOCRAT_OTHER_FIELDS}
                                                <option value="{$OTHER_FIELD_NAME}" {if $OTTOCRAT_FIELD_NAME eq $OTHER_FIELD_NAME}selected{/if}>{vtranslate($OTHER_FIELD_LABEL,$SOURCE_MODULE)}</option>
                                            {/foreach}
                                        </select>
                                    {else if $CUSTOM_FIELD_MAP['google_field_name'] eq 'gContact:website'}
                                        <select class="select2 stretched ottocrat_field_name" style="width:200px;" data-category="url">
                                            {foreach key=URL_FIELD_NAME item=URL_FIELD_LABEL from=$OTTOCRAT_URL_FIELDS}
                                                <option value="{$URL_FIELD_NAME}" {if $OTTOCRAT_FIELD_NAME eq $URL_FIELD_NAME}selected{/if}>{vtranslate($URL_FIELD_LABEL,$SOURCE_MODULE)}</option>
                                            {/foreach}
                                        </select>
                                    {/if}
                                </td>
                                <td>
                                    ...
                                    {if $CUSTOM_FIELD_MAP['google_field_name'] eq 'gd:email'}
                                        ...
                                        <input type="text" class="google-custom-label" style="visibility:{if $CUSTOM_FIELD_MAP['google_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                               value="{if $CUSTOM_FIELD_MAP['google_field_type'] eq 'custom'}{$CUSTOM_FIELD_MAP['google_custom_label']}{/if}" data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]"/>
                                    {else if $CUSTOM_FIELD_MAP['google_field_name'] eq 'gd:phoneNumber'}
                                        {assign var=GOOGLE_TYPES value=$GOOGLE_FIELDS['phone']['types']}
                                    ...
                                    {else if $CUSTOM_FIELD_MAP['google_field_name'] eq 'gContact:userDefinedField'}
                                        ...
                                        <input type="text" class="google-custom-label" value="{$CUSTOM_FIELD_MAP['google_custom_label']}" style="width:190px;" data-validation-engine="validate[required,funcCall[Ottocrat_Base_Validator_Js.invokeValidation]]"/>
                                    {else if $CUSTOM_FIELD_MAP['google_field_name'] eq 'gContact:website'}
