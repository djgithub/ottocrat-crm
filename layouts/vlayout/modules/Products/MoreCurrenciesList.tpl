			...
			<table width="100%" border="0" cellpadding="5" cellspacing="0" class="table table-bordered">
				...
				{foreach item=price key=count from=$PRICE_DETAILS}
					<tr data-currency-id={$price.curname}>
						...
						<td>
							<div class="row-fluid">
								<input {$disable_value} type="text" size="10" class="span9 convertedPrice" name="{$price.curname}" id="{$price.curname}" value="{$price.curvalue}" data-validation-engine="validate[funcCall[Ottocrat_Currency_Validator_Js.invokeValidation]]" data-decimal-seperator='{$USER_MODEL->get('currency_decimal_separator')}' data-group-seperator='{$USER_MODEL->get('currency_grouping_separator')}' />
							</div>
						</td>
						<td>
							<div class="row-fluid">
