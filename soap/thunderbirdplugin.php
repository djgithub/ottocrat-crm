...
$server->configureWSDL('ottocratsoap');
...
function track_email($user_name,$password,$contact_ids, $date_sent, $email_subject, $email_body)
{
	if(authentication($user_name,$password))
	{
		...
		$email->save("Emails");
		$query = "select fieldid from ottocrat_field where fieldname = 'email' and tabid = 4 and ottocrat_field.presence in (0,2)";
		...
		$sql = "select email from ottocrat_contactdetails inner join ottocrat_crmentity on ottocrat_crmentity.crmid = ottocrat_contactdetails.contactid where ottocrat_crmentity.deleted =0 and ottocrat_contactdetails.contactid=?";
		...
		if(isset($camodulerow))
		{
			$emailid = $camodulerow["email"];
		    	...
		    	$user_emailid = str_replace('>','&gt;',$user_emailid);
			$query = 'insert into ottocrat_emaildetails values (?,?,?,?,?,?,?,?)';
...
function retrieve_account_id($account_name,$user_id)
{
	...
	$query = "select ottocrat_account.accountname accountname,ottocrat_account.accountid accountid from ottocrat_account inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_account.accountid where ottocrat_crmentity.deleted=0 and ottocrat_account.accountname=?";
...
function AddContact($user_name,$first_name, $last_name, $email_address ,$account_name , $salutation , $title, $phone_mobile, $reports_to,$primary_address_street,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country,$alt_address_city,$alt_address_street,$alt_address_state,$alt_address_postalcode,$alt_address_country,$office_phone="",$home_phone="",$fax="",$department="",$password,$description="")
{
	if(authentication($user_name,$password))
	{
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
	    	$sql1 = "select fieldname,columnname from ottocrat_field where tabid=4 and block <> 75 and block <> 6 and block <> 5 and ottocrat_field.presence in (0,2)";
			$params1 = array();
	  	} else {
	    	$profileList = getCurrentUserProfileList();
	    	$sql1 = "select fieldname,columnname from ottocrat_field inner join ottocrat_profile2field on ottocrat_profile2field.fieldid=ottocrat_field.fieldid inner join ottocrat_def_org_field on ottocrat_def_org_field.fieldid=ottocrat_field.fieldid where ottocrat_field.tabid=4 and ottocrat_field.block <> 75 and ottocrat_field.block <> 6 and ottocrat_field.block <> 5 and ottocrat_field.displaytype in (1,2,4) and ottocrat_profile2field.visible=0 and ottocrat_def_org_field.visible=0 and ottocrat_field.presence in (0,2)";
			$params1 = array();
...
function AddLead($user_name, $first_name, $last_name, $email_address ,$account_name , $salutation , $title, $phone_mobile, $reports_to ,$primary_address_street , $website ,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country,$alt_address_city,$alt_address_street,$alt_address_state,$alt_address_postalcode,$alt_address_country,$office_phone="",$home_phone="",$fax="",$department="",$password,$description="")
{
	if(authentication($user_name,$password))
	{
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
	    	$sql1 = "select fieldname,columnname from ottocrat_field where tabid=7 and block <> 14 and ottocrat_field.presence in (0,2)";
			$params1 = array();
	  	} else {
	    	$profileList = getCurrentUserProfileList();
	    	$sql1 = "select fieldname,columnname from ottocrat_field inner join ottocrat_profile2field on ottocrat_profile2field.fieldid=ottocrat_field.fieldid inner join ottocrat_def_org_field on ottocrat_def_org_field.fieldid=ottocrat_field.fieldid where ottocrat_field.tabid=7 and ottocrat_field.block <> 14 and ottocrat_field.displaytype in (1,2,4) and ottocrat_profile2field.visible=0 and ottocrat_def_org_field.visible=0 and ottocrat_field.presence in (0,2)";
	  		$params1 = array();
			if (count($profileList) > 0) {
				$sql1 .= " and ottocrat_profile2field.profileid in (". generateQuestionMarks($profileList) .")";
				array_push($params1, $profileList);
...
function create_session($user_name, $password,$version)
{
  ...
  include('ottocratversion.php');
	...
	if($password != "" && $user_name != '')
	{
		...
		{
			$query = "select id from ottocrat_users where user_name=? and user_password=?";
