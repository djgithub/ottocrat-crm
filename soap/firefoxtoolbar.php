...
function LogintoOttocratCRM($user_name,$password,$version)
{
	global $log,$adb;
	require_once('modules/Users/Users.php');
	include('ottocratversion.php');
	if($version != $ottocrat_current_version)
	{
...
function create_account($username,$sessionid,$accountname,$email,$phone,$primary_address_street,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country)
{
	...
	if(isPermitted("Accounts","EditView") == "yes")
	{
		$query = "SELECT accountname FROM ottocrat_account,ottocrat_crmentity WHERE accountname =? and ottocrat_account.accountid = ottocrat_crmentity.crmid and ottocrat_crmentity.deleted != 1";
...
function GetPicklistValues($username,$sessionid,$tablename)
{
	...
	{
		$query = "select " . $adb->sql_escape_string($tablename) . " from ottocrat_". $adb->sql_escape_string($tablename);		
	...
	{
		$query = "select " .$adb->sql_escape_string($tablename) . " from ottocrat_". $adb->sql_escape_string($tablename) ." inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_". $adb->sql_escape_string($tablename) .".picklist_valueid where roleid=? and picklistid in (select picklistid from ottocrat_". $adb->sql_escape_string($tablename)." ) order by sortid";	
...
function unsetServerSessionId($id)
{
	...
	$adb->query("delete from ottocrat_soapservice where type='FireFox' and id=$id");
...
function getServerSessionId($id)
{
	...
	$query = "select * from ottocrat_soapservice where type='FireFox' and id={$id}";
