...
$NAMESPACE = 'http://www.ottocrat.com/products/crm';
 ...
 */
class Ottocrat_Soap_CustomerPortal {
...
function _getTicketModComments($ticketId) {
	global $adb;
	$sql = "SELECT * FROM ottocrat_modcomments
			INNER JOIN ottocrat_crmentity ON ottocrat_modcomments.modcommentsid = ottocrat_crmentity.crmid AND deleted = 0
	...
	for($i=0; $i<$rows; $i++) {
		...
		if(!empty($customer)) {
			$emailResult = $adb->pquery('SELECT * FROM ottocrat_portalinfo WHERE id = ?', array($customer));
...
function get_combo_values($input_array)
{
	...
	$output = Array();
	$sql = "select  productid, productname from ottocrat_products inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_products.productid where ottocrat_crmentity.deleted=0";
	...
	$roleres = $adb->pquery("SELECT roleid from ottocrat_user2role where userid = ?",array($userid));
	$RowCount = $adb->num_rows($roleres);
	if($RowCount > 0){
		$admin_role = $adb->query_result($roleres,0,'roleid');
	}
	$result1 = $adb->pquery("select ottocrat_ticketpriorities.ticketpriorities from ottocrat_ticketpriorities inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_ticketpriorities.picklist_valueid and ottocrat_role2picklist.roleid='$admin_role' order by sortorderid", array());
	for($i=0;$i<$adb->num_rows($result1);$i++)
	{
		$output['ticketpriorities']['ticketpriorities'][$i] = $adb->query_result($result1,$i,"ticketpriorities");
	}

	$result2 = $adb->pquery("select ottocrat_ticketseverities.ticketseverities from ottocrat_ticketseverities inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_ticketseverities.picklist_valueid and ottocrat_role2picklist.roleid='$admin_role' order by sortorderid", array());
	for($i=0;$i<$adb->num_rows($result2);$i++)
	{
		$output['ticketseverities']['ticketseverities'][$i] = $adb->query_result($result2,$i,"ticketseverities");
	}

	$result3 = $adb->pquery("select ottocrat_ticketcategories.ticketcategories from ottocrat_ticketcategories inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_ticketcategories.picklist_valueid and ottocrat_role2picklist.roleid='$admin_role' order by sortorderid", array());
	...
	if(!vtlib_isModuleActive('ServiceContracts')) {
		...
		$servicequery = "SELECT ottocrat_servicecontracts.servicecontractsid,ottocrat_servicecontracts.subject
							FROM ottocrat_servicecontracts
							INNER JOIN ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_servicecontracts.servicecontractsid
									AND ottocrat_crmentity.deleted = 0
							WHERE ottocrat_servicecontracts.sc_related_to = ?";
		$params = array($id);
		$showAll = show_all('HelpDesk');
		if($showAll == 'true') {
			$servicequery .= ' OR ottocrat_servicecontracts.sc_related_to = (SELECT accountid FROM ottocrat_contactdetails WHERE contactid=? AND accountid <> 0)
								OR ottocrat_servicecontracts.sc_related_to IN
											(SELECT contactid FROM ottocrat_contactdetails WHERE accountid =
													(SELECT accountid FROM ottocrat_contactdetails WHERE contactid=? AND accountid <> 0))
							';
...
function get_KBase_details($input_array)
{
	...
	$category_query = "select ottocrat_faqcategories.faqcategories from ottocrat_faqcategories inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_faqcategories.picklist_valueid and ottocrat_role2picklist.roleid='$admin_role'";
	...
	if($check == true) {
		$product_query = "select productid, productname from ottocrat_products inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_products.productid where ottocrat_crmentity.deleted=0";
	...
	$faq_query = "select ottocrat_faq.*, ottocrat_crmentity.createdtime, ottocrat_crmentity.modifiedtime from ottocrat_faq " .
		"inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_faq.id " .
		"where ottocrat_crmentity.deleted=0 and ottocrat_faq.status='Published' order by ottocrat_crmentity.modifiedtime DESC";
	...
	for($k=0;$k<$faq_noofrows;$k++)
	{
		...
		$faq_comment_query = "select * from ottocrat_faqcomments where faqid=? order by createdtime DESC";
...
function save_faq_comment($input_array)
{
	...
	if(trim($comment) != '')
	{
		$faq_query = "insert into ottocrat_faqcomments values(?,?,?,?)";
...
function get_tickets_list($input_array) {
	...
	{
		$contactquery = "SELECT contactid, accountid FROM ottocrat_contactdetails " .
			" INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_contactdetails.contactid" .
			" AND ottocrat_crmentity.deleted = 0 " .
			" WHERE (accountid = (SELECT accountid FROM ottocrat_contactdetails WHERE contactid = ?)  AND accountid != 0) OR contactid = ?";
	...
	$query = "SELECT ottocrat_troubletickets.*, ottocrat_crmentity.smownerid,ottocrat_crmentity.createdtime, ottocrat_crmentity.modifiedtime, '' AS setype
		FROM ottocrat_troubletickets
		INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_troubletickets.ticketid AND ottocrat_crmentity.deleted = 0
		WHERE (ottocrat_troubletickets.contact_id IN (". generateQuestionMarks($entity_ids_list) .")";
	if($acc_id) {
		$query .= " OR ottocrat_troubletickets.parent_id = $acc_id) ";
	} else {
		$query .= ')';
	}
	...
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldlabel => $fieldname) {
			...
			if($fieldname == 'title'){
				$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=HelpDesk&action=index&fun=detail&ticketid='.$ticketid).'">'.$fieldvalue.'</a>';
			}
			if($fieldname == 'parent_id') {
				$crmid = $fieldvalue;
				if ($crmid != '') {
					$fieldvalues = getEntityName('Accounts', array($crmid));
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Accounts&action=index&id='.$crmid).'">'.$fieldvalues[$crmid].'</a>';
				} else {
					$fieldvalue = '';
				}
			}
			if($fieldname == 'contact_id') {
				if(!empty($fieldvalue)) {
					...
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Contacts&action=index&id='.$fieldvalue).'">'.$fieldvalues[$fieldvalue].'</a>';
				} else {
					$fieldvalue = '';
				}
			}
...
function create_ticket($input_array)
{
	...
	$accountResult = $adb->pquery('SELECT accountid FROM ottocrat_contactdetails WHERE contactid = ?', array($parent_id));
	$accountId = $adb->query_result($accountResult, 0, 'accountid');
	if(!empty($accountId)) $ticket->column_fields['parent_id'] = $accountId;

	$ticket->save("HelpDesk");

	$ticketresult = $adb->pquery("select ottocrat_troubletickets.ticketid from ottocrat_troubletickets
		inner join ottocrat_crmentity on ottocrat_crmentity.crmid = ottocrat_troubletickets.ticketid inner join ottocrat_ticketcf on ottocrat_ticketcf.ticketid = ottocrat_troubletickets.ticketid
		where ottocrat_crmentity.deleted=0 and ottocrat_troubletickets.ticketid = ?", array($ticket->id));
	...
	if($servicecontractid != ''){
		$res = $adb->pquery("insert into ottocrat_crmentityrel values(?,?,?,?)",
...
function authenticate_user($username,$password,$version,$login = 'true')
{
	...
	$sql = "select id, user_name, user_password,last_login_time, support_start_date, support_end_date
				from ottocrat_portalinfo
					inner join ottocrat_customerdetails on ottocrat_portalinfo.id=ottocrat_customerdetails.customerid
					inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_portalinfo.id
				where ottocrat_crmentity.deleted=0 and user_name=? and user_password = ?
					and isactive=1 and ottocrat_customerdetails.portal=1
					and ottocrat_customerdetails.support_start_date <= ? and ottocrat_customerdetails.support_end_date >= ?";
	...
	if($login != 'false')
	{
		...
		$sql="insert into ottocrat_soapservice values(?,?,?)";
...
function change_password($input_array)
{
	...
	$sql = "update ottocrat_portalinfo set user_password=? where id=? and user_name=?";
...
function update_login_details($input_array)
{
	...
	if($flag == 'login')
	{
		$sql = "update ottocrat_portalinfo set login_time=? where id=?";
		$result = $adb->pquery($sql, array($current_time, $id));
	}
	elseif($flag == 'logout')
	{
		$sql = "update ottocrat_portalinfo set logout_time=?, last_login_time=login_time where id=?";
		$result = $adb->pquery($sql, array($current_time, $id));
	}
...
function send_mail_for_password($mailid)
{
	...
	$sql = "select * from ottocrat_portalinfo  where user_name = ? ";
	...
	$fromquery = "select ottocrat_users.user_name, ottocrat_users.email1 from ottocrat_users inner join ottocrat_crmentity on ottocrat_users.id = ottocrat_crmentity.smownerid inner join ottocrat_contactdetails on ottocrat_contactdetails.contactid=ottocrat_crmentity.crmid where ottocrat_contactdetails.email =?";
	...
	$mailserverresult = $adb->pquery("select * from ottocrat_systems where server_type=?", array('email'));
...
function get_ticket_creator($input_array)
{
	...
	$res = $adb->pquery("select smcreatorid from ottocrat_crmentity where crmid=?", array($ticketid));
...
function get_picklists($input_array)
{
	...
	$res = $adb->pquery("select ottocrat_". $picklist_name.".* from ottocrat_". $picklist_name." inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_". $picklist_name.".picklist_valueid and ottocrat_role2picklist.roleid='$admin_role'", array());
...
function get_ticket_attachments($input_array)
{
	...
	$query = "select ottocrat_troubletickets.ticketid, ottocrat_attachments.*,ottocrat_notes.filename,ottocrat_notes.filelocationtype from ottocrat_troubletickets " .
		"left join ottocrat_senotesrel on ottocrat_senotesrel.crmid=ottocrat_troubletickets.ticketid " .
		"left join ottocrat_notes on ottocrat_notes.notesid=ottocrat_senotesrel.notesid " .
		"inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_notes.notesid " .
		"left join ottocrat_seattachmentsrel on ottocrat_seattachmentsrel.crmid=ottocrat_notes.notesid " .
		"left join ottocrat_attachments on ottocrat_attachments.attachmentsid = ottocrat_seattachmentsrel.attachmentsid " .
		"and ottocrat_crmentity.deleted = 0 where ottocrat_troubletickets.ticketid =?";
...
function get_filecontent($input_array)
{
	...
	$query = 'SELECT ottocrat_attachments.path FROM ottocrat_attachments
	INNER JOIN ottocrat_seattachmentsrel ON ottocrat_seattachmentsrel.attachmentsid = ottocrat_attachments.attachmentsid
	INNER JOIN ottocrat_notes ON ottocrat_notes.notesid = ottocrat_seattachmentsrel.crmid
	INNER JOIN ottocrat_senotesrel ON ottocrat_senotesrel.notesid = ottocrat_notes.notesid
	INNER JOIN ottocrat_troubletickets ON ottocrat_troubletickets.ticketid = ottocrat_senotesrel.crmid
	WHERE ottocrat_troubletickets.ticketid = ? AND ottocrat_attachments.name = ? AND ottocrat_attachments.attachmentsid = ?';
...
function add_ticket_attachment($input_array)
{
	...
	$attachmentid = $adb->getUniqueID("ottocrat_crmentity");
	...
	$crmquery = "insert into ottocrat_crmentity (crmid,setype,description,createdtime) values(?,?,?,?)";
	$crmresult = $adb->pquery($crmquery, array($attachmentid, 'HelpDesk Attachment', $description, $date_var));

	$attachmentquery = "insert into ottocrat_attachments(attachmentsid,name,description,type,path) values(?,?,?,?,?)";
	$attachmentreulst = $adb->pquery($attachmentquery, array($attachmentid, $filename, $description, $filetype, $upload_filepath));

	$relatedquery = "insert into ottocrat_seattachmentsrel values(?,?)";
	...
	$related_doc = 'insert into ottocrat_seattachmentsrel values (?,?)';
	$res = $adb->pquery($related_doc,array($focus->id,$attachmentid));

	$tic_doc = 'insert into ottocrat_senotesrel values(?,?)';
...
function getServerSessionId($id)
{
	...
	$sessionid = Ottocrat_Soap_CustomerPortal::lookupSessionId($id);
	if($sessionid === false) {
		$query = "select * from ottocrat_soapservice where type='customer' and id=?";
		$result = $adb->pquery($query, array($id));
		if($adb->num_rows($result) > 0) {
			$sessionid = $adb->query_result($result,0,'sessionid');
			Ottocrat_Soap_CustomerPortal::updateSessionId($id, $sessionid);
		}
	}
	return $sessionid;
}
...
function unsetServerSessionId($id)
{
	...
	$id = (int) $id;
	Ottocrat_Soap_CustomerPortal::updateSessionId($id, false);

	$adb->pquery("delete from ottocrat_soapservice where type='customer' and id=?", array($id));
...
function get_account_name($accountid)
{
	...
	$res = $adb->pquery("select accountname from ottocrat_account where accountid=?", array($accountid));
...
function get_contact_name($contactid)
{
	...
	if($contactid != '')
	{
		$sql = "select firstname,lastname from ottocrat_contactdetails where contactid=?";
...
function get_check_account_id($id)
{
	...
	$res = $adb->pquery("select accountid from ottocrat_contactdetails where contactid=?", array($id));
...
function get_vendor_name($vendorid)
{
	...
	$res = $adb->pquery("select vendorname from ottocrat_vendor where vendorid=?", array($vendorid));
...
function get_list_values($id,$module,$sessionid,$only_mine='true')
{
	...
	if($module == 'Quotes')
	{
		$query = "select distinct ottocrat_quotes.*,ottocrat_crmentity.smownerid,
		case when ottocrat_quotes.contactid is not null then ottocrat_quotes.contactid else ottocrat_quotes.accountid end as entityid,
		case when ottocrat_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype,
		ottocrat_potential.potentialname,ottocrat_account.accountid
		from ottocrat_quotes left join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_quotes.quoteid
		...
		where ottocrat_crmentity.deleted=0 and (ottocrat_quotes.accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
	...
	{
		$query ="select distinct ottocrat_invoice.*,ottocrat_crmentity.smownerid,
		case when ottocrat_invoice.contactid !=0 then ottocrat_invoice.contactid else ottocrat_invoice.accountid end as entityid,
		case when ottocrat_invoice.contactid !=0 then 'Contacts' else 'Accounts' end as setype
		from ottocrat_invoice
		left join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_invoice.invoiceid
		where ottocrat_crmentity.deleted=0 and (accountid in (". generateQuestionMarks($entity_ids_list) .") or contactid in  (". generateQuestionMarks($entity_ids_list) ."))";
		$params = array($entity_ids_list,$entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}
	else if ($module == 'Documents')
	{
		$query ="select ottocrat_notes.*, ottocrat_crmentity.*, ottocrat_senotesrel.crmid as entityid, '' as setype,ottocrat_attachmentsfolder.foldername from ottocrat_notes " .
		"inner join ottocrat_crmentity on ottocrat_crmentity.crmid = ottocrat_notes.notesid " .
		"left join ottocrat_senotesrel on ottocrat_senotesrel.notesid=ottocrat_notes.notesid " .
		"LEFT JOIN ottocrat_attachmentsfolder ON ottocrat_attachmentsfolder.folderid = ottocrat_notes.folderid " .
		"where ottocrat_crmentity.deleted = 0 and  ottocrat_senotesrel.crmid in (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}else if ($module == 'Contacts'){
		$query = "select ottocrat_contactdetails.*,ottocrat_crmentity.smownerid from ottocrat_contactdetails
		 inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_contactdetails.contactid
		 where ottocrat_crmentity.deleted = 0 and contactid IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
	}else if ($module == 'Assets') {
		$accountRes = $adb->pquery("SELECT accountid FROM ottocrat_contactdetails
						INNER JOIN ottocrat_crmentity ON ottocrat_contactdetails.contactid = ottocrat_crmentity.crmid
		...
		$query = "select ottocrat_assets.*, ottocrat_assets.account as entityid , ottocrat_crmentity.smownerid from ottocrat_assets
						inner join ottocrat_crmentity on ottocrat_assets.assetsid = ottocrat_crmentity.crmid
						left join ottocrat_account on ottocrat_account.accountid = ottocrat_assets.account
						left join ottocrat_products on ottocrat_products.productid = ottocrat_assets.product
						where ottocrat_crmentity.deleted = 0 and account = ?";
		$params = array($accountid);
		$fields_list['Related To'] = 'entityid';
		}
	}else if ($module == 'Project') {
		$query = "SELECT ottocrat_project.*, ottocrat_crmentity.smownerid
					FROM ottocrat_project
					INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_project.projectid
					WHERE ottocrat_crmentity.deleted = 0 AND ottocrat_project.linktoaccountscontacts IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($module == 'Quotes')
			{
				if($fieldname =='subject'){
					...
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?&module=Quotes&action=index&id='.$fieldid).'">'.$fieldvalue.'</a>';
				}
			...
			if($module == 'Invoice')
			{
				if($fieldname =='subject'){
					...
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?&module=Invoice&action=index&status=true&id='.$fieldid).'">'.$fieldvalue.'</a>';
				}
				if(in_array($fieldname, $fieldValuesToRound)){
					$fieldvalue = round($fieldvalue, 2);
				}
			...
			if($module == 'Documents')
			{
				if($fieldname == 'title'){
					...
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?&module=Documents&action=index&id='.$fieldid).'">'.$fieldvalue.'</a>';
				}
				if( $fieldname == 'filename'){
					...
					if($fileactive == 1){
						if($filetype == 'I'){
							$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?&downloadfile=true&folderid='.$folderid.'&filename='.$filename.'&module=Documents&action=index&id='.$fieldid).'">'.$fieldvalue.'</a>';
						}
						elseif($filetype == 'E'){
							$fieldvalue = '<a target="_blank" href="'.Ottocrat_Request:: encryptLink($filename).'" onclick = "updateCount('.$fieldid.');">'.$filename.'</a>';
						}
					}else{
						$fieldvalue = $filename;
					}
			...
			if($module == 'Services'){
				if($fieldname == 'servicename'){
					...
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Services&action=index&id='.$fieldid).'">'.$fieldvalue.'</a>';
				}
			...
			if($module == 'Contacts'){
				if($fieldname == 'lastname' || $fieldname == 'firstname'){
					...
					$fieldvalue ='<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Contacts&action=index&id='.$fieldid).'">'.$fieldvalue.'</a>';
				}
			}
			if($module == 'Project'){
				if($fieldname == 'projectname'){
					$fieldid = $adb->query_result($res,$j,'projectid');
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Project&action=index&id='.$fieldid).'">'.$fieldvalue.'</a>';
				}
			}
			if($fieldname == 'entityid' || $fieldname == 'contactid' || $fieldname == 'accountid' || $fieldname == 'potentialid' || $fieldname == 'account' || $fieldname == 'linktoaccountscontacts') {
				...
				if ($crmid != '' && $modulename != '') {
					...
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Contacts&action=index&id='.$crmid).'">'.$fieldvalues[$crmid].'</a>';
			...
			if($module == 'Assets' && $fieldname == 'assetname') {
					...
					$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Assets&action=index&id='.$assetid).'">'.$assetname.'</a>';
			}
			if($fieldname == 'product' && $module == 'Assets'){
				$crmid= $adb->query_result($res,$j,'product');
				$fres = $adb->pquery('select ottocrat_products.productname from ottocrat_products where productid=?',array($crmid));
				$productname = $adb->query_result($fres,0,'productname');
				$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Products&action=index&id='.$crmid).'">'.$productname.'</a>';
			}
			if($fieldname == 'smownerid'){
				$fieldvalue = getOwnerName($fieldvalue);
			}
...
function get_filecontent_detail($id,$folderid,$module,$customerid,$sessionid)
{
	...
	if($module == 'Documents')
	{
		$query="SELECT filetype FROM ottocrat_notes WHERE notesid =?";
		$res = $adb->pquery($query, array($id));
		$filetype = $adb->query_result($res, 0, "filetype");
		updateDownloadCount($id);

		$fileidQuery = 'select attachmentsid from ottocrat_seattachmentsrel where crmid = ?';
		$fileres = $adb->pquery($fileidQuery,array($id));
		$fileid = $adb->query_result($fileres,0,'attachmentsid');

		$filepathQuery = 'select path,name from ottocrat_attachments where attachmentsid = ?';
	...
	{
		$query ='select ottocrat_attachments.*,ottocrat_seattachmentsrel.* from ottocrat_attachments inner join ottocrat_seattachmentsrel on ottocrat_seattachmentsrel.attachmentsid=ottocrat_attachments.attachmentsid where ottocrat_seattachmentsrel.crmid =?';
...
function updateDownloadCount($id){
	...
	$updateDownloadCount = "UPDATE ottocrat_notes SET filedownloadcount = filedownloadcount+1 WHERE notesid = ?";
...
function get_salesorder_name($id)
{
	...
	$res = $adb->pquery(" select subject from ottocrat_salesorder where salesorderid=?", array($id));
...
function get_invoice_detail($id,$module,$customerid,$sessionid)
{
	...
	$fieldquery = "SELECT fieldname, columnname, fieldlabel,block,uitype FROM ottocrat_field WHERE tabid = ? AND displaytype in (1,2,4) ORDER BY block,sequence";
	$fieldres = $adb->pquery($fieldquery,array(getTabid($module)));
	$nooffields = $adb->num_rows($fieldres);
	$query = "select ottocrat_invoice.*,ottocrat_crmentity.* ,ottocrat_invoicebillads.*,ottocrat_invoiceshipads.*,
		ottocrat_invoicecf.* from ottocrat_invoice
		inner join ottocrat_crmentity on ottocrat_crmentity.crmid = ottocrat_invoice.invoiceid
		LEFT JOIN ottocrat_invoicebillads ON ottocrat_invoice.invoiceid = ottocrat_invoicebillads.invoicebilladdressid
		LEFT JOIN ottocrat_invoiceshipads ON ottocrat_invoice.invoiceid = ottocrat_invoiceshipads.invoiceshipaddressid
		INNER JOIN ottocrat_invoicecf ON ottocrat_invoice.invoiceid = ottocrat_invoicecf.invoiceid
		where ottocrat_invoice.invoiceid=?";
	$res = $adb->pquery($query, array($id));
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		$blocknameQuery = "select blocklabel from ottocrat_blocks where blockid = ?";
		...
		if($fieldname == 'subject' && $fieldvalue !='')
		{
			...
			$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?downloadfile=true&module=Invoice&action=index&id='.$fieldid).'">'.$fieldvalue.'</a>';
		}
...
function get_product_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	$query = array();
	$params = array();
	$query[] = "SELECT ottocrat_products.*,ottocrat_seproductsrel.crmid as entityid, ottocrat_seproductsrel.setype FROM ottocrat_products
		INNER JOIN ottocrat_crmentity on ottocrat_products.productid = ottocrat_crmentity.crmid
		LEFT JOIN ottocrat_seproductsrel on ottocrat_seproductsrel.productid = ottocrat_products.productid
		WHERE ottocrat_seproductsrel.crmid in (". generateQuestionMarks($entity_ids_list).") and ottocrat_crmentity.deleted = 0 ";
	$params[] = array($entity_ids_list);

	$checkQuotes = checkModuleActive('Quotes');
	if($checkQuotes == true){
		$query[] = "select distinct ottocrat_products.*,
			case when ottocrat_quotes.contactid is not null then ottocrat_quotes.contactid else ottocrat_quotes.accountid end as entityid,
			case when ottocrat_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype
			from ottocrat_quotes INNER join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_quotes.quoteid
			...
			where ottocrat_inventoryproductrel.productid = ottocrat_products.productid AND ottocrat_crmentity.deleted=0 and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
	...
	for($k=0;$k<count($query);$k++)
	{
		...
		for( $j= 0;$j < $noofdata[$k]; $j++)
		{
			...
			foreach($fields_list as $fieldlabel=> $fieldname) {
				...
				$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Products&action=index&productid='.$fieldid).'">'.$fieldvalue.'</a>';
...
function get_details($id,$module,$customerid,$sessionid)
{
	...
	if($module == 'Quotes'){
		$query =  "SELECT
			ottocrat_quotes.*,ottocrat_crmentity.*,ottocrat_quotesbillads.*,ottocrat_quotesshipads.*,
			ottocrat_quotescf.* FROM ottocrat_quotes
			...
			WHERE ottocrat_quotes.quoteid=(". generateQuestionMarks($id) .") AND ottocrat_crmentity.deleted = 0";

	}
	else if($module == 'Documents'){
		$query =  "SELECT
			ottocrat_notes.*,ottocrat_crmentity.*,ottocrat_attachmentsfolder.foldername,ottocrat_notescf.*
			FROM ottocrat_notes
			INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_notes.notesid
			...
			WHERE ottocrat_notes.notesid=(". generateQuestionMarks($id) .") AND ottocrat_crmentity.deleted=0";
	}
	else if($module == 'HelpDesk'){
		$query ="SELECT
			ottocrat_troubletickets.*,ottocrat_crmentity.smownerid,ottocrat_crmentity.createdtime,ottocrat_crmentity.modifiedtime,
			ottocrat_ticketcf.*,ottocrat_crmentity.description  FROM ottocrat_troubletickets
			...
			WHERE (ottocrat_troubletickets.ticketid=(". generateQuestionMarks($id) .") AND ottocrat_crmentity.deleted = 0)";
	}
	else if($module == 'Services'){
		$query ="SELECT ottocrat_service.*,ottocrat_crmentity.*,ottocrat_servicecf.*  FROM ottocrat_service
			INNER JOIN ottocrat_crmentity
				ON ottocrat_crmentity.crmid = ottocrat_service.serviceid AND ottocrat_crmentity.deleted = 0
			LEFT JOIN ottocrat_servicecf
				ON ottocrat_service.serviceid = ottocrat_servicecf.serviceid
			WHERE ottocrat_service.serviceid= (". generateQuestionMarks($id) .")";
	}
	else if($module == 'Contacts'){
		$query = "SELECT ottocrat_contactdetails.*,ottocrat_contactaddress.*,ottocrat_contactsubdetails.*,ottocrat_contactscf.*" .
			" ,ottocrat_crmentity.*,ottocrat_customerdetails.*
			...
			WHERE ottocrat_contactdetails.contactid = (". generateQuestionMarks($id) .") AND ottocrat_crmentity.deleted = 0";
	}
	else if($module == 'Accounts'){
		$query = "SELECT ottocrat_account.*,ottocrat_accountbillads.*,ottocrat_accountshipads.*,ottocrat_accountscf.*,
			ottocrat_crmentity.* FROM ottocrat_account
			INNER JOIN ottocrat_crmentity
		...
		" WHERE ottocrat_account.accountid = (". generateQuestionMarks($id) .") AND ottocrat_crmentity.deleted = 0";
	}
	else if ($module == 'Products'){
		$query = "SELECT ottocrat_products.*,ottocrat_productcf.*,ottocrat_crmentity.* " .
		"FROM ottocrat_products " .
		"INNER JOIN ottocrat_crmentity " .
			"ON ottocrat_crmentity.crmid = ottocrat_products.productid " .
		"LEFT JOIN ottocrat_productcf " .
			"ON ottocrat_productcf.productid = ottocrat_products.productid " .
		"LEFT JOIN ottocrat_vendor
			ON ottocrat_vendor.vendorid = ottocrat_products.vendor_id " .
		"WHERE ottocrat_products.productid = (". generateQuestionMarks($id) .") AND ottocrat_crmentity.deleted = 0";
	} else if($module == 'Assets') {
		$query = "SELECT ottocrat_assets.*, ottocrat_assetscf.*, ottocrat_crmentity.*
		FROM ottocrat_assets
		INNER JOIN ottocrat_crmentity
		ON ottocrat_assets.assetsid = ottocrat_crmentity.crmid
		INNER JOIN ottocrat_assetscf
		ON ottocrat_assetscf.assetsid = ottocrat_assets.assetsid
		WHERE ottocrat_crmentity.deleted = 0 AND ottocrat_assets.assetsid = (". generateQuestionMarks($id) .")";
	} else if ($module == 'Project') {
		$query = "SELECT ottocrat_project.*, ottocrat_projectcf.*, ottocrat_crmentity.*
					FROM ottocrat_project
					INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_project.projectid
					LEFT JOIN ottocrat_projectcf ON ottocrat_projectcf.projectid = ottocrat_project.projectid
					WHERE ottocrat_project.projectid = ? AND ottocrat_crmentity.deleted = 0";
	}

	$params = array($id);
	$res = $adb->pquery($query,$params);

	$fieldquery = "SELECT fieldname,columnname,fieldlabel,blocklabel,uitype FROM ottocrat_field
		INNER JOIN  ottocrat_blocks on ottocrat_blocks.blockid=ottocrat_field.block WHERE ottocrat_field.tabid = ? AND displaytype in (1,2,4)
		ORDER BY ottocrat_field.block,ottocrat_field.sequence";
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		if($module=='Quotes')
		{
			if($fieldname == 'subject' && $fieldvalue !=''){
				...
				$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?downloadfile=true&module=Quotes&action=index&id='.$fieldid).'">'.$fieldvalue.'</a>';
			}
			if($fieldname == 'total'){
				$sym = getCurrencySymbol($res,0,'currency_id');
				$fieldvalue = $sym.$fieldvalue;
		...
		if($columnname == 'product_id') {
			...
			$fieldvalue = '<a href="'.Ottocrat_Request:: encryptLink('index.php?module=Products&action=index&productid='.$fieldvalue).'">'.$fieldvalues[$fieldvalue].'</a>';
		}
		if($module == 'Products'){
		...
		if($module == 'Assets' ){
			if($fieldname == 'account'){
				...
				$accountres = $adb->pquery("select ottocrat_account.accountname from ottocrat_account where accountid=?",array($accountid));
				$accountname = $adb->query_result($accountres,0,'accountname');
				$fieldvalue = $accountname;
			}
			if($fieldname == 'product'){
				$productid = $adb->query_result($res,0,'product');
				$productres = $adb->pquery("select ottocrat_products.productname from ottocrat_products where productid=?",array($productid));
			...
			if($fieldname == 'invoiceid'){
				...
				$invoiceres = $adb->pquery("select ottocrat_invoice.subject from ottocrat_invoice where invoiceid=?",array($invoiceid));
...
function check_permission($customerid, $module, $entityid) {
	...
	$faqquery = "select id from ottocrat_faq";
	...
	switch($module) {
		case 'Products'	: 	$query = "SELECT ottocrat_seproductsrel.productid FROM ottocrat_seproductsrel
								INNER JOIN ottocrat_crmentity
								ON ottocrat_seproductsrel.productid=ottocrat_crmentity.crmid
								WHERE ottocrat_seproductsrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts).")
									AND ottocrat_crmentity.deleted=0
									AND ottocrat_seproductsrel.productid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
							$query = "SELECT ottocrat_inventoryproductrel.productid, ottocrat_inventoryproductrel.id
													FROM ottocrat_inventoryproductrel
													INNER JOIN ottocrat_crmentity
													ON ottocrat_inventoryproductrel.productid=ottocrat_crmentity.crmid
													LEFT JOIN ottocrat_quotes
													ON ottocrat_inventoryproductrel.id = ottocrat_quotes.quoteid
													WHERE ottocrat_crmentity.deleted=0
														AND (ottocrat_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or ottocrat_quotes.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
														AND ottocrat_inventoryproductrel.productid = ?";
							...
							$query = "SELECT ottocrat_inventoryproductrel.productid, ottocrat_inventoryproductrel.id
													...
													INNER JOIN ottocrat_crmentity
													ON ottocrat_inventoryproductrel.productid=ottocrat_crmentity.crmid
													LEFT JOIN ottocrat_invoice
													ON ottocrat_inventoryproductrel.id = ottocrat_invoice.invoiceid
													WHERE ottocrat_crmentity.deleted=0
														AND (ottocrat_invoice.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or ottocrat_invoice.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
														AND ottocrat_inventoryproductrel.productid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
		...
		case 'Quotes'	:	$query = "SELECT ottocrat_quotes.quoteid
								FROM ottocrat_quotes
								INNER JOIN ottocrat_crmentity
								ON ottocrat_quotes.quoteid=ottocrat_crmentity.crmid
								WHERE ottocrat_crmentity.deleted=0
									AND (ottocrat_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or ottocrat_quotes.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND ottocrat_quotes.quoteid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Invoice'	:	$query = "SELECT ottocrat_invoice.invoiceid
								FROM ottocrat_invoice
								INNER JOIN ottocrat_crmentity
								ON ottocrat_invoice.invoiceid=ottocrat_crmentity.crmid
								WHERE ottocrat_crmentity.deleted=0
									AND (ottocrat_invoice.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or ottocrat_invoice.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND ottocrat_invoice.invoiceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Documents'	: 	$query = "SELECT ottocrat_senotesrel.notesid FROM ottocrat_senotesrel
									INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_senotesrel.notesid AND ottocrat_crmentity.deleted = 0
									WHERE ottocrat_senotesrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND ottocrat_senotesrel.notesid = ?";
								$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
								if ($adb->num_rows($res) > 0) {
									return true;
								}
								if(checkModuleActive('Project')) {
									$query = "SELECT ottocrat_senotesrel.notesid FROM ottocrat_senotesrel
										INNER JOIN ottocrat_project ON ottocrat_project.projectid = ottocrat_senotesrel.crmid
										WHERE ottocrat_project.linktoaccountscontacts IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
										AND ottocrat_senotesrel.notesid = ?";
									$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
									if ($adb->num_rows($res) > 0) {
										return true;
								...
								$query = "SELECT ottocrat_senotesrel.notesid FROM ottocrat_senotesrel
															INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_senotesrel.notesid AND ottocrat_crmentity.deleted = 0
															WHERE ottocrat_senotesrel.crmid IN (". generateQuestionMarks($faq_id) .")
															AND ottocrat_senotesrel.notesid = ?";
		...
		case 'HelpDesk'	:	if($acc_id) $accCondition = "OR ottocrat_troubletickets.parent_id = $acc_id";
							$query = "SELECT ottocrat_troubletickets.ticketid FROM ottocrat_troubletickets
									INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_troubletickets.ticketid AND ottocrat_crmentity.deleted = 0
									WHERE (ottocrat_troubletickets.contact_id IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") $accCondition )
									AND ottocrat_troubletickets.ticketid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
							...
							$query = "SELECT ottocrat_troubletickets.ticketid FROM ottocrat_troubletickets
									...
									WHERE ottocrat_crmentity.deleted = 0 AND
											(ottocrat_crmentityrel.crmid IN
												(SELECT projectid FROM ottocrat_project WHERE linktoaccountscontacts
													IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") AND ottocrat_crmentityrel.relcrmid = $entityid)
											OR ottocrat_crmentityrel.relcrmid IN
												(SELECT projectid FROM ottocrat_project WHERE linktoaccountscontacts
													IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") AND ottocrat_crmentityrel.crmid = $entityid)
										AND ottocrat_troubletickets.ticketid = ?)";
		...
		case 'Services'	:	$query = "SELECT ottocrat_service.serviceid FROM ottocrat_service
									INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_service.serviceid AND ottocrat_crmentity.deleted = 0
									LEFT JOIN ottocrat_crmentityrel ON (ottocrat_crmentityrel.relcrmid=ottocrat_service.serviceid OR ottocrat_crmentityrel.crmid=ottocrat_service.serviceid)
									WHERE (ottocrat_crmentityrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")  OR " .
		 							"(ottocrat_crmentityrel.relcrmid IN (".generateQuestionMarks($allowed_contacts_and_accounts).") AND ottocrat_crmentityrel.module = 'Services'))
									AND ottocrat_service.serviceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts,$allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Accounts' : 	$query = "SELECT ottocrat_account.accountid FROM ottocrat_account " .
									"INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_account.accountid " .
									"INNER JOIN ottocrat_contactdetails ON ottocrat_contactdetails.accountid = ottocrat_account.accountid " .
									"WHERE ottocrat_crmentity.deleted = 0 and ottocrat_contactdetails.contactid = ? and ottocrat_contactdetails.accountid = ?";
		...
		case 'Assets' : $query = "SELECT ottocrat_assets.assetname FROM ottocrat_assets
								INNER JOIN ottocrat_crmentity ON  ottocrat_assets.assetsid = ottocrat_crmentity.crmid
								WHERE ottocrat_crmentity.deleted = 0 and ottocrat_assets.account = ? ";
						$accountid = '';
						$accountRes = $adb->pquery("SELECT accountid FROM ottocrat_contactdetails
								INNER JOIN ottocrat_crmentity ON ottocrat_contactdetails.contactid = ottocrat_crmentity.crmid
		...
		case 'Project'	:	$query = "SELECT ottocrat_project.projectid FROM ottocrat_project
									INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_project.projectid AND ottocrat_crmentity.deleted = 0
									WHERE ottocrat_project.linktoaccountscontacts IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND ottocrat_project.projectid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
...
function get_documents($id,$module,$customerid,$sessionid)
{
	...
	$query ="select ottocrat_notes.title,'Documents' ActivityType, ottocrat_notes.filename,
		crm2.createdtime,ottocrat_notes.notesid,ottocrat_notes.folderid,
		ottocrat_notes.notecontent description, ottocrat_users.user_name, ottocrat_notes.filelocationtype
		from ottocrat_notes
		LEFT join ottocrat_senotesrel on ottocrat_senotesrel.notesid= ottocrat_notes.notesid
		INNER join ottocrat_crmentity on ottocrat_crmentity.crmid= ottocrat_senotesrel.crmid
		LEFT join ottocrat_crmentity crm2 on crm2.crmid=ottocrat_notes.notesid and crm2.deleted=0
		LEFT JOIN ottocrat_groups
		ON ottocrat_groups.groupid = ottocrat_crmentity.smownerid
		LEFT join ottocrat_users on crm2.smownerid= ottocrat_users.id
		where ottocrat_crmentity.crmid=?";
	$res = $adb->pquery($query,array($id));
	$noofdata = $adb->num_rows($res);
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldname => $fieldlabel) {
			...
			if($fieldname == 'filename'){
				...
				if($filetype == 'I'){
					...
					$fieldvalue = '<a target="_blank" href="'.Ottocrat_Request:: encryptLink(''.$filename).'">'.$filename.'</a>';
				}
			}
			$output[1][$module]['data'][$j][$i]['fielddata'] = $fieldvalue;
			$i++;
...
function get_project_components($id,$module,$customerid,$sessionid) {
	...
	if ($module == 'ProjectTask') {
		$query ="SELECT ottocrat_projecttask.*, ottocrat_crmentity.smownerid
				FROM ottocrat_projecttask
				INNER JOIN ottocrat_project ON ottocrat_project.projectid = ottocrat_projecttask.projectid AND ottocrat_project.projectid = ?
				INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_projecttask.projecttaskid AND ottocrat_crmentity.deleted = 0";
	} elseif ($module == 'ProjectMilestone') {
		$query ="SELECT ottocrat_projectmilestone.*, ottocrat_crmentity.smownerid
				FROM ottocrat_projectmilestone
				INNER JOIN ottocrat_project ON ottocrat_project.projectid = ottocrat_projectmilestone.projectid AND ottocrat_project.projectid = ?
				INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_projectmilestone.projectmilestoneid AND ottocrat_crmentity.deleted = 0";
	}
...
function get_project_tickets($id,$module,$customerid,$sessionid) {
	...
	$query = "SELECT ottocrat_troubletickets.*, ottocrat_crmentity.smownerid FROM ottocrat_troubletickets
		INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_troubletickets.ticketid
		INNER JOIN ottocrat_crmentityrel ON (ottocrat_crmentityrel.relcrmid = ottocrat_crmentity.crmid OR ottocrat_crmentityrel.crmid = ottocrat_crmentity.crmid)
		WHERE ottocrat_crmentity.deleted = 0 AND (ottocrat_crmentityrel.crmid = ? OR ottocrat_crmentityrel.relcrmid = ?)";
...
function get_service_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	$query[] = "select ottocrat_service.*," .
		"case when ottocrat_crmentityrel.crmid != ottocrat_service.serviceid then ottocrat_crmentityrel.crmid else ottocrat_crmentityrel.relcrmid end as entityid, " .
		 "'' as setype from ottocrat_service " .
		 "inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_service.serviceid " .
		 "left join ottocrat_crmentityrel on (ottocrat_crmentityrel.relcrmid=ottocrat_service.serviceid or ottocrat_crmentityrel.crmid=ottocrat_service.serviceid) " .
		 "where ottocrat_crmentity.deleted = 0 and " .
		 "( ottocrat_crmentityrel.crmid in (".generateQuestionMarks($entity_ids_list).") OR " .
		 "(ottocrat_crmentityrel.relcrmid in (".generateQuestionMarks($entity_ids_list).") AND ottocrat_crmentityrel.module = 'Services')" .
		 ")";
	...
	$checkQuotes = checkModuleActive('Quotes');
	if($checkQuotes == true){
		$query[] = "select distinct ottocrat_service.*,
			case when ottocrat_quotes.contactid is not null then ottocrat_quotes.contactid else ottocrat_quotes.accountid end as entityid,
			case when ottocrat_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype
			from ottocrat_quotes INNER join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_quotes.quoteid
			left join ottocrat_inventoryproductrel on ottocrat_inventoryproductrel.id=ottocrat_quotes.quoteid
			left join ottocrat_service on ottocrat_service.serviceid = ottocrat_inventoryproductrel.productid
			where ottocrat_inventoryproductrel.productid = ottocrat_service.serviceid AND ottocrat_crmentity.deleted=0 and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
	$checkInvoices = checkModuleActive('Invoice');
	if($checkInvoices == true){
		...
		for( $j= 0;$j < $noofdata[$k]; $j++)
		{
			...
			foreach($fields_list as $fieldlabel=> $fieldname) {
				...
				if($fieldname == 'entityid') {
					...
					$module = $adb->query_result($res[$k],$j,'setype');
					if($module == ''){
						$module = $adb->query_result($adb->pquery("SELECT setype FROM ottocrat_crmentity WHERE crmid = ?", array($crmid)),0,'setype');
					}
					if ($crmid != '' && $module != '') {
						$fieldvalues = getEntityName($module, array($crmid));
						if($module == 'Contacts')
...
function get_modules()
{
	...
	$modules = Ottocrat_Soap_CustomerPortal::lookupAllowedModules();
	if($modules === false) {
		$modules = array();

		$query = $adb->pquery("SELECT ottocrat_customerportal_tabs.* FROM ottocrat_customerportal_tabs
			INNER JOIN ottocrat_tab ON ottocrat_tab.tabid = ottocrat_customerportal_tabs.tabid
			WHERE ottocrat_tab.presence = 0 AND ottocrat_customerportal_tabs.visible = 1", array());
		...
		Ottocrat_Soap_CustomerPortal::updateAllowedModules($modules);
	}
...
function show_all($module){
	...
	$query = $adb->pquery("SELECT prefvalue from ottocrat_customerportal_prefs where tabid = ?", array($tabid));
	$norows = $adb->num_rows($query);
	if($norows > 0){
...
function getRelatedServiceContracts($crmid){
	...
	$query = "SELECT * FROM ottocrat_servicecontracts " .
	"INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_servicecontracts.servicecontractsid AND ottocrat_crmentity.deleted = 0 " .
	"LEFT JOIN ottocrat_crmentityrel ON ottocrat_crmentityrel.crmid = ottocrat_servicecontracts.servicecontractsid " .
	"WHERE (ottocrat_crmentityrel.relcrmid = ? and ottocrat_crmentityrel.module= 'ServiceContracts')";
...
function getPortalUserid() {
	...
	$userid = Ottocrat_Soap_CustomerPortal::lookupPrefValue('userid');
	if($userid === false) {
		$res = $adb->pquery("SELECT prefvalue FROM ottocrat_customerportal_prefs WHERE prefkey = 'userid' AND tabid = 0", array());
		$norows = $adb->num_rows($res);
		if($norows > 0) {
			$userid = $adb->query_result($res,0,'prefvalue');
			// Update the cache information now.
			Ottocrat_Soap_CustomerPortal::updatePrefValue('userid', $userid);
		}
	}
	return $userid;
...
function getDefaultAssigneeId() {
	...
	// Look the value from cache first
	$defaultassignee = Ottocrat_Soap_CustomerPortal::lookupPrefValue('defaultassignee');
	if($defaultassignee === false) {
		$res = $adb->pquery("SELECT prefvalue FROM ottocrat_customerportal_prefs WHERE prefkey = 'defaultassignee' AND tabid = 0", array());
		$norows = $adb->num_rows($res);
		if($norows > 0) {
			$defaultassignee = $adb->query_result($res,0,'prefvalue');
			// Update the cache information now.
			Ottocrat_Soap_CustomerPortal::updatePrefValue('defaultassignee', $defaultassignee);
		}
	}
