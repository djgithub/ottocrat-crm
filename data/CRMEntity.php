 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/ottocrat_crm/ottocratcrm/data/CRMEntity.php,v 1.16 2005/04/29 04:21:31 mickie Exp $
...
class CRMEntity {
	...
	static function isBulkSaveMode() {
		...
		if (isset($OTTOCRAT_BULK_SAVE_MODE) && $OTTOCRAT_BULK_SAVE_MODE) {
			return true;
		}
		return false;
	}
	...
	function uploadAndSaveFile($id, $module, $file_details) {
		...
		if ($save_file == 'true' && $upload_status == 'true') {
			//This is only to update the attached filename in the ottocrat_notes ottocrat_table for the Notes module
			if ($module == 'Contacts' || $module == 'Products') {
			...
			if ($module == 'Contacts') {
				$att_sql = "select ottocrat_seattachmentsrel.attachmentsid  from ottocrat_seattachmentsrel inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_seattachmentsrel.attachmentsid where ottocrat_crmentity.setype='Contacts Image' and ottocrat_seattachmentsrel.crmid=?";
				...
				if ($attachmentsid != '') {
					...
					$crm_delquery = "delete from ottocrat_crmentity where crmid=?";
					$adb->pquery($crm_delquery, array($attachmentsid));
					$sql5 = 'insert into ottocrat_seattachmentsrel values(?,?)';
	...
	/** Function to insert values in the ottocrat_crmentity for the specified module
	...
	function insertIntoCrmEntity($module, $fileid = '') {
		...
		if ($this->mode == 'edit') {
			...
			if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
				$sql = "update ottocrat_crmentity set smownerid=?,modifiedby=?,description=?, modifiedtime=? where crmid=?";
				$params = array($ownerid, $current_user->id, $description_val, $adb->formatDate($date_var, true), $this->id);
			} else {
				$profileList = getCurrentUserProfileList();
				$perm_qry = "SELECT columnname FROM ottocrat_field INNER JOIN ottocrat_profile2field ON ottocrat_profile2field.fieldid = ottocrat_field.fieldid INNER JOIN ottocrat_def_org_field ON ottocrat_def_org_field.fieldid = ottocrat_field.fieldid WHERE ottocrat_field.tabid = ? AND ottocrat_profile2field.visible = 0 AND ottocrat_profile2field.readonly = 0 AND ottocrat_profile2field.profileid IN (" . generateQuestionMarks($profileList) . ") AND ottocrat_def_org_field.visible = 0 and ottocrat_field.tablename='ottocrat_crmentity' and ottocrat_field.displaytype in (1,3) and ottocrat_field.presence in (0,2);";
				...
				if (is_array($columname) && in_array("description", $columname)) {
					...
					$sql = "update ottocrat_crmentity set smownerid=?,modifiedby=?, modifiedtime=? where crmid=?";
	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		$log->info("function insertIntoEntityTable " . $module . ' ottocrat_table name ' . $table_name);
		global $adb;
		$insertion_mode = $this->mode;

		//Checkin whether an entry is already is present in the ottocrat_table to update
		...
		if ($insertion_mode == 'edit') {
			...
			require('user_privileges/user_privileges_' . $current_user->id . '.php');
			if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
				$sql = "select * from ottocrat_field where tabid in (" . generateQuestionMarks($tabid) . ") and tablename=? and displaytype in (1,3) and presence in (0,2) group by columnname";
				...
				if (count($profileList) > 0) {
					$sql = "SELECT *
			  			FROM ottocrat_field
			  			INNER JOIN ottocrat_profile2field
			  			ON ottocrat_profile2field.fieldid = ottocrat_field.fieldid
			  			INNER JOIN ottocrat_def_org_field
			  			ON ottocrat_def_org_field.fieldid = ottocrat_field.fieldid
			  			WHERE ottocrat_field.tabid = ?
			  			AND ottocrat_profile2field.visible = 0 AND ottocrat_profile2field.readonly = 0
			  			AND ottocrat_profile2field.profileid IN (" . generateQuestionMarks($profileList) . ")
			  			AND ottocrat_def_org_field.visible = 0 and ottocrat_field.tablename=? and ottocrat_field.displaytype in (1,3) and ottocrat_field.presence in (0,2) group by columnname";
			...
			if ($table_index_column == 'id' && $table_name == 'ottocrat_users') {
				$currentuser_id = $adb->getUniqueID("ottocrat_users");
				$this->id = $currentuser_id;
			}
			$column = array($table_index_column);
			$value = array($this->id);
			$sql = "select * from ottocrat_field where tabid=? and tablename=? and displaytype in (1,3,4) and ottocrat_field.presence in (0,2)";
			$params = array($tabid, $table_name);
		}
		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			if (isset($this->column_fields[$fieldname])) {
				if ($uitype == 56) {
					...
					if ($this->column_fields[$fieldname] == null) {
						$fileQuery = $adb->pquery("SELECT filename from ottocrat_notes WHERE notesid = ?", array($this->id));
						$fldvalue = null;
						if (isset($fileQuery)) {
					...
					if (empty($fldvalue)) {
						$query = "SELECT email1 FROM ottocrat_users WHERE id = ?";
			...
			if ($insertion_mode == 'edit') {
				if ($table_name != 'ottocrat_ticketcomments' && $uitype != 4) {
		...
		if ($insertion_mode == 'edit') {
			if ($module == 'Potentials') {
				$dbquery = 'select sales_stage from ottocrat_potential where potentialid = ?';
				...
				if ($sales_stage != $_REQUEST['sales_stage'] && $_REQUEST['sales_stage'] != '') {
					...
					$sql = "insert into ottocrat_potstagehistory values(?,?,?,?,?,?,?,?)";
	 ...
	 * The function will get the attachmentsid for the given entityid from ottocrat_seattachmentsrel table and get the attachmentsname from ottocrat_attachments table
	...
	function getOldFileName($notesid) {
		...
		global $adb;
		$query1 = "select * from ottocrat_seattachmentsrel where crmid=?";
		$result = $adb->pquery($query1, array($notesid));
		$noofrows = $adb->num_rows($result);
		if ($noofrows != 0)
			$attachmentid = $adb->query_result($result, 0, 'attachmentsid');
		if ($attachmentid != '') {
			$query2 = "select * from ottocrat_attachments where attachmentsid=?";
	...
	function retrieve_entity_info($record, $module) {
		...
		if (isset($this->multirow_tables)) {
			...
			$multirow_tables = array(
				'ottocrat_campaignrelstatus',
				'ottocrat_attachments',
				//'ottocrat_inventoryproductrel',
				//'ottocrat_cntactivityrel',
				'ottocrat_email_track'
			);
		}
		...
		if ($cachedModuleFields === false) {
			...
			$sql0 = "SELECT fieldname, fieldid, fieldlabel, columnname, tablename, uitype, typeofdata,presence FROM ottocrat_field WHERE tabid=?";
		...
		if ($cachedModuleFields) {
			...
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				// Added to avoid picking shipping tax fields for Inventory modules, the shipping tax detail are stored in ottocrat_inventoryshippingrel
				// table, but in ottocrat_field table we have set tablename as ottocrat_inventoryproductrel.
			...
			$column_clause .= 'ottocrat_crmentity.deleted';

			if (isset($required_tables['ottocrat_crmentity'])) {
				$from_clause  = ' ottocrat_crmentity';
				unset($required_tables['ottocrat_crmentity']);
				foreach ($required_tables as $tablename => $tableindex) {
					...
					$from_clause .= sprintf(' %s %s ON %s.%s=%s.%s', $join_type,
						$tablename, $tablename, $tableindex, 'ottocrat_crmentity', 'crmid');
				}
			}

			$where_clause .= ' ottocrat_crmentity.crmid=?';
			$params[] = $record;
	...
	function save($module_name, $fileid = '') {
		...
		$em->triggerEvent("ottocrat.entity.beforesave.modifiable", $entityData);
		$em->triggerEvent("ottocrat.entity.beforesave", $entityData);
		...
		if($em) {
		//Event triggering code
		$em->triggerEvent("ottocrat.entity.aftersave", $entityData);
		$em->triggerEvent("ottocrat.entity.aftersave.final", $entityData);
		//Event triggering code ends
	}
...
//Added by Richie
	...
	function mark_deleted($id) {
		global $current_user;
		$date_var = date("Y-m-d H:i:s");
		$query = "UPDATE ottocrat_crmentity set deleted=1,modifiedtime=?,modifiedby=? where crmid=?";
	...
	// the keys are user defined, and don't directly map to the bean's ottocrat_fields
	// the value is the method name within that bean that will do extra
	// processing for that ottocrat_field. example: 'full_name'=>'get_names_from_full_name'
	...
	/**
	 * Function to check if the custom ottocrat_field ottocrat_table exists
	...
	/**
	 * function to construct the query to fetch the custom ottocrat_fields
	 * return the query to fetch the custom ottocrat_fields
	 */
	function constructCustomQueryAddendum($tablename, $module) {
		global $adb;
		$tabid = getTabid($module);
		$sql1 = "select columnname,fieldlabel from ottocrat_field where generatedtype=2 and tabid=? and ottocrat_field.presence in (0,2)";
	...
	function track_view($user_id, $current_module, $id = '') {
		$this->log->debug("About to call ottocrat_tracker (user_id, module_name, item_id)($user_id, $current_module, $this->id)");
	...
	function initRequiredFields($module) {
		...
		$tabid = getTabId($module);
		$sql = "select * from ottocrat_field where tabid= ? and typeofdata like '%M%' and uitype not in ('53','70') and ottocrat_field.presence in (0,2)";
	...
	function trash($module, $id) {
		...
		if(!self::isBulkSaveMode()) {
            ...
            $em->triggerEvent("ottocrat.entity.beforedelete", $entityData);
        }
		...
		$sql_recentviewed = 'DELETE FROM ottocrat_tracker WHERE user_id = ? AND item_id = ?';
		$this->db->pquery($sql_recentviewed, array($current_user->id, $id));

		if($em){
            $em->triggerEvent("ottocrat.entity.afterdelete", $entityData);
        }
	}
	...
	function unlinkDependencies($module, $id) {
		...
		$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM ottocrat_field WHERE fieldid IN (
			SELECT fieldid FROM ottocrat_fieldmodulerel WHERE relmodule=?)', array($module));
		...
		for ($i = 0; $i < $numOfFields; $i++) {
			...
			if ($tableName == 'ottocrat_inventoryproductrel') {
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		$query = 'DELETE FROM ottocrat_crmentityrel WHERE (crmid=? AND relmodule=? AND relcrmid=?) OR (relcrmid=? AND module=? AND crmid=?)';
		$params = array($id, $return_module, $return_id, $id, $return_module, $return_id);
		$this->db->pquery($query, $params);

		$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM ottocrat_field WHERE fieldid IN (
			SELECT fieldid FROM ottocrat_fieldmodulerel WHERE module=? AND relmodule=?)', array($currentModule, $return_module));
		$numOfFields = $this->db->num_rows($fieldRes);
		for ($i = 0; $i < $numOfFields; $i++) {
			$tabId = $this->db->query_result($fieldRes, $i, 'tabid');
			$tableName = $this->db->query_result($fieldRes, $i, 'tablename');
	...
	function restore($module, $id) {
		...
		$date_var = date("Y-m-d H:i:s");
		$query = 'UPDATE ottocrat_crmentity SET deleted=0,modifiedtime=?,modifiedby=? WHERE crmid = ?';
		...
		$entityData = VTEntityData::fromCRMEntity($this);
		//Event triggering code
		$em->triggerEvent("ottocrat.entity.afterrestore", $entityData);
		//Event triggering code ends
	...
	function restoreRelatedRecords($module, $record) {
		...
		$result = $this->db->pquery('SELECT * FROM ottocrat_relatedlists_rb WHERE entityid = ?', array($record));
		...
		for ($i = 0; $i < $numRows; $i++) {
			...
			if (strtoupper($action) == RB_RECORD_UPDATED) {
				...
				if ($rel_table == 'ottocrat_crmentity' && $rel_column == 'deleted') {
				...
				if ($rel_table == 'ottocrat_seproductrel') {
		...
		$this->db->pquery('DELETE FROM ottocrat_relatedlists_rb WHERE entityid = ?', array($record));
	}
	...
	function initSortByField($module) {
		...
		$sql = "SELECT columnname FROM ottocrat_field " .
				" WHERE (fieldname not like '%\_id' OR fieldname in ('assigned_user_id'))" .
				" AND tabid in (" . generateQuestionMarks($tabid) . ") and ottocrat_field.presence in (0,2)";
		$params = array($tabid);
	...
	function setModuleSeqNumber($mode, $module, $req_str = '', $req_no = '') {
		...
		if ($mode == "configure" && $req_no != '') {
			$check = $adb->pquery("select cur_id from ottocrat_modentity_num where semodule=? and prefix = ?", array($module, $req_str));
			if ($adb->num_rows($check) == 0) {
				$numid = $adb->getUniqueId("ottocrat_modentity_num");
				$active = $adb->pquery("select num_id from ottocrat_modentity_num where semodule=? and active=1", array($module));
				$adb->pquery("UPDATE ottocrat_modentity_num SET active=0 where num_id=?", array($adb->query_result($active, 0, 'num_id')));

				$adb->pquery("INSERT into ottocrat_modentity_num values(?,?,?,?,?,?)", array($numid, $module, $req_str, $req_no, $req_no, 1));
				return true;
				...
				if ($req_no < $num_check) {
					...
					$adb->pquery("UPDATE ottocrat_modentity_num SET active=0 where active=1 and semodule=?", array($module));
					$adb->pquery("UPDATE ottocrat_modentity_num SET cur_id=?, active = 1 where prefix=? and semodule=?", array($req_no, $req_str, $module));
					return true;
				}
			}
		} else if ($mode == "increment") {
			//when we save new invoice we will increment the invoice id and write
			$check = $adb->pquery("select cur_id,prefix from ottocrat_modentity_num where semodule=? and active = 1", array($module));
			...
			$req_no.= $temp . ($curid + 1);
			$adb->pquery("UPDATE ottocrat_modentity_num SET cur_id=? where cur_id=? and active=1 AND semodule=?", array($req_no, $curid, $module));
	...
	function isModuleSequenceConfigured($module) {
		...
		$result = $adb->pquery('SELECT 1 FROM ottocrat_modentity_num WHERE semodule = ? AND active = 1', array($module));
	...
	function updateMissingSeqNumber($module) {
		...
		$tabid = getTabid($module);
		$fieldinfo = $adb->pquery("SELECT * FROM ottocrat_field WHERE tabid = ? AND uitype = 4", Array($tabid));
			...
			if ($fld_table == $this->table_name) {
				...
				if ($records && $adb->num_rows($records)) {
					...
					if ($old_cur_id != $cur_id) {
						$adb->pquery("UPDATE ottocrat_modentity_num set cur_id=? where semodule=? and active=1", Array($cur_id, $module));
					}
				}
			} else {
	...
	function get_attachments($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>'ottocrat_users.first_name',
														'last_name' => 'ottocrat_users.last_name'), 'Users');
		$query = "select case when (ottocrat_users.user_name not like '') then $userNameSql else ottocrat_groups.groupname end as user_name," .
				...
				ottocrat_seattachmentsrel.attachmentsid attachmentsid, ottocrat_crmentity.smownerid smownerid, ottocrat_notes.notesid crmid,
				ottocrat_notes.notecontent description,ottocrat_notes.*
				from ottocrat_notes
	...
	function isViewed($crmid = false) {
		...
		if ($crmid) {
			global $adb;
			$result = $adb->pquery("SELECT viewedtime,modifiedtime,smcreatorid,smownerid,modifiedby FROM ottocrat_crmentity WHERE crmid=?", Array($crmid));
	...
	function markAsViewed($userid) {
		global $adb;
		$adb->pquery("UPDATE ottocrat_crmentity set viewedtime=? WHERE crmid=? AND smownerid=?", Array(date('Y-m-d H:i:s', time()), $this->id, $userid));
	}
	...
	function save_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				$checkpresence = $adb->pquery("SELECT crmid FROM ottocrat_senotesrel WHERE crmid = ? AND notesid = ?", Array($crmid, $relcrmid));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence))
					continue;

				$adb->pquery("INSERT INTO ottocrat_senotesrel(crmid, notesid) VALUES(?,?)", array($crmid, $relcrmid));
			} else {
				$checkpresence = $adb->pquery("SELECT crmid FROM ottocrat_crmentityrel WHERE
					crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?", Array($crmid, $module, $relcrmid, $with_module));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence))
					continue;

				$adb->pquery("INSERT INTO ottocrat_crmentityrel(crmid, module, relcrmid, relmodule) VALUES(?,?,?,?)", Array($crmid, $module, $relcrmid, $with_module));
			}
		}
	}
	...
	function delete_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				$adb->pquery("DELETE FROM ottocrat_senotesrel WHERE crmid=? AND notesid=?", Array($crmid, $relcrmid));
			} else {
				$adb->pquery("DELETE FROM ottocrat_crmentityrel WHERE (crmid=? AND module=? AND relcrmid=? AND relmodule=?) OR (relcrmid=? AND relmodule=? AND crmid=? AND module=?)",
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$query .= " INNER JOIN ottocrat_crmentityrel ON (ottocrat_crmentityrel.relcrmid = ottocrat_crmentity.crmid OR ottocrat_crmentityrel.crmid = ottocrat_crmentity.crmid)";
		$query .= $more_relation;
		...
		$query .= " WHERE ottocrat_crmentity.deleted = 0 AND (ottocrat_crmentityrel.crmid = $id OR ottocrat_crmentityrel.relcrmid = $id)";
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		if ($numOfFields > 0) {
			...
			$query .= " WHERE ottocrat_crmentity.deleted = 0 AND $this->table_name.$this->table_index = $id";
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		foreach ($transferEntityIds as $transferId) {
			...
			$relatedRecords = $adb->pquery("SELECT relcrmid, relmodule FROM ottocrat_crmentityrel WHERE crmid=? AND module=?" .
					" AND relcrmid NOT IN (SELECT relcrmid FROM ottocrat_crmentityrel WHERE crmid=? AND module=?)", array($transferId, $module, $entityId, $module));
			...
			for ($i = 0; $i < $numOfRecords; $i++) {
				...
				$adb->pquery("UPDATE ottocrat_crmentityrel SET crmid=? WHERE relcrmid=? AND relmodule=? AND crmid=? AND module=?", array($entityId, $relcrmid, $relmodule, $transferId, $module));
			}
			...
			$parentRecords = $adb->pquery("SELECT crmid, module FROM ottocrat_crmentityrel WHERE relcrmid=? AND relmodule=?" .
					" AND crmid NOT IN (SELECT crmid FROM ottocrat_crmentityrel WHERE relcrmid=? AND relmodule=?)", array($transferId, $module, $entityId, $module));
			...
			for ($i = 0; $i < $numOfRecords; $i++) {
				...
				$adb->pquery("UPDATE ottocrat_crmentityrel SET relcrmid=? WHERE crmid=? AND module=? AND relcrmid=? AND relmodule=?", array($entityId, $parcrmid, $parmodule, $transferId, $module));
			}
			$adb->pquery("UPDATE ottocrat_modcomments SET related_to = ? WHERE related_to = ?", array($entityId, $transferId));
		}
	...
	function generateReportsQuery($module, $queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT ottocrat_field.fieldname,ottocrat_field.tablename,ottocrat_field.fieldid from ottocrat_field INNER JOIN ottocrat_tab on ottocrat_tab.name = ? WHERE ottocrat_tab.tabid=ottocrat_field.tabid AND ottocrat_field.uitype IN (10) and ottocrat_field.presence in (0,2)", array($module));
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				$ui10_modules_query = $adb->pquery("SELECT relmodule FROM ottocrat_fieldmodulerel WHERE fieldid=?", array($field_id));

				if ($adb->num_rows($ui10_modules_query) > 0) {

					// Capture the forward table dependencies due to dynamic related-field
					$crmentityRelModuleFieldTable = "ottocrat_crmentityRel$module$field_id";
					...
					if ($queryPlanner->requireTable($crmentityRelModuleFieldTable, $matrix)) {
						$relquery.= " left join ottocrat_crmentity as $crmentityRelModuleFieldTable on $crmentityRelModuleFieldTable.crmid = $tab_name.$field_name and ottocrat_crmentityRel$module$field_id.deleted=0";
					}
		...
		$query = "from $moduletable inner join ottocrat_crmentity on ottocrat_crmentity.crmid=$moduletable.$moduleindex";
	...
	function generateReportsSecQuery($module, $secmodule,$queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT ottocrat_field.fieldname,ottocrat_field.tablename,ottocrat_field.fieldid from ottocrat_field INNER JOIN ottocrat_tab on ottocrat_tab.name = ? WHERE ottocrat_tab.tabid=ottocrat_field.tabid AND ottocrat_field.uitype IN (10) and ottocrat_field.presence in (0,2)", array($secmodule));
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				if ($adb->num_rows($ui10_modules_query) > 0) {
					// Capture the forward table dependencies due to dynamic related-field
					$crmentityRelSecModuleTable = "ottocrat_crmentityRel$secmodule$field_id";
					...
					if ($queryPlanner->requireTable($crmentityRelSecModuleTable, $matrix)) {
						$relquery .= " left join ottocrat_crmentity as $crmentityRelSecModuleTable on $crmentityRelSecModuleTable.crmid = $tab_name.$field_name and $crmentityRelSecModuleTable.deleted=0";
					}
					for ($j = 0; $j < $adb->num_rows($ui10_modules_query); $j++) {
						$rel_mod = $adb->query_result($ui10_modules_query, $j, 'relmodule');
						$rel_obj = CRMEntity::getInstance($rel_mod);
		...
		$matrix->setDependency("ottocrat_crmentity$secmodule", array("ottocrat_groups$secmodule", "ottocrat_users$secmodule", "ottocrat_lastModifiedBy$secmodule"));
		$matrix->addDependency($tablename, "ottocrat_crmentity$secmodule");
		...
		if ($queryPlanner->requireTable("ottocrat_crmentity$secmodule", $matrix)) {
			$query .= " left join ottocrat_crmentity as ottocrat_crmentity$secmodule on ottocrat_crmentity$secmodule.crmid = $tablename.$tableindex AND ottocrat_crmentity$secmodule.deleted=0";
		}

		// Add the pre-joined custom table query
		$query .= " ".$cfquery;

		if ($queryPlanner->requireTable("ottocrat_groups$secmodule")) {
			$query .= " left join ottocrat_groups as ottocrat_groups" . $secmodule . " on ottocrat_groups" . $secmodule . ".groupid = ottocrat_crmentity$secmodule.smownerid";
		}
		if ($queryPlanner->requireTable("ottocrat_users$secmodule")) {
			$query .= " left join ottocrat_users as ottocrat_users" . $secmodule . " on ottocrat_users" . $secmodule . ".id = ottocrat_crmentity$secmodule.smownerid";
		}
		if ($queryPlanner->requireTable("ottocrat_lastModifiedBy$secmodule")) {
			$query .= " left join ottocrat_users as ottocrat_lastModifiedBy" . $secmodule . " on ottocrat_lastModifiedBy" . $secmodule . ".id = ottocrat_crmentity" . $secmodule . ".modifiedby";
		}
	...
	function getListViewSecurityParameter($module) {
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1
			...
			$sec_query .= " and (ottocrat_crmentity.smownerid in($current_user->id) or ottocrat_crmentity.smownerid
					in (select ottocrat_user2role.userid from ottocrat_user2role
							...
							where ottocrat_role.parentrole like '" . $current_user_parent_role_seq . "::%') or ottocrat_crmentity.smownerid
					in(select shareduserid from ottocrat_tmp_read_user_sharing_per
						where userid=" . $current_user->id . " and tabid=" . $tabid . ") or (";
			if (sizeof($current_user_groups) > 0) {
				$sec_query .= " ottocrat_groups.groupid in (" . implode(",", $current_user_groups) . ") or ";
			}
			$sec_query .= " ottocrat_groups.groupid in(select ottocrat_tmp_read_group_sharing_per.sharedgroupid
						from ottocrat_tmp_read_group_sharing_per where userid=" . $current_user->id . " and tabid=" . $tabid . "))) ";
		}
		return $sec_query;
	}
	...
	function getRelationQuery($module, $secmodule, $table_name, $column_name, $queryPlanner) {
		...
		$secQuery = "select $table_name.* from $table_name inner join ottocrat_crmentity on " .
				"ottocrat_crmentity.crmid=$table_name.$column_name and ottocrat_crmentity.deleted=0";

		$secQueryTempTableQuery = $queryPlanner->registerTempTable($secQuery, array($column_name, $fields[1], $prifieldname));

		$query = '';
		if ($pritablename == 'ottocrat_crmentityrel') {
			$condition = "($table_name.$column_name={$tmpname}.{$secfieldname} " .
					"OR $table_name.$column_name={$tmpname}.{$prifieldname})";
			$query = " left join ottocrat_crmentityrel as $tmpname ON ($condvalue={$tmpname}.{$secfieldname} " .
			...
			$condition = "$table_name.$column_name=$tmpname.$secfieldname";
            if($pritablename == 'ottocrat_seactivityrel') {
                ...
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname
                    AND $tmpname.activityid IN (SELECT crmid FROM ottocrat_crmentity WHERE setype='$tmpModule' AND deleted = 0))";
            } else if($pritablename == 'ottocrat_senotesrel') {
                    $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname
                    AND $tmpname.notesid IN (SELECT crmid FROM ottocrat_crmentity WHERE setype='Documents' AND deleted = 0))";
            } else if($pritablename == 'ottocrat_inventoryproductrel' && ($module =="Products" || $module =="Services") && ($secmodule == "Invoice" || $secmodule == "SalesOrder" || $secmodule == "PurchaseOrder" || $secmodule == "Quotes")) {
                /** In ottocrat_inventoryproductrel table, we'll have same product related to quotes/invoice/salesorder/purchaseorder
                ...
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname AND $tmpname.id in 
                        (select crmid from ottocrat_crmentity where setype='$secmodule' and deleted=0))";
            } else {
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname)";
            }
            ...
            // To get all the Contacts from ottocrat_contpotentialrel table
            $condition .= " OR $table_name.potentialid = ottocrat_contpotentialrel.potentialid";
            $query .= " left join ottocrat_contpotentialrel on  ottocrat_contpotentialrel.contactid = ottocrat_contactdetails.contactid";
        }else if($module == "Potentials" && $secmodule == "Contacts"){
            // To get all the Potentials from ottocrat_contpotentialrel table
            $condition .= " OR $table_name.contactid = ottocrat_contpotentialrel.contactid";
            $query .= " left join ottocrat_contpotentialrel on ottocrat_potential.potentialid = ottocrat_contpotentialrel.potentialid";
        }
	...
	function add_related_to($module, $fieldname) {
		...
		$query = "SELECT ottocrat_crmentity.deleted, $focus1->table_name.*
					FROM $focus1->table_name
					INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid=$focus1->table_name.$focus1->table_index
						where $entityName=? and ottocrat_crmentity.deleted=0";
	...
	function filterInactiveFields($module) {
		...
		if ($cachedModuleFields) {
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				$tableName = str_replace("ottocrat_", "", $fieldinfo['tablename']);
	...
	function buildSearchQueryForFieldTypes($uitypes, $value=false) {
		...
		$query .=" INNER JOIN ottocrat_crmentity ON $this->table_name.$this->table_index = ottocrat_crmentity.crmid AND deleted = 0 ";
	...
	function getNonAdminUserAccessQuery($user, $parentRole, $userGroups) {
		$query = "(SELECT $user->id as id) UNION (SELECT ottocrat_user2role.userid AS userid FROM " .
				"ottocrat_user2role INNER JOIN ottocrat_users ON ottocrat_users.id=ottocrat_user2role.userid " .
				"INNER JOIN ottocrat_role ON ottocrat_role.roleid=ottocrat_user2role.roleid WHERE " .
				"ottocrat_role.parentrole like '$parentRole::%')";
		if (count($userGroups) > 0) {
			$query .= " UNION (SELECT groupid FROM ottocrat_groups where" .
	...
	function getNonAdminModuleAccessQuery($module, $user) {
		...
		if (!empty($sharingRuleInfo) && (count($sharingRuleInfo['ROLE']) > 0 ||
			...
			$query = " (SELECT shareduserid FROM ottocrat_tmp_read_user_sharing_per " .
					"WHERE userid=$user->id AND tabid=$tabId) UNION (SELECT " .
					"ottocrat_tmp_read_group_sharing_per.sharedgroupid FROM " .
					"ottocrat_tmp_read_group_sharing_per WHERE userid=$user->id AND tabid=$tabId)";
		}
		return $query;
	}
		...
		if ($is_admin == false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2]
            ...
            // for secondary module we should join the records even if record is not there(primary module without related record)
                if($scope == ''){
                    ...
                    $query = " INNER JOIN $tableName $tableName$scope ON $tableName$scope.id = " .
                            "ottocrat_crmentity$scope.smownerid OR ottocrat_crmentity$scope.smownerid IS NULL";
                }
            }
		return $query;
	}
	...
	function setRelationTables($secmodule) {
		$rel_tables = array(
			"Documents" => array("ottocrat_senotesrel" => array("crmid", "notesid"),
	...
	function trackLinkedInfo($module, $crmid, $with_module, $with_crmid) {
		...
		$adb->pquery('UPDATE ottocrat_crmentity SET modifiedtime = ?, modifiedby = ? WHERE crmid = ?', array($currentTime, $current_user->id, $crmid));
