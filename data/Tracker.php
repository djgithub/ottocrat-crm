 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/ottocrat_crm/sugarcrm/data/Tracker.php,v 1.15 2005/04/28 05:44:22 samk Exp $
...
class Tracker {
    var $log;
    var $db;
    var $table_name = "ottocrat_tracker";

    // Tracker ottocrat_table
    ...
    /**
     * Add this new item to the ottocrat_tracker ottocrat_table.  If there are too many items (global config for now)
...
//No genius required. Just add an if case and change the query so that it puts the tracker entry whenever you touch on the DetailView of the required entity
	 ...
	 #if condition added to skip ottocrat_faq in last viewed history
     ...
     * return - return the array of result set rows from the query.  All of the ottocrat_table ottocrat_fields are included
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
     */
...
//        $query = "SELECT * from $this->table_name WHERE user_id='$user_id' ORDER BY id DESC";
	$query = "SELECT * from $this->table_name inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_tracker.item_id WHERE user_id=? and ottocrat_crmentity.deleted=0 ORDER BY id DESC";
