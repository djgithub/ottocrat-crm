...
class Install_Index_view extends Ottocrat_View_Controller {
	...
	public function preProcess(Ottocrat_Request $request) {
		...
		if(is_file($configFileName) && filesize($configFileName) > 0) {
			...
			$defaultModuleInstance = Ottocrat_Module_Model::getInstance($defaultModule);
	...
	public function Step1(Ottocrat_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$viewer->view('Step1.tpl', $moduleName);
	}

	public function Step2(Ottocrat_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$viewer->view('Step2.tpl', $moduleName);
	}

	public function Step3(Ottocrat_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
	...
	public function Step4(Ottocrat_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
	...
	public function Step5(Ottocrat_Request $request) {
	...
	public function Step6(Ottocrat_Request $request) {
		$moduleName = $request->getModule();
		$viewer = $this->getViewer($request);
	...
	public function Step7(Ottocrat_Request $request) {
		...
		$moduleName = $request->getModule();
                $webuiInstance = new Ottocrat_WebUI();
                ...
                if(!$isInstalled){
                    ...
                    $viewer->assign('CURRENT_VERSION', $_SESSION['ottocrat_version']);
