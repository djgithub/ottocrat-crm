...
class Install_Utils_Model {
	...
	/**
	 * Function returns the php.ini file settings required for installing ottocratCRM
	 * @return <Array>
	 */
	...
	/**
	 * Variable has the recommended php settings for smooth running of ottocratCRM
	 * @var <Array>
	 */
	...
	/**
	 * Returns the recommended php settings for ottocratCRM
	 * @return type
	 */
	...
	/**
	 * Function checks for ottocratCRM installation prerequisites
	 * @return <Array>
	 */
		...
		if($db_type) {
			...
			if(@$conn->Connect($db_hostname,$db_username,$db_password)) {
				...
				if(@$conn->Connect($db_hostname, $db_username, $db_password, $db_name)) {
					...
					if(!$db_utf8_support) {
						$db_utf8_support = Ottocrat_Util_Helper::checkDbUTF8Support($conn);
					}
				}
				$conn->Close();
			}
	...
	public static function installModules() {
		...
		$moduleFolders = array('packages/ottocrat/mandatory', 'packages/ottocrat/optional');
