...
class HelpDesk extends CRMEntity {
	var $log;
	var $db;
	var $table_name = "ottocrat_troubletickets";
	var $table_index= 'ticketid';
	var $tab_name = Array('ottocrat_crmentity','ottocrat_troubletickets','ottocrat_ticketcf');
	var $tab_name_index = Array('ottocrat_crmentity'=>'crmid','ottocrat_troubletickets'=>'ticketid','ottocrat_ticketcf'=>'ticketid','ottocrat_ticketcomments'=>'ticketid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('ottocrat_ticketcf', 'ticketid');
	...
	var $search_fields = Array(
		//'Ticket ID' => Array('ottocrat_crmentity'=>'crmid'),
		'Ticket No' =>Array('ottocrat_troubletickets'=>'ticket_no'),
		'Title' => Array('ottocrat_troubletickets'=>'title')
		);
	...
	//var $groupTable = Array('ottocrat_ticketgrouprelation','ticketid');
	...
	function save_module($module)
	{
		...
		$this->insertIntoTicketCommentTable("ottocrat_ticketcomments",$module);

		//Inserting into ottocrat_attachments
	...
	/** Function to insert values in ottocrat_ticketcomments  for the specified tablename and  module
	...
	function insertIntoTicketCommentTable($table_name, $module)
	{
		...
		if ($comment != '') {
			$sql = "insert into ottocrat_ticketcomments values(?,?,?,?,?,?)";
	...
	*/
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (ottocrat_users.user_name not like '') then $userNameSql else ottocrat_groups.groupname end as user_name," .
					" ottocrat_activity.*, ottocrat_cntactivityrel.contactid, ottocrat_contactdetails.lastname, ottocrat_contactdetails.firstname," .
					" ottocrat_crmentity.crmid, ottocrat_recurringevents.recurringtype, ottocrat_crmentity.smownerid, ottocrat_crmentity.modifiedtime," .
					" ottocrat_seactivityrel.crmid as parent_id " .
					" from ottocrat_activity inner join ottocrat_seactivityrel on ottocrat_seactivityrel.activityid=ottocrat_activity.activityid" .
					...
					" where ottocrat_seactivityrel.crmid=".$id." and ottocrat_crmentity.deleted=0 and (activitytype NOT IN ('Emails'))" .
							" AND ( ottocrat_activity.status is NULL OR ottocrat_activity.status != 'Completed' )" .
							" and ( ottocrat_activity.eventstatus is NULL OR ottocrat_activity.eventstatus != 'Held') ";
	...
	function get_ticket_history($ticketid)
	{
		...
		$query="select title,update_log from ottocrat_troubletickets where ticketid=?";
	...
	**/
	function getColumnNames_Hd()
	{
		...
		{
			$sql1 = "select fieldlabel from ottocrat_field where tabid=13 and block <> 30 and ottocrat_field.uitype <> '61' and ottocrat_field.presence in (0,2)";
			$params1 = array();
		}else
		{
			$profileList = getCurrentUserProfileList();
			$sql1 = "select ottocrat_field.fieldid,fieldlabel from ottocrat_field inner join ottocrat_profile2field on ottocrat_profile2field.fieldid=ottocrat_field.fieldid inner join ottocrat_def_org_field on ottocrat_def_org_field.fieldid=ottocrat_field.fieldid where ottocrat_field.tabid=13 and ottocrat_field.block <> 30 and ottocrat_field.uitype <> '61' and ottocrat_field.displaytype in (1,2,3,4) and ottocrat_profile2field.visible=0 and ottocrat_def_org_field.visible=0 and ottocrat_field.presence in (0,2)";
			$params1 = array();
	...
	**/
	function getCustomerName($id)
	{
		...
		$log->debug("Entering getCustomerName(".$id.") method ...");
        	global $adb;
	        $sql = "select * from ottocrat_portalinfo inner join ottocrat_troubletickets on ottocrat_troubletickets.contact_id = ottocrat_portalinfo.id where ottocrat_troubletickets.ticketid=?";
	...
	// Function to create, export query for helpdesk module
        ...
        */
        function create_export_query($where)
        {
				...
				//Ticket changes--5198
				$fields_list = 	str_replace(",ottocrat_ticketcomments.comments as 'Add Comment'",' ',$fields_list);
				...
				INNER JOIN ottocrat_troubletickets
					ON ottocrat_troubletickets.ticketid =ottocrat_crmentity.crmid
				...
				LEFT JOIN ottocrat_contactdetails
					ON ottocrat_contactdetails.contactid = ottocrat_troubletickets.contact_id
				LEFT JOIN ottocrat_ticketcf
	...
	function get_history($id)
	{
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
							'ottocrat_users.first_name', 'last_name' => 'ottocrat_users.last_name'), 'Users');
		$query = "SELECT ottocrat_activity.activityid, ottocrat_activity.subject, ottocrat_activity.status, ottocrat_activity.eventstatus, ottocrat_activity.date_start, ottocrat_activity.due_date,ottocrat_activity.time_start,ottocrat_activity.time_end,ottocrat_activity.activitytype, ottocrat_troubletickets.ticketid, ottocrat_troubletickets.title, ottocrat_crmentity.modifiedtime,ottocrat_crmentity.createdtime, ottocrat_crmentity.description,
case when (ottocrat_users.user_name not like '') then $userNameSql else ottocrat_groups.groupname end as user_name
				...
				inner join ottocrat_troubletickets on ottocrat_troubletickets.ticketid = ottocrat_seactivityrel.crmid
				inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_activity.activityid
                                left join ottocrat_groups on ottocrat_groups.groupid=ottocrat_crmentity.smownerid
				left join ottocrat_users on ottocrat_users.id=ottocrat_crmentity.smownerid
	...
	function constructUpdateLog($focus, $mode, $assigned_group_name, $assigntype)
	{
		...
		else
		{
			...
			$tktresult = $adb->pquery("select * from ottocrat_troubletickets where ticketid=?", array($ticketid));
			$crmresult = $adb->pquery("select * from ottocrat_crmentity where crmid=?", array($ticketid));
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"ottocrat_seactivityrel","Attachments"=>"ottocrat_seattachmentsrel","Documents"=>"ottocrat_senotesrel");

		$tbl_field_arr = Array("ottocrat_seactivityrel"=>"activityid","ottocrat_seattachmentsrel"=>"attachmentsid","ottocrat_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("ottocrat_seactivityrel"=>"crmid","ottocrat_seattachmentsrel"=>"crmid","ottocrat_senotesrel"=>"crmid");
	...
	function generateReportsSecQuery($module,$secmodule, $queryplanner) {
		...
		$matrix->setDependency("ottocrat_crmentityHelpDesk",array("ottocrat_groupsHelpDesk","ottocrat_usersHelpDesk","ottocrat_lastModifiedByHelpDesk"));
		$matrix->setDependency("ottocrat_troubletickets",array("ottocrat_crmentityHelpDesk","ottocrat_ticketcf","ottocrat_crmentityRelHelpDesk","ottocrat_productsRel"));
		$matrix->setDependency("ottocrat_crmentityRelHelpDesk",array("ottocrat_accountRelHelpDesk","ottocrat_contactdetailsRelHelpDesk"));


		if (!$queryplanner->requireTable('ottocrat_troubletickets', $matrix)) {
			return '';
		}
		// TODO Support query planner
		$query = $this->getRelationQuery($module,$secmodule,"ottocrat_troubletickets","ticketid", $queryplanner);

		if ($queryplanner->requireTable("ottocrat_crmentityHelpDesk",$matrix)){
		    $query .=" left join ottocrat_crmentity as ottocrat_crmentityHelpDesk on ottocrat_crmentityHelpDesk.crmid=ottocrat_troubletickets.ticketid and ottocrat_crmentityHelpDesk.deleted=0";
		}
		if ($queryplanner->requireTable("ottocrat_ticketcf")){
		    $query .=" left join ottocrat_ticketcf on ottocrat_ticketcf.ticketid = ottocrat_troubletickets.ticketid";
		}
		if ($queryplanner->requireTable("ottocrat_crmentityRelHelpDesk",$matrix)){
		...
		if ($queryplanner->requireTable("ottocrat_accountRelHelpDesk")){
		...
		if ($queryplanner->requireTable("ottocrat_contactdetailsRelHelpDesk")){
		...
		if ($queryplanner->requireTable("ottocrat_productsRel")){
		...
		if ($queryplanner->requireTable("ottocrat_groupsHelpDesk")){
		    $query .=" left join ottocrat_groups as ottocrat_groupsHelpDesk on ottocrat_groupsHelpDesk.groupid = ottocrat_crmentityHelpDesk.smownerid";
		}
		if ($queryplanner->requireTable("ottocrat_usersHelpDesk")){
		    $query .=" left join ottocrat_users as ottocrat_usersHelpDesk on ottocrat_usersHelpDesk.id = ottocrat_crmentityHelpDesk.smownerid";
		}
		if ($queryplanner->requireTable("ottocrat_lastModifiedByHelpDesk")){
		    $query .=" left join ottocrat_users as ottocrat_lastModifiedByHelpDesk on ottocrat_lastModifiedByHelpDesk.id = ottocrat_crmentityHelpDesk.modifiedby ";
		}
        if ($queryplanner->requireTable("ottocrat_createdbyHelpDesk")){
			$query .= " left join ottocrat_users as ottocrat_createdbyHelpDesk on ottocrat_createdbyHelpDesk.id = ottocrat_crmentityHelpDesk.smcreatorid ";
		}
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("ottocrat_seactivityrel"=>array("crmid","activityid"),"ottocrat_troubletickets"=>"ticketid"),
			"Documents" => array("ottocrat_senotesrel"=>array("crmid","notesid"),"ottocrat_troubletickets"=>"ticketid"),
			"Products" => array("ottocrat_troubletickets"=>array("ticketid","product_id")),
			"Services" => array("ottocrat_crmentityrel"=>array("crmid","relcrmid"),"ottocrat_troubletickets"=>"ticketid"),
		);
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			$sql = 'UPDATE ottocrat_troubletickets SET parent_id=? WHERE ticketid=?';
			$this->db->pquery($sql, array(null, $id));
			$se_sql= 'DELETE FROM ottocrat_seticketsrel WHERE ticketid=?';
			$this->db->pquery($se_sql, array($id));
		} elseif($return_module == 'Contacts') {
			$sql = 'UPDATE ottocrat_troubletickets SET contact_id=? WHERE ticketid=?';
			$this->db->pquery($sql, array(null, $id));
			$se_sql= 'DELETE FROM ottocrat_seticketsrel WHERE ticketid=?';
			$this->db->pquery($se_sql, array($id));
		}elseif($return_module == 'Products') {
			$sql = 'UPDATE ottocrat_troubletickets SET product_id=? WHERE ticketid=?';
			$this->db->pquery($sql, array(null, $id));
		} else {
	...
	public static function getTicketEmailContents($entityData, $toOwner=false) {
		...
		$sql = "SELECT * FROM ottocrat_ticketcf WHERE ticketid = ?";
		...
		foreach ($cffields as $cfOneField) {
			if ($cfOneField != 'ticketid' && $cfOneField != 'from_portal') {
				...
				$sql = "SELECT fieldlabel FROM ottocrat_field WHERE columnname = ? and ottocrat_field.presence in (0,2)";
	...
	public static function getPortalTicketEmailContents($entityData) {
		...
		$portalUrl = "<a href='".$PORTAL_URL."/".Ottocrat_Request:: encryptLink("index.php?module=HelpDesk&action=index&ticketid=" . $entityId . "&fun=detail")."'>"
