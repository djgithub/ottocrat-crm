	...
	function handleEvent($eventName, $entityData) {
		...
		if($eventName == 'ottocrat.entity.aftersave.final') {
			...
			if ($moduleName == 'HelpDesk') {
				...
				$adb->pquery('UPDATE ottocrat_ticketcf SET from_portal=0 WHERE ticketid=?', array($ticketId));
			}
		}
	}
}
...
function HelpDesk_nofifyOnPortalTicketCreation($entityData) {
	...
	$result = $adb->pquery("SELECT email, concat (firstname,' ',lastname) as name FROM ottocrat_contactdetails WHERE contactid=?", array($parentId));
...
function HelpDesk_notifyOnPortalTicketComment($entityData) {
	...
	//get the contact email id who creates the ticket from portal and use this email as from email id in email
	$result = $adb->pquery("SELECT lastname, firstname, email FROM ottocrat_contactdetails WHERE contactid=?", array($parentId));
...
function HelpDesk_notifyParentOnTicketChange($entityData) {
	...
	//To get the emailoptout ottocrat_field value and then decide whether send mail about the tickets or not
	if(!empty($contactId[0])) {
		$result = $adb->pquery('SELECT email, emailoptout, lastname, firstname FROM ottocrat_contactdetails WHERE
		...
		//Get the status of the ottocrat_portal user. if the customer is active then send the ottocrat_portal link in the mail
		if($parent_email != '') {
		...
		$result = $adb->pquery("SELECT accountname, emailoptout, email1 FROM ottocrat_account WHERE accountid=?",
									array($accountId[1]));
		$emailoptout = $adb->query_result($result,0,'emailoptout');
		$parent_email = $adb->query_result($result,0,'email1');
		$parentname = $adb->query_result($result,0,'accountname');
	}
	//added condition to check the emailoptout(this is for contacts and ottocrat_accounts.)
	if($emailoptout == 0) {
		if($isPortalUser == 1) {
