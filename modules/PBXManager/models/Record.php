...
class PBXManager_Record_Model extends Ottocrat_Record_Model{
    
    const moduletableName = 'ottocrat_pbxmanager';
    const lookuptableName = 'ottocrat_pbxmanager_phonelookup';
    const entitytableName = 'ottocrat_crmentity';
    ...
    public function saveRecordWithArrray($params){
        $moduleModel = Ottocrat_Module_Model::getInstance('PBXManager');
        $recordModel = Ottocrat_Record_Model::getCleanInstance('PBXManager');
    ...
    public static function getUserNumbers(){
        $numbers = null;
        $db = PearDatabase::getInstance();
        $query = 'SELECT id, phone_crm_extension FROM ottocrat_users';
        $result = $db->pquery($query, array());
