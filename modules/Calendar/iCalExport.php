...
//$ical_query = "SELECT * FROM ottocrat_activity WHERE (( STATUS != 'Completed' AND STATUS != 'Deferred' ) OR STATUS IS NULL)
//               AND (( eventstatus != 'Held' AND eventstatus != 'Not Held' ) OR eventstatus IS NULL )";

global $current_user,$adb,$default_timezone;
$filename = $_REQUEST['filename'];
$ical_query = "select ottocrat_activity.*,ottocrat_crmentity.description,ottocrat_activity_reminder.reminder_time from ottocrat_activity inner join ottocrat_crmentity on ottocrat_activity.activityid = ottocrat_crmentity.crmid " .
	...
	" where ottocrat_crmentity.deleted = 0 and ottocrat_crmentity.smownerid = " . $current_user->id . 
