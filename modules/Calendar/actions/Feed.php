...
class Calendar_Feed_Action extends Ottocrat_BasicAjax_Action {
	...
	protected function pullEvents($start, $end, &$result, $userid = false,$color = null,$textColor = 'white') {
		...
		$moduleModel = Ottocrat_Module_Model::getInstance('Events');
		if($userid){
			$focus = new Users();
			$focus->id = $userid;
		...
		$query.= " AND ottocrat_activity.activitytype NOT IN ('Emails','Task') AND ";
        $hideCompleted = $currentUser->get('hidecompletedevents');
        if($hideCompleted)
            $query.= "ottocrat_activity.eventstatus != 'HELD' AND ";
        ...
        $query.= " AND ottocrat_crmentity.smownerid IN (".  generateQuestionMarks($params).")";
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			if(!$currentUser->isAdminUser() && $visibility == 'Private' && $userid && $userid != $currentUser->getId()) {
				...
				$item['url']   = Ottocrat_Request:: encryptLink(sprintf('index.php?module=Calendar&view=Detail&record=%s', $crmid));
			}
	...
	protected function pullTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$moduleModel = Ottocrat_Module_Model::getInstance('Calendar');
		...
		$query.= " AND ottocrat_activity.activitytype = 'Task' AND ";
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $hideCompleted = $currentUser->get('hidecompletedevents');
        if($hideCompleted)
            $query.= "ottocrat_activity.status != 'Completed' AND ";
	...
	protected function pullPotentials($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		foreach ($records as $record) {
			...
			$item['url']   = Ottocrat_Request:: encryptLink(sprintf('index.php?module=Potentials&view=Detail&record=%s', $crmid));
			$item['color'] = $color;
			$item['textColor'] = $textColor;
			$result[] = $item;
		}
	...
	protected function pullContactsBySupportEndDate($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		foreach ($records as $record) {
			...
			$item['url']   = Ottocrat_Request:: encryptLink(sprintf('index.php?module=Contacts&view=Detail&record=%s', $crmid));
			$item['color'] = $color;
			$item['textColor'] = $textColor;
			$result[] = $item;
		}
	...
	protected  function pullContactsByBirthday($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT firstname,lastname,birthday,crmid FROM ottocrat_contactdetails";
		...
		$query.= " INNER JOIN ottocrat_crmentity ON ottocrat_contactdetails.contactid = ottocrat_crmentity.crmid";
		$query.= " WHERE ottocrat_crmentity.deleted=0 AND smownerid IN (".  generateQuestionMarks($params) .") AND";
	...
	protected function pullInvoice($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		foreach ($records as $record) {
			...
			$item['url']   = Ottocrat_Request:: encryptLink(sprintf('index.php?module=Invoice&view=Detail&record=%s', $crmid));
			$item['color'] = $color;
			$item['textColor'] = $textColor;
			$result[] = $item;
		}
	...
	protected function pullProjects($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT projectname, startdate, targetenddate, crmid FROM ottocrat_project";
		$query.= " INNER JOIN ottocrat_crmentity ON ottocrat_project.projectid = ottocrat_crmentity.crmid";
		$query.= " WHERE ottocrat_crmentity.deleted=0 AND smownerid IN (". generateQuestionMarks($params) .") AND ";
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			$item['url']   = Ottocrat_Request:: encryptLink(sprintf('index.php?module=Project&view=Detail&record=%s', $crmid));
			$item['color'] = $color;
			$item['textColor'] = $textColor;
			$result[] = $item;
		}
	...
	protected function pullProjectTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT projecttaskname, startdate, enddate, crmid FROM ottocrat_projecttask";
		$query.= " INNER JOIN ottocrat_crmentity ON ottocrat_projecttask.projecttaskid = ottocrat_crmentity.crmid";
		$query.= " WHERE ottocrat_crmentity.deleted=0 AND smownerid IN (". generateQuestionMarks($params) .") AND ";
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			$item['url']   = Ottocrat_Request:: encryptLink(sprintf('index.php?module=ProjectTask&view=Detail&record=%s', $crmid));
			$item['color'] = $color;
			$item['textColor'] = $textColor;
			$result[] = $item;
		}
