...
// functions added for group calendar	-Jaguar
	...
	function status_availability($owner,$userid,$activity_id,$avail_date,$activity_start_time,$activity_end_time)	
	{
		...
		if( $owner != $userid)
		{
			...
			$usr_query="select activityid,ottocrat_activity.date_start,ottocrat_activity.due_date, ottocrat_activity.time_start,ottocrat_activity.duration_hours,ottocrat_activity.duration_minutes,ottocrat_crmentity.smownerid from ottocrat_activity,ottocrat_crmentity where ottocrat_crmentity.crmid=ottocrat_activity.activityid and ('".$avail_date."' like date_start) and ottocrat_crmentity.smownerid=? and ottocrat_activity.activityid !=?  and ottocrat_crmentity.deleted=0";
		}
		else
		{
			$usr_query="select activityid,ottocrat_activity.date_start,ottocrat_activity.due_date, ottocrat_activity.time_start,ottocrat_activity.duration_hours,ottocrat_activity.duration_minutes,ottocrat_crmentity.smownerid from ottocrat_activity,ottocrat_crmentity where ottocrat_crmentity.crmid=ottocrat_activity.activityid and ('".$avail_date."' like date_start) and ottocrat_crmentity.smownerid=? and ottocrat_activity.activityid !=? and ottocrat_crmentity.deleted=0";
		...
		if($avail_flag!="true")
		{
			$recur_query="SELECT ottocrat_activity.activityid, ottocrat_activity.time_start, ottocrat_activity.duration_hours, ottocrat_activity.duration_minutes , ottocrat_crmentity.smownerid, ottocrat_recurringevents.recurringid, ottocrat_recurringevents.recurringdate as date_start from ottocrat_activity inner join ottocrat_crmentity on ottocrat_activity.activityid = ottocrat_crmentity.crmid inner join ottocrat_recurringevents on ottocrat_activity.activityid=ottocrat_recurringevents.activityid where ('".$avail_date."' like ottocrat_recurringevents.recurringdate) and ottocrat_crmentity.smownerid=? and ottocrat_activity.activityid !=? and ottocrat_crmentity.deleted=0";
	 	...
	 	if($avail_flag == "true")
                {
                        $availability=' <IMG SRC="' . ottocrat_imageurl('busy.gif', $theme). '">';
                }
                else
                {
                        $availability=' <IMG SRC="' . ottocrat_imageurl('free.gif', $theme). '">';
                }
