...
/**
 * To get the lists of ottocrat_users id who shared their calendar with specified user
 * @param $sharedid -- The shared user id :: Type integer
 * @returns $shared_ids -- a comma seperated ottocrat_users id  :: Type string
 */
function getSharedCalendarId($sharedid)
{
	global $adb;
	$query = "SELECT * from ottocrat_sharedcalendar where sharedid=?";
...
/**
 * Function to get the ottocrat_activity details for mail body
	...
	if(!empty($only_for_user)) {
		if($only_for_user != 'ALL') {
			...
			if($only_for_user == $current_user->id) {
				...
				$qcondition = " AND ottocrat_crmentity.smownerid IN (" . $user_group_ids .")";
			} else {
				$qcondition = " AND ottocrat_crmentity.smownerid = "  . $adb->sql_escape_string($only_for_user);
			}
		}
	}
	return $qcondition;
