 ...
 */
class Calendar_Module_Model extends Ottocrat_Module_Model {
	...
	public function getCalendarViewUrl() {
		return Ottocrat_Request:: encryptLink( 'index.php?module='.$this->get('name').'&view='	.$this->getCalendarViewName());
	}
	...
	public function getImportResultUrl() {
		return Ottocrat_Request:: encryptLink( 'index.php?module='.$this->getName().'&view=ImportResult');
	}
	...
	public function getExportQuery($focus = '', $where = '') {
		...
		$query = "SELECT ottocrat_activity.*, ottocrat_crmentity.description, ottocrat_activity_reminder.reminder_time FROM ottocrat_activity
					INNER JOIN ottocrat_crmentity ON ottocrat_activity.activityid = ottocrat_crmentity.crmid
					LEFT JOIN ottocrat_activity_reminder ON ottocrat_activity_reminder.activity_id = ottocrat_activity.activityid AND ottocrat_activity_reminder.recurringid = 0
					WHERE ottocrat_crmentity.deleted = 0 AND ottocrat_crmentity.smownerid = $userId AND ottocrat_activity.activitytype NOT IN ('Emails')";
		return $query;
	}
	...
	public function getDetailViewUrl($id) {
		return Ottocrat_Request:: encryptLink( 'index.php?module=Calendar&view='.$this->getDetailViewName()
			.'&record='.$id);
	}
	...
	*/
	public static function getCaledarSharedUsers($id){
        ...
        $query = "SELECT ottocrat_users.user_name, ottocrat_sharedcalendar.* FROM ottocrat_sharedcalendar
				LEFT JOIN ottocrat_users ON ottocrat_sharedcalendar.sharedid=ottocrat_users.id WHERE userid=?";
	...
	*/
	public static function getSharedUsersOfCurrentUser($id){
		...
		$query = "SELECT ottocrat_users.first_name,ottocrat_users.last_name, ottocrat_users.id as userid
			FROM ottocrat_sharedcalendar RIGHT JOIN ottocrat_users ON ottocrat_sharedcalendar.userid=ottocrat_users.id and status= 'Active'
	...
	*/
	public static function getSharedUsersInfoOfCurrentUser($id){
		...
		$query = "SELECT shareduserid,color,visible FROM ottocrat_shareduserinfo where userid = ?";
        $result = $db->pquery($query, array($id));
        $rows = $db->num_rows($result);
	...
	*/
	public static function getCalendarViewTypes($id){
		...
		$query = "SELECT * FROM ottocrat_calendar_user_activitytypes 
			...
			WHERE ottocrat_calendar_user_activitytypes.userid=?";
        $result = $db->pquery($query, array($id));
        $rows = $db->num_rows($result);
        ...
        for($i=0; $i<$rows; $i++){
			...
			$moduleInstance = Ottocrat_Module::getInstance($activityTypes['module']);
			$fieldInstance = Ottocrat_Field::getInstance($activityTypes['fieldname'], $moduleInstance);
			if($fieldInstance) {
	...
	public function getSharedCalendarViewUrl() {
		return Ottocrat_Request:: encryptLink( 'index.php?module='.$this->get('name').'&view=SharedCalendar');
	}
	...
	public function deleteSharedUsers($currentUserId){
		$db = PearDatabase::getInstance();
		$delquery = "DELETE FROM ottocrat_sharedcalendar WHERE userid=?";
	...
	public function getSharedType($currentUserId){
		...
		$query = "SELECT calendarsharedtype FROM ottocrat_users WHERE id=?";
	...
	public function getRecentRecords($limit=10) {
		...
		if($nonAdminQuery){
			$query .= " INNER JOIN ottocrat_activity ON ottocrat_crmentity.crmid = ottocrat_activity.activityid ".$nonAdminQuery;
		}
	...
	public static function getCalendarReminder() {
		...
		if($activityReminder != '' ) {
			...
			$reminderActivitiesResult = "SELECT reminderid, recordid FROM ottocrat_activity_reminder_popup
            					INNER JOIN ottocrat_activity on ottocrat_activity.activityid = ottocrat_activity_reminder_popup.recordid
								INNER JOIN ottocrat_crmentity ON ottocrat_activity_reminder_popup.recordid = ottocrat_crmentity.crmid
								WHERE ottocrat_activity_reminder_popup.status = 0
								AND ottocrat_crmentity.smownerid = ? AND ottocrat_crmentity.deleted = 0
								AND ((DATE_FORMAT(ottocrat_activity_reminder_popup.date_start,'%Y-%m-%d') <= ?)
								AND (TIME_FORMAT(ottocrat_activity_reminder_popup.time_start,'%H:%i') <= ?))
                                                                AND ottocrat_activity.eventstatus <> 'Held' LIMIT 20";
			...
			for($i=0; $i<$rows; $i++) {
				...
				$recordModels[] = Ottocrat_Record_Model::getInstanceById($recordId, 'Calendar');
			}
		}
		return $recordModels;
	}
	...
	public function getSettingLinks() {
		...
		if($currentUserModel->isAdminUser()) {
			$settingLinks[] = array(
					...
					'linkicon' => Ottocrat_Theme::getImagePath('LayoutEditor.gif')
			);
