...
class Calendar_Record_Model extends Ottocrat_Record_Model {
...
/**
	...
	public function getDetailViewUrl() {
		...
		return Ottocrat_Request:: encryptLink('index.php?module=Calendar&view='.$module->getDetailViewName().'&record='.$this->getId());
	}
	...
	function save() {
		...
		$_REQUEST['time_start'] = Ottocrat_Time_UIType::getTimeValueWithSeconds($_REQUEST['time_start']);
		$_REQUEST['time_end'] = Ottocrat_Time_UIType::getTimeValueWithSeconds($_REQUEST['time_end']);
		parent::save();
	}
	...
	public function getRecurringObject() {
		...
		$query = 'SELECT ottocrat_recurringevents.*, ottocrat_activity.date_start, ottocrat_activity.time_start, ottocrat_activity.due_date, ottocrat_activity.time_end FROM ottocrat_recurringevents
					INNER JOIN ottocrat_activity ON ottocrat_activity.activityid = ottocrat_recurringevents.activityid
					WHERE ottocrat_recurringevents.activityid = ?';
	...
	public function updateReminderStatus($status=1) {
		$db = PearDatabase::getInstance();
		$db->pquery("UPDATE ottocrat_activity_reminder_popup set status = ? where recordid = ?", array($status, $this->getId()));
