...
Class Calendar_Edit_View extends Ottocrat_Edit_View {
	...
	function Events($request, $moduleName) {
		...
		$eventModule = Ottocrat_Module_Model::getInstance($moduleName);
		...
		foreach($requestFieldList as $fieldName=>$fieldValue){
            ...
            if (empty($record) && ($fieldName == 'date_start' || $fieldName == 'due_date') && !empty($fieldValue)) { 
                if($fieldName == 'date_start'){
                    ...
                    $endTime = Ottocrat_Time_UIType::getTimeValueWithSeconds($requestFieldList['time_end']);
                    $endDateTime = Ottocrat_Datetime_UIType::getDBDateTimeValue($fieldValue." ".$endTime);
                    list($endDate, $endTime) = explode(' ', $endDateTime);
                    $fieldValue = Ottocrat_Date_UIType::getDisplayDateValue($endDate);
                }
            }
		...
		$viewer->assign('TOMORROWDATE', Ottocrat_Date_UIType::getDisplayDateValue(date('Y-m-d', time()+86400)));
		...
		if(!empty($requestContactIdValue)) {
			$existingRelatedContacts[] = array('name' => Ottocrat_Util_Helper::getRecordName($requestContactIdValue) ,'id' => $requestContactIdValue);
		}
