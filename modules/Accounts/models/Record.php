...
class Accounts_Record_Model extends Ottocrat_Record_Model {
	...
	function getAccountHierarchy() {
		...
		foreach($hierarchy['entries'] as $accountId => $accountInfo) {
			...
			if($matches != null) {
				...
				$recordModel = Ottocrat_Record_Model::getCleanInstance('Accounts');
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM ottocrat_crmentity WHERE setype = ? AND label = ? AND deleted = 0";
