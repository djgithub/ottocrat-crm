...
class Accounts_DetailView_Model extends Ottocrat_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		foreach($linkModelList['DETAILVIEW'] as $link) {
			if($link->linklabel == 'View History' || $link->linklabel == 'Send SMS') {
				...
				$link->linkurl = 'javascript:Accounts_Detail_Js.triggerAccountHierarchy("'.	Ottocrat_Request::encryptLink( $linkURL).'");';
				unset($linkModelList['DETAILVIEW'][$index]);
		...
		if(!empty($SMSNotifierModuleModel) && $currentUserModel->hasModulePermission($SMSNotifierModuleModel->getId())) {
			$basicActionLink = array(
				...
				'linkurl' => 'javascript:Ottocrat_Detail_Js.triggerSendSms("'. Ottocrat_Request::encryptLink( "index.php?module='.$this->getModule()->getName().'&view=MassActionAjax&mode=showSendSMSForm").'","SMSNotifier");',
				'linkicon' => ''
			);
		...
		if($currentUserModel->hasModuleActionPermission($moduleModel->getId(), 'EditView')) {
			$massActionLink = array(
				...
				'linkurl' => 'javascript:Ottocrat_Detail_Js.triggerTransferOwnership("'. Ottocrat_Request::encryptLink( "index.php?module='.$moduleModel->getName().'&view=MassActionAjax&mode=transferOwnership").'")',
				'linkicon' => ''
			);
			$linkModelList['DETAILVIEW'][] = Ottocrat_Link_Model::getInstanceFromValues($massActionLink);
		}
