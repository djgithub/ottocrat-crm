...
class Products_Relation_Model extends Ottocrat_Relation_Model {
	...
	public function getQuery($recordModel, $actions=false){
		...
		if($functionName == 'get_product_pricebooks'){
			$selectColumnSql = $selectColumnSql.' ,ottocrat_pricebookproductrel.listprice, ottocrat_pricebook.currency_id, ottocrat_products.unit_price';
		}
		if($functionName == 'get_service_pricebooks'){
			$selectColumnSql = $selectColumnSql.' ,ottocrat_pricebookproductrel.listprice, ottocrat_pricebook.currency_id, ottocrat_service.unit_price';
		}
	...
	public function deleteRelation($sourceRecordId, $relatedRecordId) {
		...
		if(($sourceModuleName == 'Products' || $sourceModuleName == 'Services') && $relatedModuleName == 'PriceBooks') {
			...
			$priceBookModel = Ottocrat_Record_Model::getInstanceById($relatedRecordId, $relatedModuleName);
	...
	public function deleteProductToProductRelation($sourceRecordId, $relatedRecordId) {
		...
		if(!empty($sourceRecordId) && !empty($relatedRecordId)){
			$db->pquery('DELETE FROM ottocrat_seproductsrel WHERE crmid = ? AND productid = ?', array($relatedRecordId, $sourceRecordId));
			return true;
		}
	}
	...
	public function isSubProduct($subProductId){
		if(!empty($subProductId)){
			$db = PearDatabase::getInstance();
			$result = $db->pquery('SELECT crmid FROM ottocrat_seproductsrel WHERE crmid = ?', array($subProductId));
	...
	public function addListPrice($sourceRecordId, $destinationRecordId, $listPrice) {
		$sourceModuleName = $this->getParentModuleModel()->get('name');
		$relatedModuleName = $this->getRelationModuleModel()->get('name');
		$relationModuleModel = Ottocrat_Record_Model::getInstanceById($destinationRecordId, $relatedModuleName);
		
		$productModel = Ottocrat_Record_Model::getInstanceById($sourceRecordId, $sourceModuleName);
