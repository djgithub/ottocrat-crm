...
class Products_ListView_Model extends Ottocrat_ListView_Model {
	...
	public function addSubProductsQuery($listQuery){
		...
		$query = " LEFT JOIN ottocrat_seproductsrel ON ottocrat_seproductsrel.crmid = ottocrat_products.productid AND ottocrat_seproductsrel.setype='Products'";
		$splitQuery[0] .= $query;
		$productId = $this->get('productId');
		$query1 = " AND ottocrat_seproductsrel.productid = $productId";
		$splitQuery[1] .= $query1;
	...
	public function getSubProducts($subProductId){
		...
		if(!empty($subProductId)){
            $db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT ottocrat_seproductsrel.crmid from ottocrat_seproductsrel INNER JOIN
                ottocrat_crmentity ON ottocrat_seproductsrel.crmid = ottocrat_crmentity.crmid 
					AND ottocrat_crmentity.deleted = 0 AND ottocrat_seproductsrel.setype=? 
				WHERE ottocrat_seproductsrel.productid=?", array($this->getModule()->get('name'), $subProductId ));
