 ...
 * All Rights Reserved.
 *************************************************************************************/
jQuery.Class("Ottocrat_Base_Validator_Js",{
	...
	invokeValidation: function(field, rules, i, options){
		...
		var listOfValidators = Ottocrat_Base_Validator_Js.getValidator(field);
	...
	getValidator: function(field){
		...
		if(Ottocrat_Base_Validator_Js.moduleName == false && moduleEle.length > 0) {
			Ottocrat_Base_Validator_Js.moduleName = moduleEle.val();
		}
		
		var fieldInstance = Ottocrat_Field_Js.getInstance(fieldInfo);
		var validatorsOfType = Ottocrat_Base_Validator_Js.getValidatorsFromFieldType(fieldInstance);
		...
		if(fieldData.hasOwnProperty(dataValidator)){
			...
			for(var key in specialValidators){
				...
				var validatorOfNames = Ottocrat_Base_Validator_Js.getValidatorClassName(specialValidator.name);
				if(validatorOfNames != ""){
	...
	getValidatorsFromFieldType: function(fieldInstance){
		...
		validatorsOfType.push(Ottocrat_Base_Validator_Js.getValidatorClassName(fieldType));
        return validatorsOfType;
	},
	
	getValidatorClassName: function(validatorName){
		var validatorsOfType = '';
		var className = Ottocrat_Base_Validator_Js.getClassName(validatorName);
		var fallBackClassName = Ottocrat_Base_Validator_Js.getFallBackClassName(validatorName);
	...
	getClassName: function(validatorName){
		if(Ottocrat_Base_Validator_Js.moduleName != false) {
			var moduleName = Ottocrat_Base_Validator_Js.moduleName;
		} else {
	...
	getFallBackClassName: function(validatorName){
		return "Ottocrat_"+validatorName+"_Validator_Js";
	}
},{
	field: "",
	error: "",
