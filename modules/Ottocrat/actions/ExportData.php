...
class Ottocrat_ExportData_Action extends Ottocrat_Mass_Action {
	 ...
	 * @param Ottocrat_Request $request
	 */
	function ExportData(Ottocrat_Request $request) {
		$db = PearDatabase::getInstance();
		$moduleName = $request->get('source_module');

		$this->moduleInstance = Ottocrat_Module_Model::getInstance($moduleName);
	 ...
	 */
	function getExportQuery(Ottocrat_Request $request) {
		...
		switch($mode) {
			...
			case 'ExportCurrentPage' :	$pagingModel = new Ottocrat_Paging_Model();
	 ...
	 * @return <String>
	 */
	function getExportContentType(Ottocrat_Request $request) {
	...
	function sanitizeValues($arr){
		...
		if(empty ($this->fieldArray)){
			...
			foreach($this->fieldArray as $fieldName => $fieldObj){
				...
				if($fieldObj->get('table') == 'ottocrat_inventoryproductrel' && ($fieldName == 'discount_amount' || $fieldName == 'discount_percent')){
		...
		foreach($arr as $fieldName=>&$value){
			...
			if($fieldname != 'hdnTaxType' && ($uitype == 15 || $uitype == 16 || $uitype == 33)){
				...
				$value = Ottocrat_Util_Helper::getOwnerName($value);
