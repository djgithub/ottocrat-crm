...
class Ottocrat_BasicAjax_View extends Ottocrat_Basic_View {
	 ...
	 */
	function showAdvancedSearch(Ottocrat_Request $request) {
		//Modules for which search is excluded
		$excludedModuleForSearch = array('Ottocrat', 'Reports');
	 ...
	 * @param Ottocrat_Request $request
	 */
	function showSearchResults(Ottocrat_Request $request) {
		...
		if(is_array($advFilterList) && count($advFilterList) > 0) {
			...
			for($i=0; $i<$rows; ++$i) {
				...
				$recordInstance = Ottocrat_Record_Model::getInstanceById($row[0]);
			...
			$matchingRecords =  Ottocrat_Record_Model::getSearchResult($searchKey, $searchModule);
		}
