...
class Ottocrat_MassActionAjax_View extends Ottocrat_IndexAjax_View {
	 ...
	 */
	function showMassEditForm (Ottocrat_Request $request){
		...
		$recordStructureInstance = Ottocrat_RecordStructure_Model::getInstanceForModule($moduleModel, Ottocrat_RecordStructure_Model::RECORD_STRUCTURE_MODE_MASSEDIT);
		$fieldInfo = array();
	 ...
	 * @param Ottocrat_Request $request
	 */
	function showAddCommentForm(Ottocrat_Request $request){
	 ...
	 * @param Ottocrat_Request $request
	 */
	function showComposeEmailForm(Ottocrat_Request $request) {
		$moduleName = 'Emails';
		$sourceModule = $request->getModule();
		$cvId = $request->get('viewname');
		$selectedIds = $request->get('selected_ids');
		...
		$moduleModel = Ottocrat_Module_Model::getInstance($sourceModule);
        ...
        if($emailFieldCount > 1) {
            ...
            $queryWithFromClause = 'SELECT '. implode(',',$emailColumnNames). ' FROM ottocrat_crmentity ';
            foreach($emailFields as $emailFieldModel) {
                ...
                $queryWithFromClause .= ' INNER JOIN '.$fieldTableName .
                            ' ON '.$fieldTableName.'.'.$tabNameIndexList[$fieldTableName].'= ottocrat_crmentity.crmid';
            }
            $query =  $queryWithFromClause . ' WHERE ottocrat_crmentity.deleted = 0 AND crmid IN ('.  generateQuestionMarks($recordIds).') AND (';
	 ...
	 * @param Ottocrat_Request $request
	 */
	function showSendSMSForm(Ottocrat_Request $request) {
		...
		if(count($selectedIds) == 1){
			...
			$selectedRecordModel = Ottocrat_Record_Model::getInstanceById($recordId, $sourceModule);
	 ...
	 * @return integer
	 */
	function getRecordsListFromRequest(Ottocrat_Request $request, $module = false) {
		$cvId = $request->get('viewname');
		$selectedIds = $request->get('selected_ids');
		$excludedIds = $request->get('excluded_ids');
        if(empty($module)) {
	 ...
	 * @param Ottocrat_Request $request
	 */
	function showMailMergeTemplates(Ottocrat_Request $request) {
		$selectedIds = $request->get('selected_ids');
		$excludedIds = $request->get('excluded_ids');
		$cvId = $request->get('viewname');
		$module = $request->getModule();
	 ...
	 * @param Ottocrat_Request $request
	 */
	function showDuplicatesSearchForm(Ottocrat_Request $request) {
		$module = $request->getModule();
	...
	function transferOwnership(Ottocrat_Request $request){
		$module = $request->getModule();
		$moduleModel = Ottocrat_Module_Model::getInstance($module);
