...
class Ottocrat_ComposeEmail_View extends Ottocrat_Footer_View {
	...
	public function composeMailData($request){
		...
		if ($documentIds) {
			...
			foreach ($documentIds as $documentId) {
				$documentRecordModel = Ottocrat_Record_Model::getInstanceById($documentId, $sourceModule);
		...
		if ($sourceRecordId) {
			$sourceRecordModel = Ottocrat_Record_Model::getInstanceById($sourceRecordId);
		...
		foreach($selectIds as $id) {
			if ($id) {
				...
				if (count($parentIdComponents) > 1) {
					...
					$recordModel = Ottocrat_Record_Model::getInstanceById($id, $fieldModule);
				} else {
					$recordModel = Ottocrat_Record_Model::getInstanceById($id);
				}
				if($selectedFields){
		...
		$documentsModel = Ottocrat_Module_Model::getInstance('Documents');
		$documentsURL = $documentsModel->getInternalDocumentsURL();

		$emailTemplateModuleModel = Settings_Ottocrat_Module_Model::getInstance('Settings:EmailTemplates');
		...
		$viewer->assign('MODULE_IS_ACTIVE', $userPrevilegesModel->hasModulePermission(Ottocrat_Module_Model::getInstance('EmailTemplates')->getId()));
		//
	...
	function emailEdit($request){
        ...
        foreach($parentIds as $index=>$parentFieldId) {
			...
			if ($parentIdComponents[1] === '-1') {
				...
				$parentRecordModel = Ottocrat_Record_Model::getInstanceById($parentId);
			}
