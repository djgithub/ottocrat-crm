...
class Ottocrat_Detail_View extends Ottocrat_Index_View {
	...
	function preProcess(Ottocrat_Request $request, $display=true) {
		...
		$recordStrucure = Ottocrat_RecordStructure_Model::getInstanceFromRecordModel($recordModel, Ottocrat_RecordStructure_Model::RECORD_STRUCTURE_MODE_DETAIL);
		$summaryInfo = array();
...
#print_r($detailViewLinks);
	...
	public function getHeaderScripts(Ottocrat_Request $request) {
		...
		$jsFileNames = array(
			...
			'modules.Ottocrat.resources.RelatedList',
	 ...
	 * @return <type>
	 */
	function showModuleDetailView(Ottocrat_Request $request) {
		$recordId = $request->get('record');
		$moduleName = $request->getModule();
	...
	function showModuleSummaryView($request) {
		...
		$viewer = $this->getViewer($request);
		$viewer->assign('RECORD', $recordModel);
		$full_mode_url=Ottocrat_Request::encryptLink($recordModel->getDetailUrlWNoChange().'&mode=showDetailViewByMode&requestMode=full');

        $viewer->assign('FULL_MODE_URL', $full_mode_url);
		$viewer->assign('BLOCK_LIST', $moduleModel->getBlocks());
		$viewer->assign('USER_MODEL', Users_Record_Model::getCurrentUserModel());
	 ...
	 * @param Ottocrat_Request $request
	 */
	function showRecentActivities (Ottocrat_Request $request) {
	 ...
	 * @return <type>
	 */
	function showRecentComments(Ottocrat_Request $request) {
	 ...
	 * @return <type>
	 */
	function showRelatedList(Ottocrat_Request $request) {
		$moduleName = $request->getModule();
		...
		try {
			$targetControllerClass = Ottocrat_Loader::getComponentClassName('View', 'In'.$moduleName.'Relation', $relatedModuleName);
		}catch(AppException $e) {
			try {
				// If any module wants to have same view for all the relation, then invoke this.
				$targetControllerClass = Ottocrat_Loader::getComponentClassName('View', 'InRelation', $relatedModuleName);
			}catch(AppException $e) {
				// Default related list
				$targetControllerClass = Ottocrat_Loader::getComponentClassName('View', 'RelatedList', $moduleName);
			}
		}
	 ...
	 * @return <type>
	 */
	function showChildComments(Ottocrat_Request $request) {
	 ...
	 * @return <type>
	 */
	function showAllComments(Ottocrat_Request $request) {
		$parentRecordId = $request->get('record');
