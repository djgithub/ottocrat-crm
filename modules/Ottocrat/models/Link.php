...
/**
 * Ottocrat Link Model Class
 */
class Ottocrat_Link_Model extends Ottocrat_Link {
	 ...
	 * @return Ottocrat_Link_Model instance
	 */
	...
	public function getIconPath() {
		...
		return Ottocrat_Theme::getImagePath($this->getIcon());
	}
	 ...
	 * @param Ottocrat_Link_Model $link - link model
	 * @result Ottocrat_Link_Model - current Instance;
	 */
	public function addChildLink(Ottocrat_Link_Model $link) {
	 ...
	 * @result <array> - list of Ottocrat_Link_Model instances
	 */
	...
	public function convertToNativeLink() {
		...
		if(!$this->isPageLoadLink()){
           ...
           $url = Ottocrat_Util_Helper::toSafeHTML($url);
           return $url;
		}
		...
		if(!empty($module) && !empty ($sourceModule) && !empty($sourceRecord)) {
			...
			$relatedModuleModel = Ottocrat_Module_Model::getInstance($module);
	...
	/**
	 * Function to get the instance of Ottocrat Link Model from the given array of key-value mapping
	...
	/**
	 * Function to get the instance of Ottocrat Link Model from a given Ottocrat_Link object
	...
	/**
	 * Function to get all the Ottocrat Link Models for a module of the given list of link types
	 * @param <Number> $tabid
	 * @param <Array> $type
	...
	public static function getAllByType($tabid, $type = false, $parameters = false) {
		$links = Ottocrat_Cache::get('links-'.$tabid, $type);
		if(!$links) {
			$links = parent::getAllByType($tabid, $type, $parameters);
			Ottocrat_Cache::set('links-'.$tabid, $type, $links);
		}
