...
class Ottocrat_RelationListView_Model extends Ottocrat_Base_Model {
	...
	public function getSelectRelationLinks() {
		...
		foreach($selectLinkList as $selectLink) {
			$selectLinkModel[] = Ottocrat_Link_Model::getInstanceFromValues($selectLink);
		}
		return $selectLinkModel;
	}
	...
	public function getEntries($pagingModel) {
		...
		if($orderBy) {
            ...
            if($orderByFieldModuleModel && $orderByFieldModuleModel->isReferenceField()) {
                ...
                $qualifiedOrderBy = 'ottocrat_crmentity'.$orderByFieldModuleModel->get('column');
                $selectAndFromClause .= ' LEFT JOIN ottocrat_crmentity AS '.$qualifiedOrderBy.' ON '.
                ...
                $query .= ' ORDER BY '.$qualifiedOrderBy.'.label '.$sortOrder;
				 ...
				 $query .= ' ORDER BY COALESCE(CONCAT(ottocrat_users.first_name,ottocrat_users.last_name),ottocrat_groups.groupname) '.$sortOrder;
			} else{
                   ...
                   if ($relationModuleName == 'Documents' && $col == 'filename') {
                        ...
                        if ($fileName != '' && $status == 1) {
                            if ($downloadType == 'I') {
                                ...
                                $val = '<a onclick="Javascript:Documents_Index_Js.updateDownloadCount(\''.Ottocrat_Request:: encryptLink('index.php?module=Documents&action=UpdateDownloadCount&record=' . $recordId ). '\');"' .
                                        ' href="'.Ottocrat_Request:: encryptLink('index.php?module=Documents&action=DownloadFile&record=' . $recordId. '&fileid=' . $fileId ). '"' .
	...
	public function getRelationQuery() {
		...
		$query = $queryComponents[0].' ,ottocrat_crmentity.crmid FROM '.$queryComponents[1];
	...
	public static function getInstance($parentRecordModel, $relationModuleName, $label=false) {
		...
		$className = Ottocrat_Loader::getComponentClassName('Model', 'RelationListView', $parentModuleName);
		$instance = new $className();

		$parentModuleModel = $parentRecordModel->getModule();
		$relatedModuleModel = Ottocrat_Module_Model::getInstance($relationModuleName);
		$instance->setRelatedModuleModel($relatedModuleModel);
		
		$relationModel = Ottocrat_Relation_Model::getInstance($parentModuleModel, $relatedModuleModel, $label);
		...
		if(!$relationModel){
			...
			foreach ($referenceFieldOfParentModule as $fieldName=>$fieldModel) {
				...
				if(in_array($relatedModuleName, $refredModulesOfReferenceField)){
					$relationModelClassName = Ottocrat_Loader::getComponentClassName('Model', 'Relation', $parentModuleModel->getName());
	...
	public function getRelatedEntriesCount() {
		...
		if ($position) {
			...
			$relationQuery = 'SELECT COUNT(DISTINCT ottocrat_crmentity.crmid) AS count'; 
    ...
    public function getCurrencySymbol($recordId, $fieldModel) {
        ...
        if(($fieldName == 'currency_id') && ($moduleName == 'Products' || $moduleName == 'Services')) {
            $query = "SELECT currency_symbol FROM ottocrat_currency_info WHERE id = ("; 
            if($moduleName == 'Products') 
                $query .= "SELECT currency_id FROM ottocrat_products WHERE productid = ?)"; 
            else if($moduleName == 'Services')
                $query .= "SELECT currency_id FROM ottocrat_service WHERE serviceid = ?)"; 

            $result = $db->pquery($query, array($recordId)); 
            return $db->query_result($result, 0, 'currency_symbol');    
        } else if(($tableName == 'ottocrat_invoice' || $tableName == 'ottocrat_quotes' || $tableName == 'ottocrat_purchaseorder' || $tableName == 'ottocrat_salesorder') &&
            ...
            $query = "SELECT currency_symbol FROM ottocrat_currency_info WHERE id = ( SELECT currency_id FROM ".$tableName." WHERE ".$focus->table_index." = ? )";
            $result = $db->pquery($query, array($recordId)); 
            return $db->query_result($result, 0, 'currency_symbol');
        } else {
