 ...
 *************************************************************************************/
/**
 * Ottocrat Widget Model Class
 */
class Ottocrat_Widget_Model extends Ottocrat_Base_Model {
	...
	/**
	 * Function to get the instance of Ottocrat Widget Model from the given array of key-value mapping
	 * @param <Array> $valueMap
	 * @return Ottocrat_Widget_Model instance
	 */
	...
	public static function getInstance($linkId, $userId) {
		...
		$result = $db->pquery('SELECT * FROM ottocrat_module_dashboard_widgets
			INNER JOIN ottocrat_links ON ottocrat_links.linkid = ottocrat_module_dashboard_widgets.linkid
			WHERE linktype = ? AND ottocrat_links.linkid = ? AND userid = ?', array('DASHBOARDWIDGET', $linkId, $userId));
	...
	public static function updateWidgetPosition($position, $linkId, $widgetId, $userId) {
		...
		$db = PearDatabase::getInstance();
		$sql = 'UPDATE ottocrat_module_dashboard_widgets SET position=? WHERE userid=?';
	...
	public static function getInstanceWithWidgetId($widgetId, $userId) {
		...
		$result = $db->pquery('SELECT * FROM ottocrat_module_dashboard_widgets
			INNER JOIN ottocrat_links ON ottocrat_links.linkid = ottocrat_module_dashboard_widgets.linkid
			WHERE linktype = ? AND ottocrat_module_dashboard_widgets.id = ? AND userid = ?', array('DASHBOARDWIDGET', $widgetId, $userId));
	...
	public function add() {
		...
		$sql = 'SELECT id FROM ottocrat_module_dashboard_widgets WHERE linkid = ? AND userid = ?';
		...
		if(!$db->num_rows($result)) {
			$db->pquery('INSERT INTO ottocrat_module_dashboard_widgets(linkid, userid, filterid, title, data) VALUES(?,?,?,?,?)',
	...
	public function remove() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ottocrat_module_dashboard_widgets WHERE id = ? AND userid = ?',
	...
	public function getDeleteUrl() {
		$url = 'index.php?module=Ottocrat&action=RemoveWidget&linkid='. $this->get('linkid');
		$widgetid = $this->has('widgetid')? $this->get('widgetid') : $this->get('id');
		if ($widgetid) $url .= '&widgetid=' . $widgetid;
