...
class Ottocrat_ModuleMeta_Model extends Ottocrat_Base_Model {
	...
	/**
	 * creates an instance of Ottocrat_ModuleMeta_Model
	 * @param <String> $name - module name
	 * @param <Object> $user - Users Object
	 * @return Ottocrat_ModuleMeta_Model
	 */
	public static function getInstance($name, $user) {
		$self = new Ottocrat_ModuleMeta_Model();
	 ...
	 * @return <Array of Ottocrat_Field>
	 */
		...
		if(method_exists($focus, 'getImportableFields')) {
			...
			foreach($moduleFields as $fieldName => $fieldInstance) {
				if(($this->isEditableField($fieldInstance)
							&& ($fieldInstance->getTableName() != 'ottocrat_crmentity' || $fieldInstance->getColumnName() != 'modifiedby')
