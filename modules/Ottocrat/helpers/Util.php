...
class Ottocrat_Util_Helper {
	 ...
	 * @return <String> ottocrat6 displayable data
	 */
	public static function toOttocrat6SafeHTML($input) {
		$allowableTags = '<a><br>';
    ...
    public static function checkRecordExistance($recordId){
        global $adb;
        $query = 'Select deleted from ottocrat_crmentity where crmid=?';
	...
	public static function formatDateIntoStrings($date, $time = false) {
		...
		$dateTimeInUserFormat = Ottocrat_Datetime_UIType::getDisplayDateTimeValue($date . ' ' . $time);
		...
		if ($currentUser->get('hour_format') === '12') {
			$displayTime = Ottocrat_Time_UIType::getTimeValueInAMorPM($displayTime);
		}

		$today = Ottocrat_Date_UIType::getDisplayDateValue(date('Y-m-d H:i:s'));
		$tomorrow = Ottocrat_Date_UIType::getDisplayDateValue(date('Y-m-d H:i:s', strtotime('tomorrow')));
	...
	public static function getRecordName ($recordId, $checkDelete=false) {
        ...
        $query = 'SELECT label from ottocrat_crmentity where crmid=?';
        if($checkDelete) {
            $query.= ' AND deleted=0';
        }
        $result = $adb->pquery($query,array($recordId));
	...
	public static function formatDateTimeIntoDayString($dateTime) {
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$dateTimeInUserFormat = Ottocrat_Datetime_UIType::getDisplayDateTimeValue($dateTime);
    ...
    public static function getPickListValues($fieldName) {
        ...
        $query = 'SELECT '.$primaryKey.', '.$fieldName.' FROM ottocrat_'.$fieldName.' order by sortorderid';
        $values = array();
	...
	public static function getBaseCurrency() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ottocrat_currency_info WHERE defaultid < 0', array());
    ...
    public static function getRoleBasedPicklistValues($fieldName, $roleId) {
        ...
        $query = "SELECT $fieldName
                  FROM ottocrat_$fieldName
                      INNER JOIN ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_$fieldName.picklist_valueid
                  WHERE roleid=? and picklistid in (select picklistid from ottocrat_picklist) order by sortorderid";
...
/**
	...
	public static function getCreator($recordId) {
		...
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT smcreatorid FROM ottocrat_crmentity WHERE crmid = ?', array($recordId));
	 ...
	 * @param <DateTime> $dateTime
	 * @param <Ottocrat_Users_Model> $userObject
	...
	public static function convertDateTimeIntoUsersDisplayFormat($dateTime, $userObject = null) {
        ...
        if ($userModel->get('hour_format') == '12') {
            $time = Ottocrat_Time_UIType::getTimeValueInAMorPM($time);
        }
        return $date . ' ' . $time;
    }
    ...
    */
	...
	public static function getCurrentInfoOfUser() {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
	    $result = $db->pquery('SELECT * FROM ottocrat_currency_info WHERE id = ?', array($currentUser->get('currency_id')));
		if($db->num_rows($result)) return $db->query_result_rowdata($result, 0);
	}
    ...
    */
	public static function setCalendarDefaultActivityTypesForUser($userId) {
		$db = PearDatabase::getInstance();
		$userEntries = $db->pquery('SELECT 1 FROM ottocrat_calendar_user_activitytypes WHERE userid=?', array($userId));
                $activityIds = array();
		if($db->num_rows($userEntries) <= 0) {
			$queryResult = $db->pquery('SELECT id, defaultcolor FROM ottocrat_calendar_default_activitytypes', array());
			...
			foreach($activityIds as $activityId=>$color) {
				$db->pquery('INSERT INTO ottocrat_calendar_user_activitytypes (id, defaultid, userid, color) VALUES (?,?,?,?)', array($db->getUniqueID('ottocrat_calendar_user_activitytypes'), $activityId, $userId, $color));
			}
		}
    ...
    public static function isUserDeleted($userid) {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT deleted FROM ottocrat_users WHERE id = ? AND (status=? OR deleted=?)', array($userid, 'Inactive', 1));
