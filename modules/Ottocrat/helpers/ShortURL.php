 ...
 */
class Ottocrat_ShortURL_Helper {
    /*
     * @param options array(
    ...
    static function generate(array $options) {
        ...
        $sql = "INSERT INTO ottocrat_shorturls(uid, handler_path, handler_class, handler_function, handler_data, onetime) VALUES (?,?,?,?,?,?)";
    ...
    static function handle($uid) {
        ...
        $rs = $db->pquery('SELECT * FROM ottocrat_shorturls WHERE uid=?', array($uid));
        if ($rs && $db->num_rows($rs)) {
            ...
            // Delete onetime URL
            if ($record['onetime'])
                $db->pquery('DELETE FROM ottocrat_shorturls WHERE id=?', array($record['id']));
    ...
    function getInstance($id) {
        $db = PearDatabase::getInstance();
        $self = new self();
        $rs = $db->pquery('SELECT * FROM ottocrat_shorturls WHERE uid=?', array($id));
        if ($rs && $db->num_rows($rs)) {
    ...
    function delete() {
        $db = PearDatabase::getInstance();
        $db->pquery('DELETE FROM ottocrat_shorturls WHERE id=?', array($this->id));
    }
