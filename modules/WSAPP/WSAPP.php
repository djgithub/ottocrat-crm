...
class WSAPP {
	...
	function registerCustomWebservices( $operations ) {
		...
		foreach($operations as $operation_name => $operation_info) {	
			$checkres = $adb->pquery("SELECT operationid FROM ottocrat_ws_operation WHERE name=?", array($operation_name));
			if($checkres && $adb->num_rows($checkres) < 1) {
				$operation_id = $adb->getUniqueId('ottocrat_ws_operation');
			
				$operation_res = $adb->pquery(
					"INSERT INTO ottocrat_ws_operation (operationid, name, handler_path, handler_method, type, prelogin)
					VALUES (?,?,?,?,?,?)",
				...
				foreach($operation_parameters as $parameter_name => $parameter_type) {
					$adb->pquery(
						"INSERT INTO ottocrat_ws_operation_parameters (operationid, name, type, sequence)
						VALUES(?,?,?,?)", array($operation_id, $parameter_name, $parameter_type, ($parameter_index+1))
					);
					++$parameter_index;
				}
				Ottocrat_Utils::Log("Opearation $operation_name enabled successfully.");
			} else {
				Ottocrat_Utils::Log("Operation $operation_name already exists.");
			}
		}
	}
    ...
    function registerHandlers(){
        ...
        $appTypehandler = array();
        $appTypehandler['type'] = "ottocratCRM";
        $appTypehandler['handlerclass'] = "ottocratCRMHandler";
        $appTypehandler['handlerpath'] = "modules/WSAPP/Handlers/ottocratCRMHandler.php";
        $handlerDetails[] = $appTypehandler;

        foreach($handlerDetails as $appHandlerDetails)
             $adb->pquery("INSERT INTO ottocrat_wsapp_handlerdetails VALUES(?,?,?)",array($appHandlerDetails['type'],$appHandlerDetails['handlerclass'],$appHandlerDetails['handlerpath']));
    }
    
    function registerOttocratCRMApp(){
       $db = PearDatabase::getInstance();
        $appName = "ottocratCRM";
        $type  ="user";
        $uid = uniqid();
        $db->pquery("INSERT INTO ottocrat_wsapp (name, appkey,type) VALUES(?,?,?)", array($appName, $uid,$type));
    }
	...
	function registerWsappWorkflowhandler(){
		...
		$em->registerHandler('ottocrat.entity.aftersave', 'modules/WSAPP/WorkFlowHandlers/WSAPPAssignToTracker.php', 'WSAPPAssignToTracker','',$dependentEventHandlersJson);
	}

	function registerSynclibEventHandler(){
		$className='WSAPP_OttocratSyncEventHandler';
		$path = 'modules/WSAPP/synclib/handlers/OttocratSyncEventHandler.php';
		$type = 'ottocratSyncLib';
