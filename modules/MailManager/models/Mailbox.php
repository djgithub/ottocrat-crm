...
class MailManager_Mailbox_Model {
	...
	public function delete() {
		...
		$db->pquery("DELETE FROM ottocrat_mail_accounts WHERE user_id = ? AND account_id = ?", array($currentUserModel->getId(), $this->mId));
	}
	...
	public function save() {
		...
		$account_id = 1;
		$maxresult = $db->pquery("SELECT max(account_id) as max_account_id FROM ottocrat_mail_accounts", array());
		...
		if ($isUpdate) {
			$sql = "UPDATE ottocrat_mail_accounts SET display_name=?, mail_servername=?, mail_username=?, mail_password=?, mail_protocol=?, ssltype=?, sslmeth=?, box_refresh=?, sent_folder=? WHERE user_id=? AND account_id=?";
			$parameters[] = $this->mId;
		} else {
			$sql = "INSERT INTO ottocrat_mail_accounts(display_name, mail_servername, mail_username, mail_password, mail_protocol, ssltype, sslmeth, box_refresh,sent_folder, user_id, mails_per_page, account_name, status, set_default, account_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static function activeInstance($currentUserModel = false) {
		...
		$result = $db->pquery("SELECT * FROM ottocrat_mail_accounts WHERE user_id=? AND status=1 AND set_default=0", array($currentUserModel->getId()));
