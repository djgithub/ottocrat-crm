...
class MailManager_Message_Model extends Ottocrat_MailRecord  {
	...
	public static function pruneOlderInDB($waybacktime) {
		...
		$db->pquery("DELETE FROM ottocrat_mailmanager_mailrecord
		WHERE userid=? AND lastsavedtime < ?", array($currentUserModel->getId(), $waybacktime));
		$db->pquery("DELETE FROM ottocrat_mailmanager_mailattachments
		WHERE userid=? AND lastsavedtime < ?", array($currentUserModel->getId(), $waybacktime));
	}
	...
	public static function removeSavedAttachmentFiles($waybacktime) {
		...
		$mailManagerAttachments = $db->pquery("SELECT attachid, aname, path FROM ottocrat_mailmanager_mailattachments
			WHERE userid=? AND lastsavedtime < ?", array($currentUserModel->getId(), $waybacktime));

		for($i=0; $i<$db->num_rows($mailManagerAttachments); $i++) {
			$atResultRow = $db->raw_query_result_rowdata($mailManagerAttachments, $i);

			$db->pquery("UPDATE ottocrat_crmentity set deleted = 1 WHERE crmid = ?", array($atResultRow['attachid']));
	...
	public function readFromDB($uid) {
		$db = PearDatabase::getInstance();
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
		$result = $db->pquery("SELECT * FROM ottocrat_mailmanager_mailrecord
	...
	protected function loadAttachmentsFromDB($withContent, $aName=false) {
		...
		if (empty($this->_attachments)) {
			...
			$atResult = $db->pquery("SELECT {$filteredColumns} FROM ottocrat_mailmanager_mailattachments
	...
	protected function saveToDB($uid) {
		...
		$db->pquery("INSERT INTO ottocrat_mailmanager_mailrecord (userid, muid, mfrom, mto, mcc, mbcc,
		...
		if (!empty($this->_attachments)) {
			foreach($this->_attachments as $aName => $aValue) {
				...
				if(is_array($attachInfo) && !empty($attachInfo)) {
					$db->pquery("INSERT INTO ottocrat_mailmanager_mailattachments
	...
	public function __SaveAttachmentFile($filename, $filecontent) {
		...
		$attachid = $db->getUniqueId('ottocrat_crmentity');
		...
		$db->pquery("INSERT INTO ottocrat_crmentity(crmid, smcreatorid, smownerid,
		...
		$db->pquery("INSERT INTO ottocrat_attachments SET attachmentsid=?, name=?, description=?, type=?, path=?",
	...
	public function date($format = false) {
		...
		if ($date) {
			if ($format) {
				$dateTimeFormat = Ottocrat_Util_Helper::convertDateTimeIntoUsersDisplayFormat(date('Y-m-d H:i:s', strtotime($date)));
				...
				return Ottocrat_Util_Helper::convertDateTimeIntoUsersDisplayFormat(date('Y-m-d H:i:s', $date));
			}
		}
		return '';
	}
