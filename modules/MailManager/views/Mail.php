		...
		if ('open' == $this->getOperationArg($request)) {
			...
			if (!empty($to_string)) {
				...
				foreach($toArray as $to) {
					$relatedtos = MailManager::lookupMailInOttocrat($to, $currentUserModel);
					...
					if ($status === true) {
						...
						if ($realid[1] == -1) {
							$db->pquery('DELETE FROM ottocrat_salesmanactivityrel WHERE smid=? AND activityid=?',$params);
							$db->pquery('INSERT INTO ottocrat_salesmanactivityrel VALUES (?,?)', $params);
						} else {
							$db->pquery('DELETE FROM ottocrat_seactivityrel WHERE crmid=? AND activityid=?', $params);
							$db->pquery('INSERT INTO ottocrat_seactivityrel VALUES (?,?)', $params);
						}
					}
				}
			}
