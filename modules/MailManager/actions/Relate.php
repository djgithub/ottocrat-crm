...
class MailManager_Relate_Action extends Ottocrat_MailScannerAction {
	...
	public function __CreateNewEmail($mailrecord, $module, $linkfocus) {
		...
		$recordModel = Ottocrat_Record_Model::getCleanInstance('Emails');
	...
	public function __SaveAttachements($mailrecord, $basemodule, $recordModel) {
		...
		foreach($mailrecord->_attachments as $filename=>$filecontent) {
			...
			if($issaved) {
				...
				$attachRes = $db->pquery("SELECT * FROM ottocrat_attachments WHERE attachmentsid = ?", array($attachid));
	...
	public static function getSalesEntityInfo($crmid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT setype FROM ottocrat_crmentity WHERE crmid=? AND deleted=0", array($crmid));
	...
	public static function ws_modulename($modulewsid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT name FROM ottocrat_ws_entity WHERE id=?", array($modulewsid));
