 ...
 */
class PriceBooks_Record_Model extends Ottocrat_Record_Model {
	...
	function getProductsListPrice($relatedRecordId) {
		...
		$result = $db->pquery('SELECT listprice FROM ottocrat_pricebookproductrel WHERE pricebookid = ? AND productid = ?',
	...
	function deleteListPrice($relatedRecordId) {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ottocrat_pricebookproductrel WHERE pricebookid = ? AND productid = ?',
					array($this->getId(), $relatedRecordId));
	}
}