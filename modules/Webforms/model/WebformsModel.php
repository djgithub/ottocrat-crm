...
class Webforms_Model {
    ...
    function getRoundrobinOwnerId() {
        ...
        $adb->pquery("UPDATE ottocrat_webforms SET roundrobin_logic = ? WHERE id = ?", array($nextRoundrobinLogic,$this->getId()));
	...
	function retrieveFields() {
		global $adb;
		$fieldsResult = $adb->pquery("SELECT * FROM ottocrat_webforms_field WHERE webformid=?", array($this->getId()));
	...
	function save() {
		...
		if ($isNew) {
			...
			$insertSQL = "INSERT INTO ottocrat_webforms(name, targetmodule, publicid, enabled, description,ownerid,returnurl) VALUES(?,?,?,?,?,?,?)";
			...
			// Udpate
			$updateSQL = "UPDATE ottocrat_webforms SET description=? ,returnurl=?,ownerid=?,enabled=? WHERE id=?";
		...
		$adb->pquery("DELETE FROM ottocrat_webforms_field WHERE webformid=?", array($this->getId()));
		$fieldInsertSQL = "INSERT INTO ottocrat_webforms_field(webformid, fieldname, neutralizedfield, defaultvalue,required) VALUES(?,?,?,?,?)";
	...
	function delete() {
		...
		$adb->pquery("DELETE from ottocrat_webforms_field where webformid=?", array($this->getId()));
		$adb->pquery("DELETE from ottocrat_webforms where id=?", array($this->getId()));
		return true;
	}
	...
	static function retrieveWithPublicId($publicid) {
		...
		$result = $adb->pquery("SELECT * FROM ottocrat_webforms WHERE publicid=? AND enabled=?", array($publicid, 1));
	...
	static function retrieveWithId($data) {
		...
		$model = false;
		// Retrieve model and populate information
		$result = $adb->pquery("SELECT * FROM ottocrat_webforms WHERE id=?", array($id));
		if ($adb->num_rows($result)) {
			$model = new Webforms_Model($adb->fetch_array($result));
			$model->retrieveFields();
		}
	...
	static function listAll() {
		...
		$sql = "SELECT * FROM ottocrat_webforms";
	...
	static function isWebformField($webformid, $fieldname) {
		...
		$checkSQL = "SELECT 1 from ottocrat_webforms_field where webformid=? AND fieldname=?";
	...
	static function isRequired($webformid, $fieldname) {
		global $adb;
		$sql = "SELECT required FROM ottocrat_webforms_field where webformid=? AND fieldname=?";
	...
	static function retrieveDefaultValue($webformid, $fieldname) {
		...
		$sql = "SELECT defaultvalue FROM ottocrat_webforms_field WHERE webformid=? and fieldname=?";
		$result = $adb->pquery($sql, array($webformid, $fieldname));
		$defaultvalue = false;
		if ($adb->num_rows($result)) {
	...
	static function existWebformWithName($name) {
		global $adb;
		$checkSQL = "SELECT 1 FROM ottocrat_webforms WHERE name=?";
	...
	static function isActive($field, $mod) {
		global $adb;
		$tabid = getTabid($mod);
		$query = 'SELECT 1 FROM ottocrat_field WHERE fieldname = ?  AND tabid = ? AND presence IN (0,2)';
