	...
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
			...
			if($eventType == 'module.postinstall') {
				...
				// Mark the module as Standard module
				$this->updateSettings();
				$adb->pquery('UPDATE ottocrat_tab SET customized=0 WHERE name=?', array($this->LBL_WEBFORMS));
			} else if($eventType == 'module.disabled') {
			// TODO Handle actions when this module is disabled.
				global $log,$adb;
				$adb->pquery('UPDATE ottocrat_settings_field SET active= 1  WHERE  name= ?',array($this->LBL_WEBFORMS));
			} else if($eventType == 'module.enabled') {
			// TODO Handle actions when this module is enabled.
				global $log,$adb;
				$adb->pquery('UPDATE ottocrat_settings_field SET active= 0  WHERE  name= ?',array($this->LBL_WEBFORMS));
