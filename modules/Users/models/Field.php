 ...
 */
class Users_Field_Model extends Ottocrat_Field_Model {
	...
	public function getPicklistValues() {
		if($this->get('uitype') == 32) {
			return Ottocrat_Language_Handler::getAllLanguages();
		}
        else if ($this->get('uitype') == '115') {
            $db = PearDatabase::getInstance();
            
            $query = 'SELECT '.$this->getFieldName().' FROM ottocrat_'.$this->getFieldName();
    ...
    public function getDisplayValue($value, $recordId = false) {
		 ...
		 if($this->get('uitype') == 32){
			return Ottocrat_Language_Handler::getLanguageLabel($value);
		 }
