...
class Users_Module_Model extends Ottocrat_Module_Model {
	/**
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		if($sourceModule == 'Users' && $field == 'reports_to_id') {
			...
			if(!empty($record)){
				$overRideQuery = $overRideQuery. " AND ottocrat_users.id != ". $record;
			}
	...
	public function searchRecord($searchValue, $parentId=false, $parentModule=false, $relatedModule=false) {
		if(!empty($searchValue)) {
			...
			$query = 'SELECT * FROM ottocrat_users WHERE (first_name LIKE ? OR last_name LIKE ?) AND status = ?';
			...
			for($i=0; $i<$noOfRows; ++$i) {
				...
				$modelClassName = Ottocrat_Loader::getComponentClassName('Model', 'Record', 'Users');
	...
	public function checkDuplicateUser($userName){
		...
		$query = 'SELECT user_name FROM ottocrat_users WHERE user_name = ?';
    ...
    */
	public function updateBaseCurrency($currencyName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT currency_code, currency_symbol FROM ottocrat_currencies WHERE currency_name = ?', array($currencyName));
   ...
   /**
	...
	public function saveLoginHistory($username){
		...
		$query = "INSERT INTO ottocrat_loginhistory (user_name, user_ip, logout_time, login_time, status) VALUES (?,?,?,?,?)";
	...
	public function saveLogoutHistory(){
		...
		$loginIdQuery = "SELECT MAX(login_id) AS login_id FROM ottocrat_loginhistory WHERE user_name=? AND user_ip=?";
		...
		if (!empty($loginid)){
			$query = "UPDATE ottocrat_loginhistory SET logout_time =?, status=? WHERE login_id = ?";
	...
	public static function savePackagesInfo($packagesList) {
		...
		if ($disabledModulesList) {
			$updateQuery = 'UPDATE ottocrat_tab SET presence = CASE WHEN name IN (' . generateQuestionMarks($disabledModulesList) . ') THEN 1 ';
			$updateQuery .= 'ELSE 0 END WHERE presence != 2 ';
		} else {
			$updateQuery = 'UPDATE ottocrat_tab SET presence = 0 WHERE presence != 2';
		}
	...
	*/
    public function getCurrenciesList() {
	  $adb = PearDatabase::getInstance();
	   ...
	   $currency_query = 'SELECT currency_name, currency_code, currency_symbol FROM ottocrat_currencies ORDER BY currency_name';
   ...
   public function getTimeZonesList() {
	   ...
	   $timezone_query = 'SELECT time_zone FROM ottocrat_time_zone';
   ...
   public function getLanguagesList() {
	   ...
	   $language_query = 'SELECT prefix, label FROM ottocrat_language';
