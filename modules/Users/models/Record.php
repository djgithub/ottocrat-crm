...
class Users_Record_Model extends Ottocrat_Record_Model {
	...
	//	return 'index.php?module='.$this->getModuleName().'&parent=Settings&view='.$module->getDetailViewName()
	//.'&record='.$this->getId();
		return Ottocrat_Request::encryptLink('index.php?module='.$this->getModuleName().'&parent=Settings&view='.$module->getDetailViewName().'&record='.$this->getId());
	}
	...
	public function getPreferenceDetailViewUrl() {
		...
		return Ottocrat_Request::encryptLink('index.php?module='.$this->getModuleName().'&view=PreferenceDetail&record='.$this->getId());
	}
	...
	public function getProfileUrl() {
		...
		return Ottocrat_Request::encryptLink('index.php?module=Users&view=ChangePassword&mode=Profile');
	}
	...
	public function getEditViewUrl() {
		$module = $this->getModule();
		//return 'index.php?module='.$this->getModuleName().'&parent=Settings&view='.$module->getEditViewName()
		//.'&record='.$this->getId();
		return Ottocrat_Request::encryptLink('index.php?module='.$this->getModuleName().'&parent=Settings&view='.$module->getEditViewName().'&record='.$this->getId());
	}
	...
	public function getPreferenceEditViewUrl() {
		...
		return Ottocrat_Request::encryptLink('index.php?module='.$this->getModuleName().'&view=PreferenceEdit&record='.$this->getId());
	}
	...
	public function getDeleteUrl() {
		...
		return Ottocrat_Request::encryptLink('index.php?module='.$this->getModuleName().'&parent=Settings&view='.$module->getDeleteActionName().'User&record='.$this->getId());
	}
	...
	public static function getAll($onlyActive=true) {
		...
		$sql = 'SELECT id FROM ottocrat_users';
		$params = array();
		if($onlyActive) {
			$sql .= ' WHERE status = ?';
			$params[] = 'Active';
	...
	public function getImageDetails() {
		...
		if ($recordId) {
			$query = "SELECT ottocrat_attachments.* FROM ottocrat_attachments
            LEFT JOIN ottocrat_salesmanattachmentsrel ON ottocrat_salesmanattachmentsrel.attachmentsid = ottocrat_attachments.attachmentsid
            WHERE ottocrat_salesmanattachmentsrel.smid=?";
	...
	public function getAccessibleUsers($private="",$module = false) {
		...
		$accessibleUser = Ottocrat_Cache::get('ottocrat-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers');
        if(empty($accessibleUser)) {
			...
			Ottocrat_Cache::set('ottocrat-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers',$accessibleUser);
		}
	...
	public function getAllUsersOnRoles($roles) {
		...
		$sql = 'SELECT userid FROM ottocrat_user2role WHERE roleid IN ('.  generateQuestionMarks($roleIds).')';
		...
		if($noOfUsers > 0) {
			...
			$query = 'SELECT id, first_name, last_name FROM ottocrat_users WHERE status = ? AND id IN ('.  generateQuestionMarks($userIds).')';
	...
	public function getAccessibleGroups($private="",$module = false) {
        ...
        $accessibleGroups = Ottocrat_Cache::get('ottocrat-'.$private, 'accessiblegroups');
        if(!$accessibleGroups){
            $accessibleGroups = get_group_array(false, "ACTIVE", "", $private,$module);
            Ottocrat_Cache::set('ottocrat-'.$private, 'accessiblegroups',$accessibleGroups);
        }
	...
	public function deleteImage($imageId) {
		...
		$checkResult = $db->pquery('SELECT smid FROM ottocrat_salesmanattachmentsrel WHERE attachmentsid = ?', array($imageId));
		$smId = $db->query_result($checkResult, 0, 'smid');

		if ($this->getId() === $smId) {
			$db->pquery('DELETE FROM ottocrat_attachments WHERE attachmentsid = ?', array($imageId));
			$db->pquery('DELETE FROM ottocrat_salesmanattachmentsrel WHERE attachmentsid = ?', array($imageId));
			return true;
		}
		return false;
	}
	...
	function saveTagCloud() {
		$db = PearDatabase::getInstance();
		$db->pquery("UPDATE ottocrat_homestuff SET visible = ? WHERE userid=? AND stufftype='Tag Cloud'",
	...
	public static function getUserGroups($userId){
		$db = PearDatabase::getInstance();
		$groupIds = array();
		$query = "SELECT groupid FROM ottocrat_users2group WHERE userid=?";
    ...
    public static function getCount($onlyActive = false) {
        $db = PearDatabase::getInstance();
        $query = 'SELECT 1 FROM ottocrat_users ';
        $params = array();
	...
	public function getDuplicateRecordUrl() {
		...
		return Ottocrat_Request::encryptLink('index.php?module='.$this->getModuleName().'&parent=Settings&view='.$module->getEditViewName().'&record='.$this->getId().'&isDuplicate=true');
	...
	public static function getInstanceByName($userName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT id FROM ottocrat_users WHERE user_name = ?', array($userName));
	...
	public function delete() {
		...
		$db = PearDatabase::getInstance();
		$query = 'SELECT is_owner FROM ottocrat_users WHERE id = ?';
	...
	public function getActiveAdminUsers() {
		...
		$sql = 'SELECT id FROM ottocrat_users WHERE status=? AND is_admin=?';
	...
	public function getUserHash() {
		$db = PearDatabase::getInstance();
		$query = 'SELECT user_hash FROM ottocrat_users WHERE id = ?';
        ...
        public function deleteUserPermanently($userId, $newOwnerId) {
                ...
                $sql = "UPDATE ottocrat_crmentity SET smcreatorid=?,smownerid=? WHERE smcreatorid=? AND setype=?";
                $db->pquery($sql, array($newOwnerId, $newOwnerId, $userId,'ModComments'));
                
                //update history details in ottocrat_modtracker_basic 
                $sql ="update ottocrat_modtracker_basic set whodid=? where whodid=?"; 
                $db->pquery($sql, array($newOwnerId, $userId)); 

                //update comments details in ottocrat_modcomments 
                $sql ="update ottocrat_modcomments set userid=? where userid=?"; 
                $db->pquery($sql, array($newOwnerId, $userId));

                $sql = "DELETE FROM ottocrat_users WHERE id=?";
