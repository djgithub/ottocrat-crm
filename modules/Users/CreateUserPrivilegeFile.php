...
if($handle)
	{
		...
		else
		{
			...
			$relModSharArr=Array();	
			$query ="select * from ottocrat_datashare_relatedmodules";
...
function getUserModuleSharingObjects($module,$userid,$def_org_share,$current_user_roles,$parent_roles,$current_user_groups)
{
	...
	if($def_org_share[$mod_tabid] == 3 || $def_org_share[$mod_tabid] == 0)
	{
		...
		$grp_read_per=Array();
		$grp_write_per=Array();
		//Retreiving from ottocrat_role to ottocrat_role
		$query="select ottocrat_datashare_role2role.* from ottocrat_datashare_role2role inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_role2role.shareid where ottocrat_datashare_module_rel.tabid=? and ottocrat_datashare_role2role.to_roleid=?";
		...
		$query="select ottocrat_datashare_role2rs.* from ottocrat_datashare_role2rs inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_role2rs.shareid where ottocrat_datashare_module_rel.tabid=? and ottocrat_datashare_role2rs.to_roleandsubid in (". generateQuestionMarks($parRoleList) .")";
		...
		if (!empty($groupList)) {
			$query="select ottocrat_datashare_role2group.* from ottocrat_datashare_role2group inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_role2group.shareid where ottocrat_datashare_module_rel.tabid=?";
			$qparams = array($mod_tabid);

			if (count($groupList) > 0) {
				$query .= " and ottocrat_datashare_role2group.to_groupid in (". generateQuestionMarks($groupList) .")";
		...
		//Retreiving from rs to ottocrat_role
		$query="select ottocrat_datashare_rs2role.* from ottocrat_datashare_rs2role inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_rs2role.shareid where ottocrat_datashare_module_rel.tabid=? and ottocrat_datashare_rs2role.to_roleid=?";
		$result=$adb->pquery($query, array($mod_tabid, $current_user_roles));
		$num_rows=$adb->num_rows($result);
		for($i=0;$i<$num_rows;$i++)
		{
		...
		array_push($parRoleList, $current_user_roles);
		$query="select ottocrat_datashare_rs2rs.* from ottocrat_datashare_rs2rs inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_rs2rs.shareid where ottocrat_datashare_module_rel.tabid=? and ottocrat_datashare_rs2rs.to_roleandsubid in (". generateQuestionMarks($parRoleList) .")";
		$result=$adb->pquery($query, array($mod_tabid, $parRoleList));
		$num_rows=$adb->num_rows($result);
		for($i=0;$i<$num_rows;$i++)
		{
		...
		$query="select ottocrat_datashare_rs2grp.* from ottocrat_datashare_rs2grp inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_rs2grp.shareid where ottocrat_datashare_module_rel.tabid=?";
		$qparams = array($mod_tabid);
		if (count($groupList) > 0) {
			$query .= " and ottocrat_datashare_rs2grp.to_groupid in (". generateQuestionMarks($groupList) .")";
			array_push($qparams, $groupList);
		}
		$result=$adb->pquery($query, $qparams);
		$num_rows=$adb->num_rows($result);
		...
		$query="select ottocrat_datashare_grp2role.* from ottocrat_datashare_grp2role inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_grp2role.shareid where ottocrat_datashare_module_rel.tabid=? and ottocrat_datashare_grp2role.to_roleid=?";
		$result=$adb->pquery($query, array($mod_tabid, $current_user_roles));
		$num_rows=$adb->num_rows($result);
		for($i=0;$i<$num_rows;$i++)
		{
		...
		$query="select ottocrat_datashare_grp2rs.* from ottocrat_datashare_grp2rs inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_grp2rs.shareid where ottocrat_datashare_module_rel.tabid=? and ottocrat_datashare_grp2rs.to_roleandsubid in (". generateQuestionMarks($parRoleList) .")";
		$result=$adb->pquery($query, array($mod_tabid, $parRoleList));
		$num_rows=$adb->num_rows($result);
		for($i=0;$i<$num_rows;$i++)
		{
		...
		$query="select ottocrat_datashare_grp2grp.* from ottocrat_datashare_grp2grp inner join ottocrat_datashare_module_rel on ottocrat_datashare_module_rel.shareid=ottocrat_datashare_grp2grp.shareid where ottocrat_datashare_module_rel.tabid=?";
		$qparams = array($mod_tabid);
		if (count($groupList) > 0) {
			$query .= " and ottocrat_datashare_grp2grp.to_groupid in (". generateQuestionMarks($groupList) .")";
			array_push($qparams, $groupList);
		}
		$result=$adb->pquery($query, $qparams);
		$num_rows=$adb->num_rows($result);
...
function getRelatedModuleSharingArray($par_mod,$share_mod,$mod_sharingrule_members,$mod_share_read_per,$mod_share_write_per,$def_org_share)
{
	...
	if($def_org_share[$share_mod_id] == 3 || $def_org_share[$share_mod_id] == 0)
	{
		...
		{
			$query = "select ottocrat_datashare_relatedmodule_permission.* from ottocrat_datashare_relatedmodule_permission inner join ottocrat_datashare_relatedmodules on ottocrat_datashare_relatedmodules.datashare_relatedmodule_id=ottocrat_datashare_relatedmodule_permission.datashare_relatedmodule_id where ottocrat_datashare_relatedmodule_permission.shareid=? and ottocrat_datashare_relatedmodules.tabid=? and ottocrat_datashare_relatedmodules.relatedto_tabid=?";
...
function populateSharingtmptables($userid)
{
	...
	//Deleting from the existing ottocrat_tables
	$table_arr=Array('ottocrat_tmp_read_user_sharing_per', 'ottocrat_tmp_write_user_sharing_per','ottocrat_tmp_read_group_sharing_per','ottocrat_tmp_write_group_sharing_per','ottocrat_tmp_read_user_rel_sharing_per','ottocrat_tmp_write_user_rel_sharing_per','ottocrat_tmp_read_group_rel_sharing_per','ottocrat_tmp_write_group_rel_sharing_per');
...
function populateSharingPrivileges($enttype,$userid,$module,$pertype, $var_name_arr=false)
{
	...
	if($enttype=='USER')
	{
		if($pertype =='read')
		{
			$table_name='ottocrat_tmp_read_user_sharing_per';
			$var_name=$module.'_share_read_permission';
		}
		elseif($pertype == 'write')
		{
			$table_name='ottocrat_tmp_write_user_sharing_per';
	...
	elseif($enttype=='GROUP')
	{
		if($pertype =='read')
		{
			$table_name='ottocrat_tmp_read_group_sharing_per';
			$var_name=$module.'_share_read_permission';
		}
		elseif($pertype == 'write')
		{
			$table_name='ottocrat_tmp_write_group_sharing_per';
			$var_name=$module.'_share_write_permission';
		}
		// Lookup for the variable if not set through function argument
		if(!$var_name_arr) $var_name_arr=$$var_name;
...
function populateRelatedSharingPrivileges($enttype,$userid,$module,$relmodule,$pertype, $var_name_arr=false)
{
	...
	if($enttype=='USER')
	{
		if($pertype =='read')
		{
			$table_name='ottocrat_tmp_read_user_rel_sharing_per';
			$var_name=$module.'_'.$relmodule.'_share_read_permission';
		}
		elseif($pertype == 'write')
		{
			$table_name='ottocrat_tmp_write_user_rel_sharing_per';
	...
	elseif($enttype=='GROUP')
	{
		if($pertype =='read')
		{
			$table_name='ottocrat_tmp_read_group_rel_sharing_per';
			$var_name=$module.'_'.$relmodule.'_share_read_permission';
		}
		elseif($pertype == 'write')
		{
			$table_name='ottocrat_tmp_write_group_rel_sharing_per';
			$var_name=$module.'_'.$relmodule.'_share_write_permission';
		}
		// Lookup for the variable if not set through function argument
		if(!$var_name_arr) $var_name_arr=$$var_name;
