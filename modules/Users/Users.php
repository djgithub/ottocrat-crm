...
/*********************************************************************************
 * $Header: /advent/projects/wesat/ottocrat_crm/sugarcrm/modules/Users/Users.php,v 1.10 2005/04/19 14:40:48 ray Exp $
...
class Users extends CRMEntity {
    ...
    var $tab_name = Array('ottocrat_users','ottocrat_attachments','ottocrat_user2role','ottocrat_asteriskextensions');
    var $tab_name_index = Array('ottocrat_users'=>'id','ottocrat_attachments'=>'attachmentsid','ottocrat_user2role'=>'userid','ottocrat_asteriskextensions'=>'userid');

    var $table_name = "ottocrat_users";
    var $table_index= 'id';
    ...
    var $search_fields = Array(
            'Name'=>Array('ottocrat_users'=>'last_name'),
            'Email'=>Array('ottocrat_users'=>'email1'),
            'Email2'=>Array('ottocrat_users'=>'email2')
    );
    ...
    var $list_fields = Array(
            'First Name'=>Array('ottocrat_users'=>'first_name'),
            'Last Name'=>Array('ottocrat_users'=>'last_name'),
            'Role Name'=>Array('ottocrat_user2role'=>'roleid'),
            'User Name'=>Array('ottocrat_users'=>'user_name'),
			'Status'=>Array('ottocrat_users'=>'status'),
			'Email'=>Array('ottocrat_users'=>'email1'),
            'Email2'=>Array('ottocrat_users'=>'email2'),
            'Admin'=>Array('ottocrat_users'=>'is_admin'),
            'Phone'=>Array('ottocrat_users'=>'phone_work')
    );
    ...
    function encrypt_password($user_password, $crypt_type='') {
        ...
        // Fix for: http://trac.ottocrat.com/cgi-bin/trac.cgi/ticket/4923
        if($crypt_type == '') {
    ...
    function doLogin($user_password) {
        global $AUTHCFG, $adb,$WP_USER,$WP_DB,$WP_PASSWORD;
        $usr_name = $this->column_fields["user_name"];
        ...
        switch (strtoupper($AUTHCFG['authType'])) {
            ...
            $result = $this->db->requirePsSingleResult($query, array($usr_name), false);
                ...
                if($usr_name=='admin'){
                    ...
                    $adb->disconnect();
                    $this->db->resetSettings('mysqli', 'localhost', $WP_DB, $WP_USER,$WP_PASSWORD);
                    $adb->checkConnection();
                    $query = "SELECT db_name from logicint_tlrsite.tbl_user where username=? AND password=? AND status = ?";

                    $result = $this->db->requirePsSingleResult($query, array($usr_name, $encrypted_password, 'a'),false);
                  //  $this->column_fields["user_name"]=$this->db->query_result($result,0,"db_name");
                    $this->column_fields["user_name"]=$usr_name;
                }

                //print_r($result);die;

                /* echo "SELECT 1 from logicint_tlrsite.tbl_user where username='$usr_name' AND password='$encrypted_password' AND status ='a'";die;*/
    ...
    function get_user_crypt_type() {
        ...
        global $adb;
        $table_cols = $adb->getColumnNames("ottocrat_users");
...
//        $this->db->startTransaction();
    ...
    function retrieve_user_id($user_name) {
        global $adb;
        $query = "SELECT id from ottocrat_users where user_name=? AND deleted=0";
    ...
    function verify_data() {
        ...
        $query = "SELECT user_name from ottocrat_users where user_name=? AND id<>? AND deleted=0";
        $result =$this->db->pquery($query, array($usr_name, $this->id), true, "Error selecting possible duplicate users: ");
        $dup_users = $this->db->fetchByAssoc($result);

        $query = "SELECT user_name from ottocrat_users where is_admin = 'on' AND deleted=0";
        $result =$this->db->pquery($query, array(), true, "Error selecting possible duplicate ottocrat_users: ");
    ...
    function fill_in_additional_detail_fields() {
        $query = "SELECT u1.first_name, u1.last_name from ottocrat_users u1, ottocrat_users u2 where u1.id = u2.reports_to_id AND u2.id = ? and u1.deleted=0";
        $result =$this->db->pquery($query, array($this->id), true, "Error filling in additional detail ottocrat_fields") ;
    ...
    function saveentity($module) {
        ...
        foreach($this->tab_name as $table_name) {
            if($table_name == 'ottocrat_attachments') {
    ...
    function createAccessKey() {
        ...
        $updateQuery = "update ottocrat_users set accesskey=? where id=?";
    ...
    function insertIntoEntityTable($table_name, $module) {
        ...
        if($insertion_mode == 'edit') {
            ...
            $sql = "select * from ottocrat_field where tabid=? and tablename=? and displaytype in (1,3,5) and ottocrat_field.presence in (0,2)";
            $params = array($tabid, $table_name);
        }
        else {
            $column = $this->tab_name_index[$table_name];
            if($column == 'id' && $table_name == 'ottocrat_users') {
                $currentuser_id = $this->db->getUniqueID("ottocrat_users");
                $this->id = $currentuser_id;
            }
            $qparams = array($this->id);
            $tabid= getTabid($module);
            $sql = "select * from ottocrat_field where tabid=? and tablename=? and displaytype in (1,3,4,5) and ottocrat_field.presence in (0,2)";
            $params = array($tabid, $table_name);
        ...
        for($i=0; $i<$noofrows; $i++) {
            ...
            if($uitype == 31) {
                $themeList = array_keys(Ottocrat_Util_Helper::getAllSkins());
                ...
                if($current_user->id == $this->id) {
                    $_SESSION['ottocrat_authenticated_user_theme'] = $fldvalue;
                }
            } elseif($uitype == 32) {
                $languageList = Ottocrat_Language::getAll();
        ...
        if($insertion_mode == 'edit') {
            ...
            // Set the crypt_type being used, to override the DB default constraint as it is not in ottocrat_field
            if($table_name == 'ottocrat_users' && strpos('crypt_type', $column) === false) {
                $column .= ', crypt_type';
                $qparams[]= $crypt_type;
            }
            // END

			if($table_name == 'ottocrat_users' && strpos('user_hash', $column) === false) {
				$column .= ', user_hash';
    ...
    function retrieve_entity_info($record, $module) {
        ...
        $tabid = getTabid($module);
        $sql1 =  "select * from ottocrat_field where tabid=? and ottocrat_field.presence in (0,2)";
        ...
        $currency_query = "select * from ottocrat_currency_info where id=? and currency_status='Active' and deleted=0";
        $currency_result = $adb->pquery($currency_query, array($this->column_fields["currency_id"]));
        if($adb->num_rows($currency_result) == 0) {
            $currency_query = "select * from ottocrat_currency_info where id =1";
    ...
    function uploadAndSaveFile($id,$module,$file_details) {
        ...
        $current_id = $this->db->getUniqueID("ottocrat_crmentity");
        ...
        if($save_file == 'true') {
            ...
            $params1 = array($current_id, $current_user->id, $ownerid, $module." Attachment", $this->column_fields['description'], $this->db->formatString("ottocrat_crmentity","createdtime",$date_var), $this->db->formatDate($date_var, true));
            ...
            if($id != '') {
                $delquery = 'delete from ottocrat_salesmanattachmentsrel where smid = ?';
                $this->db->pquery($delquery, array($id));
            }

            $sql3='insert into ottocrat_salesmanattachmentsrel values(?,?)';
            $this->db->pquery($sql3, array($id, $current_id));

            //we should update the imagename in the users table
            $this->db->pquery("update ottocrat_users set imagename=? where id=?", array($filename, $id));
        }
        else {
    ...
    function save($module_name) {
        ...
        $query_prev_interval = $adb->pquery("SELECT reminder_interval from ottocrat_users where id=?",
                array($this->id));
		...
		Ottocrat_Util_Helper::setCalendarDefaultActivityTypesForUser($this->id);
    ...
    function getHomeStuffOrder($id) {
        ...
        $homeorder=Array();
        if($id != '') {
            $qry=" select distinct(ottocrat_homedefault.hometype) from ottocrat_homedefault inner join ottocrat_homestuff  on ottocrat_homestuff.stuffid=ottocrat_homedefault.stuffid where ottocrat_homestuff.visible=0 and ottocrat_homestuff.userid=?";
    ...
    function insertUserdetails($inVal) {
        ...
        $s3=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s6=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s7=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s8=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s9=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s10=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s11=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s12=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s13=$adb->getUniqueID("ottocrat_homestuff");
        ...
        $s14=$adb->getUniqueID("ottocrat_homestuff");
        ...
        // Non-Default Home Page widget (no entry is requried in ottocrat_homedefault below)
        $tc = $adb->getUniqueID("ottocrat_homestuff");
        $visibility=0;
        $sql="insert into ottocrat_homestuff values($tc, 15, 'Tag Cloud', $uid, $visibility, 'Tag Cloud')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s1.",'ALVT',5,'Accounts')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s2.",'HDB',5,'Dashboard')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s3.",'PLVT',5,'Potentials')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s4.",'QLTQ',5,'Quotes')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s5.",'CVLVT',5,'NULL')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s6.",'HLT',5,'HelpDesk')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s7.",'UA',5,'Calendar')";
        $adb->pquery($sql,array());

        $sql="insert into ottocrat_homedefault values(".$s8.",'GRT',5,'NULL')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s9.",'OLTSO',5,'SalesOrder')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s10.",'ILTI',5,'Invoice')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s11.",'MNL',5,'Leads')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s12.",'OLTPO',5,'PurchaseOrder')";
        $adb->pquery($sql, array());

        $sql="insert into ottocrat_homedefault values(".$s13.",'PA',5,'Calendar')";
        $adb->pquery($sql,array());

        $sql="insert into ottocrat_homedefault values(".$s14.",'LTFAQ',5,'Faq')";
        $adb->pquery($sql, array());
		 ...
		 if($this->mode == 'edit')
		 {
			 for($i = 0;$i < count($this->homeorder_array);$i++)
			 {
				 if($_REQUEST[$this->homeorder_array[$i]] != '')
				 {
                    ...
                    $qry=" update ottocrat_homestuff,ottocrat_homedefault set ottocrat_homestuff.visible=0 where ottocrat_homestuff.stuffid=ottocrat_homedefault.stuffid and ottocrat_homestuff.userid=".$id." and ottocrat_homedefault.hometype='".$this->homeorder_array[$i]."'";//To show the default Homestuff on the the Home Page
                    $result=$adb->pquery($qry, array());
                }
				 else
				 {
                    $qry="update ottocrat_homestuff,ottocrat_homedefault set ottocrat_homestuff.visible=1 where ottocrat_homestuff.stuffid=ottocrat_homedefault.stuffid and ottocrat_homestuff.userid=".$id." and ottocrat_homedefault.hometype='".$this->homeorder_array[$i]."'";//To hide the default Homestuff on the the Home Page
                    $result=$adb->pquery($qry, array());
                }
            }
            if($save_array !="")
    ...
    function resetReminderInterval($prev_reminder_interval) {
        ...
        if($prev_reminder_interval != $this->column_fields['reminder_interval'] ) {
            ...
            $adb->pquery("UPDATE ottocrat_users SET reminder_next_time=?, date_entered=? WHERE id=?",array($set_reminder_next, $this->column_fields['date_entered'], $this->id));
        }
    }
    ...
    function deleteImage() {
        $sql1 = 'SELECT attachmentsid FROM ottocrat_salesmanattachmentsrel WHERE smid = ?';
        $res1 = $this->db->pquery($sql1, array($this->id));
        if ($this->db->num_rows($res1) > 0) {
            $attachmentId = $this->db->query_result($res1, 0, 'attachmentsid');

            $sql2 = "DELETE FROM ottocrat_crmentity WHERE crmid=? AND setype='Users Attachments'";
            $this->db->pquery($sql2, array($attachmentId));

            $sql3 = 'DELETE FROM ottocrat_salesmanattachmentsrel WHERE smid=? AND attachmentsid=?';
            $this->db->pquery($sql3, array($this->id, $attachmentId));

            $sql2 = "UPDATE ottocrat_users SET imagename='' WHERE id=?";
            $this->db->pquery($sql2, array($this->id));

            $sql4 = 'DELETE FROM ottocrat_attachments WHERE attachmentsid=?';
	...
	function transformOwnerShipAndDelete($userId,$transformToUserId){
		...
		vtws_transferOwnership($userId, $transformToUserId);
			...
			//updating the   ottocrat_users table;
		$sql = "UPDATE ottocrat_users SET status=?,deleted=? where id=?";
    ...
    function mark_deleted($id) {
        ...
        $date_var = date('Y-m-d H:i:s');
        $query = "UPDATE ottocrat_users set status=?,date_modified=?,modified_user_id=? where id=?";
    ...
    public static function getActiveAdminId() {
        ...
        $sql = "SELECT id FROM ottocrat_users WHERE is_admin = 'on' AND status = 'Active' limit 1";
    ...
    */
    public function setUserPreferences($requestArray) {
		...
		if (!empty($updateData)) {
			$updateQuery = 'UPDATE ottocrat_users SET '. ( implode('=?,', array_keys($updateData)). '=?') . ' WHERE id = ?';
	...
	public function uploadOrgLogo($requestArray, $fileArray) {
		...
		if ($file_size != 0 && in_array($file_type_val, $validFileFormats)) {
			...
			//Updating Database
			$sql = 'UPDATE ottocrat_organizationdetails SET logoname = ? WHERE organization_id = ?';
    ...
    */
	public function updateBaseCurrency($requestArray) {
		...
		if (isset ($requestArray['currency_name'])) {
			...
			$result = $adb->pquery('SELECT currency_code, currency_symbol FROM ottocrat_currencies WHERE currency_name = ?', array($currency_name));
			...
			//Updating Database
			$query = 'UPDATE ottocrat_currency_info SET currency_name = ?, currency_code = ?, currency_symbol = ? WHERE id = ?';
...
class Users_CRMSetup {
	...
	public static function isFirstUser($user) {
		...
		$isFirstUser = false;
		if (is_admin($user)) {
			$query = 'SELECT COUNT(*) AS count FROM ottocrat_crmsetup';
	...
	public static function insertEntryIntoCRMSetup($id) {
		...
		$insertQuery = 'INSERT INTO ottocrat_crmsetup (userid, setup_status) VALUES (?, ?)';
	...
	public static function getUserSetupStatus($id) {
		...
		$query = 'SELECT 1 FROM ottocrat_crmsetup WHERE userid = ? AND setup_status = ?';
	...
	public static function getPackagesList() {
		...
		global $adb;
		$result = $adb->pquery('SELECT parent, name, tablabel FROM ottocrat_tab', array());
