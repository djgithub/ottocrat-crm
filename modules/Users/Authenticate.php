 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/ottocrat_crm/sugarcrm/modules/Users/Authenticate.php,v 1.10 2005/02/28 05:25:22 jack Exp $
...
if($focus->is_authenticated()) {
	...
	if($audit_trail == 'true') {
		...
		$query = "insert into ottocrat_audit_trial values(?,?,?,?,?,?)";
		$params = array($adb->getUniqueID('ottocrat_audit_trial'), $focus->id, 'Users','Authenticate','',$date_var);
	...
	$_SESSION['ottocrat_authenticated_user_theme'] = $authenticated_user_theme;
	...
	if ($userSetupStatus) {
		...
		if($isFirstUser) {
			header('Location: '.Ottocrat_Request:: encryptLink('index.php?module=Users&action=SystemSetup'));
		} else {
	...
	$sql = 'select user_name, id, crypt_type from ottocrat_users where user_name=?';
