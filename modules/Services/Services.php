...
class Services extends CRMEntity {
	...
	var $table_name = 'ottocrat_service';
	 ...
	 */
	var $customFieldTable = Array('ottocrat_servicecf', 'serviceid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('ottocrat_crmentity','ottocrat_service','ottocrat_servicecf');
	...
	var $tab_name_index = Array(
		...
		'ottocrat_service'=>'serviceid',
		'ottocrat_servicecf'=>'serviceid',
		'ottocrat_producttaxrel'=>'productid');
	...
	function save_module($module)
	{
		...
		{
			$this->insertTaxInformation('ottocrat_producttaxrel', 'Services');
			$this->insertPriceInformation('ottocrat_productcurrencyrel', 'Services');
		}
		// Update unit price value in ottocrat_productcurrencyrel
		$this->updateUnitPrice();
	}

	/**	function to save the service tax information in ottocrat_servicetaxrel table
	 *	@param string $tablename - ottocrat_tablename to save the service tax relationship (servicetaxrel)
	...
	*/
	...
	/**	function to save the service price information in ottocrat_servicecurrencyrel table
	 *	@param string $tablename - ottocrat_tablename to save the service currency relationship (servicecurrencyrel)
	 *	@param string $module	 - current module name
	 *	$return void
	*/
		...
		for($i=0;$i<count($currency_details);$i++)
		{
			...
			if($_REQUEST[$cur_checkname] == 'on' || $_REQUEST[$cur_checkname] == 1)
			{
				...
				if ($_REQUEST['base_currency'] == $cur_valuename) {
					$adb->pquery("update ottocrat_service set currency_id=?, unit_price=? where serviceid=?", array($curid, $actualPrice, $this->id));
				}
			}else{
				$curid = fetchCurrency($current_user->id);
				$adb->pquery("update ottocrat_service set currency_id=? where serviceid=?", array($curid, $this->id));
			}
		}
	...
	function updateUnitPrice() {
		$prod_res = $this->db->pquery("select unit_price, currency_id from ottocrat_service where serviceid=?", array($this->id));
	...
	function getListQuery($module, $where='') {
		...
		$query .= " LEFT JOIN ottocrat_groups
	...
	function getDuplicatesQuery($module,$table_cols,$field_values,$ui_type_arr,$select_cols='') {
		...
		$from_clause .=	" LEFT JOIN ottocrat_groups ON ottocrat_groups.groupid = ottocrat_crmentity.smownerid
	...
	function get_purchase_orders($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT ottocrat_crmentity.*,
			...
			INNER JOIN ottocrat_service
				ON ottocrat_service.serviceid = ottocrat_inventoryproductrel.productid
			LEFT JOIN ottocrat_groups
				ON ottocrat_groups.groupid = ottocrat_crmentity.smownerid
			...
			LEFT JOIN ottocrat_users
				ON ottocrat_users.id = ottocrat_crmentity.smownerid
			WHERE ottocrat_crmentity.deleted = 0
			AND ottocrat_service.serviceid = ".$id;
	...
	function get_salesorder($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT ottocrat_crmentity.*,
			ottocrat_salesorder.*,
			ottocrat_service.servicename AS servicename,
			ottocrat_account.accountname,
			case when (ottocrat_users.user_name not like '') then $userNameSql
				else ottocrat_groups.groupname end as user_name
	...
	function getPriceBookRelatedServices($query,$focus,$returnset='')
	{
		...
		if(PerformancePrefs::getBoolean('LISTVIEW_COMPUTE_PAGE_COUNT', false) === true ||
			...
			$noofrows = $adb->query_result($adb->query(Ottocrat_Functions::mkCountQuery($query)),0,'count');
		}else{
			$noofrows = null;
		}
		$module = 'PriceBooks';
		...
		for($i=0; $i<$numRows; $i++) {
			...
			if(isPermitted("PriceBooks","EditView","") == 'yes' && isPermitted('Services', 'EditView', $entity_id) == 'yes') {
				...
				$action .= '<img src="'. ottocrat_imageurl('blank.gif', $theme).'" border="0" />';
			}
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Quotes"=>"ottocrat_inventoryproductrel","PurchaseOrder"=>"ottocrat_inventoryproductrel","SalesOrder"=>"ottocrat_inventoryproductrel",
				"Invoice"=>"ottocrat_inventoryproductrel","PriceBooks"=>"ottocrat_pricebookproductrel","Documents"=>"ottocrat_senotesrel");

		$tbl_field_arr = Array("ottocrat_inventoryproductrel"=>"id","ottocrat_pricebookproductrel"=>"pricebookid","ottocrat_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("ottocrat_inventoryproductrel"=>"productid","ottocrat_pricebookproductrel"=>"productid","ottocrat_senotesrel"=>"crmid");
	...
	function generateReportsQuery($module,$queryPlanner){
	   	global $current_user;
			...
			$matrix->setDependency('ottocrat_seproductsrel',array('ottocrat_crmentityRelServices','ottocrat_accountRelServices','ottocrat_leaddetailsRelServices','ottocrat_servicecf','ottocrat_potentialRelServices'));
			$query = "from ottocrat_service
				inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_service.serviceid";
			if ($queryPlanner->requireTable("ottocrat_servicecf")){
			    $query .= " left join ottocrat_servicecf on ottocrat_service.serviceid = ottocrat_servicecf.serviceid";
			}
			if ($queryPlanner->requireTable("ottocrat_usersServices")){
			    $query .= " left join ottocrat_users as ottocrat_usersServices on ottocrat_usersServices.id = ottocrat_crmentity.smownerid";
			}
			if ($queryPlanner->requireTable("ottocrat_groupsServices")){
			    $query .= " left join ottocrat_groups as ottocrat_groupsServices on ottocrat_groupsServices.groupid = ottocrat_crmentity.smownerid";
			}
			if ($queryPlanner->requireTable("ottocrat_seproductsrel")){
			    $query .= " left join ottocrat_seproductsrel on ottocrat_seproductsrel.productid= ottocrat_service.serviceid";
			}
			if ($queryPlanner->requireTable("ottocrat_crmentityRelServices")){
			    $query .= " left join ottocrat_crmentity as ottocrat_crmentityRelServices on ottocrat_crmentityRelServices.crmid = ottocrat_seproductsrel.crmid and ottocrat_crmentityRelServices.deleted = 0";
			}
			if ($queryPlanner->requireTable("ottocrat_accountRelServices")){
			    $query .= " left join ottocrat_account as ottocrat_accountRelServices on ottocrat_accountRelServices.accountid=ottocrat_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("ottocrat_leaddetailsRelServices")){
			    $query .= " left join ottocrat_leaddetails as ottocrat_leaddetailsRelServices on ottocrat_leaddetailsRelServices.leadid = ottocrat_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("ottocrat_potentialRelServices")){
			    $query .= " left join ottocrat_potential as ottocrat_potentialRelServices on ottocrat_potentialRelServices.potentialid = ottocrat_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("ottocrat_lastModifiedByServices")){
			    $query .= " left join ottocrat_users as ottocrat_lastModifiedByServices on ottocrat_lastModifiedByServices.id = ottocrat_crmentity.modifiedby";
			}
			if ($queryPlanner->requireTable("innerService")){
			    $query .= " LEFT JOIN (
					SELECT ottocrat_service.serviceid,
							(CASE WHEN (ottocrat_service.currency_id = 1 ) THEN ottocrat_service.unit_price
								ELSE (ottocrat_service.unit_price / ottocrat_currency_info.conversion_rate) END
							) AS actual_unit_price
					FROM ottocrat_service
					LEFT JOIN ottocrat_currency_info ON ottocrat_service.currency_id = ottocrat_currency_info.id
					LEFT JOIN ottocrat_productcurrencyrel ON ottocrat_service.serviceid = ottocrat_productcurrencyrel.productid
					AND ottocrat_productcurrencyrel.currencyid = ". $current_user->currency_id . "
				) AS innerService ON innerService.serviceid = ottocrat_service.serviceid";
			}
			return $query;
	}
	...
	function generateReportsSecQuery($module,$secmodule, $queryPlanner) {
		global $current_user;
		$matrix = $queryPlanner->newDependencyMatrix();
		$matrix->setDependency('ottocrat_service',array('actual_unit_price','ottocrat_currency_info','ottocrat_productcurrencyrel','ottocrat_servicecf','ottocrat_crmentityServices'));
		$matrix->setDependency('ottocrat_crmentityServices',array('ottocrat_usersServices','ottocrat_groupsServices','ottocrat_lastModifiedByServices'));
		if (!$queryPlanner->requireTable("ottocrat_service",$matrix)){
			return '';
		}
		$query = $this->getRelationQuery($module,$secmodule,"ottocrat_service","serviceid", $queryPlanner);
		if ($queryPlanner->requireTable("innerService")){
		    $query .= " LEFT JOIN (
			SELECT ottocrat_service.serviceid,
			(CASE WHEN (ottocrat_service.currency_id = " . $current_user->currency_id . " ) THEN ottocrat_service.unit_price
			WHEN (ottocrat_productcurrencyrel.actual_price IS NOT NULL) THEN ottocrat_productcurrencyrel.actual_price
			ELSE (ottocrat_service.unit_price / ottocrat_currency_info.conversion_rate) * ". $current_user->conv_rate . " END
			) AS actual_unit_price FROM ottocrat_service
            LEFT JOIN ottocrat_currency_info ON ottocrat_service.currency_id = ottocrat_currency_info.id
            LEFT JOIN ottocrat_productcurrencyrel ON ottocrat_service.serviceid = ottocrat_productcurrencyrel.productid
			AND ottocrat_productcurrencyrel.currencyid = ". $current_user->currency_id . ")
            AS innerService ON innerService.serviceid = ottocrat_service.serviceid";
		}
		if ($queryPlanner->requireTable("ottocrat_crmentityServices",$matrix)){
		    $query .= " left join ottocrat_crmentity as ottocrat_crmentityServices on ottocrat_crmentityServices.crmid=ottocrat_service.serviceid and ottocrat_crmentityServices.deleted=0";
		}
		if ($queryPlanner->requireTable("ottocrat_servicecf")){
		    $query .= " left join ottocrat_servicecf on ottocrat_service.serviceid = ottocrat_servicecf.serviceid";
		}
		if ($queryPlanner->requireTable("ottocrat_usersServices")){
		    $query .= " left join ottocrat_users as ottocrat_usersServices on ottocrat_usersServices.id = ottocrat_crmentityServices.smownerid";
		}
		if ($queryPlanner->requireTable("ottocrat_groupsServices")){
		    $query .= " left join ottocrat_groups as ottocrat_groupsServices on ottocrat_groupsServices.groupid = ottocrat_crmentityServices.smownerid";
		}
		if ($queryPlanner->requireTable("ottocrat_lastModifiedByServices")){
		    $query .= " left join ottocrat_users as ottocrat_lastModifiedByServices on ottocrat_lastModifiedByServices.id = ottocrat_crmentityServices.modifiedby ";
		}
        if ($queryPlanner->requireTable("ottocrat_createdbyServices")){
			$query .= " left join ottocrat_users as ottocrat_createdbyServices on ottocrat_createdbyServices.id = ottocrat_crmentityServices.smcreatorid ";
		}
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Quotes" => array("ottocrat_inventoryproductrel"=>array("productid","id"),"ottocrat_service"=>"serviceid"),
			"PurchaseOrder" => array("ottocrat_inventoryproductrel"=>array("productid","id"),"ottocrat_service"=>"serviceid"),
			"SalesOrder" => array("ottocrat_inventoryproductrel"=>array("productid","id"),"ottocrat_service"=>"serviceid"),
			"Invoice" => array("ottocrat_inventoryproductrel"=>array("productid","id"),"ottocrat_service"=>"serviceid"),
			"PriceBooks" => array("ottocrat_pricebookproductrel"=>array("productid","pricebookid"),"ottocrat_service"=>"serviceid"),
			"Documents" => array("ottocrat_senotesrel"=>array("crmid","notesid"),"ottocrat_service"=>"serviceid"),
		);
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
 		...
 		if($eventType == 'module.postinstall') {
			...
			$ttModuleInstance = Ottocrat_Module::getInstance('HelpDesk');
			$ttModuleInstance->setRelatedList($moduleInstance,'Services',array('select'));

			$leadModuleInstance = Ottocrat_Module::getInstance('Leads');
			...
			$potModuleInstance = Ottocrat_Module::getInstance('Potentials');
			$potModuleInstance->setRelatedList($moduleInstance,'Services',array('select'));

			$pbModuleInstance = Ottocrat_Module::getInstance('PriceBooks');
			$pbModuleInstance->setRelatedList($moduleInstance,'Services',array('select'),'get_pricebook_services');

			// Initialize module sequence for the module
			$adb->pquery("INSERT into ottocrat_modentity_num values(?,?,?,?,?,?)",array($adb->getUniqueId("ottocrat_modentity_num"),$moduleName,'SER',1,1,1));
		...
		// TODO Handle actions after this module is updated.
			...
			$ServicesModule  = Ottocrat_Module::getInstance('Services');
			Ottocrat_Access::setDefaultSharing($ServicesModule);
		}
 	}
	...
	*/
	function get_services($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT ottocrat_service.serviceid, ottocrat_service.servicename,
			ottocrat_service.service_no, ottocrat_service.commissionrate,
			ottocrat_service.service_usageunit, ottocrat_service.unit_price,
			ottocrat_crmentity.crmid, ottocrat_crmentity.smownerid
			FROM ottocrat_service
			INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_service.serviceid
			INNER JOIN ottocrat_servicecf
				ON ottocrat_service.serviceid = ottocrat_servicecf.serviceid
			LEFT JOIN ottocrat_crmentityrel ON ottocrat_crmentityrel.relcrmid = ottocrat_service.serviceid AND ottocrat_crmentityrel.module='Services'
			LEFT JOIN ottocrat_users
				ON ottocrat_users.id=ottocrat_crmentity.smownerid
			LEFT JOIN ottocrat_groups
				ON ottocrat_groups.groupid = ottocrat_crmentity.smownerid
			WHERE ottocrat_crmentity.deleted = 0 AND ottocrat_crmentityrel.crmid = $id ";
