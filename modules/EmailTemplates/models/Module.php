...
class EmailTemplates_Module_Model extends Ottocrat_Module_Model {
	...
	public function getCreateRecordUrl() {
		return Ottocrat_Request:: encryptLink('index.php?module=' . $this->get('name') . '&view=' .
	...
	public function saveRecord($recordModel) {
		...
		if(empty($templateid)){
			$templateid = $db->getUniqueID('ottocrat_emailtemplates');
			$sql = "INSERT INTO ottocrat_emailtemplates(templatename, subject, description, body, deleted, templateid) VALUES (?,?,?,?,?,?)";
		}else{
			$sql = "UPDATE ottocrat_emailtemplates SET templatename=?, subject=?, description=?, body=?, deleted=? WHERE templateid = ?";
		}
	...
	public function deleteRecord($recordModel) {
		...
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ottocrat_emailtemplates WHERE templateid = ? ', array($recordId));
	}
	...
	public function deleteAllRecords() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ottocrat_emailtemplates', array());
	}
	...
	public function getAllModuleList(){
		...
		$query = 'SELECT DISTINCT(name) AS modulename FROM ottocrat_tab 
				  LEFT JOIN ottocrat_field ON ottocrat_field.tabid = ottocrat_tab.tabid
				  WHERE (ottocrat_field.uitype = ? AND ottocrat_tab.presence = ?)';
	...
	public function getRecordIds($skipRecords){
		...
		$query = 'SELECT templateid FROM ottocrat_emailtemplates WHERE templateid NOT IN ('.generateQuestionMarks($skipRecords).')';
