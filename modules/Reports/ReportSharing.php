...
$report_std_filter = new ottocratCRM_Smarty; 
...
function getVisibleCriteria($recordid='')
{
	...
	if($recordid!='')
	{
		$result = $adb->pquery("select sharingtype from ottocrat_report where reportid=?",array($recordid));
		$selcriteria=$adb->query_result($result,0,"sharingtype");
	}
	if($selcriteria == ""){
		$selcriteria = 'Public';
	}
	$filter_result = $adb->query("select * from ottocrat_reportfilters");
...
function getShareInfo($recordid=''){
	global $adb;
	$member_query = $adb->pquery("SELECT ottocrat_reportsharing.setype,ottocrat_users.id,ottocrat_users.user_name FROM ottocrat_reportsharing INNER JOIN ottocrat_users on ottocrat_users.id = ottocrat_reportsharing.shareid WHERE ottocrat_reportsharing.setype='users' AND ottocrat_reportsharing.reportid = ?",array($recordid));
	...
	$member_query = $adb->pquery("SELECT ottocrat_reportsharing.setype,ottocrat_groups.groupid,ottocrat_groups.groupname FROM ottocrat_reportsharing INNER JOIN ottocrat_groups on ottocrat_groups.groupid = ottocrat_reportsharing.shareid WHERE ottocrat_reportsharing.setype='groups' AND ottocrat_reportsharing.reportid = ?",array($recordid));
	$noofrows = $adb->num_rows($member_query);
	if($noofrows > 0){
		for($i=0;$i<$noofrows;$i++){
