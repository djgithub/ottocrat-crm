...
class Reports_ScheduleReports_Model extends Ottocrat_Base_Model {
	...
	public static function getInstanceById($recordId) {
		...
		if (!empty($recordId)) {
			$scheduledReportResult = $db->pquery('SELECT * FROM ottocrat_schedulereports WHERE reportid = ?', array($recordId));
	...
	public function saveScheduleReport() {
		...
		if ($scheduleid == self::$SCHEDULED_ON_SPECIFIC_DATE) {
            ...
            $currentTime = Ottocrat_Util_Helper::getActiveAdminCurrentDateTime();
		...
		if ($isReportScheduled == '0' || $isReportScheduled == '' || $isReportScheduled == false) {
			$deleteScheduledReportSql = "DELETE FROM ottocrat_schedulereports WHERE reportid=?";
			$adb->pquery($deleteScheduledReportSql, array($reportid));
		} else {
			$checkScheduledResult = $adb->pquery('SELECT 1 FROM ottocrat_schedulereports WHERE reportid=?', array($reportid));
			if ($adb->num_rows($checkScheduledResult) > 0) {
				$scheduledReportSql = 'UPDATE ottocrat_schedulereports SET scheduleid=?, recipients=?, schdate=?, schtime=?, schdayoftheweek=?, schdayofthemonth=?, schannualdates=?, specificemails=?, next_trigger_time=? WHERE reportid=?';
				...
				$scheduleReportSql = 'INSERT INTO ottocrat_schedulereports (reportid,scheduleid,recipients,schdate,schtime,schdayoftheweek,schdayofthemonth,schannualdates,next_trigger_time,specificemails) VALUES (?,?,?,?,?,?,?,?,?,?)';
	...
	public function getRecipientEmails() {
		...
		if (!empty($recipientsList) && count($recipientsList) > 0) {
			foreach ($recipientsList as $userId) {
                if(!Ottocrat_Util_Helper::isUserDeleted($userId)) {
	...
	public function sendEmail() {
		require_once 'vtlib/Ottocrat/Mailer.php';

		$ottocratMailer = new Ottocrat_Mailer();

		$recipientEmails = $this->getRecipientEmails();
        Ottocrat_Utils::ModuleLog('ScheduleReprots', $recipientEmails);
		foreach ($recipientEmails as $name => $email) {
			$ottocratMailer->AddAddress($email, $name);
		}
		vimport('~modules/Report/models/Record.php');
		$reportRecordModel = Reports_Record_Model::getInstanceById($this->get('reportid'));
		$currentTime = date('Y-m-d H:i:s');
        Ottocrat_Utils::ModuleLog('ScheduleReprots Send Mail Start ::', $currentTime);
		$reportname = decode_html($reportRecordModel->getName());
        $subject = $reportname;
        Ottocrat_Utils::ModuleLog('ScheduleReprot Name ::', $reportname);
		$ottocratMailer->Subject = $subject;
		$ottocratMailer->Body = $this->getEmailContent($reportRecordModel);
		$ottocratMailer->IsHTML();
		...
		foreach ($attachments as $attachmentName => $path) {
			$ottocratMailer->AddAttachment($path, decode_html($attachmentName));
		}
		//Added cc to account owner
		$accountOwnerId = Users::getActiveAdminId();
		$ottocratMailer->AddCC(getUserEmail($accountOwnerId), getUserFullName($accountOwnerId));
		$status = $ottocratMailer->Send(true);
	...
	public function updateNextTriggerTime() {
		$adb = PearDatabase::getInstance();
		$nextTriggerTime = $this->getNextTriggerTime();
        Ottocrat_Utils::ModuleLog('ScheduleReprot Next Trigger Time >> ', $nextTriggerTime);
		$adb->pquery('UPDATE ottocrat_schedulereports SET next_trigger_time=? WHERE reportid=?', array($nextTriggerTime, $this->get('reportid')));
        Ottocrat_Utils::ModuleLog('ScheduleReprot', 'Next Trigger Time updated');
	}
	...
	public static function getScheduledReports() {
		...
		@date_default_timezone_set($default_timezone);
		$result = $adb->pquery("SELECT reportid FROM ottocrat_schedulereports WHERE next_trigger_time = '' || next_trigger_time <= ?", array($currentTimestamp));
	...
	public static function runScheduledReports() {
		...
		foreach ($scheduledReports as $scheduledReport) {
            ...
            Ottocrat_Utils::ModuleLog('ScheduleReprot Send Mail Status ', $status);
			if($status)
