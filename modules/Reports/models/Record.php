...
class Reports_Record_Model extends Ottocrat_Record_Model {
	/**
	...
	function getDetailViewUrl() {
		...
		return Ottocrat_Request:: encryptLink('index.php?module='.$this->getModuleName().'&view='.$view.'&record='.$this->getId());
	}
	...
	public function getDuplicateRecordUrl() {
		...
		return Ottocrat_Request:: encryptLink('index.php?module='.$this->getModuleName().'&view='.$view.'&record='.$this->getId().'&isDuplicate=true');
	}
	...
	function getReportExcelURL() {
		return Ottocrat_Request:: encryptLink('index.php?module='.$this->getModuleName().'&view=ExportReport&mode=GetXLS&record='. $this->getId());
	}
	...
	function getReportCSVURL() {
		return Ottocrat_Request:: encryptLink('index.php?module='.$this->getModuleName().'&view=ExportReport&mode=GetCSV&record='. $this->getId());
	}
	...
	function getReportPrintURL() {
		return Ottocrat_Request:: encryptLink('index.php?module='.$this->getModuleName().'&view=ExportReport&mode=GetPrintReport&record='. $this->getId());
	}
	...
	public static function getInstanceById($recordId) {
		...
		$self = new self();
		$reportResult = $db->pquery('SELECT * FROM ottocrat_report WHERE reportid = ?', array($recordId));
	...
	function initialize() {
		$reportId = $this->getId();
		$this->report = Ottocrat_Report_Model::getInstance($reportId);
	}
	...
	function getPrimaryModuleFields() {
		...
		//need to add this ottocrat_crmentity:crmid:".$module."_ID:crmid:I
	...
	function getSelectedFields() {
		...
		$result = $db->pquery("SELECT ottocrat_selectcolumn.columnname FROM ottocrat_report
					INNER JOIN ottocrat_selectquery ON ottocrat_selectquery.queryid = ottocrat_report.queryid
					INNER JOIN ottocrat_selectcolumn ON ottocrat_selectcolumn.queryid = ottocrat_selectquery.queryid
					WHERE ottocrat_report.reportid = ? ORDER BY ottocrat_selectcolumn.columnindex", array($this->getId()));
	...
	function getSelectedCalculationFields() {
		...
		$result = $db->pquery('SELECT ottocrat_reportsummary.columnname FROM ottocrat_reportsummary
					INNER JOIN ottocrat_report ON ottocrat_report.reportid = ottocrat_reportsummary.reportsummaryid
					WHERE ottocrat_report.reportid=?', array($this->getId()));
	...
	function getSelectedSortFields() {
		...
		$result = $db->pquery('SELECT ottocrat_reportsortcol.* FROM ottocrat_report
					INNER JOIN ottocrat_reportsortcol ON ottocrat_report.reportid = ottocrat_reportsortcol.reportid
					WHERE ottocrat_report.reportid = ? ORDER BY ottocrat_reportsortcol.sortcolid',array($this->getId()));
	...
	function getSelectedStandardFilter() {
		...
		$result = $db->pquery('SELECT * FROM ottocrat_reportdatefilter WHERE datefilterid = ? AND startdate != ? AND enddate != ?',
	...
	function save() {
		...
		$reportId = $this->getId();
		if(empty($reportId)) {
			$reportId = $db->getUniqueID("ottocrat_selectquery");
			$this->setId($reportId);

			$db->pquery('INSERT INTO ottocrat_selectquery(queryid, startindex, numofobjects) VALUES(?,?,?)',
					array($reportId, 0, 0));
			...
			$db->pquery('INSERT INTO ottocrat_report(reportid, folderid, reportname, description,
								reporttype, queryid, state, owner, sharingtype) VALUES(?,?,?,?,?,?,?,?,?)', $reportParams);


			$secondaryModule = $this->getSecondaryModules();
			$db->pquery('INSERT INTO ottocrat_reportmodules(reportmodulesid, primarymodule, secondarymodules) VALUES(?,?,?)',
			...
			$reportId = $this->getId();
			$db->pquery('DELETE FROM ottocrat_selectcolumn WHERE queryid = ?', array($reportId));
			$this->saveSelectedFields();

			$db->pquery("DELETE FROM ottocrat_reportsharing WHERE reportid = ?", array($reportId));
			$this->saveSharingInformation();


			$db->pquery('UPDATE ottocrat_reportmodules SET primarymodule = ?,secondarymodules = ? WHERE reportmodulesid = ?',
					array($this->getPrimaryModule(), $this->getSecondaryModules(), $reportId));

			$db->pquery('UPDATE ottocrat_report SET reportname = ?, description = ?, reporttype = ?, folderid = ? WHERE
				reportid = ?', array($this->get('reportname'), $this->get('description'), $this->get('reporttype'), $this->get('folderid'), $reportId));


			$db->pquery('DELETE FROM ottocrat_reportsortcol WHERE reportid = ?', array($reportId));
			$db->pquery('DELETE FROM ottocrat_reportgroupbycolumn WHERE reportid = ?',array($reportId));
			$this->saveSortFields();

			$db->pquery('DELETE FROM ottocrat_reportsummary WHERE reportsummaryid = ?', array($reportId));
			$this->saveCalculationFields();

			$db->pquery('DELETE FROM ottocrat_reportdatefilter WHERE datefilterid = ?', array($reportId));
			$this->saveStandardFilter();
	...
	function saveSortFields() {
        ...
        if(!empty($sortFields)){
            ...
            foreach($sortFields as $fieldInfo) {
                $db->pquery('INSERT INTO ottocrat_reportsortcol(sortcolid, reportid, columnname, sortorder) VALUES (?,?,?,?)',
                        array($i, $this->getId(), $fieldInfo[0], $fieldInfo[1]));
                if(IsDateField($fieldInfo[0])) {
                    if(empty($fieldInfo[2])){
                        $fieldInfo[2] = 'None';
                    }
                    $db->pquery("INSERT INTO ottocrat_reportgroupbycolumn(reportid, sortid, sortcolname, dategroupbycriteria)
	...
	function saveCalculationFields() {
		...
		for ($i=0; $i<count($calculationFields); $i++) {
			$db->pquery('INSERT INTO ottocrat_reportsummary (reportsummaryid, summarytype, columnname) VALUES (?,?,?)',
	...
	function saveStandardFilter() {
		...
		if(!empty($standardFilter)) {
			$db->pquery('INSERT INTO ottocrat_reportdatefilter (datefilterid, datecolumnname, datefilter, startdate, enddate)
	...
	function saveSharingInformation() {
		...
		for($i=0; $i<count($sharingInfo); $i++) {
			$db->pquery('INSERT INTO ottocrat_reportsharing(reportid, shareid, setype) VALUES (?,?,?)',
        ...
        if(!empty($selectedFields)){
           for($i=0 ;$i<count($selectedFields);$i++) {
                if(!empty($selectedFields[$i])) {
                    $db->pquery("INSERT INTO ottocrat_selectcolumn(queryid, columnindex, columnname) VALUES (?,?,?)",
	...
	function saveAdvancedFilters() {
		...
		if(!empty($advancedFilter)) {
			...
			$db->pquery('DELETE FROM ottocrat_relcriteria WHERE queryid = ?', array($reportId));
			$db->pquery('DELETE FROM ottocrat_relcriteria_grouping WHERE queryid = ?', array($reportId));
			...
			foreach($advancedFilter as $groupIndex => $groupInfo) {
				...
				foreach($groupColumns as $columnIndex => $columnCondition) {
					...
					if($fieldType == 'currency') {
						if($field->getUIType() == '72') {
							...
							$advFilterValue = Ottocrat_Currency_UIType::convertToDBFormat($advFilterValue, null, true);
						} else {
							$advFilterValue = Ottocrat_Currency_UIType::convertToDBFormat($advFilterValue);
						}
					}
					...
					$db->pquery('INSERT INTO ottocrat_relcriteria (queryid, columnindex, columnname, comparator, value,
				...
				$db->pquery("INSERT INTO ottocrat_relcriteria_grouping(groupid, queryid, group_condition, condition_expression) VALUES (?,?,?,?)",
	...
	function saveScheduleInformation() {
		...
		$db->pquery('INSERT INTO ottocrat_scheduled_reports(reportid, recipients, schedule, format, next_trigger_time) VALUES
	...
	function deleteScheduling() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ottocrat_scheduled_reports WHERE reportid = ?', array($this->getId()));
	}
	...
	function move($folderId) {
		...
		$db->pquery("UPDATE ottocrat_report SET folderid = ? WHERE reportid = ?", array($folderId, $this->getId()));
	}
	...
	public function transformStandardFilter(){
		...
		if(!empty($standardFilter)){
			...
			if($fields[1] == 'createdtime' || $fields[1] == 'modifiedtime' ||($fields[0] == 'ottocrat_activity' && $fields[1] == 'date_start')){
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM ottocrat_report WHERE reportname = ?";
	...
	function showLineItemFieldsInFilter($calculationFields=false) {
		...
		if(!empty($calculationFields)) {
			foreach($calculationFields as $field) {
				if(stripos($field, 'cb:ottocrat_inventoryproductrel') !== false) {
					return true;
				}
			}
			return false;
    ...
    public function getRecordsListFromRequest(Ottocrat_Request $request) {
    ...
    function saveReportType(){
        ...
        if(!empty($data)){
            $db->pquery('DELETE FROM ottocrat_reporttype WHERE reportid = ?', array($this->getId()));
            $db->pquery("INSERT INTO ottocrat_reporttype(reportid, data) VALUES (?,?)",
    ...
    function getReportTypeInfo() {
		...
		$result = $db->pquery("SELECT data FROM ottocrat_reporttype WHERE reportid = ?", array($this->getId()));
		...
		if(is_array($fields)) foreach($fields as $module => $blocks) {
			if(is_array($blocks)) foreach($blocks as $blockLabel => $blockFields) {
				if(is_array($blockFields)) foreach($blockFields as $reportFieldInfo => $fieldLabel) {
					...
					if(empty($fieldInstance) || $fieldInfo[0] == 'ottocrat_inventoryproductrel' || $fieldInstance->getFieldDataType() == 'email'
		...
		if(is_array($secondaryModules)) {
			...
			foreach($secondaryModules as $secondaryModule) {
				if(!empty($secondaryModule)) {
					$secondaryModuleModel = Ottocrat_Module_Model::getInstance($secondaryModule);
		...
		if(is_array($fields)) foreach($fields as $module => $blocks) {
			if(is_array($blocks)) foreach($blocks as $blockLabel => $blockFields) {
				if(is_array($blockFields)) foreach($blockFields as $reportFieldInfo => $fieldLabel) {
					...
					if(empty($fieldInstance) || $fieldInfo[0] == 'ottocrat_inventoryproductrel'
