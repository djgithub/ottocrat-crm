...
class Reports_Folder_Model extends Ottocrat_Base_Model {
	...
	function save() {
		...
		$folderId = $this->getId();
		if(!empty($folderId)) {
			$db->pquery('UPDATE ottocrat_reportfolder SET foldername = ?, description = ? WHERE folderid = ?',
					array($this->getName(), $this->getDescription(), $folderId));
		} else {
			$result = $db->pquery('SELECT MAX(folderid) AS folderid FROM ottocrat_reportfolder', array());
			$folderId = (int) ($db->query_result($result, 0, 'folderid')) + 1;

			$db->pquery('INSERT INTO ottocrat_reportfolder(folderid, foldername, description, state) VALUES(?, ?, ?, ?)', array($folderId, $this->getName(), $this->getDescription(), 'CUSTOMIZED'));
	...
	function delete() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ottocrat_reportfolder WHERE folderid = ?', array($this->getId()));
	}
	...
	function getReports($pagingModel) {
		...
		$reportClassInstance = Ottocrat_Module_Model::getClassInstance('Reports');
		...
		$reportModuleModel = Ottocrat_Module_Model::getInstance('Reports');
	 ...
	 */
	function getEditUrl() {
		return Ottocrat_Request:: encryptLink('index.php?module=Reports&view=EditFolder&folderid='.$this->getId());
	}
	...
	function getDeleteUrl() {
		return Ottocrat_Request:: encryptLink( 'index.php?module=Reports&action=Folder&mode=delete&folderid='
			.$this->getId());
	}
	...
	public static function getInstanceById($folderId) {
		$folderModel = Ottocrat_Cache::get('reportsFolder',$folderId);
        if(!$folderModel){
            $db = PearDatabase::getInstance();
            $folderModel = Reports_Folder_Model::getInstance();

            $result = $db->pquery("SELECT * FROM ottocrat_reportfolder WHERE folderid = ?", array($folderId));

            if ($db->num_rows($result) > 0) {
                $values = $db->query_result_rowdata($result, 0);
                $folderModel->setData($values);
            }
            Ottocrat_Cache::set('reportsFolder',$folderId,$folderModel);
        }
        return $folderModel;
	}
    ...
    public static function getAll() {
        $db = PearDatabase::getInstance();
        $folders = Ottocrat_Cache::get('reports', 'folders');
        if (!$folders) {
            $folders = array();
            $result = $db->pquery("SELECT * FROM ottocrat_reportfolder ORDER BY foldername ASC", array());
            ...
            if ($noOfFolders > 0) {
                for ($i = 0; $i < $noOfFolders; $i++) {
                    ...
                    Ottocrat_Cache::set('reportsFolder',$values['folderid'],$folderModel);
                }
            }
            Ottocrat_Cache::set('reports','folders',$folders);
        }
        return $folders;
    }
	...
	function checkDuplicate() {
		...
		$query = 'SELECT 1 FROM ottocrat_reportfolder WHERE foldername = ?';
	...
	function hasReports() {
		...
		$result = $db->pquery('SELECT 1 FROM ottocrat_report WHERE folderid = ?', array($this->getId()));
	...
	public function getReportsCount() {
        ...
        // To get the report ids which are permitted for the user
            $query = "SELECT reportmodulesid, primarymodule from ottocrat_reportmodules";
        ...
        //End
		$sql = "SELECT count(*) AS count FROM ottocrat_report
				INNER JOIN ottocrat_reportfolder ON ottocrat_reportfolder.folderid = ottocrat_report.folderid AND 
                ottocrat_report.reportid in (".implode(',',$allowedReportIds).")";
		$fldrId = $this->getId();
		if($fldrId == 'All') {
			$fldrId = false;
		}
		...
		if (!$currentUserModel->isAdminUser()) {
			...
			if ($groupId) {
				$groupQuery = "(SELECT reportid from ottocrat_reportsharing WHERE shareid IN ($groupId) AND setype = 'groups') OR ";
			}
			
			$sql .= " AND (ottocrat_report.reportid IN (SELECT reportid from ottocrat_reportsharing WHERE $groupQuery shareid = ? AND setype = 'users')
						OR ottocrat_report.sharingtype = 'Public'
						OR ottocrat_report.owner = ?
						OR ottocrat_report.owner IN (SELECT ottocrat_user2role.userid FROM ottocrat_user2role
													...
													WHERE ottocrat_role.parentrole LIKE ?))";
	 ...
	 * @param <Ottocrat_Module_Model> - Reports Module Model
	...
	function getListViewQuery($folderId) {
        $sql = "select ottocrat_report.*, ottocrat_reportmodules.*, ottocrat_reportfolder.folderid from ottocrat_report 
                inner join ottocrat_reportfolder on ottocrat_reportfolder.folderid = ottocrat_report.folderid 
                inner join ottocrat_reportmodules on ottocrat_reportmodules.reportmodulesid = ottocrat_report.reportid ";
        
        if($folderId != "All") {
                $sql = $sql." where ottocrat_reportfolder.folderid = ".$folderId;
        }        
        return $sql;
	}
}