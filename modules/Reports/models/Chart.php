...
class Reports_Chart_Model extends Ottocrat_Base_Model {

	public static function getInstanceById($reportModel) {
		$self = new self();
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ottocrat_reporttype WHERE reportid = ?', array($reportModel->getId()));
...
abstract class Base_Chart extends Ottocrat_Base_Model{
	...
	function generateLink($field, $value) {
		...
		if(($dataFieldInfo[4] == 'D' || $dataFieldInfo[4] == 'DT') && !empty($dataFieldInfo[5])) {
			...
			$value = Ottocrat_Date_UIType::getDisplayDateTimeValue($value);
		}
		...
		return Ottocrat_Request::encryptLink($baseModuleListLink.'&search_params='. json_encode($listSearchParams));
	}
			...
			if($legendField) {
				...
				if($fieldDataType == 'picklist') {
					...
					$label =  implode(',', $labelList);
						...
						$label = Ottocrat_Date_UIType::getDisplayDateValue($row[strtolower($legendField->get('reportlabel'))]);
				} else if($fieldDataType == 'datetime') {
						$label = Ottocrat_Date_UIType::getDisplayDateTimeValue($row[strtolower($legendField->get('reportlabel'))]);
				} else {
			...
			if($groupByColumnsByFieldModel) {
				foreach($groupByColumnsByFieldModel as $gFieldModel) {
					...
					if($fieldDataType == 'picklist') {
						...
						$label = Ottocrat_Date_UIType::getDisplayDateValue($row[strtolower($gFieldModel->get('reportlabel'))]);
					} else if($fieldDataType == 'datetime') {
						$label = Ottocrat_Date_UIType::getDisplayDateTimeValue($row[strtolower($gFieldModel->get('reportlabel'))]);
					} else {
