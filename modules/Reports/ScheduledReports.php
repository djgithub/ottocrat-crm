...
class VTScheduledReport extends Reports {
	...
	public function getReportScheduleInfo() {
		...
		if(!empty($this->id)) {
			...
			if($cachedInfo == false) {
				$result = $adb->pquery('SELECT * FROM ottocrat_scheduled_reports WHERE reportid=?', array($this->id));
	...
	public function sendEmail() {
		...
		$ottocratMailer->Body    = $contents;
		$ottocratMailer->ContentType = "text/html";
		...
		foreach($attachments as $attachmentName => $path) {
			$ottocratMailer->AddAttachment($path, $attachmentName);
		}

		$ottocratMailer->Send(true);
	...
	public function updateNextTriggerTime() {
		...
		$adb->pquery('UPDATE ottocrat_scheduled_reports SET next_trigger_time=? WHERE reportid=?', array($nextTriggerTime, $this->id));
	}
	...
	public static function getScheduledReports($adb, $user) {
		...
		$currentTime = date('Y-m-d H:i:s');
		$result = $adb->pquery("SELECT * FROM ottocrat_scheduled_reports
