...
Class Reports_Edit_View extends Ottocrat_Edit_View {
	...
	function step1(Ottocrat_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$record = $request->get('record');
	...
	function step2(Ottocrat_request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$record = $request->get('record');
                ...
                foreach($blockFields as $key => $value){
                    if(isset($value)){
                        foreach($value as $key1 => $value1){
                            if($key1 == 'ottocrat_troubletickets:update_log:HelpDesk_Update_History:update_log:V'){
