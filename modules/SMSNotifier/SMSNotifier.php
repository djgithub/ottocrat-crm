...
class SMSNotifier extends SMSNotifierBase {
	...
	static function sendsms($message, $tonumbers, $ownerid = false, $linktoids = false, $linktoModule = false) {
		...
		if($linktoids !== false) {
			...
			if($linktoModule !== false) {
				...
				$linkidsetypes = $adb->pquery( "SELECT setype,crmid FROM ottocrat_crmentity WHERE crmid IN (".generateQuestionMarks($linktoids) . ")", array($linktoids) );
	...
	function detectRelatedModules() {
		...
		$result = $adb->pquery("SELECT distinct setype FROM ottocrat_crmentity WHERE crmid in (
			SELECT relcrmid FROM ottocrat_crmentityrel INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid=ottocrat_crmentityrel.crmid
			WHERE ottocrat_crmentity.crmid = ? AND ottocrat_crmentity.deleted=0)", array($this->id));
	...
	protected function isUserOrGroup($id) {
		global $adb;
		$result = $adb->pquery("SELECT 1 FROM ottocrat_users WHERE id=?", array($id));
		if($result && $adb->num_rows($result)) {
			return 'U';
		} else {
			return 'T';
	...
	protected function smsAssignedTo() {
		...
		if(count($userIds) > 0) {
	       	$phoneSqlQuery = "select phone_mobile, id from ottocrat_users WHERE status='Active' AND id in(". generateQuestionMarks($userIds) .")";
	...
	private function processFireSendSMSResponse($responses) {
		...
		foreach($responses as $response) {
			...
			$adb->pquery("INSERT INTO ottocrat_smsnotifier_status(smsnotifierid,tonumber,status,statusmessage,smsmessageid,needlookup) VALUES(?,?,?,?,?,?)",
	...
	static function smsquery($record) {
		global $adb;
		$result = $adb->pquery("SELECT * FROM ottocrat_smsnotifier_status WHERE smsnotifierid = ? AND needlookup = 1", array($record));
		if($result && $adb->num_rows($result)) {
			$provider = SMSNotifierManager::getActiveProviderInstance();
			...
			while($resultrow = $adb->fetch_array($result)) {
				...
				$adb->pquery("UPDATE ottocrat_smsnotifier_status SET status=?, statusmessage=?, needlookup=? WHERE smsmessageid = ?",
	...
	static function getSMSStatusInfo($record) {
		global $adb;
		$results = array();
		$qresult = $adb->pquery("SELECT * FROM ottocrat_smsnotifier_status WHERE smsnotifierid=?", array($record));
...
class SMSNotifierManager {
	...
	static function getActiveProviderInstance() {
		global $adb;
		$result = $adb->pquery("SELECT * FROM ottocrat_smsnotifier_servers WHERE isactive = 1 LIMIT 1", array());
		if($result && $adb->num_rows($result)) {
	...
	static function listConfiguredServer($id) {
		global $adb;
		$result = $adb->pquery("SELECT * FROM ottocrat_smsnotifier_servers WHERE id=?", array($id));
		if($result) {
	...
	static function listConfiguredServers() {
		global $adb;
		$result = $adb->pquery("SELECT * FROM ottocrat_smsnotifier_servers", array());
		$servers = array();
		if($result) {
	...
	static function updateConfiguredServer($id, $frmvalues) {
		...
		if(empty($id)) {
			$adb->pquery("INSERT INTO ottocrat_smsnotifier_servers (providertype,username,password,isactive,parameters) VALUES(?,?,?,?,?)",
				array($providertype, $username, $password, $isactive, $parameters));
		} else {
			$adb->pquery("UPDATE ottocrat_smsnotifier_servers SET username=?, password=?, isactive=?, providertype=?, parameters=? WHERE id=?",
				array($username, $password, $isactive, $providertype, $parameters, $id));
		}
	}
	static function deleteConfiguredServer($id) {
		global $adb;
		$adb->pquery("DELETE FROM ottocrat_smsnotifier_servers WHERE id=?", array($id));
	}
}
?>
