...
function getUserFldArray($fld_module,$roleid){
	...
	$query="select ottocrat_field.fieldlabel,ottocrat_field.columnname,ottocrat_field.fieldname, ottocrat_field.uitype" .
			" FROM ottocrat_field inner join ottocrat_picklist on ottocrat_field.fieldname = ottocrat_picklist.name" .
			" where (displaytype=1 and ottocrat_field.tabid=? and ottocrat_field.uitype in ('15','55','33','16') " .
			" or (ottocrat_field.tabid=? and fieldname='salutationtype' and fieldname !='vendortype')) " .
...
function getPickListModules(){
	...
	$query = 'select distinct ottocrat_field.fieldname,ottocrat_field.tabid,ottocrat_tab.tablabel, ottocrat_tab.name as tabname,uitype from ottocrat_field inner join ottocrat_tab on ottocrat_tab.tabid=ottocrat_field.tabid where uitype IN (15,33) and ottocrat_field.tabid != 29 and ottocrat_tab.presence != 1 and ottocrat_field.presence in (0,2) order by ottocrat_field.tabid ASC';
	// END
...
function getrole2picklist(){
	global $adb;
	$query = "select rolename,roleid from ottocrat_role where roleid not in('H1') order by roleid";
	$result = $adb->pquery($query, array());
	while($row = $adb->fetch_array($result)){
...
function getAllPickListValues($fieldName,$lang = Array() ){
	global $adb;
	$sql = 'SELECT * FROM ottocrat_'.$adb->sql_escape_string($fieldName);
...
function getEditablePicklistValues($fieldName, $lang= array(), $adb){
	...
	$sql="select $fieldName from ottocrat_$fieldName where presence=1 and $fieldName <> '--None--'";
	$res = $adb->query($sql);
...
function getNonEditablePicklistValues($fieldName, $lang=array(), $adb){
	$values = array();
	$fieldName = $adb->sql_escape_string($fieldName);
	$sql = "select $fieldName from ottocrat_$fieldName where presence=0";
	$result = $adb->query($sql);
	$count = $adb->num_rows($result);
	for($i=0;$i<$count;$i++){
...
function getAssignedPicklistValues($tableName, $roleid, $adb, $lang=array()){
	...
	if($adb->num_rows($result)){
		...
		$sql = "SELECT distinct ".$adb->sql_escape_string($tableName)." FROM ". $adb->sql_escape_string("ottocrat_$tableName")
				. " inner join ottocrat_role2picklist on ".$adb->sql_escape_string("ottocrat_$tableName").".picklist_valueid=ottocrat_role2picklist.picklistvalueid"
