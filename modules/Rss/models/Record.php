...
class Rss_Record_Model extends Ottocrat_Record_Model {
	...
	public function save($url) {
        ...
        $title = $this->getName();
        $id = $db->getUniqueID("ottocrat_rss");
        ...
        $sql = "INSERT INTO ottocrat_rss (rssid,rssurl,rsstitle) values (?,?,?)";
    ...
    public function delete() {
        ...
        $sql = 'DELETE FROM ottocrat_rss where rssid = ?';
    ...
    public function makeDefault() {
        ...
        $sql = 'UPDATE ottocrat_rss set starred = 0';
        $db->pquery($sql,array());
        
        $sql = 'UPDATE ottocrat_rss set starred = 1 where rssid = ?';
        $db->pquery($sql,array($recordId));
    }
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ottocrat_rss WHERE rssid = ?', array($recordId));
    ...
    public function getDefaultRss() {
        ...
        $result = $db->pquery('SELECT rssid FROM ottocrat_rss where starred = 1', array());
        $recordId = $db->query_result($result,'0','rssid');
        if($recordId) {
            $this->setId($recordId);
        } else {
            $result = $db->pquery('SELECT rssid FROM ottocrat_rss', array());
            $recordId = $db->query_result($result,'0','rssid');
            $this->setId($recordId);
        }
    }
