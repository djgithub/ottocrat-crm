...
class ModTracker {
	...
	*/	
	function vtlib_handler($moduleName, $eventType) {
		...
		$modtrackerModule = Ottocrat_Module::getInstance($currentModule);
        $otherModuleNames = $this->getModTrackerEnabledModules();
 		...
 		if($eventType == 'module.postinstall') {
			...
			$adb->pquery('INSERT INTO ottocrat_settings_field(fieldid, blockid, name, iconpath, description, linkto, sequence)
				...
				Ottocrat_Request:: encryptLink('index.php?module=ModTracker&action=BasicSettings&parenttab=Settings&formodule=ModTracker'), $seq));
    ...
    */
    function getModTrackerEnabledModules() {
        global $adb;
        $moduleResult = $adb->pquery('SELECT * FROM ottocrat_modtracker_tabs', array());
	...
	static function disableTrackingForModule($tabid){
		...
		if(!self::isModulePresent($tabid)){
				$res=$adb->pquery("INSERT INTO ottocrat_modtracker_tabs VALUES(?,?)",array($tabid,0));
				self::updateCache($tabid,0);
		} else{
				$updatevisibility = $adb->pquery("UPDATE ottocrat_modtracker_tabs SET visible = 0 WHERE tabid = ?", array($tabid));
				self::updateCache($tabid,0);
		}
		if(self::isModtrackerLinkPresent($tabid)) {
			$moduleInstance=Ottocrat_Module::getInstance($tabid);
	...
	static function enableTrackingForModule($tabid){
		global $adb;
		if(!self::isModulePresent($tabid)){
			$res=$adb->pquery("INSERT INTO ottocrat_modtracker_tabs VALUES(?,?)",array($tabid,1));
			self::updateCache($tabid,1);
		} else{
			$updatevisibility = $adb->pquery("UPDATE ottocrat_modtracker_tabs SET visible = 1 WHERE tabid = ?", array($tabid));
			self::updateCache($tabid,1);
		}
		if(!self::isModTrackerLinkPresent($tabid)){
			$moduleInstance=Ottocrat_Module::getInstance($tabid);
	...
	static function isTrackingEnabledForModule($modulename){
		...
		if(!self::getVisibilityForModule($tabid) || self::getVisibilityForModule($tabid) !== 0) {
			$query = $adb->pquery("SELECT * FROM ottocrat_modtracker_tabs WHERE ottocrat_modtracker_tabs.visible = 1
								   AND ottocrat_modtracker_tabs.tabid=?", array($tabid));
	...
	static function isModulePresent($tabid){
		...
		if(!self::checkModuleInModTrackerCache($tabid)){
			$query=$adb->pquery("SELECT * FROM ottocrat_modtracker_tabs WHERE tabid = ?",array($tabid));
			$rows = $adb->num_rows($query);
			if($rows){
	...
	static function isModtrackerLinkPresent($tabid){
		global $adb;
		$query1=$adb->pquery("SELECT * FROM ottocrat_links WHERE linktype='DETAILVIEWBASIC' AND
    ...
    function getChangedRecords($uniqueId, $mtime, $limit = 100) {
        ...
        $query = "SELECT id, module, modifiedtime, ottocrat_crmentity.crmid, smownerid, ottocrat_modtracker_basic.status
                FROM ottocrat_modtracker_basic
                INNER JOIN ottocrat_crmentity ON ottocrat_modtracker_basic.crmid = ottocrat_crmentity.crmid
                    AND ottocrat_modtracker_basic.changedon = ottocrat_crmentity.modifiedtime
        ...
        $moreQuery = "SELECT * FROM ottocrat_modtracker_basic WHERE id > ? AND changedon >= ? AND module
    ...
    static function getRecordFieldChanges($crmid, $time, $decodeHTML = false) {
        ...
        $fieldResult = $adb->pquery('SELECT * FROM ottocrat_modtracker_detail
                        INNER JOIN ottocrat_modtracker_basic ON ottocrat_modtracker_basic.id = ottocrat_modtracker_detail.id
    ...
    static function trackRelation($sourceModule, $sourceId, $targetModule, $targetId, $type) {
        ...
        $adb->pquery('INSERT INTO ottocrat_modtracker_basic(id, crmid, module, whodid, changedon, status) VALUES(?,?,?,?,?,?)',
                array($id , $sourceId, $sourceModule, $current_user->id, $currentTime, $type));

        $adb->pquery('INSERT INTO ottocrat_modtracker_relations(id, targetmodule, targetid, changedon)
