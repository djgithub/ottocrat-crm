...
class ProjectMilestone extends CRMEntity {
	...
	var $table_name = 'ottocrat_projectmilestone';
	 ...
	 */
	var $customFieldTable = Array('ottocrat_projectmilestonecf', 'projectmilestoneid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('ottocrat_crmentity', 'ottocrat_projectmilestone', 'ottocrat_projectmilestonecf');
	...
	var $tab_name_index = Array(
		...
		'ottocrat_projectmilestone'   => 'projectmilestoneid',
	    'ottocrat_projectmilestonecf' => 'projectmilestoneid');
	...
	function vtlib_handler($modulename, $event_type) {
		...
		if($event_type == 'module.postinstall') {
			...
			$projectMilestoneResult = $adb->pquery('SELECT tabid FROM ottocrat_tab WHERE name=?', array('ProjectMilestone'));
			...
			if(getTabid('CustomerPortal')) {
				$checkAlreadyExists = $adb->pquery('SELECT 1 FROM ottocrat_customerportal_tabs WHERE tabid=?', array($projectmilestoneTabid));
				if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
					...
					$adb->query("INSERT INTO ottocrat_customerportal_tabs(tabid,visible,sequence) VALUES ($projectmilestoneTabid,1,$nextSequence)");
					$adb->query("INSERT INTO ottocrat_customerportal_prefs(tabid,prefkey,prefvalue) VALUES ($projectmilestoneTabid,'showrelatedinfo',1)");
				}
			}
			...
			if (!($adb->num_rows($result))) {
				...
				$adb->pquery("INSERT INTO ottocrat_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("ottocrat_modentity_num"), $modulename, 'PM', 1, 1, 1));
			}
