...
class Settings_ExtensionStore_ExtensionStore_View extends Settings_Ottocrat_Index_View {
    ...
    public function process(Ottocrat_Request $request) {
        ...
        $loaderInfo =  $loaderRequired ? $loaderInstance->ottocrat_extensionloader_suggest() : null;
    ...
    protected function searchExtension(Ottocrat_Request $request) {
    ...
    protected function detail(Ottocrat_Request $request) {
        $viewer = $this->getViewer($request);
        $qualifiedModuleName = $request->getModule(false);
    ...
    protected function installationLog(Ottocrat_Request $request) {
        $viewer = $this->getViewer($request);
        global $Ottocrat_Utils_Log;
        $viewer->assign('OTTOCRAT_UTILS_LOG', $Ottocrat_Utils_Log);
            ...
            if ($package) {
                ...
                if (!$upgradeError) {
                    if (!$isLanguagePackage) {
                        $moduleModel = Ottocrat_Module_Model::getInstance($importedModuleName);
        ...
        if ($extensionId && $extensionModel) {
            ...
            $viewer->assign('TARGET_MODULE_INSTANCE', Ottocrat_Module_Model::getInstance($targetModuleName));
    ...
    protected function oneClickInstall(Ottocrat_Request $request) {
        $viewer = $this->getViewer($request);
        global $Ottocrat_Utils_Log;
        $viewer->assign('OTTOCRAT_UTILS_LOG', $Ottocrat_Utils_Log);
        $Ottocrat_Utils_Log = true;
