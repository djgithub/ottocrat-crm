...
vimport('~~/vtlib/Ottocrat/Package.php');
vimport ('~/libraries/PHPMarkdown/Michelf/Markdown.inc.php');

class Settings_ExtensionStore_Extension_Model extends Ottocrat_Base_Model {
	...
	public function __construct() {
		parent::__construct();
		$this->EXTENSIONSTORE_LOOKUP_URL = 'https://marketplace.ottocrat.com/api';
		global $site_URL;
		if (empty($site_URL)) {
	 ...
	 * @return <Ottocrat_Package> package object
	 */
	public function getPackage() {
		$packageModel = new Ottocrat_Package();
	...
	/**
	 * Function to check whether it is compatible with ottocrat or not
	 * @return <boolean> true/false
	 */
	public function isOttocratCompatible() {
		vimport('~~/vtlib/Ottocrat/Version.php');
		$ottocratVersion = $this->get('ottocratVersion');
		$ottocratMaxVersion = $this->get('ottocratMaxVersion');

		if ((Ottocrat_Version::check($ottocratVersion, '>=') && $ottocratMaxVersion && Ottocrat_Version::check($ottocratMaxVersion, '<'))
				|| Ottocrat_Version::check($ottocratVersion, '=')) {
			return true;
		}
		return false;
	}
    ...
    public static function getLanguageInstance($lang) {
        $sql = 'SELECT id,name,prefix FROM ottocrat_language WHERE name = ?';
	...
	public function findListings($searchTerm=null, $searchType) {
         ...
         */
	public function logoutMarketPlace(Ottocrat_Request $request) {
		    $sql = 'DELETE FROM ottocrat_extnstore_users';
		    $db = PearDatabase::getInstance();
		    $db->pquery($sql, array());
	    }
	...
	public function getInstanceFromArray($listing) {
		...
		foreach ($listing as $key => $value) {
			...
			case 'minrange':   $key = 'ottocratVersion'; break;
			case 'maxrange':   $key = 'ottocratMaxVersion'; break;
    ...
    public static function getModuleFromExtnName($extnName) {
        $moduleModel = Ottocrat_Module_Model::getInstance($extnName);
        if($moduleModel) {
            $moduleModel->set('extnType','module');
        }
        if(!$moduleModel) {
            if(self::getLanguageInstance($extnName)) {
               $moduleModel = new Ottocrat_Module_Model();
       ...
       public function getCardDetails($cardId) {
        ...
        public function getLocationUrl($extensionId, $extensionName){
            ...
            if(is_admin($current_user)){
                return Ottocrat_Request:: encryptLink( 'index.php?module=ExtensionStore&parent=Settings&view=ExtensionStore&mode=detail&extensionId='.$extensionId.'&extensionName='.$extensionName);
            }else{
                return 'https://marketplace.ottocrat.com/app/listings?id='.$extensionId;
            }
        }
}
