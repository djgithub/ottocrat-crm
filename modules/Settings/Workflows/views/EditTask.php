...
class Settings_Workflows_EditTask_View extends Settings_Ottocrat_Index_View {
	...
	public function process(Ottocrat_Request $request) {
		...
		if ($taskType === 'VTCreateEntityTask') {
			if ($taskObject->entity_type) {
				$relationModuleModel = Ottocrat_Module_Model::getInstance($taskObject->entity_type);
        ...
        if($moduleModel->getName() == 'Invoice' || $moduleModel->getName() == 'Quotes') {
            $metaVariables['Portal Pdf Url'] = '(general : (__OttocratMeta__) portalpdfurl)';
        }
		...
		if(!empty ($repeat_date)){
		    $repeat_date = Ottocrat_Date_UIType::getDisplayDateValue($repeat_date);
		}
		...
		$fromEmailFieldOptions .= '<option value="$(general : (__OttocratMeta__) supportName)<$(general : (__OttocratMeta__) supportEmailId)>"
		...
		$assignedToValues[vtranslate('LBL_USERS', 'Ottocrat')] = $userList;
		$assignedToValues[vtranslate('LBL_GROUPS', 'Ottocrat')] = $groupList;
