...
class Settings_Workflows_Edit_View extends Settings_Ottocrat_Index_View {
	...
	public function step2(Ottocrat_Request $request) {
		...
		if ($recordId) {
			...
			$selectedModule = Ottocrat_Module_Model::getInstance($selectedModuleName);
		...
		// Added to show filters only when saved from ottocrat6
	...
	function Step3(Ottocrat_Request $request) {
		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		$qualifiedModuleName = $request->getModule(false);
