	...
	public function getStructure() {
					...
					if($fieldModel->isViewable()) {
						...
						switch($fieldModel->getFieldDataType()) {
							case 'date'		:	if (($moduleName === 'Events' && in_array($fieldName, array('date_start', 'due_date'))) ||
													...
													$fieldName = $fieldName .' ($(general : (__OttocratMeta__) usertimezone))';
												} else {
													$fieldName = $fieldName .' ($_DATE_FORMAT_)';
												}
												break;
							case 'datetime'	:	$fieldName = $fieldName .' ($(general : (__OttocratMeta__) usertimezone))';	break;
							default			:	$fieldName;
						}
		...
		if ($moduleModel->isCommentEnabled()) {
			...
			foreach ($commentFieldModelsList as $commentFieldName => $commentFieldModel) {
				switch($commentFieldModel->getFieldDataType()) {
						...
						case 'datetime'	:	$commentFieldName = $commentFieldName .' ($(general : (__OttocratMeta__) usertimezone)_)';	break;
		...
		foreach($fields as $parentFieldName => $field) {
			...
			foreach($referenceModules as $refModule) {
				$moduleModel = Ottocrat_Module_Model::getInstance($refModule);
				$blockModelList = $moduleModel->getBlocks();
				foreach($blockModelList as $blockLabel=>$blockModel) {
					$fieldModelList = $blockModel->getFields();
					if (!empty ($fieldModelList)) {
						foreach($fieldModelList as $fieldName=>$fieldModel) {
							if($fieldModel->isViewable()) {
								...
								switch($fieldModel->getFieldDataType()) {
									case 'date'		:	if (($moduleName === 'Events' && in_array($fieldName, array('date_start', 'due_date'))) ||
															($moduleName === 'Calendar' && $fieldName === 'date_start')) {
															$workflowColumnName = $name .' ($(general : (__OttocratMeta__) usertimezone))';
														} else {
															$workflowColumnName = $name .' ($_DATE_FORMAT_)';
														}
														break;
									case 'datetime'	:	$workflowColumnName = $name .' ($(general : (__OttocratMeta__) usertimezone))';	break;
