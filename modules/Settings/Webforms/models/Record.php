...
class Settings_Webforms_Record_Model extends Settings_Ottocrat_Record_Model {
	...
	public function getDetailViewUrl() {
		...
		Ottocrat_Request:: encryptLink("index.php?module=".$moduleModel->getName()."&parent=".$moduleModel->getParentName()."&view=Detail&record=".$this->getId());
	}
	...
	public function getEditViewUrl() {
		$moduleModel = $this->getModule();
		Ottocrat_Request:: encryptLink("index.php?module=".$moduleModel->getName()."&parent=".$moduleModel->getParentName()."&view=Edit&record=".$this->getId());
	}
	...
	public function getDeleteUrl() {
		$moduleModel = $this->getModule();
		Ottocrat_Request:: encryptLink("index.php?module=".$moduleModel->getName()."&parent=".$moduleModel->getParentName()."&action=Delete&record=".$this->getId());
	}
	...
	public function getShowFormUrl() {
		$moduleModel = $this->getModule();
		Ottocrat_Request:: encryptLink("index.php?module=".$moduleModel->getName()."&parent=".$moduleModel->getParentName()."&view=ShowForm&record=".$this->getId());
	}
	...
	public function getRecordLinks() {
		...
		$recordLinks = array(
				...
				array(
						...
						'linkurl' => "javascript:Settings_Ottocrat_List_Js.triggerDelete(event,'".$this->getDeleteUrl()."');",
						'linkicon' => 'icon-trash'
				)
		);
	...
	public function getSelectedFieldsList($mode='') {
		if (!$this->selectedFields) {
			...
			if ($targetModule) {
				...
				$targetModuleModel = Ottocrat_Module_Model::getInstance($targetModule);
				$allFields = $targetModuleModel->getFields();

				$result = $db->pquery("SELECT * FROM ottocrat_webforms_field WHERE webformid = ? ORDER BY sequence", array($this->getId()));
	...
	public function save() {
		...
		if ($mode) {
			$updateQuery = "UPDATE ottocrat_webforms SET description = ?, returnurl = ?, ownerid = ?, enabled = ?, captcha = ? , roundrobin = ?, roundrobin_userid = ?, roundrobin_logic = ? ,targetmodule = ? WHERE id = ?";
			...
			$insertQuery = "INSERT INTO ottocrat_webforms(name, targetmodule, publicid, enabled, description, ownerid, returnurl, captcha, roundrobin, roundrobin_userid, roundrobin_logic) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		$db->pquery("DELETE FROM ottocrat_webforms_field WHERE webformid = ?", array($this->getId()));

		//Saving data of source module fields info for this webform
		$selectedFieldsData = $this->get('selectedFieldsData');
		$sourceModuleModel = Ottocrat_Module_Model::getInstance($this->get('targetmodule'));

		$fieldInsertQuery = "INSERT INTO ottocrat_webforms_field(webformid, fieldname, neutralizedfield, defaultvalue, required, sequence, hidden) VALUES(?, ?, ?, ?, ?, ?, ?)";
		foreach ($selectedFieldsData as $fieldName => $fieldDetails) {
			...
			$fieldModel = Ottocrat_Field_Model::getInstance($fieldName, $sourceModuleModel);
			...
			if ($dataType === 'date') {
				$fieldDefaultValue = Ottocrat_Date_UIType::getDBInsertedValue($fieldDefaultValue);
			}
	
			if ($dataType === 'time') {
				$fieldDefaultValue = Ottocrat_Time_UIType::getTimeValueWithSeconds($fieldDefaultValue);
			}
			...
			if ($dataType === 'reference') {
				...
				$referenceObject = OttocratWebserviceObject::fromName($db,$referenceModule);
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM ottocrat_webforms WHERE name = ?";
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		...
		$result = $db->pquery("SELECT * FROM ottocrat_webforms WHERE id = ?", array($recordId));
		if ($db->num_rows($result)) {
	 ...
	 * @return <Settings_Ottocrat_Module_Model>
	 */
	static public function getCleanInstance($moduleName) {
		$recordModel = new self();
		$moduleModel = Settings_Ottocrat_Module_Model::getInstance($moduleName);
