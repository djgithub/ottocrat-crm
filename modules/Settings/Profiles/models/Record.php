 ...
 */
class Settings_Profiles_Record_Model extends Settings_Ottocrat_Record_Model {
	...
	public function hasModuleActionPermission($module, $action) {
		$actionId = false;
		if(is_object($action) && is_a($action, 'Ottocrat_Action_Model')) {
			$actionId = $action->getId();
		} else {
			$action = Ottocrat_Action_Model::getInstance($action);
			$actionId = $action->getId();
		}
		if(!$actionId) {
			return false;
	...
	public function getProfileTabFieldModel($module, $field) {
		...
		$fieldId = false;
		if(is_object($field) && is_a($field, 'Ottocrat_Field_Model')) {
			$fieldId = $field->getId();
		} else {
			$field = Ottocrat_Field_Model::getInstance($field, $profileTabModel);
			$fieldId = $field->getId();
		}
		if(!$fieldId) {
			return false;
	...
	public function getProfileTabFieldPermissions($tabId) {
		...
		if(!$this->profile_tab_field_permissions[$tabId]) {
			...
			if($this->getId()) {
				$sql = 'SELECT * FROM ottocrat_profile2field WHERE profileid=? AND tabid=?';
	...
	public function getProfileActionPermissions() {
		...
		if(!$this->profile_action_permissions) {
			...
			if($this->getId()) {
				$sql = 'SELECT * FROM ottocrat_profile2standardpermissions WHERE profileid=?';
				$params = array($this->getId());
				$result = $db->pquery($sql, $params);
				$noOfRows = $db->num_rows($result);
				for($i=0; $i<$noOfRows; ++$i) {
	...
	public function getProfileUtilityPermissions() {
		...
		if(!$this->profile_utility_permissions) {
			...
			if($this->getId()) {
				$sql = 'SELECT * FROM ottocrat_profile2utility WHERE profileid=?';
				$params = array($this->getId());
				$result = $db->pquery($sql, $params);
				$noOfRows = $db->num_rows($result);
				for($i=0; $i<$noOfRows; ++$i) {
	...
	public function getModulePermissions() {
		if(!$this->module_permissions) {
			$allModules = Ottocrat_Module_Model::getAll(array(0), Settings_Profiles_Module_Model::getNonVisibleModulesList());
			$eventModule = Ottocrat_Module_Model::getInstance('Events');
			...
			$allTabActions = Ottocrat_Action_Model::getAll(true);
	...
	public function delete($transferToRecord) {
		...
		$db->pquery('DELETE FROM ottocrat_profile2globalpermissions WHERE profileid=?', array($profileId));
		$db->pquery('DELETE FROM ottocrat_profile2tab WHERE profileid=?', array($profileId));
		$db->pquery('DELETE FROM ottocrat_profile2standardpermissions WHERE profileid=?', array($profileId));
		$db->pquery('DELETE FROM ottocrat_profile2utility WHERE profileid=?', array($profileId));
		$db->pquery('DELETE FROM ottocrat_profile2field WHERE profileid=?', array($profileId));

		$checkSql = 'SELECT roleid, count(profileid) AS profilecount FROM ottocrat_role2profile
							WHERE roleid IN (select roleid FROM ottocrat_role2profile WHERE profileid=?) GROUP BY roleid';
		...
		for($i=0; $i<$noOfRoles; ++$i) {
			...
			if($profileCount > 1) {
				$sql = 'DELETE FROM ottocrat_role2profile WHERE roleid=? AND profileid=?';
				$params = array($roleId, $profileId);
			} else {
				$sql = 'UPDATE ottocrat_role2profile SET profileid=? WHERE roleid=? AND profileid=?';
				$params = array($transferProfileId, $roleId, $profileId);
			}
			$db->pquery($sql, $params);
		}

		$db->pquery('DELETE FROM ottocrat_profile WHERE profileid=?', array($profileId));
	}
	...
	public function save() {
		...
		$calendarModule = Ottocrat_Module_Model::getInstance('Calendar');
		$eventModule = Ottocrat_Module_Model::getInstance('Events');
		...
		$profileId = $this->getId();
		if(!$profileId) {
			$profileId = $db->getUniqueId('ottocrat_profile');
			$this->setId($profileId);
			$sql = 'INSERT INTO ottocrat_profile(profileid, profilename, description, directly_related_to_role) VALUES (?,?,?,?)';
			$params = array($profileId, $profileName, $description, $isProfileDirectlyRelatedToRole);
			$isNewProfile = true;
		} else {
			$sql = 'UPDATE ottocrat_profile SET profilename=?, description=?, directly_related_to_role=? WHERE profileid=?';
		...
		$sql = 'INSERT INTO ottocrat_profile2globalpermissions(profileid, globalactionid, globalactionpermission) VALUES (?,?,?)';
		...
		$allModuleModules = Ottocrat_Module_Model::getAll(array(0), Settings_Profiles_Module_Model::getNonVisibleModulesList());
		$allModuleModules[$eventModule->getId()] = $eventModule;
		if(count($allModuleModules) > 0) {
			$actionModels = Ottocrat_Action_Model::getAll(true);
	...
	protected function saveModulePermissions($moduleModel, $permissions) {
		...
		$db->pquery('DELETE FROM ottocrat_profile2tab WHERE profileid=? AND tabid=?', array($profileId, $tabId));
		...
		if($moduleModel->isEntityModule() || $moduleModel->isUtilityActionEnabled()) {
			if($actionPermissions || $moduleModel->isUtilityActionEnabled()) {
				$actionIdsList = Ottocrat_Action_Model::$standardActions;
				...
				foreach($actionPermissions as $actionId => $permission) {
					if(isset(Ottocrat_Action_Model::$standardActions[$actionId])) {
				...
				if ($profileActionPermissions) {
					...
					$actionsUpdateQuery = 'UPDATE ottocrat_profile2standardpermissions SET permissions = CASE ';
					...
					foreach (Ottocrat_Action_Model::$utilityActions as $utilityActionId => $utilityActionName) {
						if(!isset($utilityIdsList[$utilityActionId])) {
							$utilityIdsList[$utilityActionId] = 'off';
						}
					}
					//Utility permissions
					$utilityUpdateQuery = 'UPDATE ottocrat_profile2utility SET permission = CASE ';
					...
					$actionsInsertQuery .= 'INSERT INTO ottocrat_profile2standardpermissions(profileid, tabid, operation, permissions) VALUES ';
					foreach ($actionsIdsList as $actionId => $permission) {
						$actionEnabled = true;
						$permissionValue = $this->tranformInputPermissionValue($permission);
					...
					$utilityInsertQuery .= 'INSERT INTO ottocrat_profile2utility(profileid, tabid, activityid, permission) VALUES ';
					foreach($utilityIdsList as $actionId => $permission) {
						$permissionValue = $this->tranformInputPermissionValue($permission);
		...
		$sql = 'INSERT INTO ottocrat_profile2tab(profileid, tabid, permissions) VALUES (?,?,?)';
		...
		if(is_array($fieldPermissions)) {
			foreach($fieldPermissions as $fieldId => $stateValue) {
				$db->pquery('DELETE FROM ottocrat_profile2field WHERE profileid=? AND tabid=? AND fieldid=?',
				...
				$sql = 'INSERT INTO ottocrat_profile2field(profileid, tabid, fieldid, visible, readonly) VALUES (?,?,?,?,?)';
	 ...
	 * @return <Array> - Associate array of Ottocrat_Link_Model instances
	 */
	public function getRecordLinks() {
		...
		$recordLinks = array(
			...
			array(
				...
				'linkurl' => "javascript:Settings_Ottocrat_List_Js.triggerDelete(event,'".$this->getDeleteActionUrl()."')",
				'linkicon' => 'icon-trash'
			)
		);
	...
	public static function getAllByRole($roleId) {
		...
		$sql = 'SELECT ottocrat_profile.*
					FROM ottocrat_profile
					INNER JOIN
						ottocrat_role2profile ON ottocrat_profile.profileid = ottocrat_role2profile.profileid
						AND
						ottocrat_role2profile.roleid = ?';
		$params = array($roleId);
		$result = $db->pquery($sql, $params);
	...
	public static function getAll() {
		...
		$sql = 'SELECT * FROM ottocrat_profile';
		$params = array();
		$result = $db->pquery($sql, $params);
		$noOfProfiles = $db->num_rows($result);
		$profiles = array();
	...
	public static function getInstanceById($profileId) {
		...
		$sql = 'SELECT * FROM ottocrat_profile WHERE profileid = ?';
    ...
    public static function getInstanceByName($profileName , $checkOnlyDirectlyRelated=false, $excludedRecordId = array()) {
        $db = PearDatabase::getInstance();
        $query = 'SELECT * FROM ottocrat_profile WHERE profilename=?';
	...
	public function getUsersList($allUsers = false) {
		$db = PearDatabase::getInstance();
		$params = array(0);
		$query = 'SELECT id FROM ottocrat_users
					INNER JOIN ottocrat_user2role ON ottocrat_user2role.userid = ottocrat_users.id
					INNER JOIN ottocrat_role2profile ON ottocrat_role2profile.roleid = ottocrat_user2role.roleid
					WHERE ottocrat_users.deleted = ?';

		if (!$allUsers) {
			$query .= ' AND ottocrat_role2profile.profileid = ?';
			$params[] = $this->getId();
		}
		$result = $db->pquery($query, $params);
	...
	/**
	 * Function to save user fields in ottocrat_profile2field table
	...
	public function saveUserAccessbleFieldsIntoProfile2Field(){
		...
		if(!empty($profileId)){
			...
			//Added user fields into ottocrat_profile2field and ottocrat_def_org_field
			//We are using this field information in Email Templates.
			foreach ($userAccessbleFields as $fieldId => $fieldName) {
				$insertQuery = 'INSERT INTO ottocrat_profile2field VALUES(?,?,?,?,?)';
			...
			foreach ($userAccessbleFields as $fieldId => $fieldName) {
				if(!in_array($fieldId, $def_org_fields)){
					$insertQuery = 'INSERT INTO ottocrat_def_org_field VALUES(?,?,?,?)';
