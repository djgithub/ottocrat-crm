 ...
 */
class Settings_Roles_Record_Model extends Settings_Ottocrat_Record_Model {
	...
	public function getChildren() {
		...
		if(!$this->children) {
			...
			$sql = 'SELECT * FROM ottocrat_role WHERE parentrole LIKE ? AND depth = ?';
	...
	public function getAllChildren() {
		...
		$sql = 'SELECT * FROM ottocrat_role WHERE parentrole LIKE ?';
    ...
    public function getProfileIdList(){
        ...
        $db = PearDatabase::getInstance();
        $query = 'SELECT profileid FROM ottocrat_role2profile WHERE roleid=?';
    ...
    public function getDirectlyRelatedProfileId() {
        ...
        $query = 'SELECT directly_related_to_role, ottocrat_profile.profileid FROM ottocrat_role2profile 
                  INNER JOIN ottocrat_profile ON ottocrat_profile.profileid = ottocrat_role2profile.profileid 
                  WHERE ottocrat_role2profile.roleid=?';
	...
	public function getEditViewUrl() {
		return Ottocrat_Request:: encryptLink('index.php?module=Roles&parent=Settings&view=Edit&record='.$this->getId
			());
	}
...
//	}
	...
	public function getCreateChildUrl() {
		return Ottocrat_Request:: encryptLink('?module=Roles&parent=Settings&view=Edit&parent_roleid='.$this->getId());
	}
	...
	public function getDeleteActionUrl() {
		return Ottocrat_Request:: encryptLink('?module=Roles&parent=Settings&view=DeleteAjax&record='.$this->getId());
	}
	...
	public function getPopupWindowUrl() {
		return Ottocrat_Request:: encryptLink('module=Roles&parent=Settings&view=Popup&src_record='.$this->getId());
	}
	...
	public function save() {
		...
		if(empty($roleId)) {
			$mode = '';
			$roleIdNumber = $db->getUniqueId('ottocrat_role');
		...
		if($mode == 'edit') {
			$sql = 'UPDATE ottocrat_role SET rolename=?, parentrole=?, depth=?, allowassignedrecordsto=? WHERE roleid=?';
			$params = array($this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'), $roleId);
			$db->pquery($sql, $params);
		} else {
			$sql = 'INSERT INTO ottocrat_role(roleid, rolename, parentrole, depth, allowassignedrecordsto) VALUES (?,?,?,?,?)';
			$params = array($roleId, $this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'));
			$db->pquery($sql, $params);
			$picklist2RoleSQL = "INSERT INTO ottocrat_role2picklist SELECT '".$roleId."',picklistvalueid,picklistid,sortid
					FROM ottocrat_role2picklist WHERE roleid = ?";
		...
		if(!empty($profileIds)) {
			...
			if($noOfProfiles > 0) {
				$db->pquery('DELETE FROM ottocrat_role2profile WHERE roleid=?', array($roleId));

				$sql = 'INSERT INTO ottocrat_role2profile(roleid, profileid) VALUES (?,?)';
	...
	public function delete($transferToRole) {
		...
		$db->pquery('UPDATE ottocrat_user2role SET roleid=? WHERE roleid=?', array($transferRoleId, $roleId));

		$db->pquery('DELETE FROM ottocrat_role2profile WHERE roleid=?', array($roleId));
		$db->pquery('DELETE FROM ottocrat_group2role WHERE roleid=?', array($roleId));
		$db->pquery('DELETE FROM ottocrat_group2rs WHERE roleandsubid=?', array($roleId));

                $user_result=$db->pquery('select userid from ottocrat_user2role WHERE roleid=?',array($roleId));
		...
		$db->pquery('DELETE FROM ottocrat_role WHERE roleid=?', array($roleId));
	...
	public static function getInstanceById($roleId) {
		...
		$sql = 'SELECT * FROM ottocrat_role WHERE roleid = ?';
		$params = array($roleId);
		$result = $db->pquery($sql, $params);
	...
	public static function getBaseRole() {
		...
		$sql = 'SELECT * FROM ottocrat_role WHERE depth=0 LIMIT 1';
		$params = array();
		$result = $db->pquery($sql, $params);
		if($db->num_rows($result) > 0) {
			return self::getInstanceFromQResult($result, 0);
   ...
   public static function getInstanceByName($name, $excludedRecordId = array()) {
       $db = PearDatabase::getInstance();
       $sql = 'SELECT * FROM ottocrat_role WHERE rolename=?';
       $params = array($name);
   ...
   public function getUsers() {
	   $db = PearDatabase::getInstance();
	   $result = $db->pquery('SELECT userid FROM ottocrat_user2role WHERE roleid = ?', array($this->getId()));
