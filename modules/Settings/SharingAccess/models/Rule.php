...
/**
 * Sharng Access Ottocrat Module Model Class
 */
class Settings_SharingAccess_Rule_Model extends Ottocrat_Base_Model {
	...
	static $dataShareTableColArr = array (
		...
		self::RULE_TYPE_ROLE => array (
			self::RULE_TYPE_GROUPS => array (
				'table' => 'ottocrat_datashare_role2group',
				'source_id' => 'share_roleid',
				'target_id' => 'to_groupid'
			),
			self::RULE_TYPE_ROLE => array (
				'table' => 'ottocrat_datashare_role2role',
				'source_id' => 'share_roleid',
				'target_id' => 'to_roleid'
			),
			self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
				'table' => 'ottocrat_datashare_role2rs',
				'source_id' => 'share_roleid',
				'target_id' => 'to_roleandsubid'
			),
		),
		self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
			self::RULE_TYPE_GROUPS => array (
				'table' => 'ottocrat_datashare_rs2grp',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_groupid'
			),
			self::RULE_TYPE_ROLE => array (
				'table' => 'ottocrat_datashare_rs2role',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_roleid'
			),
			self::RULE_TYPE_ROLE_AND_SUBORDINATES => array (
				'table' => 'ottocrat_datashare_rs2rs',
				'source_id' => 'share_roleandsubid',
				'target_id' => 'to_roleandsubid'
			),
		),
	...
	public function getEditViewUrl() {
		return Ottocrat_Request:: encryptLink('?module=SharingAccess&parent=Settings&view=IndexAjax&mode=editRule&for_module='.$this->getModule()->getId().'&record='.$this->getId());
	}

	public function getDeleteActionUrl() {
		return Ottocrat_Request:: encryptLink('?module=SharingAccess&parent=Settings&action=IndexAjax&mode=deleteRule&for_module='.$this->getModule()->getId().'&record='.$this->getId());
	}
	...
	public function getSourceDetailViewUrl() {
		...
		if($sourceMemberDetails[0] == 'Groups') {
			return Ottocrat_Request:: encryptLink('index.php?parent=Settings&module=Groups&view=Detail&record='.$sourceMemberDetails[1]);
		} else if($sourceMemberDetails[0] == 'Roles') {
			return Ottocrat_Request:: encryptLink('index.php?parent=Settings&module=Roles&view=Edit&record='.$sourceMemberDetails[1]);
	...
	public function getTargetDetailViewUrl() {
		...
		if($targetMemberDetails[0] == 'Groups'){
			return Ottocrat_Request:: encryptLink('index.php?parent=Settings&module=Groups&view=Detail&record='.$targetMemberDetails[1]);
		} else if($targetMemberDetails[0] == 'Roles') {
			return Ottocrat_Request:: encryptLink('index.php?parent=Settings&module=Roles&view=Edit&record='.$targetMemberDetails[1]);
	...
	public function save() {
		...
		if(!$ruleId) {
			$ruleId = $db->getUniqueId('ottocrat_datashare_module_rel');
			$this->set('shareid', $ruleId);

			$db->pquery("INSERT INTO ottocrat_datashare_module_rel(shareid, tabid) VALUES(?,?)",
		...
		$sql = 'UPDATE ottocrat_datashare_module_rel SET relationtype=? WHERE shareid=?';
	...
	public function delete() {
		...
		$db->pquery('DELETE FROM ottocrat_datashare_module_rel WHERE shareid=?', array($ruleId));
        Settings_SharingAccess_Module_Model::recalculateSharingRules();
	}
	...
	public static function getInstance($moduleModel, $ruleId) {
		...
		$sql = 'SELECT * FROM ottocrat_datashare_module_rel WHERE tabid = ? AND shareid = ?';
	...
	public static function getAllByModule($moduleModel) {
		...
		$sql = 'SELECT * FROM ottocrat_datashare_module_rel WHERE tabid = ?';
