 ...
 */
class Settings_SharingAccess_Module_Model extends Ottocrat_Module_Model {
	...
	public function save() {
		...
		$sql = 'UPDATE ottocrat_def_org_share SET permission = ? WHERE tabid = ?';
	...
	public static function getInstance($value) {
		...
		if(Ottocrat_Utils::isNumber($value)) {
			$query = 'SELECT * FROM ottocrat_def_org_share INNER JOIN ottocrat_tab ON ottocrat_tab.tabid = ottocrat_def_org_share.tabid WHERE ottocrat_tab.tabid=?';
		} else {
			$query = 'SELECT * FROM ottocrat_def_org_share INNER JOIN ottocrat_tab ON ottocrat_tab.tabid = ottocrat_def_org_share.tabid WHERE name=?';
		}
	...
	public static function getAll($editable=false) {
		...
		$query = 'SELECT * FROM ottocrat_def_org_share INNER JOIN ottocrat_tab ON ottocrat_tab.tabid = ottocrat_def_org_share.tabid WHERE ottocrat_tab.presence IN (0,2)';
		$params = array();
		if($editable) {
	...
	public static function recalculateSharingRules() {
		...
		$result = $db->pquery('SELECT id FROM ottocrat_users WHERE deleted = ?', array(0));
