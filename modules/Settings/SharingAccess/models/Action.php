 ...
 */
class Settings_SharingAccess_Action_Model extends Ottocrat_Base_Model {
	...
	public function isModuleEnabled($module) {
		...
		$sql = 'SELECT 1 FROM ottocrat_org_share_action2tab WHERE tabid = ? AND share_action_id = ?';
	...
	public static function getInstance($value) {
		...
		if(Ottocrat_Utils::isNumber($value)) {
			$sql = 'SELECT * FROM ottocrat_org_share_action_mapping WHERE share_action_id = ?';
		} else {
			$sql = 'SELECT * FROM ottocrat_org_share_action_mapping WHERE share_action_name = ?';
		}
		$params = array($value);
		$result = $db->pquery($sql, $params);
	...
	public static function getAll($configurable=true) {
		...
		$sql = 'SELECT * FROM ottocrat_org_share_action_mapping';
		$params = array();
		if($configurable) {
