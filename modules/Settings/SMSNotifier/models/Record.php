...
class Settings_SMSNotifier_Record_Model extends Settings_Ottocrat_Record_Model {
	...
	public function getEditViewUrl() {
		...
		return Ottocrat_Request:: encryptLink('index.php?module='.$moduleModel->getName().'&parent='.$moduleModel->getParentName().'&view=Edit&record='.$this->getId());
	}
	...
	public function getDeleteUrl() {
		$moduleModel = $this->getModule();
		return Ottocrat_Request:: encryptLink('index.php?module='.$moduleModel->getName().'&parent='.$moduleModel->getParentName().'&action=Delete&record='.$this->getId());
	}
	...
	public function save() {
		...
		if ($id) {
			$query = 'UPDATE ottocrat_smsnotifier_servers SET providertype = ?, isactive = ?, username = ?, password = ?, parameters = ? WHERE id = ?';
			array_push($params, $id);
		} else {
			$query = 'INSERT INTO ottocrat_smsnotifier_servers(providertype, isactive, username, password, parameters) VALUES(?, ?, ?, ?, ?)';
		}
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ottocrat_smsnotifier_servers WHERE id = ?', array($recordId));
