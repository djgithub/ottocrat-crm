...
class Settings_Leads_Mapping_Model extends Settings_Ottocrat_Module_Model {
	...
	public function getDetailViewUrl() {
		return Ottocrat_Request:: encryptLink('index.php?parent='. $this->getParentName() .'&module='. $this->getName().'&view=MappingDetail');
	}
	...
	public function getEditViewUrl() {
		return Ottocrat_Request:: encryptLink('index.php?parent='. $this->getParentName() .'&module='. $this->getName().'&view=MappingEdit');
	}
	...
	public function getMappingDeleteUrl() {
		return Ottocrat_Request:: encryptLink('index.php?parent='. $this->getParentName() .'&module='. $this->getName() .'&action=MappingDelete');
	}
	 ...
	 * @return <Array> list of detail view link models <Ottocrat_Link_Model>
	 */
	public function getDetailViewLinks() {
		return array(Ottocrat_Link_Model::getInstanceFromValues(array(
				'linktype' => 'DETAILVIEW',
				'linklabel' => 'LBL_EDIT',
	 ...
	 * @return <Array> list of mapping link models <Ottocrat_Link_Model>
	 */
	...
	public function getMapping($editable = false) {
		if (!$this->mapping) {
			...
			$query = 'SELECT * FROM ottocrat_convertleadmapping';
			if ($editable) {
	...
	public function getFieldsInfo($fieldIdsList) {
		$leadModel = Ottocrat_Module_Model::getInstance($this->getName());
		$leadId = $leadModel->getId();

		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT fieldid, fieldlabel, uitype, typeofdata, fieldname, tablename, tabid FROM ottocrat_field WHERE fieldid IN ('. generateQuestionMarks($fieldIdsList). ')', $fieldIdsList);
		$numOfRows = $db->num_rows($result);
	...
	public function save($mapping) {
		...
		if ($deleteMappingsList) {
			$db->pquery('DELETE FROM ottocrat_convertleadmapping WHERE editable = 1 AND cfmid IN ('. generateQuestionMarks($deleteMappingsList) .')', $deleteMappingsList);
		}

		if ($createMappingsList) {
			$insertQuery = 'INSERT INTO ottocrat_convertleadmapping(leadfid, accountfid, contactfid, potentialfid) VALUES ';
		...
		if ($updateMappingsList) {
			...
			$db->pquery("UPDATE ottocrat_convertleadmapping $leadQuery, $accountQuery, $contactQuery, $potentialQuery WHERE editable = ?", array(1));
		}
	}
	...
	public static function getRestrictedFieldIdsList() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ottocrat_convertleadmapping WHERE editable = ?', array(0));
		$numOfRows = $db->num_rows($result);
	...
	public static function deleteMapping($mappingIdsList) {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ottocrat_convertleadmapping WHERE cfmid IN ('. generateQuestionMarks($mappingIdsList). ')', $mappingIdsList);
	}
}