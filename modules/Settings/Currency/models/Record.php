...
class Settings_Currency_Record_Model extends Settings_Ottocrat_Record_Model{
    ...
    public function isDisabledRestricted() {
        ...
        $query = 'SELECT 1 FROM ottocrat_users WHERE currency_id = ?';
    ...
    public static function getAllNonMapped($includedIds = array()) {
        ...
        $query = 'SELECT ottocrat_currencies.* FROM ottocrat_currencies 
                    LEFT JOIN ottocrat_currency_info ON ottocrat_currency_info.currency_name = ottocrat_currencies.currency_name
                    WHERE ottocrat_currency_info.currency_name IS NULL or ottocrat_currency_info.deleted=1';
        $params = array();
        if(!empty($includedIds)) {
            $params = $includedIds;
            $query .= ' or ottocrat_currency_info.id IN('.generateQuestionMarks($includedIds).')';
        }
        $result = $db->pquery($query,$params);
