...
class PickListHandler extends VTEventHandler {
	...
	function handleEvent($eventName, $entityData) {
		...
		if($eventName == 'ottocrat.picklist.afterrename') {
			$this->operationsAfterPicklistRename($entityData);
		} elseif($eventName == 'ottocrat.picklist.afterdelete') {
	...
	function operationsAfterPicklistRename($entityData) {
		...
		$query = "SELECT id,targetvalues FROM ottocrat_picklist_dependency where targetfield=? and tabid=?";
		...
		for($i = 0; $i < $num_rows; $i++) {
			...
			$query = 'UPDATE ottocrat_picklist_dependency SET targetvalues=? where id=? AND tabid=?';
		...
		$query= 'SELECT cvid,value,columnindex,groupid FROM ottocrat_cvadvfilter where columnname=?';
		...
		for ($i = 0; $i < $num_rows; $i++) {
			...
			$query = 'UPDATE ottocrat_cvadvfilter SET value=? where columnname=? and cvid=? and columnindex=? and groupid=?';
			$db->pquery($query, array($value, $advFiltercolumnName, $row['cvid'], $row['columnindex'], $row['groupid']));
		}
		
		//update reportsFilter values
		$query= 'SELECT queryid,value,columnindex,groupid FROM ottocrat_relcriteria where columnname=?';
		...
		for ($i = 0; $i < $num_rows; $i++) {
			...
			$value = implode(',', $explodedValueArray);
			$query = 'UPDATE ottocrat_relcriteria SET value=? where columnname=? and queryid=? and columnindex=? and groupid=?';
			$db->pquery($query, array($value, $reportFilterColumnName, $row['queryid'], $row['columnindex'], $row['groupid']));
		}
		
		//update Workflows values
		$query= 'SELECT workflow_id,test FROM com_ottocrat_workflows where module_name=? AND test != "" AND test IS NOT NULL AND test !="null" AND test LIKE "%'.$oldValue.'%"';
		...
		for($i = 0;$i < $num_rows; $i++) {
			...
			if(!empty($decodedArrayConditions)){
				...
				$query= 'UPDATE com_ottocrat_workflows SET test=? where workflow_id=?';
				$db->pquery($query, array($condtion, $row['workflow_id']));
			}
		}
		
		//update workflow task
		$query = 'SELECT task,task_id,workflow_id FROM com_ottocrat_workflowtasks where task LIKE "%'.$oldValue.'%"';
	...
	function operationsAfterPicklistDelete($entityData) {
		...
		foreach ($valueToDelete as $value) {
			//update Workflows values
			$query = 'SELECT workflow_id,test FROM com_ottocrat_workflows where module_name=? AND test != "" AND test IS NOT NULL AND test !="null" AND test LIKE "%' . $value . '%"';
			$result = $db->pquery($query, array($moduleName));
			$num_rows = $db->num_rows($result);
			for ($i = 0; $i < $num_rows; $i++) {
				$row = $db->query_result_rowdata($result, $i);
		...
		foreach ($valueToDelete as $value) {
			//update workflow task
			$query = 'SELECT task,task_id,workflow_id FROM com_ottocrat_workflowtasks where task LIKE "%' . $value . '%"';
			$result = $db->pquery($query, array());
			$num_rows = $db->num_rows($result);
