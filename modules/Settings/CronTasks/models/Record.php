...
class Settings_CronTasks_Record_Model extends Settings_Ottocrat_Record_Model {
    ...
    function getLastEndDateTime() {
        if($this->get('lastend') != NULL) {
		    $lastScannedTime = Ottocrat_Datetime_UIType::getDisplayDateTimeValue(date('Y-m-d H:i:s', $this->get('lastend')));
	...
	public function save() {
		...
		$updateQuery = "UPDATE ottocrat_cron_task SET frequency = ?, status = ? WHERE id = ?";
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		...
		$result = $db->pquery("SELECT * FROM ottocrat_cron_task WHERE id = ?", array($recordId));
    ...
    public static function getInstanceByName($name) {
		...
		$result = $db->pquery("SELECT * FROM ottocrat_cron_task WHERE name = ?", array($name));
		if ($db->num_rows($result)) {
