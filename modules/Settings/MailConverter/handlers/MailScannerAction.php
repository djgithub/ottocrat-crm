 ...
 * Mail Scanner Action
 */
class Ottocrat_MailScannerAction {
	...
	function initialize($foractionid) {
		global $adb;
		$result = $adb->pquery("SELECT * FROM ottocrat_mailscanner_actions WHERE actionid=? ORDER BY sequence", Array($foractionid));
	...
	function update($ruleid, $actiontext) {
		...
		if($this->actionid) {
			$adb->pquery("UPDATE ottocrat_mailscanner_actions SET scannerid=?, actiontype=?, module=?, lookup=? WHERE actionid=?",
				Array($this->scannerid, $this->actiontype, $this->module, $this->lookup, $this->actionid));
		} else {
			$this->sequence = $this->__nextsequence();
			$adb->pquery("INSERT INTO ottocrat_mailscanner_actions(scannerid, actiontype, module, lookup, sequence) VALUES(?,?,?,?,?)",
				Array($this->scannerid, $this->actiontype, $this->module, $this->lookup, $this->sequence));
			$this->actionid = $adb->database->Insert_ID();
		}
		$checkmapping = $adb->pquery("SELECT COUNT(*) AS ruleaction_count FROM ottocrat_mailscanner_ruleactions
		...
		if($adb->num_rows($checkmapping) && !$adb->query_result($checkmapping, 0, 'ruleaction_count')) {
			$adb->pquery("INSERT INTO ottocrat_mailscanner_ruleactions(ruleid, actionid) VALUES(?,?)",
	...
	function delete() {
		global $adb;
		if($this->actionid) {
			$adb->pquery("DELETE FROM ottocrat_mailscanner_actions WHERE actionid=?", Array($this->actionid));
			$adb->pquery("DELETE FROM ottocrat_mailscanner_ruleactions WHERE actionid=?", Array($this->actionid));
		}
	}
	...
	function __nextsequence() {
		global $adb;
		$seqres = $adb->pquery("SELECT max(sequence) AS max_sequence FROM ottocrat_mailscanner_actions", Array());
		$maxsequence = 0;
	...
	function __UpdateTicket($mailscanner, $mailrecord, $regexMatchInfo,$mailscannerrule) {
		...
		if($this->lookup == 'SUBJECT') {
			...
			if($linkfocus) {
				...
				$adb->pquery("UPDATE ottocrat_troubletickets set status=? WHERE ticketid=? AND status='Closed'", Array('Open', $linkfocus->id));
	...
	function __SaveAttachements($mailrecord, $basemodule, $basefocus) {
		...
		foreach($mailrecord->_attachments as $filename=>$filecontent) {
			$attachid = $adb->getUniqueId('ottocrat_crmentity');
			$description = $filename;
			$usetime = $adb->formatDate($date_var, true);

			$adb->pquery("INSERT INTO ottocrat_crmentity(crmid, smcreatorid, smownerid,
			...
			if($issaved) {
				...
				$adb->pquery("INSERT INTO ottocrat_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)",
					Array($document->id, $attachid));

				// Link document to base record
				$adb->pquery("INSERT INTO ottocrat_senotesrel(crmid, notesid) VALUES(?,?)",
	...
	function __SaveAttachmentFile($attachid, $filename, $filecontent) {
		...
		$adb->pquery("INSERT INTO ottocrat_attachments SET attachmentsid=?, name=?, description=?, type=?, path=?",
    ...
    function setDefaultValue($module, $moduleObj) { 
        $moduleInstance = Ottocrat_Module_Model::getInstance($module);
        
        $fieldInstances = Ottocrat_Field_Model::getAllForModule($moduleInstance);
        foreach($fieldInstances as $blockInstance) {
            foreach($blockInstance as $fieldInstance) {
                ...
                if($fieldInstance->isMandatory() && !$defaultValue) {
                    $moduleObj->column_fields[$fieldName] = Ottocrat_Util_Helper::getDefaultMandatoryValue($fieldInstance->getFieldDataType());
                }
            }
        }
    }
