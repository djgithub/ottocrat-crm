 ...
 * Scanner Rule
 */
class Ottocrat_MailScannerRule {
    // id of this instance
	var $ruleid    = false;
    ...
    function initialize($forruleid) {
        global $adb;
        $result = $adb->pquery("SELECT * FROM ottocrat_mailscanner_rules WHERE ruleid=? ORDER BY sequence", Array($forruleid));
    ...
    function initializeActions() {
        global $adb;
            if($this->ruleid) {
            $this->actions = Array();
                $actionres = $adb->pquery("SELECT actionid FROM ottocrat_mailscanner_ruleactions WHERE ruleid=?",Array($this->ruleid));
            $actioncount = $adb->num_rows($actionres);
                if($actioncount) {
                    for($index = 0; $index < $actioncount; ++$index) {
                $actionid = $adb->query_result($actionres, $index, 'actionid');
                $ruleaction = new Ottocrat_MailScannerAction($actionid);
    ...
    static function resetSequence($ruleid1, $ruleid2) {
        global $adb;
            $ruleresult = $adb->pquery("SELECT ruleid, sequence FROM ottocrat_mailscanner_rules WHERE ruleid = ? or ruleid = ?",
        ...
        $rule_partinfo = Array();
            ...
            $adb->pquery("UPDATE ottocrat_mailscanner_rules SET sequence = ? WHERE ruleid = ?", Array($rule_partinfo[$ruleid2], $ruleid1));
            $adb->pquery("UPDATE ottocrat_mailscanner_rules SET sequence = ? WHERE ruleid = ?", Array($rule_partinfo[$ruleid1], $ruleid2));
        }
    }
    ...
    function update() {
        global $adb;
        if ($this->ruleid) {
            $adb->pquery("UPDATE ottocrat_mailscanner_rules SET scannerid=?,fromaddress=?,toaddress=?,subjectop=?,subject=?,bodyop=?,body=?,matchusing=?,assigned_to=?,cc=?,bcc=?
            ...
            $adb->pquery("INSERT INTO ottocrat_mailscanner_rules(scannerid,fromaddress,toaddress,subjectop,subject,bodyop,body,matchusing,sequence,assigned_to,cc,bcc)
    ...
    function __nextsequence() {
        global $adb;
        $seqres = $adb->pquery("SELECT max(sequence) AS max_sequence FROM ottocrat_mailscanner_rules", Array());
        $maxsequence = 0;
    ...
    function delete() {
        ...
        // Delete dependencies
            ...
            if($this->ruleid) {
            $adb->pquery("DELETE FROM ottocrat_mailscanner_ruleactions WHERE ruleid = ?", Array($this->ruleid));
            $adb->pquery("DELETE FROM ottocrat_mailscanner_rules WHERE ruleid=?", Array($this->ruleid));
        }
    }
    ...
    function updateAction($actionid, $actiontext) {
        $action = $this->useaction;
            ...
            if($actionid != '' && $actiontext == '') {
                ...
                if($actionid == '') {
            $action = new Ottocrat_MailScannerAction($actionid);
            }
