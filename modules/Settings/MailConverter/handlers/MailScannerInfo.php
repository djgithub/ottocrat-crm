 ...
 */
class Ottocrat_MailScannerInfo {
	...
	function initialize($scannername) {
		global $adb;
		$result = $adb->pquery("SELECT * FROM ottocrat_mailscanner WHERE scannername=?", Array($scannername));
	...
	function initializeFolderInfo() {
		...
		if($this->scannerid) {
			$this->lastscan = Array();
			$this->rescan   = Array();
			$lastscanres = $adb->pquery("SELECT * FROM ottocrat_mailscanner_folders WHERE scannerid=?",Array($this->scannerid));
	...
	function clearLastscan() {
		global $adb;
		$adb->pquery("DELETE FROM ottocrat_mailscanner_folders WHERE scannerid=?", Array($this->scannerid));
		$this->lastscan = false;
	}
	...
	function updateAllFolderRescan($rescanFlag=false) {
		...
		$adb->pquery("UPDATE ottocrat_mailscanner_folders set rescan=? WHERE scannerid=?",
	...
	function dateBasedOnMailServerTimezone($format='d-M-Y') {
		$returnDate = NULL;
		##--Fix for trac : http://trac.ottocrat.com/cgi-bin/trac.cgi/ticket/8051-##
	...
	function updateLastscan($folderName, $rescanFolder=false, $enabledForScan=1) {
		...
		$folderInfo = $adb->pquery("SELECT folderid FROM ottocrat_mailscanner_folders WHERE scannerid=? AND foldername=?",
			Array($this->scannerid, $folderName));
		if($adb->num_rows($folderInfo)) {
			$folderid = $adb->query_result($folderInfo, 0, 'folderid');
			$adb->pquery("UPDATE ottocrat_mailscanner_folders SET lastscan=?, rescan=? WHERE folderid=?",
				Array($scannedOn, $needRescan, $folderid));
		} else {
			$adb->pquery("INSERT INTO ottocrat_mailscanner_folders(scannerid, foldername, lastscan, rescan, enabled)
	...
	function getFolderInfo() {
		...
		if($this->scannerid) {
			global $adb;
			$fldres = $adb->pquery("SELECT * FROM ottocrat_mailscanner_folders WHERE scannerid=?", Array($this->scannerid));
	...
	function updateFolderInfo($foldernames, $rescanFolder=false) {
		if($this->scannerid && !empty($foldernames)) {
			...
			$adb->pquery("DELETE FROM ottocrat_mailscanner_folders WHERE scannerid=? AND foldername NOT IN
	...
	function enableFoldersForScan($folderinfo) {
		if($this->scannerid) {
			global $adb;
			$adb->pquery("UPDATE ottocrat_mailscanner_folders set enabled=0 WHERE scannerid=?", Array($this->scannerid));
			foreach($folderinfo as $foldername=>$foldervalue) {
				$folderid = $foldervalue[folderid];
				$enabled  = $foldervalue[enabled];
				$adb->pquery("UPDATE ottocrat_mailscanner_folders set enabled=? WHERE folderid=? AND scannerid=?",
	...
	function initializeRules() {
		...
		if($this->scannerid) {
			$this->rules = Array();
			$rulesres = $adb->pquery("SELECT * FROM ottocrat_mailscanner_rules WHERE scannerid=? ORDER BY sequence",Array($this->scannerid));
			$rulescount = $adb->num_rows($rulesres);
			if($rulescount) {
				for($index = 0; $index < $rulescount; ++$index) {
					$ruleid = $adb->query_result($rulesres, $index, 'ruleid');
					$scannerrule = new Ottocrat_MailScannerRule($ruleid);
	...
	function update($otherInstance) {
		...
		if($this->scannerid == false) {
            $adb->pquery("INSERT INTO ottocrat_mailscanner(scannername,server,protocol,username,password,ssltype,
			...
			$adb->pquery("UPDATE ottocrat_mailscanner SET scannername=?,server=?,protocol=?,username=?,password=?,ssltype=?,
	...
	function delete() {
		...
		if($this->scannerid) {
			$tables = Array(
				'ottocrat_mailscanner',
				'ottocrat_mailscanner_ids',
				'ottocrat_mailscanner_folders'
			);
			foreach($tables as $table) {
				$adb->pquery("DELETE FROM $table WHERE scannerid=?", Array($this->scannerid));
			}
			$adb->pquery("DELETE FROM ottocrat_mailscanner_ruleactions
				WHERE actionid in (SELECT actionid FROM ottocrat_mailscanner_actions WHERE scannerid=?)", Array($this->scannerid));
			$adb->pquery("DELETE FROM ottocrat_mailscanner_actions WHERE scannerid=?", Array($this->scannerid));
		}
	}
	...
	static function listAll() {
		...
		global $adb;
		$result = $adb->pquery("SELECT scannername FROM ottocrat_mailscanner", array());
