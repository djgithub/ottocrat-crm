...
class Ottocrat_MailScanner {
	...
	function getMailBox() {
		if(!$this->_mailbox) {
			$this->_mailbox = new Ottocrat_MailBox($this->_scannerinfo);
	...
	function markMessageScanned($mailrecord, $crmid=false) {
		...
		$adb->pquery("INSERT INTO ottocrat_mailscanner_ids(scannerid, messageid, crmid) VALUES(?,?,?)",
	...
	function isMessageScanned($mailrecord, $lookAtFolder) {
		global $adb;
		$messages = $adb->pquery("SELECT 1 FROM ottocrat_mailscanner_ids WHERE scannerid=? AND messageid=?",
		...
		if($adb->num_rows($messages)) {
			...
			if($folderRescan && empty($relatedCRMId)) {
				$adb->pquery("DELETE FROM ottocrat_mailscanner_ids WHERE scannerid=? AND messageid=?",
					Array($this->_scannerinfo->scannerid, $mailrecord->_uniqueid));
				$isScanned = false;
			}
		}
	...
	function LookupContact($email) {
		...
		$contactid = false;
		$contactres = $adb->pquery("SELECT contactid FROM ottocrat_contactdetails INNER JOIN ottocrat_crmentity ON crmid = contactid WHERE setype = ? AND email = ? AND deleted = ?", array('Contacts', $email, 0));
	...
	function LookupLead($email) {
		...
		$leadid = false;
		$leadres = $adb->pquery("SELECT leadid FROM ottocrat_leaddetails INNER JOIN ottocrat_crmentity ON crmid = leadid WHERE setype=? AND email = ? AND converted = ? AND deleted = ?", array('Leads', $email, 0, 0));
	...
	function LookupAccount($email) {
		...
		$accountid = false;
		$accountres = $adb->pquery("SELECT accountid FROM ottocrat_account INNER JOIN ottocrat_crmentity ON crmid = accountid WHERE setype=? AND (email1 = ? OR email2 = ?) AND deleted = ?", Array('Accounts', $email, $email, 0));
	...
	function LookupTicket($subjectOrId) {
		...
		if(!$checkTicketId) {
			$ticketres = $adb->pquery("SELECT ticketid FROM ottocrat_troubletickets WHERE title = ? OR ticket_no = ?", Array($subjectOrId, $subjectOrId));
		...
		if(!$checkTicketId) {
			$ticketres = $adb->pquery("SELECT ticketid FROM ottocrat_troubletickets WHERE ticket_no = ?", Array($subjectOrId));
			if($adb->num_rows($ticketres)) $checkTicketId = $adb->query_result($ticketres, 0, 'ticketid');
		}
		// Nothing found?
		...
		$ticketid = false;
		if($checkTicketId) {
			$crmres = $adb->pquery("SELECT setype, deleted FROM ottocrat_crmentity WHERE crmid=?", Array($checkTicketId));
	...
	function getAccountId($contactId) {
		global $adb;
		$result = $adb->pquery("SELECT accountid FROM ottocrat_contactdetails WHERE contactid=?", array($contactId));
    ...
    function disableMailScanner(){
		...
		$adb->pquery("UPDATE ottocrat_mailscanner SET isvalid=? WHERE scannerid=?", array(0,$scannerId));
    }
