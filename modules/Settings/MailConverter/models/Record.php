...
class Settings_MailConverter_Record_Model extends Settings_Ottocrat_Record_Model {
	...
	public function hasRules() {
		...
		$result = $db->pquery('SELECT 1 FROM ottocrat_mailscanner_rules WHERE scannerid = ?', array($this->getId()));
	...
	public function getDefaultUrl() {
		...
		return Ottocrat_Request:: encryptLink('index.php?module=' . $moduleModel->getName() . '&parent=' . $moduleModel->getParentName() . '&record=' . $this->getId());
	}
	public function getDefaultUrlWNC() {
		$moduleModel = $this->getModule();
		return 'index.php?module=' . $moduleModel->getName() . '&parent=' . $moduleModel->getParentName() . '&record=' . $this->getId();
	}
	public function getListUrl() {
		$moduleModel = $this->getModule();
		return Ottocrat_Request:: encryptLink( 'index.php?module=' . $moduleModel->getName() . '&parent=' . $moduleModel->getParentName() .
		'&view=List');
	}
	...
	public function getScanUrl() {
		$url = $this->getDefaultUrlWNC(). '&action=ScanNow';
		return 'javascript:Settings_MailConverter_List_Js.triggerScan("'.Ottocrat_Request:: encryptLink($url).'")';
	}
	...
	public function getRulesListUrl() {
		$url = $this->getDefaultUrlWNC(). '&view=RulesList';
		return Ottocrat_Request:: encryptLink($url);
	}
	...
	public function getEditViewUrl() {
		return Ottocrat_Request:: encryptLink($this->getDefaultUrlWNC() . '&create=existing&view=Edit');
	}
	public function getEditViewUrlWNC() {
		return $this->getDefaultUrlWNC() . '&create=existing&view=Edit';
	}
	...
	public function  getCreateRuleRecordUrl() {
		...
		return 'javascript:Settings_MailConverter_Index_Js.triggerRuleEdit("'.Ottocrat_Request:: encryptLink($url).'")';
	}
	...
	public function getDeleteUrl() {
		return Ottocrat_Request:: encryptLink($this->getDefaultUrlWNC(). '&action=DeleteMailBox');
	}

	/**
	 * Function to get record links
	 * @return <Array> List of link models <Ottocrat_Link_Model>
	 */
	public function getRecordLinks() {
		...
		$recordLinks = array(
			array(
				...
				'linkurl' => "javascript:window.location.href = '" .Ottocrat_Request:: encryptLink( $this->getEditViewUrlWNC() . "&mode=step1")."'",
				'linkicon' => 'icon-pencil'
			),
			array(
				'linktype' => 'LISTVIEW',
				'linklabel' => vtranslate('LBL_SELECT_FOLDERS', $qualifiedModuleName),
				'linkurl' => "javascript:window.location.href = '" . Ottocrat_Request:: encryptLink($this->getEditViewUrlWNC() . "&mode=step2")."'",
				'linkicon' => 'icon-pencil'
			),
			array(
				'linktype' => 'LISTVIEW',
		...
		$links = array();
		if ($this->hasRules()) {
			$links[] = Ottocrat_Link_Model::getInstanceFromValues(array(
					'linktype' => 'LISTVIEW',
	...
	public function delete() {
        ...
        $scanner = new Ottocrat_MailScannerInfo(trim($this->getName()));
		$scanner->delete();
	}
	...
	public function save() {
        vimport('~~modules/Settings/MailConverter/handlers/MailScannerInfo.php');
		$scannerLatestInfo = new Ottocrat_MailScannerInfo(false, false);
		...
		$isConnected = true;
		$scannerOldInfo = new Ottocrat_MailScannerInfo($this->get('scannerOldName'));

		if(!$scannerOldInfo->compare($scannerLatestInfo)) {
            vimport('~~modules/Settings/MailConverter/handlers/MailBox.php');
			$mailBox = new Ottocrat_MailBox($scannerLatestInfo);
	...
	public function scanNow() {
		...
		if ($isValid) {
			...
			$scannerInfo = new Ottocrat_MailScannerInfo($this->getName());
			/** Start the scanning. */
			$scanner = new Ottocrat_MailScanner($scannerInfo);
	...
	public function getUpdatedFoldersList() {
        vimport('~~modules/Settings/MailConverter/handlers/MailBox.php');
		$scannerInfo = new Ottocrat_MailScannerInfo($this->getName());
		$mailBox = new Ottocrat_MailBox($scannerInfo);
	...
	public function saveFolders() {
		...
		$updateQuery = "UPDATE ottocrat_mailscanner_folders SET enabled = CASE folderid ";
	...
	public function updateSequence($sequencesList) {
		...
		$updateQuery = "UPDATE ottocrat_mailscanner_rules SET sequence = CASE";
	...
	public static function getCleanInstance() {
		...
		return $recordModel->setModule(Settings_Ottocrat_Module_Model::getInstance('Settings:MailConverter'));
	}
	...
	public static function getInstanceById($recordId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ottocrat_mailscanner WHERE scannerid = ', array($recordId));
		if ($db->num_rows($result)) {
	...
	public static function getAll() {
		$db = PearDatabase::getInstance();
		$moduleModel = Settings_Ottocrat_Module_Model::getInstance('Settings:MailConverter');

		$result = $db->pquery('SELECT * FROM ottocrat_mailscanner', array());
	...
	public static function getCount() {
		...
		$result = $db->pquery('SELECT 1 FROM ottocrat_mailscanner', array());
		$numOfRows = $db->num_rows($result);
		return $numOfRows;
	}
