...
class Settings_MailConverter_RuleRecord_Model extends Settings_Ottocrat_Record_Model {
	...
	public function getDefaultUrl() {
		return Ottocrat_Request:: encryptLink('index.php?module=MailConverter&parent=Settings&record='. $this->getId() .'&scannerId='. $this->getScannerId());
	}
	...
	public function getEditViewUrl() {
		$url = $this->getDefaultUrlWNC(). '&view=EditRule';
	...
	public function getDeleteUrl() {
		$url = $this->getDefaultUrlWNC(). '&action=DeleteRule';
		return 'javascript:Settings_MailConverter_Index_Js.triggerDeleteRule(this,"'.Ottocrat_Request:: encryptLink
		($url).'")';
	}
	 ...
	 * @return <Array> List of actions <Ottocrat_MailScannerAction>
	 */
	public function getActions() {
		$ruleId = $this->getId();
		if (!$this->actions && $ruleId) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT actionid FROM ottocrat_mailscanner_ruleactions WHERE ruleid = ?", array($ruleId));
			$numOfRows = $db->num_rows($result);

			for($i=0; $i<$numOfRows; $i++) {
				$actionId = $db->query_result($result, $i, 'actionid');
				$this->actions[$actionId] = new Ottocrat_MailScannerAction($actionId);
			}
		}
		return $this->actions;
	}
	...
	public function delete() {
		$rule = new Ottocrat_MailScannerRule($this->getId());
		$rule->delete();
	}
	...
	public function save() {
		$recordId = $this->getId();
		$ruleModel = new Ottocrat_MailScannerRule($recordId);
	...
	public static function getInstanceById($recordId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ottocrat_mailscanner_rules WHERE ruleid = ', array($recordId));
	...
	public static function getAll($scannerId) {
		...
		$result = $db->pquery("SELECT * FROM ottocrat_mailscanner_rules WHERE scannerid = ? ORDER BY sequence", array($scannerId));
		$numOfRows = $db->num_rows($result);
		for($i=0; $i<$numOfRows; $i++) {
	...
	public static function getRule($scannerId, $ruleId) {
		...
		$result = $db->pquery("SELECT * FROM ottocrat_mailscanner_rules WHERE scannerid = ? AND ruleid = ?", array($scannerId, $ruleId));
		if ($db->num_rows($result)) {
	...
	public function getAssignedTo($scannerId, $ruleId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT assigned_to FROM ottocrat_mailscanner_rules WHERE scannerid = ? AND ruleid = ?", array($scannerId, $ruleId));
