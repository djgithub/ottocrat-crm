...
class Settings_MailConverter_Module_Model extends Settings_Ottocrat_Module_Model {
	...
	public function getCreateRecordUrl() {
		...
		return 'javascript:Settings_MailConverter_List_Js.checkMailBoxMaxLimit("'.Ottocrat_Request:: encryptLink
		($url).'")';
	}
	...
	public function MailBoxExists() {
		...
		$result = $db->pquery("SELECT COUNT(*) AS count FROM ottocrat_mailscanner", array());
	...
	public function getDefaultId() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT MIN(scannerid) AS id FROM ottocrat_mailscanner", array());
	...
	public function getMailboxes() {
		$mailBox = array();
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT scannerid, scannername FROM ottocrat_mailscanner", array());
	...
	public function getScannedFolders($id) {
		$folders = array();
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT foldername FROM ottocrat_mailscanner_folders WHERE scannerid=? AND enabled=1", array($id));
		$numOfRows = $db->num_rows($result);
		for ($i = 0; $i < $numOfRows; $i++) {
	...
	public function getFolders($id) {
		...
		$scannerInfo = new Ottocrat_MailScannerInfo($scannerName);
	...
	public function getScannerName($id) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT scannername FROM ottocrat_mailscanner WHERE scannerid=?", array($id));
	...
	public function updateFolders($scannerId, $folders) {
		...
		$db->pquery("DELETE FROM ottocrat_mailscanner_folders WHERE scannerid=?", array($scannerId));
		foreach ($folders as $folder) {
			$db->pquery("INSERT INTO ottocrat_mailscanner_folders VALUES(?,?,?,?,?,?)", array('', $scannerId, $folder, $lastScan, '0', '1'));
		}
	}
