...
class Settings_LayoutEditor_Field_Model extends Ottocrat_Field_Model {
    ...
    public function delete() {
        ...
        $adb->pquery("delete from ottocrat_cvcolumnlist where columnname = ? ", array($deletecolumnname));
        $adb->pquery("delete from ottocrat_cvstdfilter where columnname = ?", array($column_cvstdfilter));
        $adb->pquery("delete from ottocrat_cvadvfilter where columnname = ?", array($deletecolumnname));
        $adb->pquery("delete from ottocrat_selectcolumn where columnname = ?", array($select_columnname));
        $adb->pquery("delete from ottocrat_relcriteria where columnname = ?", array($select_columnname));
        $adb->pquery("delete from ottocrat_reportsortcol where columnname = ?", array($select_columnname));
        $adb->pquery("delete from ottocrat_reportdatefilter where datecolumnname = ?", array($column_cvstdfilter));
        $adb->pquery("delete from ottocrat_reportsummary where columnname like ?", array('%'.$reportsummary_column.'%'));


        //Deleting from convert lead mapping ottocrat_table- Jaguar
        if($fld_module=="Leads") {
            $deletequery = 'delete from ottocrat_convertleadmapping where leadfid=?';
            ...
            $map_del_q = "update ottocrat_convertleadmapping set ".$map_del_id[$fld_module]."=0 where ".$map_del_id[$fld_module]."=?";
        ...
        if($this->getFieldDataType() == 'picklist' || $this->getFieldDataType() == 'multipicklist') {
            $deltablequery = 'drop table ottocrat_'.$adb->sql_escape_string($columnname);
            $adb->pquery($deltablequery, array());
            //To Delete Sequence Table 
            $deltableseqquery = 'drop table ottocrat_'.$adb->sql_escape_string($columnname).'_seq'; 
            $adb->pquery($deltableseqquery, array()); 
            $adb->pquery("delete from  ottocrat_picklist_dependency where sourcefield=? or targetfield=?", array($columnname,$columnname));
        }
    }
	...
	public function move($fieldNewDetails, $fieldOlderDetails) {
		...
		if ($olderBlockId == $newBlockId) {
			if ($newSequence > $olderSequence) {
				$updateQuery = 'UPDATE ottocrat_field SET sequence = sequence-1 WHERE sequence > ? AND sequence <= ? AND block = ?';
			...
			$query = 'UPDATE ottocrat_field SET sequence = ? WHERE fieldid = ?';
			$params = array($newSequence, $this->getId());
			$db->pquery($query, $params);
		} else {
			$updateOldBlockQuery = 'UPDATE ottocrat_field SET sequence = sequence-1 WHERE sequence > ? AND block = ?';
			$params = array($olderSequence, $olderBlockId);
			$db->pquery($updateOldBlockQuery, $params);

			$updateNewBlockQuery = 'UPDATE ottocrat_field SET sequence = sequence+1 WHERE sequence >= ? AND block = ?';
			$params = array($newSequence, $newBlockId);
			$db->pquery($updateNewBlockQuery, $params);

			$query = 'UPDATE ottocrat_field SET sequence = ?, block = ? WHERE fieldid = ?';
    ...
    public static function makeFieldActive($fieldIdsList=array(), $blockId) {
        $db = PearDatabase::getInstance();
		$maxSequenceQuery = "SELECT MAX(sequence) AS maxsequence FROM ottocrat_field WHERE block = ? AND presence IN (0,2) ";
		$res = $db->pquery($maxSequenceQuery,array($blockId));
		$maxSequence = $db->query_result($res,0,'maxsequence');

		$query = 'UPDATE ottocrat_field SET presence = 2, sequence = CASE';
    ...
    public function isDefaultValueOptionDisabled() {
        if($this->isMandatoryOptionDisabled() || $this->getFieldDataType() == Ottocrat_Field_Model::REFERENCE_TYPE || $this->get('uitype') == 69){
            return true;
        }
        return false;
    }
	...
	public static function getDetailsForMove($fieldIdsList = array()) {
		if ($fieldIdsList) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery('SELECT fieldid, sequence, block, fieldlabel FROM ottocrat_field WHERE fieldid IN ('. generateQuestionMarks($fieldIdsList) .')', $fieldIdsList);
	 ...
	 * @param <Ottocrat_Module_Model> $moduleInstance
	...
	public static function getInstanceFromBlockIdList($blockId, $moduleInstance= false) {
		...
		$query = 'SELECT * FROM ottocrat_field WHERE block IN('. generateQuestionMarks($blockId) .') AND ottocrat_field.displaytype IN (1,2,4) ORDER BY sequence';
