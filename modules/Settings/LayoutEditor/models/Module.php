...
class Settings_LayoutEditor_Module_Model extends Ottocrat_Module_Model {
    ...
    public function addField($fieldType, $blockId, $params) {
        ...
        $max_fieldid = $db->getUniqueID("ottocrat_field");
        ...
        if (isset($focus->customFieldTable)) {
            ...
            $tableName= 'ottocrat_'.strtolower($moduleName).'cf';
        }
        ...
        $blockModel = Ottocrat_Block_Model::getInstance($blockId, $this);
    ...
    public function getTypeDetailsForAddField($fieldType,$params) {
        switch ($fieldType) {
                ...
                Case 'Decimal' :
                               ...
                               // Fix for http://trac.ottocrat.com/cgi-bin/trac.cgi/ticket/6363
                               $uichekdata='NN~O';
                               break;
               Case 'Percent' :
                               $uitype = 9;
    ...
    public function checkFIeldExists($fieldLabel) {
        ...
        $query = 'SELECT 1 FROM ottocrat_field WHERE tabid IN ('.  generateQuestionMarks($tabId).') AND fieldlabel=?';
	...
	public static function getEntityModulesList() {
		...
		$query = 'SELECT name FROM ottocrat_tab WHERE
	...
	public function getRelations() {
		if($this->relations === null) {
			$this->relations = Ottocrat_Relation_Model::getAllRelations($this, false);
		}
