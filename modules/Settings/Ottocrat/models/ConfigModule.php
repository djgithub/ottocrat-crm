...
class Settings_Ottocrat_ConfigModule_Model extends Settings_Ottocrat_Module_Model {
	...
	public function getMenuItem() {
		$menuItem = Settings_Ottocrat_MenuItem_Model::getInstance('LBL_CONFIG_EDITOR');
		return $menuItem;
	}
	...
	public function getEditViewUrl() {
		...
		return '?module=Ottocrat&parent=Settings&view=ConfigEditorEdit&block='.$menuItem->get('blockid').'&fieldid='.$menuItem->get('fieldid');
	}
	...
	public function getDetailViewUrl() {
		$menuItem = $this->getMenuItem();
		return '?module=Ottocrat&parent=Settings&view=ConfigEditorDetail&block='.$menuItem->get('blockid').'&fieldid='.$menuItem->get('fieldid');
	}
	...
	public function getPicklistValues($fieldName) {
		if ($fieldName === 'default_module') {
			...
			$query = 'SELECT name, tablabel FROM ottocrat_tab WHERE presence IN (' . generateQuestionMarks($presence) . ') AND isentitytype = ? AND name NOT IN (' . generateQuestionMarks($restrictedModules) . ')';
...
//			'history_max_viewed'			=> array('label' => 'LBL_MAX_HISTORY_VIEWED',				'fieldType' => 'input'),
	 ...
	 * @return <Settings_Ottocrat_ConfigModule_Model> $moduleModel
	 */
