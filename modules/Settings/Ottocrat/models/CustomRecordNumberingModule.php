...
class Settings_Ottocrat_CustomRecordNumberingModule_Model extends Ottocrat_Module_Model {
	 ...
	 * @return <Settings_Ottocrat_CustomRecordNumberingModule_Model> $moduleModel
	 */
	 ...
	 * @return <Array> list of supported modules <Ottocrat_Module_Model>
	 */
	public static function getSupportedModules() {
		$db = PearDatabase::getInstance();

		$sql = "SELECT tabid, name FROM ottocrat_tab WHERE isentitytype = ? AND presence = ? AND tabid IN (SELECT DISTINCT tabid FROM ottocrat_field WHERE uitype = ?)";
		$result = $db->pquery($sql, array(1, 0, 4));
		$numOfRows = $db->num_rows($result);

		for($i=0; $i<$numOfRows; $i++) {
			$tabId = $db->query_result($result, $i, 'tabid');
			$modulesModels[$tabId] = Settings_Ottocrat_CustomRecordNumberingModule_Model::getInstance($db->query_result($result, $i, 'name'), $tabId);
		}
	...
	public function setModuleSequence() {
		...
		if (!$status) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT cur_id FROM ottocrat_modentity_num WHERE semodule = ? AND prefix = ?", array($moduleName, $prefix));
