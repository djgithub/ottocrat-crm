 ...
 */
class Settings_Groups_Member_Model extends Ottocrat_Base_Model {
	...
	public static function getAllByTypeForGroup($groupModel, $type) {
		...
		if($type == self::MEMBER_TYPE_USERS) {
			$sql = 'SELECT ottocrat_users.id, ottocrat_users.last_name, ottocrat_users.first_name FROM ottocrat_users
							INNER JOIN ottocrat_users2group ON ottocrat_users2group.userid = ottocrat_users.id
							WHERE ottocrat_users2group.groupid = ?';
		...
		if($type == self::MEMBER_TYPE_GROUPS) {
			$sql = 'SELECT ottocrat_groups.groupid, ottocrat_groups.groupname FROM ottocrat_groups
							INNER JOIN ottocrat_group2grouprel ON ottocrat_group2grouprel.containsgroupid = ottocrat_groups.groupid
							WHERE ottocrat_group2grouprel.groupid = ?';
			$params = array($groupModel->getId());
			$result = $db->pquery($sql, $params);
		...
		if($type == self::MEMBER_TYPE_ROLES) {
			$sql = 'SELECT ottocrat_role.roleid, ottocrat_role.rolename FROM ottocrat_role
							INNER JOIN ottocrat_group2role ON ottocrat_group2role.roleid = ottocrat_role.roleid
							WHERE ottocrat_group2role.groupid = ?';
			$params = array($groupModel->getId());
			$result = $db->pquery($sql, $params);
		...
		if($type == self::MEMBER_TYPE_ROLE_AND_SUBORDINATES) {
			$sql = 'SELECT ottocrat_role.roleid, ottocrat_role.rolename FROM ottocrat_role
							INNER JOIN ottocrat_group2rs ON ottocrat_group2rs.roleandsubid = ottocrat_role.roleid
							WHERE ottocrat_group2rs.groupid = ?';
			$params = array($groupModel->getId());
			$result = $db->pquery($sql, $params);
			$noOfRoles = $db->num_rows($result);
