 ...
 */
class Settings_Groups_Record_Model extends Settings_Ottocrat_Record_Model {
	...
	public function getEditViewUrl() {
		return Ottocrat_Request:: encryptLink('?module=Groups&parent=Settings&view=Edit&record='.$this->getId());
	}
	...
	public function getDeleteActionUrl() {
		return Ottocrat_Request:: encryptLink('index.php?module=Groups&parent=Settings&view=DeleteAjax&record='.$this->getId());
	}
    ...
    public function getDetailViewUrl() {
        return Ottocrat_Request:: encryptLink('?module=Groups&parent=Settings&view=Detail&record='.$this->getId());
    }
	...
	public function save() {
		...
		if (empty($groupId)) {
			$mode = '';
			$groupId = $db->getUniqueId('ottocrat_users');
			$this->setId($groupId);
		}

		if ($mode == 'edit') {
			$sql = 'UPDATE ottocrat_groups SET groupname=?, description=? WHERE groupid=?';
			$params = array($this->getName(), $this->getDescription(), $groupId);
		} else {
			$sql = 'INSERT INTO ottocrat_groups(groupid, groupname, description) VALUES (?,?,?)';
		...
		if (is_array($members)) {
			$db->pquery('DELETE FROM ottocrat_users2group WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM ottocrat_group2grouprel WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM ottocrat_group2role WHERE groupid=?', array($groupId));
			$db->pquery('DELETE FROM ottocrat_group2rs WHERE groupid=?', array($groupId));
			...
			for ($i = 0; $i < $noOfMembers; ++$i) {
				...
				if ($idComponents && count($idComponents) == 2) {
					...
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_USERS) {
						$db->pquery('INSERT INTO ottocrat_users2group(userid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_GROUPS) {
						$db->pquery('INSERT INTO ottocrat_group2grouprel(containsgroupid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLES) {
						$db->pquery('INSERT INTO ottocrat_group2role(roleid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLE_AND_SUBORDINATES) {
						$db->pquery('INSERT INTO ottocrat_group2rs(roleandsubid, groupid) VALUES (?,?)', array($memberId, $groupId));
					}
				}
			}
		}
	...
	protected function transferOwnership($transferToGroup) {
		...
		$query = 'UPDATE ottocrat_crmentity SET smownerid=? WHERE smownerid=?';
	...
	public function delete($transferToGroup) {
		...
		$em->triggerEvent("ottocrat.entity.beforegroupdelete", $entityData);
		...
		$db->pquery('DELETE FROM ottocrat_group2rs WHERE groupid=?', array($groupId));
		$db->pquery('DELETE FROM ottocrat_users2group WHERE groupid=?', array($groupId));
		$db->pquery("DELETE FROM ottocrat_reportsharing WHERE shareid=? AND setype='groups'", array($groupId));

		$db->pquery('DELETE FROM ottocrat_groups WHERE groupid=?', array($groupId));
	}
	...
	public static function getAll() {
		...
		$sql = 'SELECT * FROM ottocrat_groups';
		$params = array();
	...
	public static function getInstance($value) {
		...
		if (Ottocrat_Utils::isNumber($value)) {
			$sql = 'SELECT * FROM ottocrat_groups WHERE groupid = ?';
		} else {
			$sql = 'SELECT * FROM ottocrat_groups WHERE groupname = ?';
		}
		$params = array($value);
		$result = $db->pquery($sql, $params);
