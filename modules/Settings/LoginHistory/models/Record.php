...
class Settings_LoginHistory_Record_Model extends Settings_Ottocrat_Record_Model {
	...
	public function getAccessibleUsers(){
		...
		$query = 'SELECT user_name, first_name, last_name FROM ottocrat_users';
	...
	public function getDisplayValue($fieldName, $recordId = false) {
		if($fieldName == 'login_time' || $fieldName == 'logout_time'){
			if($this->get($fieldName) != '0000-00-00 00:00:00'){
				return Ottocrat_Datetime_UIType::getDateTimeValue($this->get($fieldName));
			}else{
				return '---';
			}
		} else {
