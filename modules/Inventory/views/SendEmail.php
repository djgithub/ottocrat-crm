...
class Inventory_SendEmail_View extends Ottocrat_ComposeEmail_View {
     ...
     */
    public function composeMailData(Ottocrat_Request $request) {
        parent::composeMailData($request);
        $viewer = $this->getViewer($request);
        $inventoryRecordId = $request->get('record');
        $recordModel = Ottocrat_Record_Model::getInstanceById($inventoryRecordId, $request->getModule());
    ...
    public function populateTo($request){
        ...
        foreach($toEmailConsiderableFields as $fieldName){
            ...
            $fieldLabel = Ottocrat_Util_Helper::getLabel($fieldValue);
            $referenceModuleModel = Ottocrat_Module_Model::getInstance($referenceModule);
