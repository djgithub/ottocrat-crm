...
class Inventory_Detail_View extends Ottocrat_Detail_View {
	 ...
	 * @return type
	 */
	function showDetailViewByMode(Ottocrat_Request $request) {
	 ...
	 * @param Ottocrat_Request $request
	 */
	function showLineItemDetails(Ottocrat_Request $request) {
		...
		if ($taxtype == 'group') {
			...
			for($i=0; $i<$taxCount; $i++) {
				$taxDetails[$i]['amount'] = Ottocrat_Currency_UIType::transformDisplayValue($taxDetails[$i]['amount'], null, true);
			}
		...
		for($i=0; $i<$taxCount; $i++) {
			$shippingTaxDetails[$i]['amount'] = Ottocrat_Currency_UIType::transformDisplayValue($shippingTaxDetails[$i]['amount'], null, true);
		}
		...
		foreach ($currencyFieldsList as $fieldName) {
			$finalDetails[$fieldName] = Ottocrat_Currency_UIType::transformDisplayValue($finalDetails[$fieldName], null, true);
		}
		...
		for ($i=1; $i<=$productsCount; $i++) {
			...
			if ($taxtype == 'individual') {
				...
				for($j=0; $j<$taxCount; $j++) {
					$taxDetails[$j]['amount'] = Ottocrat_Currency_UIType::transformDisplayValue($taxDetails[$j]['amount'], null, true);
				}
			...
			foreach ($currencyFieldsList as $fieldName) {
				$product[$fieldName.$i] = Ottocrat_Currency_UIType::transformDisplayValue($product[$fieldName.$i], null, true);
			}
