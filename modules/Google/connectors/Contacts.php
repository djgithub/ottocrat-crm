...
require_once 'vtlib/Ottocrat/Net/Client.php'; 
...
Class Google_Contacts_Connector extends WSAPP_TargetConnector {
    ...
    /**
     * Tarsform Google Records to Ottocrat Records
    ...
    protected function fireRequest($url,$headers,$params=array(),$method='POST') { 
        $httpClient = new Ottocrat_Net_Client($url); 
    ...
    public function getContacts($SyncState) {
        ...
        if (count($feed['entry']) > 0) { 
            ...
            $maxModifiedTime = date('Y-m-d H:i:s', strtotime(Google_Contacts_Model::ottocratFormat($lastEntry['updated']['$t'])) + 1); 
            if ($this->totalRecords > $this->maxResults) {
                if (!Google_Utils_Helper::getSyncTime('Contacts', $user)) {
                    $query['updated-min'] = $this->googleFormat(date('Y-m-d H:i:s', strtotime(Google_Contacts_Model::ottocratFormat($lastEntry['updated']['$t'])))); 
...
/**
    ...
    /**
     * Push the ottocrat records to google
     * @param <array> $records ottocrat records to be pushed to google
     * @return <array> pushed records
     */
    ...
    /**
     * Tarsform  Ottocrat Records to Google Records
     * @param <array> $vtContacts 
     * @return <array> tranformed ottocrat Records
     */
