...
//		}
    ...
    function hasStoredToken($service, $accessToken = false, $requestToken = false) {
        ...
        if (!$accessToken && !$requestToken){
            $query = "SELECT  1 FROM ottocrat_google_oauth WHERE  userid=? and service=?";
            $params = array($this->userId, $service);
        }
        else if ($accessToken){
            $query = "SELECT  access_token FROM ottocrat_google_oauth WHERE  userid=? and service=? AND access_token<>? AND access_token IS NOT NULL";
            $params = array($this->userId, $service, '');
        }
        else if ($requestToken){
            $query = "SELECT  request_token FROM ottocrat_google_oauth WHERE  userid=? and service=? AND request_token<>? AND request_token IS NOT NULL";
            $params = array($this->userId, $service, '');
        }
    ...
    protected function storeAccessToken($service, $token) {
        ...
        $query = "INSERT INTO ottocrat_google_oauth(service,access_token,userid) VALUES(?,?,?)";
        $params = array($service, base64_encode(serialize($token)), $user->getid());
        if (self::hasStoredToken($service, false, true)) {
            $query = "UPDATE ottocrat_google_oauth SET access_token=? WHERE userid=? AND  service=?";
    ...
    protected function retreiveAccessToken($service) {
        ...
        $query = "SELECT access_token FROM ottocrat_google_oauth WHERE userid=? AND service =?";
        $params = array($this->userId, $service);
    ...
    protected function storeRequestToken($service, $token) {
        ...
        $query = "INSERT INTO ottocrat_google_oauth(service,request_token,userid) values(?,?,?)";
    ...
    protected function retrieveRequestToken($service) {
        ...
        $query = "SELECT request_token FROM ottocrat_google_oauth WHERE userid=? AND service =?";
    ...
    function getHttpClient($service) {
	 ...
	 public static function initializeSchema(){
		 if(!Ottocrat_Utils::CheckTable('ottocrat_google_oauth')) {
                Ottocrat_Utils::CreateTable('ottocrat_google_oauth',
