...
class Emails_Record_Model extends Ottocrat_Record_Model {
	...
	public function getDetailViewUrl() {
		...
		return 'Javascript:Ottocrat_Index_Js.showEmailPreview("'.$this->getId().'","'.$parentId.'")';
	}
	...
	public function send() {
		...
		foreach($toEmailInfo as $id => $emails) {
			...
			$moduleLanguageStrings = Ottocrat_Language_Handler::getModuleStringsFromFile($currentLanguage,$parentModule);
	...
	function getFromEmailAddress() {
		...
		$fromEmail = false;
		$result = $db->pquery('SELECT from_email_field FROM ottocrat_systems WHERE server_type=?', array('email'));
	...
	function getAttachmentDetails() {
		...
		$attachmentRes = $db->pquery("SELECT * FROM ottocrat_attachments
						INNER JOIN ottocrat_seattachmentsrel ON ottocrat_attachments.attachmentsid = ottocrat_seattachmentsrel.attachmentsid
						WHERE ottocrat_seattachmentsrel.crmid = ?", array($this->getId()));
	...
	public function getRelatedDocuments() {
		...
		$documentRes = $db->pquery("SELECT * FROM ottocrat_senotesrel
						INNER JOIN ottocrat_crmentity ON ottocrat_senotesrel.notesid = ottocrat_crmentity.crmid AND ottocrat_senotesrel.crmid = ?
						INNER JOIN ottocrat_notes ON ottocrat_notes.notesid = ottocrat_senotesrel.notesid
						INNER JOIN ottocrat_seattachmentsrel ON ottocrat_seattachmentsrel.crmid = ottocrat_notes.notesid
						INNER JOIN ottocrat_attachments ON ottocrat_attachments.attachmentsid = ottocrat_seattachmentsrel.attachmentsid
						WHERE ottocrat_crmentity.deleted = 0", array($this->getId()));
	...
	public function saveDocumentDetails() {
		...
		for ($i=0; $i<$count; $i++) {
			$db->pquery("INSERT INTO ottocrat_senotesrel(crmid, notesid) VALUES(?, ?)", array($record, $documentIds[$i]));
		}
	}
    ...
    public function deleteDocumentLink($idList = array()){
        $db = PearDatabase::getInstance();
        $query =  'DELETE FROM ottocrat_senotesrel where crmid=?';
    ...
    public function deleteAttachment($emailAttachmentDetails = array()) {
        ...
        $db->pquery('UPDATE ottocrat_crmentity SET deleted=0 WHERE crmid IN('.generateQuestionMarks($attachmentIdList).')',$attachmentIdList);
        $db->pquery('DELETE FROM ottocrat_attachments WHERE attachmentsid IN('.generateQuestionMarks($attachmentIdList).')',$attachmentIdList);
        $db->pquery('DELETE FROM ottocrat_seattachmentsrel WHERE crmid=? and attachmentsid IN('.generateQuestionMarks($attachmentIdList).')',
	...
	public function checkUploadSize($documentIds = false) {
		...
		if (!empty ($documentIds)) {
			$count = count($documentIds);
			for ($i=0; $i<$count; $i++) {
				$documentRecordModel = Ottocrat_Record_Model::getInstanceById($documentIds[$i], 'Documents');
	...
	public function updateTrackDetails($parentId) {
		...
		$db->pquery("INSERT INTO ottocrat_email_access(crmid, mailid, accessdate, accesstime) VALUES(?, ?, ?, ?)", array($parentId, $recordId, date('Y-m-d'), date('Y-m-d H:i:s')));

		$result = $db->pquery("SELECT 1 FROM ottocrat_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		if ($db->num_rows($result)>0) {
			$db->pquery("UPDATE ottocrat_email_track SET access_count = access_count+1 WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		} else {
			$db->pquery("INSERT INTO ottocrat_email_track(crmid, mailid, access_count) values(?, ?, ?)", array($parentId, $recordId, 1));
		}
	}
	...
	public function getAccessCountValue($parentId) {
		...
		$result = $db->pquery("SELECT access_count FROM ottocrat_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $this->getId()));
	...
	public function isSentMail(){
		if(!array_key_exists('email_flag', $this->getData())){
			$db = PearDatabase::getInstance();
			$query = 'SELECT email_flag FROM ottocrat_emaildetails WHERE emailid=?';
	...
	function getEntityType($id) {
		...
		if (!empty($id)) {
			...
			if(!in_array($relatedModule, $emailRelatedModules)){
				$sql = 'SELECT id FROM ottocrat_users WHERE id=?';
				$result = $db->pquery($sql, array($id));
				if($db->num_rows($result) > 0){
					$relatedModule = 'Users';
				}
