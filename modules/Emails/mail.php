...
/**   Function used to send email
  ...
  *   $from_email	-- currently loggedin ottocrat_users's email id. you can give as '' if you are not in HelpDesk module
  ...
  *   $attachment	-- whether we want to attach the currently selected file or all ottocrat_files.[values = current,all] - optional
  *   $emailid		-- id of the email object which will be used to get the ottocrat_attachments
  */
function send_mail($module,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',$logo='', $useGivenFromEmailAddress=false)
{
	...
	//Get the email id of assigned_to user -- pass the value and name, name must be "user_name" or "id"(field names of ottocrat_users ottocrat_table)
    ...
    if($cachedFromEmail === null) {
        $query = "select from_email_field from ottocrat_systems where server_type=?";
        $params = array('email');
...
/**	Function to get the user Email id based on column name and column value
  *	$name -- column name of the ottocrat_users ottocrat_table
  *	$val  -- column value
  */
function getUserEmailId($name,$val)
{
	...
	if($val != '')
	{
		...
		$sql = "SELECT email1, email2, secondaryemail  from ottocrat_users WHERE status='Active' AND ". $adb->sql_escape_string($name)." = ?";
...
function addSignature($contents, $fromname)
{
    ...
    if($sign === null) {
        $result = $adb->pquery("select signature, first_name, last_name from ottocrat_users where user_name=?", array($fromname));
...
/**	Function to set all the Mailer properties
  ...
  *	$attachment	-- whether we want to attach the currently selected file or all ottocrat_files.
  				[values = current,all] - optional
  *	$emailid	-- id of the email object which will be used to get the ottocrat_attachments - optional
  */
function setMailerProperties($mail,$subject,$contents,$from_email,$from_name,$to_email,$attachment='',$emailid='',$module='',$logo='')
{
    ...
    if(empty($userFullName)) {
        $rs = $adb->pquery("select first_name,last_name from ottocrat_users where user_name=?", array($from_name));
	...
	//This will add all the ottocrat_files which are related to this record or email
...
function setMailServerProperties($mail)
{
	...
	$res = $adb->pquery("select * from ottocrat_systems where server_type=?", array('email'));
...
/**     Function to add all the ottocrat_files as attachment with the mail object
  *     $mail -- reference of the mail object
  *     $record -- email id ie., record id which is used to get the all ottocrat_attachments from database
  */
function addAllAttachments($mail,$record)
{
	...
	//Retrieve the ottocrat_files from database where avoid the file which has been currently selected
	$sql = "select ottocrat_attachments.* from ottocrat_attachments inner join ottocrat_seattachmentsrel on ottocrat_attachments.attachmentsid = ottocrat_seattachmentsrel.attachmentsid inner join ottocrat_crmentity on ottocrat_crmentity.crmid = ottocrat_attachments.attachmentsid where ottocrat_crmentity.deleted=0 and ottocrat_seattachmentsrel.crmid=?";
...
function MailSend($mail)
{
	...
	{
		 $log->info("Mail has been sent from the ottocratCRM system : Status : '".$mail->ErrorInfo."'");
		return 1;
	}
}
...
/**	Function to get the Parent email id from HelpDesk to send the details about the ticket via email
  ...
  *	$parentid -- id of the parent ie., contact or ottocrat_account
  */
function getParentMailId($parentmodule,$parentid)
{
	...
	$adb->println("Inside the function getParentMailId. \n parent module and id => ".$parentmodule."&".$parentid);
        ...
        {
                $tablename = 'ottocrat_contactdetails';
                $idname = 'contactid';
		$first_email = 'email';
...
function parseEmailErrorString($mail_error_str)
{
	...
	foreach($mail_status as $key => $val)
	{
		...
		if($status_str[1] != 1 && $status_str[1] != '')
		{
			...
			{
				$adb->println("first elseif part - status will be 0 which is the case of assigned to ottocrat_users's email is empty.");
...
function getDefaultAssigneeEmailIds($groupId) {
	...
	if($groupId != '') {
		...
		$result = $adb->pquery('SELECT email1,email2,secondaryemail FROM ottocrat_users WHERE ottocrat_users.id IN
											('.  generateQuestionMarks($userGroups->group_users).') AND ottocrat_users.status= ?',
