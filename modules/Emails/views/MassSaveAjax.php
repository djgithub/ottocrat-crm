...
class Emails_MassSaveAjax_View extends Ottocrat_Footer_View {
	 ...
	 */
	public function massSave(Ottocrat_Request $request) {
		...
		$result = Ottocrat_Util_Helper::transformUploadedFiles($_FILES, true);
		$_FILES = $result['file'];
        ...
        if(!empty($parentEmailId) && !empty ($recordId)) {
            $parentEmailModel = Ottocrat_Record_Model::getInstanceById($parentEmailId);
		...
		if ($recordModel->checkUploadSize($documentIds)) {
			...
			if($flag == 'SENT') {
				...
				if ($status === true) {
					// This is needed to set ottocrat_email_track table as it is used in email reporting
