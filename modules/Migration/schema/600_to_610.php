...
include_once 'modules/com_ottocrat_workflow/VTTaskManager.inc';
include_once 'include/utils/utils.php';

if(defined('OTTOCRAT_UPGRADE')) {
        //Collating all module package updates here
	updateVtlibModule('Import', 'packages/ottocrat/mandatory/Import.zip');
        updateVtlibModule('PBXManager', 'packages/ottocrat/mandatory/PBXManager.zip');
	...
	updateVtlibModule('WSAPP', 'packages/ottocrat/mandatory/WSAPP.zip');
        updateVtlibModule('Arabic_ar_ae', 'packages/ottocrat/optional/Arabic_ar_ae.zip');
        updateVtlibModule('Assets', 'packages/ottocrat/optional/Assets.zip');
        updateVtlibModule('EmailTemplates', 'packages/ottocrat/optional/EmailTemplates.zip');
	...
	updateVtlibModule('SMSNotifier', "packages/ottocrat/optional/SMSNotifier.zip");
        updateVtlibModule("Sweden_sv_se","packages/ottocrat/optional/Sweden_sv_se.zip");
	updateVtlibModule("Webforms","packages/ottocrat/optional/Webforms.zip");
        installVtlibModule('ExtensionStore', 'packages/ottocrat/marketplace/ExtensionStore.zip');
}
...
$unWanted=array(
 ...
 "modules/com_ottocrat_workflow/resources/functional.js",
 "modules/com_ottocrat_workflow/resources/parallelexecuter.js",
 "modules/com_ottocrat_workflow/resources/editworkflowscript.js",
 "modules/com_ottocrat_workflow/resources/createtodotaskscript.js",
 "modules/com_ottocrat_workflow/resources/fieldexpressionpopup.js",
 "modules/com_ottocrat_workflow/resources/workflowlistscript.js",
 "modules/com_ottocrat_workflow/resources/fieldvalidator.js",
 "modules/com_ottocrat_workflow/resources/updatefieldstaskscript.js",
 "modules/com_ottocrat_workflow/resources/jquery-1.2.6.js",
 "modules/com_ottocrat_workflow/resources/json2.js",
 "modules/com_ottocrat_workflow/resources/ottocratwebservices.js",
 "modules/com_ottocrat_workflow/resources/jquery.timepicker.js",
 "modules/com_ottocrat_workflow/resources/createentitytaskscript.js",
 "modules/com_ottocrat_workflow/resources/edittaskscript.js",
 "modules/com_ottocrat_workflow/resources/createeventtaskscript.js",
 "modules/com_ottocrat_workflow/resources/emailtaskscript.js",
 "modules/FieldFormulas/editexpressionscript.js",              
 "modules/FieldFormulas/jquery-1.2.6.js",
 "modules/FieldFormulas/json2.js",
 "modules/FieldFormulas/ottocratwebservices.js",
...
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_ottocrat_workflows ADD schtypeid INT(10)", array());
}
$result = $adb->pquery("show columns from com_ottocrat_workflows like ?", array('schtime'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_ottocrat_workflows ADD schtime TIME", array());
}
$result = $adb->pquery("show columns from com_ottocrat_workflows like ?", array('schdayofmonth'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_ottocrat_workflows ADD schdayofmonth VARCHAR(100)", array());
}
$result = $adb->pquery("show columns from com_ottocrat_workflows like ?", array('schdayofweek'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_ottocrat_workflows ADD schdayofweek VARCHAR(100)", array());
}
$result = $adb->pquery("show columns from com_ottocrat_workflows like ?", array('schannualdates'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_ottocrat_workflows ADD schannualdates VARCHAR(100)", array());
}
$result = $adb->pquery("show columns from com_ottocrat_workflows like ?", array('nexttrigger_time'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE com_ottocrat_workflows ADD nexttrigger_time DATETIME", array());
}
...
//73 starts
$query = 'SELECT 1 FROM ottocrat_currencies WHERE currency_name=?';
$result = $adb->pquery($query, array('Sudanese Pound'));
if($adb->num_rows($result) <= 0){
    //Inserting Currency Sudanese Pound to ottocrat_currencies
    Migration_Index_View::ExecuteQuery('INSERT INTO ottocrat_currencies (currencyid,currency_name,currency_code,currency_symbol) VALUES ('.$adb->getUniqueID("ottocrat_currencies").',"Sudanese Pound","SDG","£")',array());
    Ottocrat_Utils::AddColumn('ottocrat_mailmanager_mailattachments', 'cid', 'VARCHAR(100)');
}
//73 ends
...
$adb->query("CREATE TABLE IF NOT EXISTS ottocrat_feedback (userid INT(19), dontshow VARCHAR(19) default false);");

//75 ends

//76 starts
$moduleInstance = Ottocrat_Module::getInstance('Calendar');
$fieldInstance = Ottocrat_Field::getInstance('activitytype',$moduleInstance);

$fieldInstance->setPicklistValues(array('Mobile Call'));
//76 ends

//77 starts
$sql = "ALTER TABLE ottocrat_products MODIFY productname VARCHAR( 100 )";
Migration_Index_View::ExecuteQuery($sql,array());
echo "<br>Updated to varchar(100) for productname";

$result = $adb->pquery('SELECT 1 FROM ottocrat_currencies WHERE currency_name = ?', array('CFA Franc BCEAO'));
    if(!$adb->num_rows($result)) {
        Migration_Index_View::ExecuteQuery('INSERT INTO ottocrat_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
            array($adb->getUniqueID('ottocrat_currencies'), 'CFA Franc BCEAO', 'XOF', 'CFA'));
    }
$result = $adb->pquery('SELECT 1 FROM ottocrat_currencies WHERE currency_name = ?', array('CFA Franc BEAC'));
    if(!$adb->num_rows($result)) {
        Migration_Index_View::ExecuteQuery('INSERT INTO ottocrat_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
            array($adb->getUniqueID('ottocrat_currencies'), 'CFA Franc BEAC', 'XAF', 'CFA'));
    }    
echo "<br>Added CFA Franc BCEAO and CFA Franc BEAC currencies";

$sql = "ALTER TABLE ottocrat_loginhistory MODIFY user_name VARCHAR( 255 )";
Migration_Index_View::ExecuteQuery($sql,array());

$sql = "UPDATE ottocrat_activitytype SET presence = '0' WHERE activitytype ='Mobile Call'";
Migration_Index_View::ExecuteQuery($sql,array());
...
//79 starts
Migration_Index_View::ExecuteQuery("CREATE TABLE IF NOT EXISTS ottocrat_shareduserinfo
						(userid INT(19) NOT NULL default 0, shareduserid INT(19) NOT NULL default 0,
						color VARCHAR(50), visible INT(19) default 1);", array());

Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_mailscanner_rules ADD assigned_to INT(10), ADD cc VARCHAR(255), ADD bcc VARCHAR(255)', array());
$assignedToId = Users::getActiveAdminId();
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_mailscanner_rules SET assigned_to=?", array($assignedToId));
echo "<br> Adding assigned to, cc, bcc fields for mail scanner rules";


//Schema changes for ottocrat_troubletickets hours & days column
Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_troubletickets MODIFY hours decimal(25,8)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_troubletickets MODIFY days decimal(25,8)', array());

Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET defaultvalue=? WHERE tablename=? and fieldname=?", array('1', 'ottocrat_pricebook', 'active'));
echo "<br> updated default value for pricebooks active";

$relationId = $adb->getUniqueID('ottocrat_relatedlists');
$contactTabId = getTabid('Contacts');
$vendorTabId = getTabId('Vendors');
$actions = 'SELECT';

$query = 'SELECT max(sequence) as maxsequence FROM ottocrat_relatedlists where tabid = ?';
$result = $adb->pquery($query, array($contactTabId));
$sequence = $adb->query_result($result, 0 ,'maxsequence');

$query = 'INSERT INTO ottocrat_relatedlists VALUES(?,?,?,?,?,?,?,?)';
...
//Schema changes for ottocrat_troubletickets hours & days column
Migration_Index_View::ExecuteQuery('UPDATE ottocrat_field set typeofdata=? WHERE fieldname IN(?,?) AND tablename = ?', array('N~O', 'hours', 'days', 'ottocrat_troubletickets'));

//79 ends

//80 starts
//Added recurring enddate column for events,to ottocrat_recurringevents table
Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_recurringevents ADD COLUMN recurringenddate date', array());
echo "added field recurring enddate to ottocrat_recurringevents to save untill date of repeat events";
...
//82 starts
Migration_Index_View::ExecuteQuery("ALTER TABLE ottocrat_mailscanner CHANGE timezone time_zone VARCHAR(10)", array());
...
//84 starts
$query = "ALTER table ottocrat_relcriteria modify comparator varchar(20)";
Migration_Index_View::ExecuteQuery($query, array());

//To copy imagename saved in ottocrat_attachments for products and contacts into respectively base table
//to support filters on imagename field
$productIdSql = 'SELECT productid,name FROM ottocrat_seattachmentsrel INNER JOIN ottocrat_attachments ON
                                        ottocrat_seattachmentsrel.attachmentsid = ottocrat_attachments.attachmentsid INNER JOIN ottocrat_products ON
                                        ottocrat_products.productid = ottocrat_seattachmentsrel.crmid';
...
foreach ($productImageMap as $productId => $imageNames) {
        ...
        Migration_Index_View::ExecuteQuery('UPDATE ottocrat_products SET imagename = ? WHERE productid = ?',array($implodedNames,$productId));
}
echo 'updating image information for products table is completed';

$ContactIdSql = 'SELECT contactid,name FROM ottocrat_seattachmentsrel INNER JOIN ottocrat_attachments ON
                                        ottocrat_seattachmentsrel.attachmentsid = ottocrat_attachments.attachmentsid INNER JOIN ottocrat_contactdetails ON
                                        ottocrat_contactdetails.contactid = ottocrat_seattachmentsrel.crmid';
...
for ($i = 0; $i < $numOfRows; $i++) {
        ...
        Migration_Index_View::ExecuteQuery('UPDATE ottocrat_contactdetails SET imagename = ? WHERE contactid = ?',array($imageName,$contactId));
}
...
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_relatedlists SET actions=? WHERE label=? and tabid=? ",array('ADD,SELECT', 'PriceBooks', $productsTabId));
echo '<br>Updated PriceBooks related list actions for products and services';

$adb->pquery("CREATE TABLE IF NOT EXISTS ottocrat_schedulereports(
            reportid INT(10),
            scheduleid INT(3),
            recipients TEXT,
            schdate VARCHAR(20),
...
Ottocrat_Cron::register('ScheduleReports', 'cron/modules/Reports/ScheduleReports.service', 900);

Migration_Index_View::ExecuteQuery('UPDATE ottocrat_cron_task set description = ?  where name = "ScheduleReports" ', array("Recommended frequency for ScheduleReports is 15 mins"));
Migration_Index_View::ExecuteQuery('UPDATE ottocrat_cron_task set module = ? where name = "ScheduleReports" ', array("Reports"));
...
$module = Ottocrat_Module::getInstance('Users');
  if ($module) {
      $blockInstance = Ottocrat_Block::getInstance('LBL_CALENDAR_SETTINGS', $module);
      if ($blockInstance) {
          $desField = Ottocrat_Field::getInstance('defaulteventstatus', $module);
          if(!$desField) {
          ...
          $datField = Ottocrat_Field::getInstance('defaultactivitytype', $module);
          if(!$datField) {
          $fieldInstance1 = new Ottocrat_Field();
...
//85 starts
Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_account ALTER isconvertedfromlead SET DEFAULT ?', array('0'));
Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_contactdetails ALTER isconvertedfromlead SET DEFAULT ?', array('0'));
Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_potential ALTER isconvertedfromlead SET DEFAULT ?', array('0'));
Migration_Index_View::ExecuteQuery('Update ottocrat_account SET isconvertedfromlead = ? where isconvertedfromlead is NULL',array('0'));
Migration_Index_View::ExecuteQuery('Update ottocrat_contactdetails SET isconvertedfromlead = ? where isconvertedfromlead is NULL',array('0'));
Migration_Index_View::ExecuteQuery('Update ottocrat_potential SET isconvertedfromlead = ? where isconvertedfromlead is NULL',array('0'));
...
//87 starts
$result = $adb->pquery('SELECT 1 FROM ottocrat_currencies WHERE currency_name = ?', array('Haiti, Gourde'));
if(!$adb->num_rows($result)) {
                    Migration_Index_View::ExecuteQuery('INSERT INTO ottocrat_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
                    array($adb->getUniqueID('ottocrat_currencies'), 'Haiti, Gourde', 'HTG', 'G'));
}
//87 ends   

//88 starts
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_currencies SET currency_symbol=? WHERE currency_code=?", array('₹','INR'));
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_currency_info SET currency_symbol=? WHERE currency_code=?", array('₹','INR'));

Migration_Index_View::ExecuteQuery('UPDATE ottocrat_projecttaskstatus set presence = 0 where projecttaskstatus in (?,?,?,?,?)',
...
$updateQuery = "UPDATE ottocrat_field SET sequence = CASE fieldname ";
...
$query = "SELECT tabid FROM ottocrat_def_org_share";
...
if(!$exist){
        $ruleid = $adb->getUniqueID('ottocrat_def_org_share');
        $shareaccessquery = "INSERT INTO ottocrat_def_org_share VALUES(?,?,?,?)";
...
//92 starts
$result = $adb->pquery('SELECT max(templateid) AS maxtemplateid FROM ottocrat_emailtemplates', array());
Migration_Index_View::ExecuteQuery('UPDATE ottocrat_emailtemplates_seq SET id = ?', array(1 + ((int)$adb->query_result($result, 0, 'maxtemplateid'))));

 $result = $adb->pquery("SELECT 1 FROM ottocrat_eventhandlers WHERE event_name=? AND handler_class=?",
                                    array('ottocrat.entity.aftersave','Ottocrat_RecordLabelUpdater_Handler'));
if($adb->num_rows($result) <= 0) {
    $lastMaxCRMId = 0;
    do {
        $rs = $adb->pquery("SELECT crmid,setype FROM ottocrat_crmentity WHERE crmid > ? LIMIT 500", array($lastMaxCRMId));
        if (!$adb->num_rows($rs)) {
            break;
        }
    ...
    Ottocrat_Event::register($homeModule, 'ottocrat.entity.aftersave', 'Ottocrat_RecordLabelUpdater_Handler', 'modules/Ottocrat/handlers/RecordLabelUpdater.php');
...
$updateQuery = "update ottocrat_campaignrelstatus set campaignrelstatus=? where campaignrelstatus=?";
...
//94 starts
$result = $adb->pquery('SELECT 1 FROM ottocrat_currencies WHERE currency_name = ?', array('Libya, Dinar'));
if(!$adb->num_rows($result)) {
        Migration_Index_View::ExecuteQuery('INSERT INTO ottocrat_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
        array($adb->getUniqueID('ottocrat_currencies'), 'Libya, Dinar', 'LYD', 'LYD'));
}

//Start: Customer - Feature #17656 Allow users to add/remove date format with the date fields in workflow send mail task.
$fieldResult = $adb->pquery('SELECT fieldname, name, typeofdata FROM ottocrat_field
                                INNER JOIN ottocrat_tab ON ottocrat_tab.tabid = ottocrat_field.tabid WHERE typeofdata LIKE ?', array('D%'));
...
$taskIdsList = array();
$result = $adb->pquery('SELECT task_id, module_name FROM com_ottocrat_workflowtasks
                        INNER JOIN com_ottocrat_workflows ON com_ottocrat_workflows.workflow_id = com_ottocrat_workflowtasks.workflow_id
...
$timeZone = '($(general : (__OttocratMeta__) usertimezone))';
foreach ($taskIdsList as $taskId => $taskModuleName) {
        ...
        $properties = get_object_vars($task);
        foreach ($properties as $propertyName => $propertyValue) {
                $propertyValue = str_replace('$(general : (__OttocratMeta__) date)', "(general : (__OttocratMeta__) date) $dateFormat", $propertyValue);
...
// To update ottocrat_modcomments table for permormance issue
$datatypeQuery = "ALTER TABLE ottocrat_modcomments MODIFY COLUMN related_to int(19)";
...
echo '</br>';
$indexQuery = "ALTER TABLE ottocrat_modcomments ADD INDEX relatedto_idx (related_to)";
...
$maxActionIdResult = $adb->pquery('SELECT MAX(actionid) AS maxid FROM ottocrat_actionmapping', array());
$maxActionId = $adb->query_result($maxActionIdResult, 0, 'maxid');
Migration_Index_View::ExecuteQuery('INSERT INTO ottocrat_actionmapping(actionid, actionname, securitycheck) VALUES(?,?,?)', array($maxActionId+1 ,'Print', '0'));
echo "<br> added print to ottocrat_actionnmapping";
$module = Ottocrat_Module_Model::getInstance('Reports');
...
//95 starts
Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_webforms MODIFY COLUMN description TEXT',array());
require_once 'vtlib/Ottocrat/Module.php';
$module = Ottocrat_Module::getInstance('Users');
if ($module) {
    $blockInstance = Ottocrat_Block::getInstance('LBL_CALENDAR_SETTINGS', $module);
    if ($blockInstance) {
        $hideCompletedField = Ottocrat_Field::getInstance('hidecompletedevents', $module);
...
$entityModulesModels = Ottocrat_Module_Model::getEntityModules();
$modules = array();
...
foreach($modules as $module){
    ...
    if($moduleInstance){
        $result = Migration_Index_View::ExecuteQuery("select blocklabel from ottocrat_blocks where tabid=? and sequence = ?", array($moduleInstance->id, 1));
        ...
        if($block){
            ...
            $field->table = 'ottocrat_crmentity';
...
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET presence=0 WHERE fieldname='unit_price' and columnname='unit_price'", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE ottocrat_portal ADD createdtime datetime", array());

$adb->query("CREATE TABLE IF NOT EXISTS ottocrat_calendar_default_activitytypes (id INT(19), module VARCHAR(50), fieldname VARCHAR(50), defaultcolor VARCHAR(50));");

$result = Migration_Index_View::ExecuteQuery('SELECT * FROM ottocrat_calendar_default_activitytypes', array());
if ($adb->num_rows($result) <= 0) {
        ...
        foreach($calendarViewTypes as $module=>$viewInfo) {
                foreach($viewInfo as $fieldname=>$color) {
                        Migration_Index_View::ExecuteQuery('INSERT INTO ottocrat_calendar_default_activitytypes (id, module, fieldname, defaultcolor) VALUES (?,?,?,?)', array($adb->getUniqueID('ottocrat_calendar_default_activitytypes'), $module, $fieldname, $color));
                }
        }
        echo '<br>Default Calendar view types added to the table.<br>';
}
$adb->query("CREATE TABLE IF NOT EXISTS ottocrat_calendar_user_activitytypes (id INT(19), defaultid INT(19), userid INT(19), color VARCHAR(50), visible INT(19) default 1);");

$result = Migration_Index_View::ExecuteQuery('SELECT * FROM ottocrat_calendar_user_activitytypes', array());
if ($adb->num_rows($result) <= 0) {
    $queryResult = Migration_Index_View::ExecuteQuery('SELECT id, defaultcolor FROM ottocrat_calendar_default_activitytypes', array());
    ...
    foreach($allUsers as $userId=>$userModel) {
            foreach($activityIds as $activityId=>$color) {
                   Migration_Index_View::ExecuteQuery('INSERT INTO ottocrat_calendar_user_activitytypes (id, defaultid, userid, color) VALUES (?,?,?,?)', array($adb->getUniqueID('ottocrat_calendar_user_activitytypes'), $activityId, $userId, $color));
            }
    }
    echo '<br>Default Calendar view types added to the table for all existing users';
}

Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET quickcreate = ? WHERE tabid = 8 AND (fieldname = ? OR fieldname = ?);", array(0,"filename","filelocationtype"));
    ...
    if($entityModulesModels){
        foreach($entityModulesModels as $moduleInstance){
            if($moduleInstance){
                ...
                $fieldInstance = Ottocrat_Field::getInstance($fieldNameToDelete,$moduleInstance);
                if($fieldInstance){
...
//97 starts
    ...
    Migration_Index_View::ExecuteQuery('DELETE FROM ottocrat_eventhandlers WHERE handler_path IN ('.generateQuestionMarks($handlers) .')', $handlers);

    //delete modtracker detail view links
    Migration_Index_View::ExecuteQuery('DELETE FROM ottocrat_links WHERE linktype = ? AND handler_class = ? AND linkurl like "javascript:ModTrackerCommon.showhistory%"',
                    array('DETAILVIEWBASIC', 'ModTracker'));

    //Added New field in mailmanager
    Migration_Index_View::ExecuteQuery('ALTER TABLE ottocrat_mail_accounts ADD COLUMN sent_folder VARCHAR(50)', array());
...
if(!defined('INSTALLATION_MODE')) {
    $moduleInstance = Ottocrat_Module_Model::getInstance('PBXManager');
    if(!$moduleInstance){ 
       echo '<br>Installing PBX Manager starts<br>'; 
       installVtlibModule('PBXManager', 'packages/ottocrat/mandatory/PBXManager.zip'); 
    }else{ 
        $result = $adb->pquery('SELECT server, port FROM ottocrat_asterisk', array());
        ...
        foreach ($modules as $module) {
            ...
            //Data migrate from old columns to new columns in ottocrat_pbxmanager 
            $query = 'SELECT * FROM ottocrat_pbxmanager';
            $result = $adb->pquery($query, array());
            $params = array();
            ...
            for ($i = 0; $i < $rowCount; $i++) {
                ...
                $userIdQuery = $adb->pquery('SELECT userid FROM ottocrat_asteriskextensions WHERE asterisk_extension = ?', array($callto));
                ...
                //Update query 
                $adb->pquery('UPDATE ottocrat_pbxmanager SET customer = ? AND user = ? AND totalduration = ? AND callstatus = ? WHERE pbxmanagerid = ?', array($customer, $user, $timeofcall, $callstatus, $pbxmanagerid));
            }
            ...
            $extensionResult = $adb->pquery('SELECT userid, asterisk_extension FROM ottocrat_asteriskextensions', array());
            for ($i = 0; $i < $adb->num_rows($extensionResult); $i++) {
                ...
                $adb->pquery('UPDATE ottocrat_users SET phone_crm_extension = ? WHERE id = ?', array($extensionNumber, $userId));
            }
            ...
            Ottocrat_Link::addLink(0, $headerScriptLinkType, $incominglinkLabel, 'modules/PBXManager/resources/PBXManagerJS.js', '', '', $handlerInfo);
            ...
            //Existing Asterisk extension block removed from ottocrat_users if exist 
            $moduleInstance = Ottocrat_Module_Model::getInstance('Users');
...
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET presence=? WHERE tabid=? AND fieldname=?;", array(1, $tabId, "callfrom"));
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET presence=? WHERE tabid=? AND fieldname=?;", array(1, $tabId, "callto"));
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET presence=? WHERE tabid=? AND fieldname=?;", array(1, $tabId, "timeofcall"));
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET presence=? WHERE tabid=? AND fieldname=?;", array(1, $tabId, "status"));
echo '<br>Hiding previous PBXManager fields done.<br>'; 
//PBXManager porting ends.

//Making document module fields masseditable
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"notes_title")); 
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"assigned_user_id")); 
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"notecontent")); 
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"fileversion")); 
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"filestatus")); 
Migration_Index_View::ExecuteQuery("UPDATE ottocrat_field SET masseditable = ? WHERE tabid = 8 AND fieldname = ?;", array(1,"folderid")); 

//Add Vat ID to Company Details 
Ottocrat_Utils::AddColumn('ottocrat_organizationdetails', 'vatid', 'VARCHAR(100)');

//Add Column trial for ottocrat_tab table if not exists
$result = $adb->pquery("SHOW COLUMNS FROM ottocrat_tab LIKE ?", array('trial'));
if (!($adb->num_rows($result))) {
    $adb->pquery("ALTER TABLE ottocrat_tab ADD trial INT(1) NOT NULL DEFAULT 0",array());
}

##--http://trac.ottocrat.com/cgi-bin/trac.cgi/ticket/7635--##
...
$relatedToQuery = "SELECT fieldid FROM ottocrat_field WHERE tabid=? AND fieldname=?";
$calendarInstance = Ottocrat_Module::getInstance('Calendar');
$tabId = $calendarInstance->getId();
$result = $adb->pquery($relatedToQuery, array($tabId, 'parent_id'));
$fieldId = $adb->query_result($result,0, 'fieldid');
$insertQuery = "INSERT INTO ottocrat_fieldmodulerel (fieldid,module,relmodule,status,sequence) VALUES(?,?,?,?,?)";
$relModule = 'Calendar';
foreach ($moduleArray as $module) {
    $adb->pquery($insertQuery, array($fieldId, $module, $relModule, NULL, NULL));
}
//For contacts the fieldname is contact_id
$contactsRelatedToQuery = "SELECT fieldid FROM ottocrat_field WHERE tabid=? AND fieldname=?";
...
$insertContactsQuery = "INSERT INTO ottocrat_fieldmodulerel (fieldid,module,relmodule,status,sequence) VALUES(?,?,?,?,?)";
$module = 'Contacts';
...
//Adding is_owner to existing ottocrat users

$usersModuleInstance = Ottocrat_Module::getInstance('Users');
$usersBlockInstance = Ottocrat_Block::getInstance('LBL_USERLOGIN_ROLE', $usersModuleInstance);

$usersFieldInstance = Ottocrat_Field::getInstance('is_owner', $usersModuleInstance);
...
$adb = PearDatabase::getInstance();
$idResult = $adb->pquery('SELECT id FROM ottocrat_users WHERE is_admin = ? AND status=?', array('on', 'Active'));
if ($adb->num_rows($idResult) > 0) {
    for($i = 0;$i<=$adb->num_rows($idResult);$i++) {
        $userid = $adb->query_result($idResult, $i, 'id');
        $adb->pquery('UPDATE ottocrat_users SET is_owner=? WHERE id=?', array(1, $userid));
        echo '<br>Account Owner Informnation saved in ottocrat';
...
Migration_Index_View::ExecuteQuery("CREATE TABLE IF NOT EXISTS ottocrat_reporttype(
                        reportid INT(10),
                        data text,
						PRIMARY KEY (`reportid`),
						CONSTRAINT `fk_1_ottocrat_reporttype` FOREIGN KEY (`reportid`) REFERENCES `ottocrat_report` (`reportid`) ON DELETE CASCADE)
                        ENGINE=InnoDB DEFAULT CHARSET=utf8;", array()); 

//Configuration Editor fix
$sql = "UPDATE ottocrat_settings_field SET name = ? WHERE name = ?";
