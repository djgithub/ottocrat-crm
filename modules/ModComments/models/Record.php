 ...
 */
class ModComments_Record_Model extends Ottocrat_Record_Model {
	...
	public function getImagePath() {
		...
		if($commentor) {
			...
			if (!empty($customer) && $isMailConverterType != 1) {
                $recordModel = Ottocrat_Record_Model::getInstanceById($customer);
	...
	public static function getInstanceById($record) {
		...
		$result = $db->pquery('SELECT ottocrat_modcomments.*, ottocrat_crmentity.smownerid,
					ottocrat_crmentity.createdtime, ottocrat_crmentity.modifiedtime FROM ottocrat_modcomments
	...
	public function getParentRecordModel() {
		...
		return Ottocrat_Record_Model::getInstanceById($parentRecordId);
	...
	public function getCommentedByModel() {
		$customer = $this->get('customer');
		if(!empty($customer)) {
			return Ottocrat_Record_Model::getInstanceById($customer, 'Contacts');
		} else {
			$commentedBy = $this->get('smownerid');
			if($commentedBy) {
			    $commentedByModel = Ottocrat_Record_Model::getInstanceById($commentedBy, 'Users');
			    if(empty($commentedByModel->entity->column_fields['user_name'])) {
				$activeAdmin = Users::getActiveAdminUser();
				$commentedByModel = Ottocrat_Record_Model::getInstanceById($activeAdmin->id, 'Users');
			    }
	 ...
	 * @param <Ottocrat_Paging_Model> - paging model
	...
	public static function getRecentComments($parentRecordId, $pagingModel){
		...
		$listView = Ottocrat_ListView_Model::getInstance('ModComments');
		...
		$query = $query ." AND related_to = ? ORDER BY ottocrat_crmentity.createdtime DESC
	...
	public static function getAllParentComments($parentId) {
		...
		$query = $query ." AND related_to = ? AND parent_comments = '' ORDER BY ottocrat_crmentity.createdtime DESC";
	...
	public function getChildCommentsCount() {
		...
		$query = 'SELECT 1 FROM ottocrat_modcomments WHERE parent_comments = ? AND related_to = ?';
