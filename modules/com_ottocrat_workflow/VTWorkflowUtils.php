...
<?php
	...
	function checkModuleWorkflow($modulename) {
		...
		$query = "SELECT name FROM ottocrat_tab WHERE name not in (" . generateQuestionMarks($modules_not_supported) . ") AND isentitytype=1 AND presence = 0 AND tabid = ?";
	...
	function vtGetModules($adb) {
		...
		$sql = "select distinct ottocrat_field.tabid, name
			...
			where ottocrat_tab.name not in(" . generateQuestionMarks($modules_not_supported) . ") and ottocrat_tab.isentitytype=1 and ottocrat_tab.presence in (0,2) ";
