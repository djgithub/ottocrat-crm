...
class Documents_Module_Model extends Ottocrat_Module_Model {
	...
	public static function getAllFolders() {
		...
		$result = $db->pquery('SELECT * FROM ottocrat_attachmentsfolder ORDER BY sequence', array());
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		if($sourceModule === 'Emails' && $field === 'composeEmail') {
			$condition = ' (( ottocrat_notes.filelocationtype LIKE "%I%")) AND ottocrat_notes.filename != "" AND ottocrat_notes.filestatus = 1';
		} else {
			$condition = " ottocrat_notes.notesid NOT IN (SELECT notesid FROM ottocrat_senotesrel WHERE crmid = '$record') AND ottocrat_notes.filestatus = 1";
		}
    ...
    public function getSettingLinks() {
		...
		$settingsLinks[] = array(
				...
				'linkurl' => Ottocrat_Request:: encryptLink('index.php?parent=Settings&module=Picklist&source_module='.$this->getName()),
				'linkicon' => ''
		);
