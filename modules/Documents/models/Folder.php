...
class Documents_Folder_Model extends Ottocrat_Base_Model {
	...
	public function checkDuplicate() {
		...
		$result = $db->pquery("SELECT 1 FROM ottocrat_attachmentsfolder WHERE foldername = ?", array($folderName));
	...
	public function hasDocuments() {
		...
		$result = $db->pquery("SELECT 1 FROM ottocrat_notes
						INNER JOIN ottocrat_attachmentsfolder ON ottocrat_attachmentsfolder.folderid = ottocrat_notes.folderid
						INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_notes.notesid
						WHERE ottocrat_attachmentsfolder.folderid = ?
						AND ottocrat_attachmentsfolder.foldername != 'Default'
						AND ottocrat_crmentity.deleted = 0", array($folderId));
		$num_rows = $db->num_rows($result);
		if ($num_rows>0) {
			return true;
		}
	...
	public function save() {
		...
		$result = $db->pquery("SELECT max(sequence) AS max, max(folderid) AS max_folderid FROM ottocrat_attachmentsfolder", array());
		...
		$db->pquery("INSERT INTO ottocrat_attachmentsfolder(folderid,foldername, description, createdby, sequence) VALUES(?, ?, ?, ?, ?)", $params);
	...
	public function delete() {
		$db = PearDatabase::getInstance();
		$folderId = $this->getId();
		$result = $db->pquery("DELETE FROM ottocrat_attachmentsfolder WHERE folderid = ? AND foldername != 'Default'", array($folderId));
		return $this;
	}
	...
	public static function getInstanceById($folderId) {
		...
		$result = $db->pquery("SELECT * FROM ottocrat_attachmentsfolder WHERE folderid = ?", array($folderId));
		$num_rows = $db->num_rows($result);
		if ($num_rows > 0) {
	...
	public function getDeleteUrl() {
		$folderName = $this->getName();
		return Ottocrat_Request:: encryptLink( "index.php?module=Documents&action=Folder&mode=delete&foldername=$folderName");
	}
