...
class Events_Record_Model extends Calendar_Record_Model {
	...
	public function getEditViewUrl() {
		...
		return Ottocrat_Request:: encryptLink('index.php?module=Calendar&view='.$module->getEditViewName().'&record='.$this->getId());
	}
	...
	public function getDeleteUrl() {
		$module = $this->getModule();
		return Ottocrat_Request:: encryptLink('index.php?module=Calendar&action='.$module->getDeleteActionName().'&record='.$this->getId());
	}
    ...
    public function getDuplicateRecordUrl(){
        $module = $this->getModule();
		return Ottocrat_Request:: encryptLink('index.php?module=Calendar&view='.$module->getEditViewName().'&record='.$this->getId().'&isDuplicate=true');
    ...
    public function getRelatedContactInfo() {
        ...
        foreach($contactIdList as $contactId) {
            $relatedContactInfo[] = array('name' => Ottocrat_Util_Helper::getRecordName($contactId) ,'id' => $contactId);
        }
     ...
     public function getInvities() {
         $adb = PearDatabase::getInstance();
         $sql = "select ottocrat_invitees.* from ottocrat_invitees where activityid=?";
     ...
     public function getInviteUserMailData() {
            ...
            $cont_qry = "select * from ottocrat_cntactivityrel where activityid=?";
            ...
            foreach($cont_id as $key=>$id) {
                if($id != '') {
                    $contact_name = Ottocrat_Util_Helper::getRecordName($id);
			...
			$parentName = '';
			if($parentId != '') {
				$parentName = Ottocrat_Util_Helper::getRecordName($parentId);
			}
