...
class Import_FileReader_Reader {
	...
	public function  __construct($request, $user) {
		...
		$this->user = $user;
        $this->moduleModel = Ottocrat_Module_Model::getInstance($this->request->get('module'));
	}
	 ...
	 * @param $fieldObject <Ottocrat_Field_Model>
    ...
    public function getModuleFieldDBColumnType() {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT tablename FROM ottocrat_field WHERE tabid=? GROUP BY tablename', array($this->moduleModel->getId()));
        $tables = array();
