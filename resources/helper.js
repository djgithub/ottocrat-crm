...
jQuery.Class("Ottocrat_Helper_Js",{
	...
	requestToShowComposeEmailForm : function(selectedId,fieldname,fieldmodule){
		...
		var emailsMassEditInstance = Ottocrat_Helper_Js.getEmailMassEditInstance();
	...
	getInternalMailer  : function(selectedId,fieldname,fieldmodule){
		var module = 'Emails';
		var cacheResponse = Ottocrat_Helper_Js.checkServerConfigResponseCache;
		var  checkServerConfigPostOperations = function (data) {
			if(data == true){
				Ottocrat_Helper_Js.requestToShowComposeEmailForm(selectedId,fieldname,fieldmodule);
			} else {
				alert(app.vtranslate('JS_EMAIL_SERVER_CONFIGURATION'));
			}
		}
		if(cacheResponse === ''){
			var checkServerConfig = Ottocrat_Helper_Js.checkServerConfig(module);
			checkServerConfig.then(function(data){
				Ottocrat_Helper_Js.checkServerConfigResponseCache = data;
				checkServerConfigPostOperations(Ottocrat_Helper_Js.checkServerConfigResponseCache);
			});
		} else {
			checkServerConfigPostOperations(Ottocrat_Helper_Js.checkServerConfigResponseCache);
		}
