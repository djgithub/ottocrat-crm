 ...
 * @package vtlib
 */
class Ottocrat_Mailer extends PHPMailer {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('ottocrat_mailer_queue');
	}
	...
	function initialize() {
		...
		global $adb;
		$result = $adb->pquery("SELECT * FROM ottocrat_systems WHERE server_type=?", Array('email'));
...
//			$this->Sender= getReturnPath($this->Host);
	...
	function initFromTemplate($emailtemplate) {
		global $adb;
		$result = $adb->pquery("SELECT * from ottocrat_emailtemplates WHERE templatename=? AND foldername=?",
	...
	*/
	function addSignature($userId) {
		global $adb;
		$sign = nl2br($adb->query_result($adb->pquery("select signature from ottocrat_users where id=?", array($userId)),0,"signature"));
		$this->Signature = $sign;
	}
	...
	function ConfigSenderInfo($fromemail, $fromname='', $replyto='') {
		...
		$this->From = $fromemail;
		//fix for (http://trac.ottocrat.com/cgi-bin/trac.cgi/ticket/8001)
	 ...
	 * @param String ottocrat CRM Email template name to use
	 */
	...
	function __initializeQueue() {
		if(!$this->_queueinitialized) {
			if(!Ottocrat_Utils::CheckTable('ottocrat_mailer_queue')) {
				Ottocrat_Utils::CreateTable('ottocrat_mailer_queue',
			...
			if(!Ottocrat_Utils::CheckTable('ottocrat_mailer_queueinfo')) {
				Ottocrat_Utils::CreateTable('ottocrat_mailer_queueinfo',
					'(id INTEGER, name VARCHAR(100), email VARCHAR(100), type VARCHAR(7))',
					true);
			}
			if(!Ottocrat_Utils::CheckTable('ottocrat_mailer_queueattachments')) {
				Ottocrat_Utils::CreateTable('ottocrat_mailer_queueattachments',
	...
	function __AddToQueue($linktoid) {
		if($this->__initializeQueue()) {
			...
			$adb->pquery('INSERT INTO ottocrat_mailer_queue(id,fromname,fromemail,content_type,subject,body,mailer,relcrmid) VALUES(?,?,?,?,?,?,?,?)',
			...
			foreach($this->to as $toinfo) {
				...
				$adb->pquery('INSERT INTO ottocrat_mailer_queueinfo(id, name, email, type) VALUES(?,?,?,?)',
			...
			foreach($this->attachment as $attachmentinfo) {
				...
				$adb->pquery('INSERT INTO ottocrat_mailer_queueattachments(id, path, name, encoding, type) VALUES(?,?,?,?,?)',
	 ...
	 */
	static function dispatchQueue(Ottocrat_Mailer_Listener $listener=null) {
		global $adb;
		if(!Ottocrat_Utils::CheckTable('ottocrat_mailer_queue')) return;

		$mailer = new self();
		$queue = $adb->pquery('SELECT * FROM ottocrat_mailer_queue WHERE failed != ?', array(1));
		if($adb->num_rows($queue)) {
			for($index = 0; $index < $adb->num_rows($queue); ++$index) {
				...
				$emails = $adb->pquery('SELECT * FROM ottocrat_mailer_queueinfo WHERE id=?', Array($queueid));
				...
				$attachments = $adb->pquery('SELECT * FROM ottocrat_mailer_queueattachments WHERE id=?', Array($queueid));
				...
				if($sent) {
					Ottocrat_Event::trigger('ottocrat.mailer.mailsent', $relcrmid);
					if($listener) {
						$listener->mailsent($queueid);
					}
					$adb->pquery('DELETE FROM ottocrat_mailer_queue WHERE id=?', Array($queueid));
					$adb->pquery('DELETE FROM ottocrat_mailer_queueinfo WHERE id=?', Array($queueid));
					$adb->pquery('DELETE FROM ottocrat_mailer_queueattachments WHERE id=?', Array($queueid));
				} else {
					if($listener) {
						$listener->mailerror($queueid);
					}
					$adb->pquery('UPDATE ottocrat_mailer_queue SET failed=?, failreason=? WHERE id=?', Array(1, $mailer->ErrorInfo, $queueid));
				}
			}
		}
	}
 ...
 * @package vtlib
 */
abstract class Ottocrat_Mailer_Listener {
