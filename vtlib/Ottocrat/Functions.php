...
class Ottocrat_Functions {
	...
	static function getTranslatedString($str, $module = '') {
		return Ottocrat_Language_Handler::getTranslatedString($str, $module);
	}
	...
	static function userCurrencyId($userid) {
		...
		if (!isset(self::$userIdCurrencyIdCache[$userid])) {
			$result = $adb->pquery('SELECT id,currency_id FROM ottocrat_users', array());
	...
	protected static function getCurrencyInfo($currencyid) {
		...
		if (!isset(self::$currencyInfoCache[$currencyid])) {
			$result = $adb->pquery('SELECT * FROM ottocrat_currency_info', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getCurrencyName($currencyid, $show_symbol = true) {
		...
		if ($show_symbol) {
			return sprintf("%s : %s", Ottocrat_Deprecated::getTranslatedCurrencyString($currencyInfo['currency_name']), $currencyInfo['currency_symbol']);
		}
	...
	protected static function getBasicModuleInfo($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT tabid, name, ownedby FROM ottocrat_tab', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleData($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT * FROM ottocrat_tab', array());
			while ($row = $adb->fetch_array($result)) {
				self::$moduleIdNameCache[$row['tabid']] = $row;
				self::$moduleNameIdCache[$row['name']]  = $row;
	...
	static function getEntityModuleInfo($mixed) {
		...
		if ($name && !isset(self::$moduleEntityCache[$name])) {
			global $adb;
			$result = $adb->pquery('SELECT fieldname,modulename,tablename,entityidfield,entityidcolumn from ottocrat_entityname', array());
			while ($row = $adb->fetch_array($result)) {
	...
	protected static function getCRMRecordMetadata($mixedid) {
		...
		if ($missing) {
			$sql = sprintf("SELECT crmid, setype, label FROM ottocrat_crmentity WHERE %s", implode(' OR ', array_fill(0, count($missing), 'crmid=?')));
	...
	static function updateCRMRecordLabel($module, $id, $label) {
		...
		if ($labelInfo) {
			...
			$adb->pquery('UPDATE ottocrat_crmentity SET label=? WHERE crmid=?', array($label, $id));
	...
	static function computeCRMRecordLabels($module, $ids) {
		...
		if ($module) {
			...
			if ($ids) {
				...
				if ($module == 'Groups') {
					$metainfo = array('tablename' => 'ottocrat_groups','entityidfield' => 'groupid','fieldname' => 'groupname');
				} else if ($module == 'DocumentFolders') { 
                                        $metainfo = array('tablename' => 'ottocrat_attachmentsfolder','entityidfield' => 'folderid','fieldname' => 'foldername'); 
                                } else {
	...
	static function getGroupName($id) {
		...
		if (!self::$groupIdNameCache[$id]) {
			$result = $adb->pquery('SELECT groupid, groupname FROM ottocrat_groups');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getUserName($id) {
		...
		if (!self::$userIdNameCache[$id]) {
			$result = $adb->pquery('SELECT id, user_name FROM ottocrat_users');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleFieldInfos($mixed) {
		...
		$module = $moduleInfo['name'];
                ...
                $no_of_fields = $adb->pquery('SELECT COUNT(fieldname) FROM ottocrat_field WHERE tabid=?',array(self::getModuleId($module)));
		...
		if ($module && (!isset(self::$moduleFieldInfoByNameCache[$module]) || ((int)$fields_count != (int)$cached_fields_count))) {
			$result =
				($module == 'Calendar')?
				$adb->pquery('SELECT * FROM ottocrat_field WHERE tabid=? OR tabid=?', array(9, 16)) :
				$adb->pquery('SELECT * FROM ottocrat_field WHERE tabid=?', array(self::getModuleId($module)));
	...
	static function getModuleFieldInfoWithId($fieldid) {
		global $adb;
		$result = $adb->pquery('SELECT * FROM ottocrat_field WHERE fieldid=?', array($fieldid));
	...
	static function getInventoryTermsAndCondition() {
		global $adb;
		$sql = "select tandc from ottocrat_inventory_tandc";
	...
	static function getTicketComments($ticketid) {
		...
		$commentlist = '';
		$sql = "SELECT commentcontent FROM ottocrat_modcomments WHERE related_to = ?";
	...
	static function getTagCloudView($id = "") {
		...
		if ($id == '') {
			...
			$query = "select visible from ottocrat_homestuff where userid=? and stufftype='Tag Cloud'";
	...
	static function transformFieldTypeOfData($table_name, $column_name, $type_of_data) {
		...
		static $new_field_details = Array(
			...
			"ottocrat_contactdetails:accountid" => "V",
			"ottocrat_contactsubdetails:birthday" => "D",
			"ottocrat_contactdetails:email" => "V",
			"ottocrat_contactdetails:secondaryemail" => "V",
			//Potential Related Fields
			"ottocrat_potential:campaignid" => "V",
			//Account Related Fields
			"ottocrat_account:parentid" => "V",
			"ottocrat_account:email1" => "V",
			"ottocrat_account:email2" => "V",
			//Lead Related Fields
			"ottocrat_leaddetails:email" => "V",
			"ottocrat_leaddetails:secondaryemail" => "V",
			//Documents Related Fields
			"ottocrat_senotesrel:crmid" => "V",
			//Calendar Related Fields
			"ottocrat_seactivityrel:crmid" => "V",
			"ottocrat_seactivityrel:contactid" => "V",
			"ottocrat_recurringevents:recurringtype" => "V",
			//HelpDesk Related Fields
			"ottocrat_troubletickets:parent_id" => "V",
			"ottocrat_troubletickets:product_id" => "V",
			//Product Related Fields
			"ottocrat_products:discontinued" => "C",
			"ottocrat_products:vendor_id" => "V",
			"ottocrat_products:parentid" => "V",
			//Faq Related Fields
			"ottocrat_faq:product_id" => "V",
			//Vendor Related Fields
			"ottocrat_vendor:email" => "V",
			//Quotes Related Fields
			"ottocrat_quotes:potentialid" => "V",
			"ottocrat_quotes:inventorymanager" => "V",
			"ottocrat_quotes:accountid" => "V",
			//Purchase Order Related Fields
			"ottocrat_purchaseorder:vendorid" => "V",
			"ottocrat_purchaseorder:contactid" => "V",
			//SalesOrder Related Fields
			"ottocrat_salesorder:potentialid" => "V",
			"ottocrat_salesorder:quoteid" => "V",
			"ottocrat_salesorder:contactid" => "V",
			"ottocrat_salesorder:accountid" => "V",
			//Invoice Related Fields
			"ottocrat_invoice:salesorderid" => "V",
			"ottocrat_invoice:contactid" => "V",
			"ottocrat_invoice:accountid" => "V",
			//Campaign Related Fields
			"ottocrat_campaign:product_id" => "V",
			//Related List Entries(For Report Module)
			"ottocrat_activityproductrel:activityid" => "V",
			"ottocrat_activityproductrel:productid" => "V",
			"ottocrat_campaigncontrel:campaignid" => "V",
			"ottocrat_campaigncontrel:contactid" => "V",
			"ottocrat_campaignleadrel:campaignid" => "V",
			"ottocrat_campaignleadrel:leadid" => "V",
			"ottocrat_cntactivityrel:contactid" => "V",
			"ottocrat_cntactivityrel:activityid" => "V",
			"ottocrat_contpotentialrel:contactid" => "V",
			"ottocrat_contpotentialrel:potentialid" => "V",
			"ottocrat_pricebookproductrel:pricebookid" => "V",
			"ottocrat_pricebookproductrel:productid" => "V",
			"ottocrat_seactivityrel:crmid" => "V",
			"ottocrat_seactivityrel:activityid" => "V",
			"ottocrat_senotesrel:crmid" => "V",
			"ottocrat_senotesrel:notesid" => "V",
			"ottocrat_seproductsrel:crmid" => "V",
			"ottocrat_seproductsrel:productid" => "V",
			"ottocrat_seticketsrel:crmid" => "V",
			"ottocrat_seticketsrel:ticketid" => "V",
			"ottocrat_vendorcontactrel:vendorid" => "V",
			"ottocrat_vendorcontactrel:contactid" => "V",
			"ottocrat_pricebook:currency_id" => "V",
		);
	...
	static function getPickListValuesFromTableForRole($tablename, $roleid) {
		global $adb;
		$query = "select $tablename from ottocrat_$tablename inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_$tablename.picklist_valueid where roleid=? and picklistid in (select picklistid from ottocrat_picklist) order by sortid";
	...
	static function getActivityType($id) {
		global $adb;
		$query = "select activitytype from ottocrat_activity where activityid=?";
	...
	static function getInvoiceStatus($id) {
		global $adb;
		$result = $adb->pquery("SELECT invoicestatus FROM ottocrat_invoice where invoiceid=?", array($id));
    ...
    */
    static function getUnitPrice($productid, $module='Products') {
        ...
        if($module == 'Services') {
            $query = "select unit_price from ottocrat_service where serviceid=?";
        } else {
            $query = "select unit_price from ottocrat_products where productid=?";
        }
    ...
    /**
    * Function to fetch the list of ottocrat_groups from group ottocrat_table
    * Takes no value as input
    * returns the query result set object
    */
    static function get_group_options() {
        global $adb, $noof_group_rows;
        $sql = "select groupname,groupid from ottocrat_groups";
        $result = $adb->pquery($sql, array());
