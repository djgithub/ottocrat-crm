...
include_once('vtlib/Ottocrat/Zip.php');
include_once('vtlib/Ottocrat/Cron.php');
/**
 * Provides API to package ottocrat CRM module and associated files.
 * @package vtlib
 */
class Ottocrat_PackageExport {
	 ...
	 * Constructor
	 */
	function Ottocrat_PackageExport() {
	...
	function __initExport($module, $moduleInstance) {
		if($moduleInstance->isentitytype) {
			...
			Ottocrat_Utils::checkFileAccessForInclusion("modules/$module/$module.php");
		}
	...
	function export_Dependencies($moduleInstance) {
		...
		$sqlresult = $adb->pquery("SELECT * FROM ottocrat_tab_info WHERE tabid = ?", array($moduleid));
		...
		for($i=0; $i<$noOfPreferences; ++$i) {
			...
			if($prefName == 'ottocrat_min_version') {
				$ottocratMinVersion = $prefValue;
			}
			if($prefName == 'ottocrat_max_version') {
				$ottocratMaxVersion = $prefValue;
			}
	...
	function export_Module($moduleInstance) {
		...
		$sqlresult = $adb->pquery("SELECT * FROM ottocrat_parenttabrel WHERE tabid = ?", array($moduleid));
		$parenttabid = $adb->query_result($sqlresult, 0, 'parenttabid');
		$menu = Ottocrat_Menu::getInstance($parenttabid);
		$parent_name = $menu->label;

		$sqlresult = $adb->pquery("SELECT * FROM ottocrat_tab WHERE tabid = ?", array($moduleid));
	...
	function export_Tables($moduleInstance) {
		...
		if($moduleInstance->isentitytype) {
			...
			foreach($tables as $table) {
				...
				$this->outputNode('<![CDATA['.Ottocrat_Utils::CreateTableSql($table).']]>', 'sql');
	...
	function export_Blocks($moduleInstance) {
		global $adb;
		$sqlresult = $adb->pquery("SELECT * FROM ottocrat_blocks WHERE tabid = ?", Array($moduleInstance->id));
	...
	function export_Fields($moduleInstance, $blockid) {
		...
		$fieldresult = $adb->pquery("SELECT * FROM ottocrat_field WHERE tabid=? AND block=?", Array($moduleInstance->id, $blockid));
		$fieldcount = $adb->num_rows($fieldresult);

		if(empty($fieldcount)) return;

		$entityresult = $adb->pquery("SELECT * FROM ottocrat_entityname WHERE tabid=?", Array($moduleInstance->id));
		...
		for($index = 0; $index < $fieldcount; ++$index) {
			...
			if($uitype == '10') {
				$relatedmodres = $adb->pquery("SELECT * FROM ottocrat_fieldmodulerel WHERE fieldid=?", Array($fieldid));
	...
	function export_CustomViews($moduleInstance) {
		...
		$customviewres = $adb->pquery("SELECT * FROM ottocrat_customview WHERE entitytype = ?", Array($moduleInstance->name));
		...
		for($cvindex = 0; $cvindex < $customviewcount; ++$cvindex) {
			...
			$cvcolumnres = $adb->pquery("SELECT * FROM ottocrat_cvcolumnlist WHERE cvid=?", array($cvid));
			...
			for($index = 0; $index < $cvcolumncount; ++$index) {
				...
				$cvcolumnruleres = $adb->pquery("SELECT * FROM ottocrat_cvadvfilter WHERE cvid=? AND columnname=?",
				...
				if($cvcolumnrulecount) {
					...
					for($rindex = 0; $rindex < $cvcolumnrulecount; ++$rindex) {
						...
						$cvcolumnrulecomp  = Ottocrat_Filter::translateComparator($cvcolumnrulecomp, true);
	...
	function export_SharingAccess($moduleInstance) {
		...
		$deforgshare = $adb->pquery("SELECT * FROM ottocrat_def_org_share WHERE tabid=?", Array($moduleInstance->id));
	...
	function export_Events($moduleInstance) {
		$events = Ottocrat_Event::getAll($moduleInstance);
		if(!$events) return;
	...
	function export_Actions($moduleInstance) {
		...
		global $adb;
		$result = $adb->pquery('SELECT distinct(actionname) FROM ottocrat_profile2utility, ottocrat_actionmapping
			WHERE ottocrat_profile2utility.activityid=ottocrat_actionmapping.actionid and tabid=?', Array($moduleInstance->id));
	...
	function export_RelatedLists($moduleInstance) {
		...
		global $adb;
		$result = $adb->pquery("SELECT * FROM ottocrat_relatedlists WHERE tabid = ?", Array($moduleInstance->id));
		if($adb->num_rows($result)) {
			...
			for($index = 0; $index < $adb->num_rows($result); ++$index) {
				...
				$relModuleInstance = Ottocrat_Module::getInstance($row['related_tabid']);
	...
	function export_CronTasks($moduleInstance){
        $cronTasks = Ottocrat_Cron::listAllInstancesByModule($moduleInstance->name);
        $this->openNode('crons');
