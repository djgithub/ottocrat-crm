...
/**
 * Provides API to work with ottocrat CRM Module Blocks
 * @package vtlib
 */
class Ottocrat_Block {
	...
	function __getUniqueId() {
		...
		$maxblockid = $adb->getUniqueID('ottocrat_blocks');
		return $maxblockid;
	}
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) as max_sequence from ottocrat_blocks where tabid = ?", Array($this->module->id));
		$maxseq = 0;
	 ...
	 * @param Ottocrat_Module Instance of module to which this block is associated
	 * @access private
	 */
	...
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO ottocrat_blocks(blockid,tabid,blocklabel,sequence,show_title,visible,create_view,edit_view,detail_view,iscustom)
	...
	function __delete() {
		...
		$adb->pquery("DELETE FROM ottocrat_blocks WHERE blockid=?", Array($this->id));
		self::log("DONE");
	}

	/**
	 * Save this block instance
	 * @param Ottocrat_Module Instance of the module to which this block is associated
	 */
	...
	function delete($recursive=true) {
		if($recursive) {
			$fields = Ottocrat_Field::getAllForBlock($this);
	 ...
	 * @param Ottocrat_Field Instance of field to add to this block.
	 ...
	 * @param Ottocrat_Module Instance of the module if block label is passed
	 */
	static function getInstance($value, $moduleInstance=false) {
		...
		if($moduleInstance && $cache->getBlockInstance($value, $moduleInstance->id)){
			...
			if(Ottocrat_Utils::isNumber($value)) {
				$query = "SELECT * FROM ottocrat_blocks WHERE blockid=?";
				$queryParams = Array($value);
			} else {
				$query = "SELECT * FROM ottocrat_blocks WHERE blocklabel=? AND tabid=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM ottocrat_blocks WHERE tabid=? ORDER BY sequence";
	 ...
	 * @param Ottocrat_Module Instnace of module to use
	...
	static function deleteForModule($moduleInstance, $recursive=true) {
		global $adb;
		if($recursive) Ottocrat_Field::deleteForModule($moduleInstance);
		$adb->pquery("DELETE FROM ottocrat_blocks WHERE tabid=?", Array($moduleInstance->id));
