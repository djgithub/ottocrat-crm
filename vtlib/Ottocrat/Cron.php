...
class Ottocrat_Cron {
    ...
    function updateStatus($status) {
        ...
        self::querySilent('UPDATE ottocrat_cron_task SET status=? WHERE id=?', array($status, $this->getId()));
    }
    ...
    */
    function updateFrequency($frequency) {
        self::querySilent('UPDATE ottocrat_cron_task SET frequency=? WHERE id=?', array($frequency, $this->getId()));
    }
    ...
    function markRunning() {
        $time = time();
        self::querySilent('UPDATE ottocrat_cron_task SET status=?, laststart=?, lastend=? WHERE id=?', array(self::$STATUS_RUNNING, $time, 0, $this->getId()));
    ...
    function markFinished() {
        $time = time();
        self::querySilent('UPDATE ottocrat_cron_task SET status=?, lastend=? WHERE id=?', array(self::$STATUS_ENABLED, $time, $this->getId()));
    ...
    protected static function initializeSchema() {
        if(!self::$schemaInitialized) {
            if(!Ottocrat_Utils::CheckTable('ottocrat_cron_task')) {
                Ottocrat_Utils::CreateTable('ottocrat_cron_task',
    ...
    static function nextSequence() {
        global $adb;
        $result = self::querySilent('SELECT MAX(sequence) FROM ottocrat_cron_task ORDER BY SEQUENCE');
    ...
    static function register($name, $handler_file, $frequency, $module = 'Home', $status = 1, $sequence = 0, $description = '') {
        ...
        self::querySilent('INSERT INTO ottocrat_cron_task (name, handler_file, frequency, status, sequence,module,description) VALUES(?,?,?,?,?,?,?)',
    ...
    static function deregister($name) {
        self::querySilent('DELETE FROM ottocrat_cron_task WHERE name=?', array($name));
    ...
    static function listAllActiveInstances($byStatus = 0) {
        ...
        $instances = array();
        if($byStatus == 0) {
            $result = self::querySilent('SELECT * FROM ottocrat_cron_task WHERE status <> ? ORDER BY SEQUENCE',array(self::$STATUS_DISABLED   ));
        }
        else {
            $result = self::querySilent('SELECT * FROM ottocrat_cron_task  ORDER BY SEQUENCE');

        }
        if ($result && $adb->num_rows($result)) {
            while ($row = $adb->fetch_array($result)) {
                $instances[] = new Ottocrat_Cron($row);
            }
        }
        return $instances;
    }
    ...
    static function getInstance($name) {
        ...
        if ($instance === false) {
            $result = self::querySilent('SELECT * FROM ottocrat_cron_task WHERE name=?', array($name));
            if ($result && $adb->num_rows($result)) {
                $instance = new Ottocrat_Cron($adb->fetch_array($result));
            }
        }
        return $instance;
    }
    ...
    static function getInstanceById($id) {
        ...
        if ($instance === false) {
            $result = self::querySilent('SELECT * FROM ottocrat_cron_task WHERE id=?', array($id));
            if ($result && $adb->num_rows($result)) {
                $instance = new Ottocrat_Cron($adb->fetch_array($result));
            }
        }
    ...
    static function listAllInstancesByModule($module) {
        ...
        $instances = array();
        $result = self::querySilent('SELECT * FROM ottocrat_cron_task WHERE module=?',array($module));
        if ($result && $adb->num_rows($result)) {
            while ($row = $adb->fetch_array($result)) {
                $instances[] = new Ottocrat_Cron($row);
            }
	...
	public function log($completed = false){
		global $adb;
		 $result = self::querySilent('SELECT id,iteration from ottocrat_cron_log where start = ? AND name=?',array($this->getLastStart(),$this->getName()));
		  if ($result && $adb->num_rows($result) > 0) {
			  $row = $adb->fetch_array($result);
			  if($completed){
				  self::querySilent('UPDATE ottocrat_cron_log set status = ?,end = ? where id = ?',array(self::$STATUS_COMPLETED,time(),$row['id']));
			  } else{

				 self::querySilent('UPDATE ottocrat_cron_log set iteration = ? where id = ?',array($row['iteration']+1,$row['id']));
			  }
		  } else {
			  self::querySilent('INSERT INTO ottocrat_cron_log (name,start,iteration,status) VALUES(?,?,?,?)',
	 ...
	 public function isSentLogMail(){
		 global $adb;
		 $result = self::querySilent('SELECT 1 from ottocrat_cron_log where start = ? AND name=? AND iteration >= 4 ',array($this->getLastStart(),$this->getName()));
		 if ($result && $adb->num_rows($result)) {
			 return true;
		 }	else {
			return false;
	 ...
	 public function getIterations(){
		 global $adb;
		 $result = self::querySilent('SELECT iteration from ottocrat_cron_log where start = ? AND name=?',array($this->getLastStart(),$this->getName()));
		 if ($result && $adb->num_rows($result)) {
			 $row = $adb->fetch_array($result);
	 ...
	 public function getCompletedTime(){
		 global $adb;
		 $result = self::querySilent('SELECT start,end from ottocrat_cron_log where start = ? AND name=?',array($this->getLastStart(),$this->getName()));
		 if ($result && $adb->num_rows($result)) {
			$row = $adb->fetch_array($result);
