...
/**
 * Provides API to work with ottocrat CRM Custom View (Filter)
 * @package vtlib
 */
class Ottocrat_Filter {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('ottocrat_customview');
	}

	/**
	 * Initialize this filter instance
	 * @param Ottocrat_Module Instance of the module to which this filter is associated.
	 * @access private
	 */
	function initialize($valuemap, $moduleInstance=false) {
		$this->id = $valuemap[cvid];
		$this->name= $valuemap[viewname];
		$this->module=$moduleInstance? $moduleInstance: Ottocrat_Module::getInstance($valuemap[tabid]);
	}

	/**
	 * Create this instance
	 * @param Ottocrat_Module Instance of the module to which this filter should be associated with
	 * @access private
	 */
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO ottocrat_customview(cvid,viewname,setdefault,setmetrics,entitytype) VALUES(?,?,?,?,?)", 
		...
		if(!$this->status) {
			...
			$adb->pquery("UPDATE ottocrat_customview SET status=? WHERE cvid=?", Array($this->status, $this->id));
	...
	function __delete() {
		global $adb;
		$adb->pquery("DELETE FROM ottocrat_cvadvfilter WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM ottocrat_cvcolumnlist WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM ottocrat_customview WHERE cvid=?", Array($this->id));
	}
	 ...
	 * @param Ottocrat_Field Instance of the field
	 * @access private
	 */
	...
	function addField($fieldInstance, $index=0) {
		...
		$adb->pquery("UPDATE ottocrat_cvcolumnlist SET columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC", 
			Array($this->id, $index));
		$adb->pquery("INSERT INTO ottocrat_cvcolumnlist(cvid,columnindex,columnname) VALUES(?,?,?)", Array($this->id, $index, $cvcolvalue));
	...
	function addRule($fieldInstance, $comparator, $comparevalue, $index=0, $group=1, $condition='and') {
		...
		$adb->pquery("UPDATE ottocrat_cvadvfilter set columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC",
			Array($this->id, $index));		
		$adb->pquery("INSERT INTO ottocrat_cvadvfilter(cvid, columnindex, columnname, comparator, value, groupid, column_condition) VALUES(?,?,?,?,?,?,?)",
			Array($this->id, $index, $cvcolvalue, $comparator, $comparevalue, $group, $condition));

		Ottocrat_Utils::Log("Adding Condition " . self::translateComparator($comparator,true) ." on $fieldInstance->name of $this->name filter ... DONE");
	 ...
	 * @access private
	 * @internal Used from Ottocrat_PackageExport also
	 */
	 ...
	 * @param Ottocrat_Module Instance of the module to use when filtername is used
	 */
	static function getInstance($value, $moduleInstance=false) {
		...
		if(Ottocrat_Utils::isNumber($value)) {
			$query = "SELECT * FROM ottocrat_customview WHERE cvid=?";
			$queryParams = Array($value);
		} else {
			$query = "SELECT * FROM ottocrat_customview WHERE viewname=? AND entitytype=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM ottocrat_customview WHERE entitytype=?";
	...
	static function deleteForModule($moduleInstance) {
		...
		$cvidres = $adb->pquery("SELECT cvid FROM ottocrat_customview WHERE entitytype=?", Array($moduleInstance->name));
		if($adb->num_rows($cvidres)) {
			...
			if(!empty($cvids)) {
				$adb->pquery("DELETE FROM ottocrat_cvadvfilter WHERE cvid  IN (" . implode(',', $cvids) . ")", array());
				$adb->pquery("DELETE FROM ottocrat_cvcolumnlist WHERE cvid IN (" . implode(',', $cvids) . ")", array());
				$adb->pquery("DELETE FROM ottocrat_customview WHERE cvid   IN (" . implode(',', $cvids) . ")", array());
			}
		}
	}
}
