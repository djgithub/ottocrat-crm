 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once('vtlib/Ottocrat/PackageExport.php');
include_once('vtlib/Ottocrat/Unzip.php');
include_once('vtlib/Ottocrat/Module.php');
include_once('vtlib/Ottocrat/Event.php');
include_once('vtlib/Ottocrat/Cron.php');

/**
 * Provides API to import module into ottocrat CRM
 * @package vtlib
 */
class Ottocrat_PackageImport extends Ottocrat_PackageExport {
	 ...
	 * Constructor
	 */
	function Ottocrat_PackageImport() {
		parent::__construct();
	}
	...
	function checkZip($zipfile) {
		...
		$language_modulename = null;
			...
			if(count($matches)) {
				...
				if($isModuleBundle === 'true' && (!empty($this->_modulexml)) &&
						...
						(!empty($this->_modulexml->dependencies->ottocrat_version))) {
		...
		if(!empty($this->_modulexml) &&
			!empty($this->_modulexml->dependencies) &&
			!empty($this->_modulexml->dependencies->ottocrat_version)) {
				$ottocratVersion = (string)$this->_modulexml->dependencies->ottocrat_version;
				if(version_compare($ottocratVersion, '6.0.0rc', '>=') === true) {
					$ottocratversion_found = true;
				}
		}

		$validzip = false;
		if($manifestxml_found && $languagefile_found && $ottocratversion_found)
			$validzip = true;

                if($manifestxml_found && $layoutfile_found && $ottocratversion_found)
			$validzip = true;
                
                if($manifestxml_found && $extensionfile_found && $ottocratversion_found)
			$validzip = true;
	...
	function initImport($zipfile, $overwrite=true) {
		...
		if($module != null) {
			...
			$unzip = new Ottocrat_Unzip($zipfile, $overwrite);
	 ...
	 * @access private
	 */
	function getDependentOttocratVersion() {
		return $this->_modulexml->dependencies->ottocrat_version;
	}
	 ...
	 * @access private
	 */
	function getDependentMaxOttocratVersion() {
		return $this->_modulexml->dependencies->ottocrat_max_version;
	}
	...
	function import_Module() {
		...
		$ottocratMinVersion = $this->_modulexml->dependencies->ottocrat_version;
		$ottocratMaxVersion = $this->_modulexml->dependencies->ottocrat_max_version;

		$moduleInstance = new Ottocrat_Module();
		...
		$moduleInstance->minversion = (!$ottocratMinVersion)? false : $ottocratMinVersion;
		$moduleInstance->maxversion = (!$ottocratMaxVersion)?  false : $ottocratMaxVersion;
		$moduleInstance->save();
		...
		Ottocrat_Module::fireEvent($moduleInstance->name,
			Ottocrat_Module::EVENT_MODULE_POSTINSTALL);
	...
	function import_Tables($modulenode) {
		...
		foreach($modulenode->tables->table as $tablenode) {
			...
			if(Ottocrat_Utils::IsCreateSql($tablesql)) {
				if(!Ottocrat_Utils::checkTable($tablename)) {
					self::log("SQL: $tablesql ... ", false);
					Ottocrat_Utils::ExecuteQuery($tablesql);
					self::log("DONE");
				}
			} else {
				if(Ottocrat_Utils::IsDestructiveSql($tablesql)) {
	...
	function import_CustomView($modulenode, $moduleInstance, $customviewnode) {
		...
		$filterInstance = new Ottocrat_Filter();
	...
	function import_Event($modulenode, $moduleInstance, $eventnode) {
		...
		Ottocrat_Event::register($moduleInstance,
	...
	function import_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
		$relModuleInstance = Ottocrat_Module::getInstance($relatedlistnode->relatedmodule);
	...
	function import_CronTasks($modulenode){
		...
		foreach ($modulenode->crons->cron as $cronTask){
			if(empty($cronTask->status)){
                            $cronTask->status = Ottocrat_Cron::$STATUS_DISABLED;
                        } else {
                            $cronTask->status = Ottocrat_Cron::$STATUS_ENABLED;
                        } 
			if((empty($cronTask->sequence))){
				$cronTask->sequence=Ottocrat_Cron::nextSequence();
			}
			Ottocrat_Cron::register("$cronTask->name","$cronTask->handler", "$cronTask->frequency", "$modulenode->name","$cronTask->status","$cronTask->sequence","$cronTask->description");
		}
	}
}
?>
