...
/**
 * Provides basic API to work with ottocrat CRM Fields
 * @package vtlib
 */
class Ottocrat_FieldBasic {
     ...
     * @param Array 
     * @param Ottocrat_Module Instance of module to which this field belongs
     * @param Ottocrat_Block Instance of block to which this field belongs
     * @access private
     */
    function initialize($valuemap, $moduleInstance = false, $blockInstance = false) {
        ...
        $this->block = $blockInstance ? $blockInstance : Ottocrat_Block::getInstance($valuemap['block'], $moduleInstance);
    }
...
// Add helpinfo column to the ottocrat_field table
        if (empty(self::$__cacheSchemaChanges['ottocrat_field.helpinfo'])) {
            Ottocrat_Utils::AddColumn('ottocrat_field', 'helpinfo', ' TEXT');
            self::$__cacheSchemaChanges['ottocrat_field.helpinfo'] = true;
        }
        if (empty(self::$__cacheSchemaChanges['ottocrat_field.summaryfield'])) {
            Ottocrat_Utils::AddColumn('ottocrat_field', 'summaryfield', ' INT(10) NOT NULL DEFAULT 0');
            self::$__cacheSchemaChanges['ottocrat_field.summaryfield'] = 0;
        }
    }
    ...
    function __getUniqueId() {
        global $adb;
        return $adb->getUniqueID('ottocrat_field');
    }
    ...
    function __getNextSequence() {
        global $adb;
        $result = $adb->pquery("SELECT MAX(sequence) AS max_seq FROM ottocrat_field WHERE tabid=? AND block=?", Array($this->getModuleId(), $this->getBlockId()));
        $maxseq = 0;
    ...
    function __getNextQuickCreateSequence() {
        global $adb;
        $result = $adb->pquery("SELECT MAX(quickcreatesequence) AS max_quickcreateseq FROM ottocrat_field WHERE tabid=?", Array($this->getModuleId()));
     ...
     * @param Ottocrat_Block Instance of the block to use
     * @access private
     */
...
// Initialize other variables which are not done
        ...
        $adb->pquery("INSERT INTO ottocrat_field (tabid, fieldid, columnname, tablename, generatedtype,
...
// Set the field status for mass-edit (if set)
        $adb->pquery('UPDATE ottocrat_field SET masseditable=? WHERE fieldid=?', Array($this->masseditable, $this->id));

        Ottocrat_Profile::initForField($this);

        if (!empty($this->columntype)) {
            Ottocrat_Utils::AddColumn($this->table, $this->column, $this->columntype);
        }
    ...
    function __delete() {
        ...
        Ottocrat_Profile::deleteForField($this);

        $adb->pquery("DELETE FROM ottocrat_field WHERE fieldid=?", Array($this->id));
     ...
     * @param Ottocrat_Block Instance of block to which this field should be added.
     */
...
// Make sure to initialize the core tables first
        ...
        global $adb;
        $adb->pquery('UPDATE ottocrat_field SET helpinfo=? WHERE fieldid=?', Array($helptext, $this->id));
    ...
    function setMassEditable($value) {
        global $adb;
        $adb->pquery('UPDATE ottocrat_field SET masseditable=? WHERE fieldid=?', Array($value, $this->id));
    ...
    function setSummaryField($value) { 
        global $adb; 
        $adb->pquery('UPDATE ottocrat_field SET summaryfield=? WHERE fieldid=?', Array($value, $this->id)); 
