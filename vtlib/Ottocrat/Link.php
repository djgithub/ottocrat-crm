...
include_once('vtlib/Ottocrat/Utils/StringTemplate.php');
include_once 'vtlib/Ottocrat/LinkData.php';
...
class Ottocrat_Link {
	...
	static function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('ottocrat_links');
	}
	...
	static function __initSchema() {
		/* ottocrat_links is already core product table */
		/*if(empty(self::$__cacheSchemaChanges['ottocrat_links'])) {
			if(!Ottocrat_Utils::CheckTable('ottocrat_links')) {
				Ottocrat_Utils::CreateTable(
					...
					true);
				Ottocrat_Utils::ExecuteQuery(
					'CREATE INDEX link_tabidtype_idx on ottocrat_links(tabid,linktype)');
			}
			self::$__cacheSchemaChanges['ottocrat_links'] = true;
		}*/
	}
	...
	static function addLink($tabid, $type, $label, $url, $iconpath='',$sequence=0, $handlerInfo=null) {
		global $adb;
		self::__initSchema();
		$checkres = $adb->pquery('SELECT linkid FROM ottocrat_links WHERE tabid=? AND linktype=? AND linkurl=? AND linkicon=? AND linklabel=?',
			Array($tabid, $type, $url, $iconpath, $label));
		if(!$adb->num_rows($checkres)) {
			$uniqueid = self::__getUniqueId();
			$sql = 'INSERT INTO ottocrat_links (linkid,tabid,linktype,linklabel,linkurl,linkicon,'.
			'sequence';
	...
	static function deleteLink($tabid, $type, $label, $url=false) {
		...
		self::__initSchema();
		if($url) {
			$adb->pquery('DELETE FROM ottocrat_links WHERE tabid=? AND linktype=? AND linklabel=? AND linkurl=?',
				Array($tabid, $type, $label, $url));
			self::log("Deleting Link ($type - $label - $url) ... DONE");
		} else {
			$adb->pquery('DELETE FROM ottocrat_links WHERE tabid=? AND linktype=? AND linklabel=?',
	...
	static function deleteAll($tabid) {
		global $adb;
		self::__initSchema();
		$adb->pquery('DELETE FROM ottocrat_links WHERE tabid=?', Array($tabid));
	...
	static function getAllByType($tabid, $type=false, $parameters=false) {
		...
		if($type) {
			...
			if(is_array($type)) { 
				...
				if($tabid === self::IGNORE_MODULE) {
					$sql = 'SELECT * FROM ottocrat_links WHERE linktype IN ('.
						Ottocrat_Utils::implodestr('?', count($type), ',') .') ';
					$params = $type;
					...
					if(count($permittedTabIdList) > 0 && $current_user->is_admin !== 'on') {
						...
						$sql .= ' and tabid IN ('.
							Ottocrat_Utils::implodestr('?', count($permittedTabIdList), ',').')';
						$params[] = $permittedTabIdList;
					}
					$result = $adb->pquery($sql, Array($adb->flatten_array($params)));
				} else {
					$result = $adb->pquery('SELECT * FROM ottocrat_links WHERE (tabid=? OR tabid=0) AND linktype IN ('.
						Ottocrat_Utils::implodestr('?', count($type), ',') .')',
				...
				if($tabid === self::IGNORE_MODULE) {
					$result = $adb->pquery('SELECT * FROM ottocrat_links WHERE linktype=?', Array($type));
				} else {
					$result = $adb->pquery('SELECT * FROM ottocrat_links WHERE (tabid=? OR tabid=0) AND linktype=?', Array($tabid, $type));				
				}
			}
		} else {
			$result = $adb->pquery('SELECT * FROM ottocrat_links WHERE tabid=?', Array($tabid));
		}

		$strtemplate = new Ottocrat_StringTemplate();
		if($parameters) {
		...
		while($row = $adb->fetch_array($result)){
			...
			if(!empty($row['handler_path']) && isFileAccessible($row['handler_path'])) {
				...
				$linkData = new Ottocrat_LinkData($instance, $current_user);
	 ...
	 * @param Ottocrat_LinkData $linkData
	 * @return Boolean
	 */
