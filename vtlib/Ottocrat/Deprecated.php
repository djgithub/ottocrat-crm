...
class Ottocrat_Deprecated {
	...
	static function getBlockId($tabid, $label) {
		global $adb;
		$query = "select blockid from ottocrat_blocks where tabid=? and blocklabel = ?";
	...
	static function getParentTabFromModule($module) {
		...
		if (file_exists('tabdata.php') && (filesize('tabdata.php') != 0) && file_exists('parent_tabdata.php') && (filesize('parent_tabdata.php') != 0)) {
			...
			$sql = "select ottocrat_parenttab.* from ottocrat_parenttab inner join ottocrat_parenttabrel on ottocrat_parenttabrel.parenttabid=ottocrat_parenttab.parenttabid inner join ottocrat_tab on ottocrat_tab.tabid=ottocrat_parenttabrel.tabid where ottocrat_tab.name=?";
	...
	/*static function checkParentTabExists($parenttab) {
		...
		if (file_exists('parent_tabdata.php') && (filesize('parent_tabdata.php') != 0)) {
			...
			$result = "select 1 from ottocrat_parenttab where parenttab_label = ?";
			$noofrows = $adb->num_rows($result);
			if ($noofrows > 0)
				return true;
			else
	...
	static function createModuleMetaFile() {
		...
		$sql1 = "select * from ottocrat_actionmapping";
		...
		$sql2 = "select * from ottocrat_actionmapping where securitycheck=0";
	...
	static function createModuleGroupMetaFile() {
		global $adb;
		$sql = "select parenttabid,parenttab_label from ottocrat_parenttab where visible=0 order by sequence";
		$result = $adb->pquery($sql, array());
		$num_rows = $adb->num_rows($result);
		$result_array = Array();
		for ($i = 0; $i < $num_rows; $i++) {
		...
		if (file_exists($filename)) {
			if (is_writable($filename)) {
				...
				foreach ($result_array as $parid => $parvalue) {
					$childArray = Array();
					//$sql = "select * from ottocrat_parenttabrel where parenttabid=? order by sequence";
					// vtlib customization: Disabling the tab item based on presence
					$sql = "select * from ottocrat_parenttabrel where parenttabid=?
						and tabid in (select tabid from ottocrat_tab where presence in (0,2)) order by sequence";
					// END
	...
	static function getTemplateDetails($templateid) {
		global $adb;
		$returndata = Array();
		$result = $adb->pquery("select body, subject from ottocrat_emailtemplates where templateid=?", array($templateid));
	...
	static function getAnnouncements() {
		global $adb;
		$sql = " select * from ottocrat_announcement inner join ottocrat_users on ottocrat_announcement.creatorid=ottocrat_users.id";
		$sql.=" AND ottocrat_users.is_admin='on' AND ottocrat_users.status='Active' AND ottocrat_users.deleted = 0";
		$result = $adb->pquery($sql, array());
	...
	static function getModuleTranslationStrings($language, $module) {
		...
		$newStrings = Ottocrat_Language_Handler::getModuleStringsFromFile($language, $module);
	...
	static function getIdOfCustomViewByNameAll($module) {
		...
		if (!isset($cvidCache[$module])) {
			$qry_res = $adb->pquery("select cvid from ottocrat_customview where viewname='All' and entitytype=?", array($module));
	...
	static function SaveTagCloudView($id = "") {
		...
		if ($id == '') {
			...
			$query = "update ottocrat_homestuff set visible = ? where userid=? and stufftype='Tag Cloud'";
	...
	static function getSettingsBlockId($label) {
		global $adb;
		$blockid = '';
		$query = "select blockid from ottocrat_settings_blocks where label = ?";
	...
	static function getSqlForNameInDisplayFormat($input, $module, $glue = ' ') {
		$entity_field_info = Ottocrat_Functions::getEntityModuleInfoFieldsFormatted($module);
		$fieldsName = $entity_field_info['fieldname'];
		if(is_array($fieldsName)) {
	...
	static function getModuleSequenceNumber($module, $recordId) {
		...
		switch ($module) {
			case "Invoice":
				$res = $adb->query("SELECT invoice_no FROM ottocrat_invoice WHERE invoiceid = $recordId");
				$moduleSeqNo = $adb->query_result($res, 0, 'invoice_no');
				break;
			case "PurchaseOrder":
				$res = $adb->query("SELECT purchaseorder_no FROM ottocrat_purchaseorder WHERE purchaseorderid = $recordId");
				$moduleSeqNo = $adb->query_result($res, 0, 'purchaseorder_no');
				break;
			case "Quotes":
				$res = $adb->query("SELECT quote_no FROM ottocrat_quotes WHERE quoteid = $recordId");
				$moduleSeqNo = $adb->query_result($res, 0, 'quote_no');
				break;
			case "SalesOrder":
				$res = $adb->query("SELECT salesorder_no FROM ottocrat_salesorder WHERE salesorderid = $recordId");
	...
	static function getModuleFieldTypeOfDataInfos($tables, $tabid='') {
		$result = array();
		if (!empty($tabid)) {
			$module = Ottocrat_Functions::getModuleName($tabid);
			$fieldInfos = Ottocrat_Functions::getModuleFieldInfos($tabid);
			foreach ($fieldInfos as $name => $field) {
				if (($field['displaytype'] == '1' || $field['displaytype'] == '3') &&
						($field['presence'] == '0' || $field['presence'] == '2')) {

					$label = Ottocrat_Functions::getTranslatedString($field['fieldlabel'], $module);
	...
	static function return_app_list_strings_language($language, $module='Ottocrat') {
		$strings = Ottocrat_Language_Handler::getModuleStringsFromFile($language, $module);
