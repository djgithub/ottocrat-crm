 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once('vtlib/Ottocrat/PackageImport.php');

/**
 * Provides API to update module into ottocrat CRM
 * @package vtlib
 */
class Ottocrat_PackageUpdate extends Ottocrat_PackageImport {
     ...
     * Constructor
     */
    function Ottocrat_PackageUpdate() {
        parent::__construct();
    }
     ...
     * @param Ottocrat_Module Instance of the module to update
                ...
                foreach ($installSequenceArray as $sequence) {
                    foreach ($buildModuleArray as $moduleInfo) {
                        if($moduleInfo['install_sequence'] == $sequence) {
                            $moduleInstance = Ottocrat_Module::getInstance($moduleInfo['name']);
    ...
    function update_Module($moduleInstance) {
        ...
        Ottocrat_Module::fireEvent($moduleInstance->name,
                Ottocrat_Module::EVENT_MODULE_PREUPDATE);
        ...
        Ottocrat_Module::fireEvent($moduleInstance->name,
                Ottocrat_Module::EVENT_MODULE_POSTUPDATE);
    }
                ...
                if(!empty($migrationnode->tables) && !empty($migrationnode->tables->table)) {
                    foreach($migrationnode->tables->table as $tablenode) {
                        ...
                        if(Ottocrat_Utils::IsDestructiveSql($tablesql)) {
                            ...
                            Ottocrat_Utils::ExecuteQuery($tablesql, true);
                            self::log("DONE");
                        }
                    }
                }
    ...
    function update_Blocks($modulenode, $moduleInstance) {
        ...
        foreach($modulenode->blocks->block as $blocknode) {
            $blockInstance = Ottocrat_Block::getInstance((string)$blocknode->label, $moduleInstance);
            if(!$blockInstance) {
    ...
    function update_Fields($blocknode, $blockInstance, $moduleInstance) {
        ...
        foreach($blocknode->fields->field as $fieldnode) {
            $fieldInstance = Ottocrat_Field::getInstance((string)$fieldnode->fieldname, $moduleInstance);
            if(!$fieldInstance) {
    ...
    function update_CustomViews($modulenode, $moduleInstance) {
        ...
        foreach($modulenode->customviews->customview as $customviewnode) {
            $filterInstance = Ottocrat_Filter::getInstance($customviewnode->viewname, $moduleInstance);
            if(!$filterInstance) {
    ...
    function update_Event($modulenode, $moduleInstance, $eventnode) {
        //Ottocrat_Event::register($moduleInstance, $eventnode->eventname, $eventnode->classname, $eventnode->filename);
    ...
    function update_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
        $relModuleInstance = Ottocrat_Module::getInstance((string)$relatedlistnode->relatedmodule);
	...
	function update_CronTasks($modulenode) {
		...
		$cronTasks = Ottocrat_Cron::listAllInstancesByModule($modulenode->name);
		foreach ($modulenode->crons->cron as $importCronTask) {
			foreach($cronTasks as $cronTask) {
				if($cronTask->getName() == $importCronTask->name && $importCronTask->handler == $cronTask->getHandlerFile()) {
					Ottocrat_Cron::deregister($importCronTask->name);
				}
			}
			...
			if((empty($importCronTask->sequence))){
				$importCronTask->sequence=Ottocrat_Cron::nextSequence();
			}
			Ottocrat_Cron::register("$importCronTask->name","$importCronTask->handler", "$importCronTask->frequency", "$modulenode->name","$importCronTask->status","$importCronTask->sequence","$importCronTask->description");
		}
	}
}
?>
