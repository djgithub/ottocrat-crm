 ...
 * @package vtlib
 */
class Ottocrat_LanguageExport extends Ottocrat_Package {

	const TABLENAME = 'ottocrat_language';
	...
	function __initExport($languageCode) {
		...
		Ottocrat_Utils::checkFileAccessForInclusion("languages/$languageCode/Ottocrat.php");
	...
	function export_Language($prefix) {
		...
		$sqlresult = $adb->pquery("SELECT * FROM ottocrat_language WHERE prefix = ?", array($prefix));
