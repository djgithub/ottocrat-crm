...
include_once('vtlib/Ottocrat/ModuleBasic.php');
/**
 * Provides API to work with ottocrat CRM Modules
 * @package vtlib
 */
class Ottocrat_Module extends Ottocrat_ModuleBasic {
	...
	function __getRelatedListUniqueId() {
		global $adb;
		return $adb->getUniqueID('ottocrat_relatedlists');
	}
	...
	function __getNextRelatedListSequence() {
		global $adb;
		$max_sequence = 0;
		$result = $adb->pquery("SELECT max(sequence) as maxsequence FROM ottocrat_relatedlists WHERE tabid=?", Array($this->id));
	 ...
	 * @param Ottocrat_Module Instance of target module with which relation should be setup
	 ...
	 *
	 * @internal Creates table ottocrat_crmentityrel if it does not exists
	 */
	function setRelatedList($moduleInstance, $label='', $actions=false, $function_name='get_related_list') {
		...
		if(!Ottocrat_Utils::CheckTable('ottocrat_crmentityrel')) {
			Ottocrat_Utils::CreateTable(
				'ottocrat_crmentityrel',
		...
		// Add column to ottocrat_relatedlists to save extended actions
		Ottocrat_Utils::AddColumn('ottocrat_relatedlists', 'actions', 'VARCHAR(50)');

		$adb->pquery("INSERT INTO ottocrat_relatedlists(relation_id,tabid,related_tabid,name,sequence,label,presence,actions) VALUES(?,?,?,?,?,?,?,?)",
	...
	function unsetRelatedList($moduleInstance, $label='', $function_name='get_related_list') {
		...
		$adb->pquery("DELETE FROM ottocrat_relatedlists WHERE tabid=? AND related_tabid=? AND name=? AND label=?",
	...
	function addLink($type, $label, $url, $iconpath='', $sequence=0, $handlerInfo=null) {
		Ottocrat_Link::addLink($this->id, $type, $label, $url, $iconpath, $sequence, $handlerInfo);
	}
	...
	function deleteLink($type, $label, $url=false) {
		Ottocrat_Link::deleteLink($this->id, $type, $label, $url);
	}
	 ...
	 */
	function getLinks() {
		return Ottocrat_Link::getAll($this->id);
	}
	...
	function getLinksForExport() {
		return Ottocrat_Link::getAllForExport($this->id);
	}
	...
	function initWebservice() {
		Ottocrat_Webservice::initialize($this);
	}
	...
	function deinitWebservice() {
		Ottocrat_Webservice::uninitialize($this);
	}
	...
	static function getInstance($value) {
		$instance = false;
		$data = Ottocrat_Functions::getModuleData($value);
		if ($data) {
			$instance = new self();
	...
	static function getClassInstance($modulename) {
		...
		if(Ottocrat_Utils::checkFileAccessForInclusion($filepath, false)) {
