...
class Ottocrat_Tools_Console_Controller {
	const PROMPT_ANY      = 1;
	...
	protected function welcome() {
		if ($this->interactive) {
			echo "Welcome to Ottocrat CRM Creator.\n";
	...
	protected function handleCreateModule() {
		$controller = new Ottocrat_Tools_Console_ModuleController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateLanguage() {
		$controller = new Ottocrat_Tools_Console_LanguageController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateTestLanguage() {
		$controller = new Ottocrat_Tools_Console_TestLanguageController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateLayout() {
		$controller = new Ottocrat_Tools_Console_LayoutController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleImportModule() {
		$controller = new Ottocrat_Tools_Console_ImportController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleUpdateModule() {
		$controller = new Ottocrat_Tools_Console_UpdateController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleRemoveModule() {
		$controller = new Ottocrat_Tools_Console_RemoveController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
...
class Ottocrat_Tools_Console_ModuleController extends Ottocrat_Tools_Console_Controller {
	...
	public function find($name) {
		return Ottocrat_Module::getInstance($name);
	}
	...
	protected function create($moduleInformation) {
		...
		/** Common fields that should be in every module, linked to ottocrat CRM core table */
		$field2 = new Ottocrat_Field();
		$field2->name = 'assigned_user_id';
		$field2->label = 'Assigned To';
		$field2->table = 'ottocrat_crmentity';
		...
		$field3->table = 'ottocrat_crmentity';
		...
		$field4->table = 'ottocrat_crmentity';
	...
	protected function createFiles(Ottocrat_Module $module, Ottocrat_Field $entityField) {
...
class Ottocrat_Tools_Console_LayoutController extends Ottocrat_Tools_Console_Controller {
	...
	protected function create($layoutInformation) {
		...
		foreach ($files as $file) {
			$tplfolder = $layoutdir . "modules/Ottocrat";
...
class Ottocrat_Tools_Console_LanguageController extends Ottocrat_Tools_Console_Controller {
	...
	protected function deploy($languageInformation) {
		...
		$check = $db->pquery('SELECT 1 FROM ottocrat_language WHERE prefix=?', $languageInformation['prefix']);
		if ($check && $db->num_rows($check)) {
			;
		} else {
			$db->pquery('INSERT INTO ottocrat_language (id,name,prefix,label,lastupdated,isdefault,active) VALUES(?,?,?,?,?,?,?)',
					array($db->getUniqueId('ottocrat_language'), $languageInformation['label'], $languageInformation['prefix'],
							$languageInformation['label'], date('Y-m-d H:i:s'), 0, 1));
		}
	}
}

class Ottocrat_Tools_Console_TestLanguageController extends Ottocrat_Tools_Console_LanguageController {
...
class Ottocrat_Tools_Console_ImportController extends Ottocrat_Tools_Console_Controller {
...
class Ottocrat_Tools_Console_UpdateController extends Ottocrat_Tools_Console_Controller {
...
class Ottocrat_Tools_Console_RemoveController extends Ottocrat_Tools_Console_Controller {
...
if (php_sapi_name() == 'cli') {
	Ottocrat_Tools_Console_Controller::run();
} else {
