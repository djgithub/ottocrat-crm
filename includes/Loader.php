...
class Ottocrat_Loader {
	...
	static function resolveNameToPath($qualifiedName, $fileExtension='php') {
		...
		// TO handle loading ottocrat files
	 ...
	 * @throws AppException
	 */
	public static function getComponentClassName($componentType, $componentName, $moduleName='Ottocrat') {
		...
		$fallBackModuleDir = $fallBackModuleClassPath = 'Ottocrat';
		...
		if(strpos($moduleName, ':') > 0) {
			...
			if($baseModule == 'Settings')  $baseModule = 'Settings:Ottocrat';
		...
		$moduleSpecificComponentFilePath = Ottocrat_Loader::resolveNameToPath('modules.'.$moduleDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		if(!empty($firstFallBackDir) && !empty($firstFallBackClassPath)) {
			$fallBackComponentFilePath = Ottocrat_Loader::resolveNameToPath('modules.'.$firstFallBackDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		if(!empty($secondFallBackModuleDir) && !empty($secondFallBackModuleClassPath)) {
			$fallBackComponentFilePath = Ottocrat_Loader::resolveNameToPath('modules.'.$secondFallBackModuleDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		$fallBackComponentFilePath = Ottocrat_Loader::resolveNameToPath('modules.'.$fallBackModuleDir.'.'.$componentTypeDirectory.'.'.$componentName);
	...
	public static function autoLoad($className) {
		...
		if($noOfParts > 2) {
            ...
            return Ottocrat_Loader::includeOnce($filePath);
		}
		return false;
	}
}
...
function vimport($qualifiedName) {
	return Ottocrat_Loader::includeOnce($qualifiedName);
}

function vimport_try($qualifiedName) {
	return Ottocrat_Loader::includeOnce($qualifiedName, true);
}

function vimport_path($qualifiedName) {
	return Ottocrat_Loader::includePath($qualifiedName);
}

spl_autoload_register('Ottocrat_Loader::autoLoad');
