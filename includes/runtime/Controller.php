 ...
 */
abstract class Ottocrat_Controller {
	...
	abstract function getViewer(Ottocrat_Request $request);
 ...
 */
abstract class Ottocrat_Action_Controller extends Ottocrat_Controller {
	function __construct() {
		parent::__construct();
	}

	function getViewer(Ottocrat_Request $request) {
 ...
 */
abstract class Ottocrat_View_Controller extends Ottocrat_Action_Controller {
	...
	function getViewer(Ottocrat_Request $request) {
		if(!$this->viewer) {
			...
			$viewer->assign('OTTOCRAT_VERSION', $ottocrat_current_version);
			$this->viewer = $viewer;
		}
		return $this->viewer;
	}
	 ...
	 * @return <array> - array of Ottocrat_JsScript_Model
	 */
	function getHeaderScripts(Ottocrat_Request $request){
		$headerScriptInstances = array();
		$languageHandlerShortName = Ottocrat_Language_Handler::getShortLanguageName();
	...
	function checkAndConvertJsScripts($jsFileNames) {
		...
		foreach($jsFileNames as $jsFileName) {
			...
			$jsScript = new Ottocrat_JsScript_Model();
			...
			$completeFilePath = Ottocrat_Loader::resolveNameToPath($jsFileName, $fileExtension);

			if(file_exists($completeFilePath)) {
				if (strpos($jsFileName, '~') === 0) {
					$filePath = ltrim(ltrim($jsFileName, '~'), '/');
					// if ~~ (reference is outside ottocrat6 folder)
				...
				$fallBackFilePath = Ottocrat_Loader::resolveNameToPath(Ottocrat_JavaScript::getBaseJavaScriptPath().'/'.$jsFileName, 'js');
				if(file_exists($fallBackFilePath)) {
					$filePath = str_replace('.','/', $jsFileName) . '.js';
					$jsScriptInstances[$jsFileName] = $jsScript->set('src', Ottocrat_JavaScript::getFilePath($filePath));
				}
			}
		}
	 ...
	 * @return <Array of Ottocrat_CssScript_Model>
	 *
	...
	function checkAndConvertCssStyles($cssFileNames, $fileExtension='css') {
		...
		foreach($cssFileNames as $cssFileName) {
			...
			$completeFilePath = Ottocrat_Loader::resolveNameToPath($cssFileName, $fileExtension);
			$filePath = NULL;
			if(file_exists($completeFilePath)) {
				if (strpos($cssFileName, '~') === 0) {
					...
					$filePath = Ottocrat_Theme::getStylePath($filePath);
				}
	 ...
	 * @param Ottocrat_Request $request
	 */
	function getJSLanguageStrings(Ottocrat_Request $request) {
		$moduleName = $request->getModule(false);
		return Ottocrat_Language_Handler::export($moduleName, 'jsLanguageStrings');
	}
}