...
class Ottocrat_Viewer extends SmartyBC {
	...
	public function getTemplatePath($templateName, $moduleName='') {
		...
		if(!empty($moduleName) && file_exists($completeFilePath)) {
			...
			return "modules/Ottocrat/$templateName";
		}
	}
	 ...
	 * @return Ottocrat_Viewer instance
	 */
...
function vtemplate_path($templateName, $moduleName='') {
	$viewerInstance = Ottocrat_Viewer::getInstance();
	$args = func_get_args();
	return call_user_func_array(array($viewerInstance, 'getTemplatePath'), $args);
}

/**
 * Generated cache friendly resource URL linked with version of Ottocrat
 */
function vresource_url($url) {
    global $ottocrat_current_version;
    if (stripos($url, '://') === false) {
        $url = $url .'?v='.$ottocrat_current_version;
    }
    return $url;
}