...
function getPriceBookRelatedProducts($query,$focus,$returnset='')
{
	...
	for($i=0; $i<$numRows; $i++) {
		...
		if(isPermitted("PriceBooks","EditView","") == 'yes' && isPermitted('Products', 'EditView', $entity_id) == 'yes') {
			$action .= '<img style="cursor:pointer;" src="'. ottocrat_imageurl('editfield.gif', $theme).'" border="0" onClick="fnvshobj(this,\'editlistprice\'),editProductListPrice(\''.$entity_id.'\',\''.$pricebook_id.'\',\''.number_format($listprice, $no_of_decimal_places,'.','').'\')" alt="'.$app_strings["LBL_EDIT_BUTTON"].'" title="'.$app_strings["LBL_EDIT_BUTTON"].'"/>';
		} else {
		...
		if(isPermitted("PriceBooks","Delete","") == 'yes' && isPermitted('Products', 'Delete', $entity_id) == 'yes') {
			if($action != "")
				$action .= '&nbsp;|&nbsp;';
			$action .= '<img src="'. ottocrat_imageurl('delete.gif', $theme).'" onclick="if(confirm(\''.$app_strings['ARE_YOU_SURE'].'\')) deletePriceBookProductRel('.$entity_id.','.$pricebook_id.');" alt="'.$app_strings["LBL_DELETE"].'" title="'.$app_strings["LBL_DELETE"].'" style="cursor:pointer;" border="0">';
		}
		if($action != "")
			$entries[] = $action;
...
function CheckColumnPermission($tablename, $columnname, $module)
{
	...
	if (!array_key_exists($cachekey, $cache)) {
		$res = $adb->pquery("select fieldname from ottocrat_field where tablename=? and columnname=? and ottocrat_field.presence in (0,2)", array($tablename, $columnname));
