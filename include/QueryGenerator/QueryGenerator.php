...
class QueryGenerator {
	...
	public function initForCustomViewById($viewId) {
		...
		foreach ($this->customViewColumnList as $customViewColumnInfo) {
			...
			if(empty($details[2]) && $details[1] == 'crmid' && $details[0] == 'ottocrat_crmentity') {
				$name = 'id';
			...
			if(count($filtercolumns) > 0) {
				...
				foreach ($filtercolumns as $index=>$filter) {
                    ...
                    if($nameComponents[2] == 'due_date' && $nameComponents[3] == 'Events_End_Date_&_Time')
                        $nameComponents[4] = 'DT';
					if(empty($nameComponents[2]) && $nameComponents[1] == 'crmid' && $nameComponents[0] == 'ottocrat_crmentity') {
		...
		foreach ($this->fields as $field) {
			...
			if($this->meta->getEntityName() == 'Calendar' && ($field == 'date_start' || $field == 'due_date' || $field == 'taskstatus' || $field == 'eventstatus')) {
				if($field=='date_start') {
					...
					$sql = "CASE WHEN (ottocrat_activity.status not like '') THEN ottocrat_activity.status ELSE ottocrat_activity.eventstatus END AS ";
					if ( $field == 'taskstatus') {
						$sql .= "status";
					} else {
						$sql .= $field;
	...
	public function getFromClause() {
			...
			if($field->getFieldDataType() == 'reference') {
				...
				foreach($moduleList as $module) {
					if($module == 'Users' && $baseModule != 'Users') {
						$tableJoinCondition[$fieldName]['ottocrat_users'.$fieldName] = $field->getTableName().
								".".$field->getColumnName()." = ottocrat_users".$fieldName.".id";
						$tableJoinCondition[$fieldName]['ottocrat_groups'.$fieldName] = $field->getTableName().
								".".$field->getColumnName()." = ottocrat_groups".$fieldName.".groupid";
						$tableJoinMapping['ottocrat_users'.$fieldName] = 'LEFT JOIN ottocrat_users AS';
						$tableJoinMapping['ottocrat_groups'.$fieldName] = 'LEFT JOIN ottocrat_groups AS';
						$i++;
					}
				}
			} elseif($field->getFieldDataType() == 'owner') {
				$tableList['ottocrat_users'] = 'ottocrat_users';
				$tableList['ottocrat_groups'] = 'ottocrat_groups';
				$tableJoinMapping['ottocrat_users'] = 'LEFT JOIN';
				$tableJoinMapping['ottocrat_groups'] = 'LEFT JOIN';
		...
		foreach ($this->whereFields as $fieldName) {
			...
			if($field->getFieldDataType() == 'reference') {
				...
				// This is special condition as the data is not stored in the base table, 
                                ...
                                if ($fieldName == 'parent_id' && $field->getTableName() == 'ottocrat_seactivityrel') {
                                    $tableJoinMapping[$field->getTableName()] = 'LEFT JOIN';
                                } else if ($fieldName == 'contact_id' && $field->getTableName() == 'ottocrat_cntactivityrel') {
                                    $tableJoinMapping[$field->getTableName()] = "LEFT JOIN";
                                } else {
                                    $tableJoinMapping[$field->getTableName()] = 'INNER JOIN';
                                }
		...
		foreach ($tableList as $tableName) {
			...
			} elseif($tableName == 'ottocrat_groups') {
				$field = $moduleFields[$ownerField];
				$sql .= " $tableJoinMapping[$tableName] $tableName ON ".$field->getTableName().".".
		...
		if( $this->meta->getTabName() == 'Documents') {
			$tableJoinCondition['folderid'] = array(
				'ottocrat_attachmentsfolderfolderid'=>"$baseTable.folderid = ottocrat_attachmentsfolderfolderid.folderid"
			);
			$tableJoinMapping['ottocrat_attachmentsfolderfolderid'] = 'INNER JOIN ottocrat_attachmentsfolder';
		}
			...
			foreach ($this->referenceModuleField as $index=>$conditionInfo) {
				...
				if(!in_array($tableName, $referenceFieldTableList)) {
					if($referenceFieldObject->getFieldName() == 'parent_id' && ($this->getModule() == 'Calendar' || $this->getModule() == 'Events')) {
						$sql .= ' LEFT JOIN ottocrat_seactivityrel ON ottocrat_seactivityrel.activityid = ottocrat_activity.activityid ';
					}
					...
					if($referenceFieldObject->getFieldName() == 'contact_id' && ($this->getModule() == 'Calendar' || $this->getModule() == 'Events')) {
						$sql .= ' LEFT JOIN ottocrat_cntactivityrel ON ottocrat_cntactivityrel.activityid = ottocrat_activity.activityid ';
					}
        ...
        foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
                    ...
                    if($fieldName == 'created_user_id'){
                        $concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"ottocrat_users$fieldName.first_name",'last_name'=>"ottocrat_users$fieldName.last_name"), 'Users');
                        $fieldSql .= "$fieldGlue (trim($concatSql) $valueSql)";
                    }else{
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"ottocrat_users.first_name",'last_name'=>"ottocrat_users.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or "."ottocrat_groups.groupname $valueSql)";
                    }
					...
					if($fieldName == 'date_start') {
						$dateFieldColumnName = 'ottocrat_activity.date_start';
						$timeFieldColumnName = 'ottocrat_activity.time_start';
					} else {
						$dateFieldColumnName = 'ottocrat_activity.due_date';
						$timeFieldColumnName = 'ottocrat_activity.time_end';
					}
					if($operator == 'bw') {
			...
			foreach ($advfilterlist as $groupindex=>$groupcolumns) {
				...
				if(count($filtercolumns) > 0) {
					...
					foreach ($filtercolumns as $index=>$filter) {
						...
						if(empty($name[2]) && $name[1] == 'crmid' && $name[0] == 'ottocrat_crmentity') {
							$name = $this->getSQLColumn('id');
						} else {
							$name = $name[2];
						}
