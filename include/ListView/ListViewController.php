			...
			foreach ($listViewFields as $fieldName) {
				...
				if($module == 'Documents' && $fieldName == 'filename') {
					...
					if($fileName != '' && $status == 1) {
						if($downloadType == 'I' ) {
							$value = '<a onclick="Javascript:Documents_Index_Js.updateDownloadCount(\''.Ottocrat_Request:: encryptLink('index.php?module=Documents&action=UpdateDownloadCount&record='.$recordId).'\');"'.
                    ...
                    if($module == 'Calendar' && ($fieldName == 'taskstatus' || $fieldName == 'eventstatus' || $fieldName == 'activitytype')) {
                        $value = Ottocrat_Language_Handler::getTranslatedString($value,$module);
						...
						$value = "<font color='red'>". Ottocrat_Language_Handler::getTranslatedString('LBL_NOT_ACCESSIBLE',
								$module)."</font>";
					} else {
						$value =  Ottocrat_Language_Handler::getTranslatedString($value,$module);
						$value = textlength_check($value);
					...
					if($value != '' && $value != '0000-00-00') {
						...
						if($fieldDataType == 'datetime') {
							$value = Ottocrat_Datetime_UIType::getDateTimeValue($value);
					...
					if(!empty($value)){
						...
						if($userModel->get('hour_format') == '12'){
							$value = Ottocrat_Time_UIType::getTimeValueInAMorPM($value);
						}
					}
...
//							$value = CurrencyField::appendCurrencySymbol($currencyValue, $currencySymbol);
					...
					if($current_user->internal_mailer == 1){
						...
						$value = "<a class='emailField' onclick=\"Ottocrat_Helper_Js.getInternalMailer($recordId,".
					...
					$value = '<a href="'.Ottocrat_Request:: encryptLink("index.php?module=Roles&parent=Settings&view=Edit&record=".$value).'">'.textlength_check(getRoleName($value)).'</a>';
                    ...
                    if($outgoingCallPermission && !empty($value)) {
                        ...
                        $value = '<a class="phoneField" data-value="'.$phoneNumber.'" record="'.$recordId.'" onclick="Ottocrat_PBXManager_Js.registerPBXOutboundCall(\''.$phoneNumber.'\', '.$recordId.')">'.textlength_check($value).'</a>';
                    }else {
                        $value = textlength_check($value);
                    }
					...
					$result1 = $this->db->pquery("SELECT access_count FROM ottocrat_email_track WHERE ".
