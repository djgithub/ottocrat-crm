...
class OttocratWebserviceObject{
	...
	private function OttocratWebserviceObject($entityId,$entityName,$handler_path,$handler_class){
		$this->id = $entityId;
		$this->name = $entityName;
		// Quick Fix to override default Actor class & path (good to update DB itself)
		if ($entityName == 'CompanyDetails') {
			$handler_path = 'include/Webservices/Custom/OttocratCompanyDetails.php';
			$handler_class= 'OttocratCompanyDetails';
		}
		// END
	...
	static function fromName($adb,$entityName){
		...
		if(!isset(self::$_fromNameCache[$entityName])) {
			...
			$result = null;
			if ($cacheLength == 0) {
				$result = $adb->pquery("select * from ottocrat_ws_entity where name=?",array($entityName));
			} else {
				// Could repeat more number of times...so let us pull rest of details into cache.
				$result = $adb->pquery("select * from ottocrat_ws_entity", array());
			}
		...
		if($rowData) {
			return new OttocratWebserviceObject($rowData['id'],$rowData['name'],
	...
	static function fromId($adb,$entityId){		
		...
		if(!isset(self::$_fromIdCache[$entityId])) {
			$result = $adb->pquery("select * from ottocrat_ws_entity where id=?",array($entityId));
			if($result){
				$rowCount = $adb->num_rows($result);
				if($rowCount === 1){
	...
	static function fromQuery($adb,$query){
		...
		if($found === 1){
			return OttocratWebserviceObject::fromName($adb,trim($matches[1]));
		}
