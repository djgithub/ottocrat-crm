...
require_once 'include/Webservices/DescribeObject.php';
	...
	function vtws_sync($mtime,$elementType,$syncType,$user){
        ...
        if ($userAndGroupSync) {
        $groupresult = $adb->pquery("select groupid from ottocrat_users2group where userid=?", array($user->id));
		...
		if($typed){
		 ...
		 $baseCRMTable = " ottocrat_crmentity ";
		...
		foreach($accessableModules as $elementType){
			...
			$fromClause.= ' ) ottocrat_ws_sync ON (ottocrat_crmentity.crmid = ottocrat_ws_sync.crmid)';
	...
	function vtws_getEmailFromClause(){
		$q = "FROM ottocrat_activity
				...
				LEFT JOIN ottocrat_seattachmentsrel ON ottocrat_activity.activityid = ottocrat_seattachmentsrel.crmid
				LEFT JOIN ottocrat_attachments ON ottocrat_seattachmentsrel.attachmentsid = ottocrat_attachments.attachmentsid
				LEFT JOIN ottocrat_email_track ON ottocrat_activity.activityid = ottocrat_email_track.mailid
				INNER JOIN ottocrat_emaildetails ON ottocrat_activity.activityid = ottocrat_emaildetails.emailid
				LEFT JOIN ottocrat_users ottocrat_users2 ON ottocrat_emaildetails.idlists = ottocrat_users2.id
				LEFT JOIN ottocrat_groups ottocrat_groups2 ON ottocrat_emaildetails.idlists = ottocrat_groups2.groupid";
		return $q;
	}

	function getSyncQueryBaseTable($elementType){
		if($elementType!="Calendar" && $elementType!="Events"){
			return "ottocrat_crmentity";
		}
		else{
			$activityCondition = getCalendarTypeCondition($elementType);
			$query = "ottocrat_crmentity INNER JOIN ottocrat_activity ON (ottocrat_crmentity.crmid = ottocrat_activity.activityid and $activityCondition)";
			return $query;
		}
	}
	...
	function getCalendarTypeCondition($elementType){
		if($elementType == "Events")
			$activityCondition = "ottocrat_activity.activitytype !='Task' and ottocrat_activity.activitytype !='Emails'";
		else
			$activityCondition = "ottocrat_activity.activitytype ='Task'";
