 ...
 *************************************************************************************/
	...
	class OperationManager{
		...
		private function fillOperationDetails($operationName){
			$sql = "select * from ottocrat_ws_operation where name=?";
		...
		private function fillOperationParameters(){
			$sql = "select * from ottocrat_ws_operation_parameters where operationid=? order by sequence";
			...
			try{
				...
				if(!$this->preLogin){
					...
					if(is_array($userDetails)){
						...
						global $adb;
						$webserviceObject = OttocratWebserviceObject::fromName($adb,"Users");
						$userId = vtws_getId($webserviceObject->getEntityId(),$userDetails->id);
						$ottocratVersion = vtws_getOttocratVersion();
						$resp = array("sessionName"=>$this->sessionManager->getSessionId(),"userId"=>$userId,"version"=>$API_VERSION,"ottocratVersion"=>$ottocratVersion);
						return $resp;
					}
				}
