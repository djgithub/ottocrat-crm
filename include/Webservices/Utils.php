...
/**
 * get current ottocrat version from the database.
 */
function vtws_getOttocratVersion(){
	global $adb;
	$query = 'select * from ottocrat_version';
...
function vtws_getWebserviceGroupFromGroups($groups){
	global $adb;
	$webserviceObject = OttocratWebserviceObject::fromName($adb,'Groups');
...
function getEmailFieldId($meta, $entityId){
	...
	$query="SELECT fieldid,fieldlabel,columnname FROM ottocrat_field WHERE tabid=?
...
function vtws_getModuleNameList(){
	...
	$sql = "select name from ottocrat_tab where isentitytype=1 and name not in ('Rss','Webmails',".
...
function vtws_getWebserviceEntities(){
	...
	$sql = "select name,id,ismodule from ottocrat_ws_entity";
	$res = $adb->pquery($sql, array());
	$moduleArray = Array();
	$entityArray = Array();
	while($row = $adb->fetchByAssoc($res)){
...
/**
 *
 * @param OttocratWebserviceObject $webserviceObject
 * @return CRMEntity
 */
...
function vtws_isRecordOwnerUser($ownerId){
	...
	if (!array_key_exists($ownerId, $cache)) {
		$result = $adb->pquery("select first_name from ottocrat_users where id = ?",array($ownerId));
		$rowCount = $adb->num_rows($result);
...
function vtws_isRecordOwnerGroup($ownerId){
	...
	static $cache = array();
	if (!array_key_exists($ownerId, $cache)) {
		$result = $adb->pquery("select groupname from ottocrat_groups where groupid = ?",array($ownerId));
		$rowCount = $adb->num_rows($result);
...
function vtws_getCalendarEntityType($id){
	...
	$sql = "select activitytype from ottocrat_activity where activityid=?";
...
function vtws_getWebserviceEntityId($entityName, $id){
	global $adb;
	$webserviceObject = OttocratWebserviceObject::fromName($adb,$entityName);
...
function vtws_addModuleTypeWebserviceEntity($moduleName,$filePath,$className){
	global $adb;
	$checkres = $adb->pquery('SELECT id FROM ottocrat_ws_entity WHERE name=? AND handler_path=? AND handler_class=?',
		array($moduleName, $filePath, $className));
	if($checkres && $adb->num_rows($checkres) == 0) {
		$isModule=1;
		$entityId = $adb->getUniqueID("ottocrat_ws_entity");
		$adb->pquery('insert into ottocrat_ws_entity(id,name,handler_path,handler_class,ismodule) values (?,?,?,?,?)',
...
function vtws_deleteWebserviceEntity($moduleName) {
	global $adb;
	$adb->pquery('DELETE FROM ottocrat_ws_entity WHERE name=?',array($moduleName));
}

function vtws_addDefaultActorTypeEntity($actorName,$actorNameDetails,$withName = true){
	$actorHandler = array('file'=>'include/Webservices/OttocratActorOperation.php',
		'class'=>'OttocratActorOperation');
	if($withName == true){
...
function vtws_addActorTypeName($entityId,$fieldNames,$indexColumn,$tableName){
	global $adb;
	$adb->pquery('insert into ottocrat_ws_entity_name(entity_id,name_fields,index_field,table_name) values (?,?,?,?)',
...
function vtws_getName($id,$user){
	...
	$webserviceObject = OttocratWebserviceObject::fromId($adb,$id);
...
function vtws_addWebserviceOperation($name,$handlerFilePath,$handlerMethodName,$requestType,$preLogin = 0){
	global $adb;
	$createOperationQuery = "insert into ottocrat_ws_operation(operationid,name,handler_path,handler_method,type,prelogin)
		values (?,?,?,?,?,?);";
	...
	$operationId = $adb->getUniqueID("ottocrat_ws_operation");
...
function vtws_addWebserviceOperationParam($operationId,$paramName,$paramType,$sequence){
	...
	$createOperationParamsQuery = "insert into ottocrat_ws_operation_parameters(operationid,name,type,sequence)
		values (?,?,?,?);";
...
function vtws_getModuleHandlerFromName($name,$user){
	global $adb, $log;
	$webserviceObject = OttocratWebserviceObject::fromName($adb,$name);
	$handlerPath = $webserviceObject->getHandlerPath();
	$handlerClass = $webserviceObject->getHandlerClass();
...
function vtws_getActorEntityName ($name, $idList) {
	...
	$entity = OttocratWebserviceObject::fromName($db, $name);
...
function vtws_getActorEntityNameById ($entityId, $idList) {
	...
	$nameList = array();
	$webserviceObject = OttocratWebserviceObject::fromId($db, $entityId);
	$query = "select * from ottocrat_ws_entity_name where entity_id = ?";
...
function vtws_getConvertLeadFieldMapping(){
	global $adb;
	$sql = "select * from ottocrat_convertleadmapping";
...
function vtws_getRelatedNotesAttachments($id,$relatedId) {
	...
	$sql = "select * from ottocrat_senotesrel where crmid=?";
	$result = $adb->pquery($sql, array($id));
	if($result === false){
		return false;
	}
	...
	$sql="insert into ottocrat_senotesrel(crmid,notesid) values (?,?)";
	for($i=0; $i<$rowCount;++$i ) {
	...
	$sql = "select * from ottocrat_seattachmentsrel where crmid=?";
	$result = $adb->pquery($sql, array($id));
	if($result === false){
		return false;
	}
	...
	$sql = "insert into ottocrat_seattachmentsrel(crmid,attachmentsid) values (?,?)";
	for($i=0;$i<$rowCount;++$i) {
...
function vtws_saveLeadRelatedProducts($leadId, $relatedId, $setype) {
	...
	$result = $adb->pquery("select * from ottocrat_seproductsrel where crmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		$resultNew = $adb->pquery("insert into ottocrat_seproductsrel values(?,?,?)", array($relatedId, $productId, $setype));
		if($resultNew === false){
			return false;
		}
	}
...
function vtws_saveLeadRelations($leadId, $relatedId, $setype) {
	...
	$result = $adb->pquery("select * from ottocrat_crmentityrel where crmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		$adb->pquery("insert into ottocrat_crmentityrel values(?,?,?,?)",
		array($relatedId, $setype, $recordId, $recordModule));
		if($resultNew === false){
			return false;
		}
	}
	$result = $adb->pquery("select * from ottocrat_crmentityrel where relcrmid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
...
function vtws_getRelatedActivities($leadId,$accountId,$contactId,$relatedId) {
	...
	global $adb;
	$sql = "select * from ottocrat_seactivityrel where crmid=?";
	...
	for($i=0;$i<$rowCount;++$i) {
		...
		$sql ="select setype from ottocrat_crmentity where crmid=?";
		...
		if($type != "Emails") {
				if(!empty($accountId)){
					$sql = "insert into ottocrat_seactivityrel(crmid,activityid) values (?,?)";
				...
				if(!empty($contactId)){
					$sql="insert into ottocrat_cntactivityrel(contactid,activityid) values (?,?)";
...
function vtws_saveLeadRelatedCampaigns($leadId, $relatedId, $seType) {
	...
	$result = $adb->pquery("select * from ottocrat_campaignleadrel where leadid=?", array($leadId));
	if($result === false){
		return false;
	}
	$rowCount = $adb->num_rows($result);
	for($i = 0; $i < $rowCount; ++$i) {
		...
		if($seType == 'Accounts') {
			$resultNew = $adb->pquery("insert into ottocrat_campaignaccountrel (campaignid, accountid) values(?,?)",
				array($campaignId, $relatedId));
		} elseif ($seType == 'Contacts') {
			$resultNew = $adb->pquery("insert into ottocrat_campaigncontrel (campaignid, contactid) values(?,?)",
				array($campaignId, $relatedId));
		}
		if($resultNew === false){
			return false;
...
function vtws_transferOwnership($ownerId, $newOwnerId, $delete=true) {
	$db = PearDatabase::getInstance();
	//Updating the smcreatorid,smownerid, modifiedby in ottocrat_crmentity
	$sql = "UPDATE ottocrat_crmentity SET smcreatorid=? WHERE smcreatorid=? AND setype<>?";
	$db->pquery($sql, array($newOwnerId, $ownerId,'ModComments'));

	$sql = "UPDATE ottocrat_crmentity SET smownerid=? WHERE smownerid=? AND setype<>?";
	$db->pquery($sql, array($newOwnerId, $ownerId, 'ModComments'));

	$sql = "update ottocrat_crmentity set modifiedby=? where modifiedby=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	//deleting from ottocrat_tracker
	if ($delete) {
		$sql = "delete from ottocrat_tracker where user_id=?";
		$db->pquery($sql, array($ownerId));
	}

	//updating the ottocrat_import_maps
	$sql ="update ottocrat_import_maps set assigned_user_id=? where assigned_user_id=?";
	$db->pquery($sql, array($newOwnerId, $ownerId));

	if(Ottocrat_Utils::CheckTable('ottocrat_customerportal_prefs')) {
		$query = 'UPDATE ottocrat_customerportal_prefs SET prefvalue = ? WHERE prefkey = ? AND prefvalue = ?';
	...
	//delete from ottocrat_homestuff
	if ($delete) {
		$sql = "delete from ottocrat_homestuff where userid=?";
		$db->pquery($sql, array($ownerId));
	}

	//delete from ottocrat_users to ottocrat_role ottocrat_table
	if ($delete) {
		$sql = "delete from ottocrat_users2group where userid=?";
		$db->pquery($sql, array($ownerId));
	}

	$sql = "select tabid,fieldname,tablename,columnname from ottocrat_field left join ".
	"ottocrat_fieldmodulerel on ottocrat_field.fieldid=ottocrat_fieldmodulerel.fieldid where uitype ".
...
function vtws_updateWebformsRoundrobinUsersLists($ownerId, $newOwnerId){
    $db = PearDatabase::getInstance();
    $sql = 'SELECT id,roundrobin_userid FROM ottocrat_webforms;';
    $result = $db->pquery($sql, array());
        ...
        if(is_array($usersList)){
            if(($key = array_search($ownerId, $usersList)) !== false){
                ...
                if(count($usersList) == 0){
                    $db->pquery('UPDATE ottocrat_webforms SET roundrobin_userid = ?,roundrobin = ? where id =?',array("--None--",0,$webformId));
                }
                else{
                    $usersList = json_encode($usersList);
                    $db->pquery('UPDATE ottocrat_webforms SET roundrobin_userid = ? where id =?',array($usersList,$webformId));
                }
            }
        }
    }
...
function vtws_transferOwnershipForWorkflowTasks($ownerModel, $newOwnerModel) {
	...
	$query = "SELECT task,task_id,workflow_id FROM com_ottocrat_workflowtasks where task LIKE '%".$nameSearchValue."%' OR task LIKE '%".$idSearchValue.
	...
	for ($i = 0; $i < $num_rows; $i++) {
		...
		require_once 'modules/com_ottocrat_workflow/tasks/'.$className.'.inc';
		...
		if(array_key_exists("field_value_mapping",$unserializeTask)) {
			...
			if (!empty($fieldMapping)) {
				...
				$query = 'UPDATE com_ottocrat_workflowtasks SET task=? where workflow_id=? AND task_id=?';
