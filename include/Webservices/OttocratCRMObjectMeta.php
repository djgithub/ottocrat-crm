...
class OttocratCRMObjectMeta extends EntityMeta {
	...
	function OttocratCRMObjectMeta($webserviceObject,$user){
		...
		if(in_array('ottocrat_crmentity',$instance->tab_name)){
			$this->defaultTableList = array('ottocrat_crmentity');
		}else{
	...
	private function computeAccess(){
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0){
			...
			$sql = "select * from ottocrat_profile2globalpermissions where profileid in (".generateQuestionMarks($profileList).");";
			...
			$sql = 'select * from ottocrat_profile2tab where profileid in ('.generateQuestionMarks($profileList).') and tabid = ?;';
			...
			$sql = "select * from ottocrat_profile2standardpermissions where profileid in (".generateQuestionMarks($profileList).") and tabid=?";
			$result = $adb->pquery($sql,array($profileList,$this->getTabId()));
	...
	function hasAssignPrivilege($webserviceId){
		...
		$webserviceObject = OttocratWebserviceObject::fromId($adb,$ownerTypeId);
	...
	private function retrieveMetaForBlock($block){
		...
		require('user_privileges/user_privileges_'.$this->user->id.'.php');
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0){
			$sql = "select *, '0' as readonly from ottocrat_field where tabid =? and block in (".generateQuestionMarks($block).") and displaytype in (1,2,3,4,5)";
			$params = array($tabid, $block);	
		}else{
			$profileList = getCurrentUserProfileList();
			
			if (count($profileList) > 0) {
				$sql = "SELECT ottocrat_field.*, ottocrat_profile2field.readonly
						FROM ottocrat_field
						...
						WHERE ottocrat_field.tabid =? AND ottocrat_profile2field.visible = 0 
						AND ottocrat_profile2field.profileid IN (". generateQuestionMarks($profileList) .")
						AND ottocrat_def_org_field.visible = 0 and ottocrat_field.block in (".generateQuestionMarks($block).") and ottocrat_field.displaytype in (1,2,3,4,5) and ottocrat_field.presence in (0,2) group by columnname";
				...
				$sql = "SELECT ottocrat_field.*, ottocrat_profile2field.readonly
						...
						AND ottocrat_profile2field.visible = 0 
						AND ottocrat_def_org_field.visible = 0 and ottocrat_field.block in (".generateQuestionMarks($block).") and ottocrat_field.displaytype in (1,2,3,4,5) and ottocrat_field.presence in (0,2) group by columnname";
				$params = array($tabid, $block);
			}
		}
	...
	function getObjectEntityName($webserviceId){
		...
		if($this->objectName == 'Users'){
			$sql = "select user_name from ottocrat_users where id=? and deleted=0";
			...
			$sql = "select setype from ottocrat_crmentity where crmid=? and deleted=0";
			$result = $adb->pquery($sql , array($id));
			if($result != null && isset($result)){
				if($adb->num_rows($result)>0){
	...
	function exists($recordId){
		...
		if($this->objectName == 'Users'){
			if (array_key_exists($recordId, $user_exists_cache)) {
				...
				$sql = "select 1 from ottocrat_users where id=? and deleted=0 and status='Active'";
			}
			
		}else{
			$sql = "select 1 from ottocrat_crmentity where crmid=? and deleted=0 and setype='".
				$this->getTabName()."'";
		}
