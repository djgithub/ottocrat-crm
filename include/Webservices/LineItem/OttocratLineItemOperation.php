...
require_once "include/Webservices/LineItem/OttocratInventoryOperation.php";
...
/**
 * Description of OttocratLineItemOperation
 */
class OttocratLineItemOperation  extends OttocratActorOperation {
	...
	public function __construct($webserviceObject,$user,$adb,$log) {
		...
		$this->meta = new OttocratLineItemMeta($this->entityTableName,$webserviceObject,$adb,$user);
	...
	private function updateTaxes($createdElement){
		if(count($this->taxList) > 0 ) {
			...
			$id = $id[1];
			$sql = 'UPDATE ottocrat_inventoryproductrel set ';
	...
	function getProductPrice($productId){
		...
		$sql = "select unit_price from ottocrat_products where productid=?";
	...
	public function getCollectiveTaxList(){
		$db = PearDatabase::getInstance();
		$sql = 'select * from ottocrat_inventorytaxinfo where deleted=0';
		$params = array();
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
		if(!$transactionSuccessful){
	...
	private function getProductTaxList($productId){
		$db = PearDatabase::getInstance();
		$sql = 'select * from ottocrat_producttaxrel inner join ottocrat_inventorytaxinfo on
			ottocrat_producttaxrel.taxid=ottocrat_inventorytaxinfo.taxid and deleted=0
			where productid=?';
		$params = array($productId);
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
