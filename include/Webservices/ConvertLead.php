...
function vtws_convertlead($entityvalues, $user) {
	...
	$leadObject = OttocratWebserviceObject::fromName($adb, 'Leads');
	...
	foreach ($availableModules as $entityName) {
		if ($entityvalues['entities'][$entityName]['create']) {
			...
			$entityObject = OttocratWebserviceObject::fromName($adb, $entityvalue['name']);
			...
			try {
				...
				if ($entityvalue['name'] == 'Accounts') {
					$sql = "SELECT ottocrat_account.accountid FROM ottocrat_account,ottocrat_crmentity WHERE ottocrat_crmentity.crmid=ottocrat_account.accountid AND ottocrat_account.accountname=? AND ottocrat_crmentity.deleted=0";
...
function vtws_updateConvertLeadStatus($entityIds, $leadId, $user) {
	...
	if ($entityIds['Accounts'] != '' || $entityIds['Contacts'] != '') {
		$sql = "UPDATE ottocrat_leaddetails SET converted = 1 where leadid=?";
		$result = $adb->pquery($sql, array($leadIdComponents[1]));
		if ($result === false) {
		...
		$sql = "DELETE FROM ottocrat_campaignleadrel WHERE leadid=?";
		$adb->pquery($sql, array($leadIdComponents[1]));

		$sql = "DELETE FROM ottocrat_tracker WHERE item_id=?";
		$adb->pquery($sql, array($leadIdComponents[1]));

		//update the modifiedtime and modified by information for the record
		$leadModifiedTime = $adb->formatDate(date('Y-m-d H:i:s'), true);
		$crmentityUpdateSql = "UPDATE ottocrat_crmentity SET modifiedtime=?, modifiedby=? WHERE crmid=?";
