...
class OttocratCRMActorMeta extends EntityMeta {
	protected $pearDB;
	protected static $fieldTypeMapping = array();
	protected static $referenceTypeMapping = array();
	
	function OttocratCRMActorMeta($tableName,$webserviceObject,$adb,$user){
	...
	protected function getReferenceList($dbField, $tableName){
		...
		if(!isset(OttocratCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			$this->getFieldType($dbField, $tableName);
		}
		$fieldTypeData = OttocratCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
		
		if (empty(OttocratCRMActorMeta::$referenceTypeMapping)) {
			$sql = "SELECT * FROM ottocrat_ws_entity_referencetype";
			...
			for ($index =0, $count = $this->pearDB->num_rows($result); $index < $count; ++$index) {
				...
				OttocratCRMActorMeta::$referenceTypeMapping[$row['fieldtypeid']][] = $row['type'];
			}
		}
		
		$referenceTypes = array();
		if (isset(OttocratCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']])) {
			$referenceTypes = OttocratCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']];
		}
	...
	protected function getFieldType($dbField,$tableName){
		...
		if(isset(OttocratCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			if(OttocratCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] === 'null'){
				return null;
			}
			$row = OttocratCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
			return $row['fieldtype'];
		}
		
		if (empty(OttocratCRMActorMeta::$fieldTypeMapping)) {
			// Optimization to avoid repeated initialization
			$sql = "select * from ottocrat_ws_entity_fieldtype";
			$result = $this->pearDB->pquery($sql,array());
			$rowCount = $this->pearDB->num_rows($result);
			while ($rowCount) {
				$row = $this->pearDB->query_result_rowdata($result,$rowCount-1);
				OttocratCRMActorMeta::$fieldTypeMapping[$row['table_name']][$row['field_name']] = $row;
				--$rowCount;
			}
		}
		
		if (!isset(OttocratCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])) {
			OttocratCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] = 'null';
			return null;
		}
		return OttocratCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name]['fieldtype'];
	}
	...
	public function getNameFields(){
		$query = "select name_fields from ottocrat_ws_entity_name where entity_id = ?";
	...
	public function getEntityDeletedQuery() {
		if($this->getEntityName() == 'Currency'){
			return 'ottocrat_currency_info.deleted=0';
		}
