...
class OttocratModuleOperation extends WebserviceEntityOperation {
	protected $tabId;
	protected $isEntity = true;
	protected $partialDescribeFields = null;
	
	public function OttocratModuleOperation($webserviceObject,$user,$adb,$log){
	...
	protected function getMetaInstance(){
		if(empty(WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id])){
			WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id]  = new OttocratCRMObjectMeta($this->webserviceObject,$this->user);
		}
		return WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id];
	}
	
	public function create($elementType,$element){
		$crmObject = new OttocratCRMObject($elementType, false);
	...
	public function retrieve($id){
		...
		$crmObject = new OttocratCRMObject($this->tabId, true);
    ...
    public function relatedIds($id, $relatedModule, $relatedLabel, $relatedHandler=null) {
		...
		$sourceRecordModel = Ottocrat_Record_Model::getInstanceById($ids[1], $sourceModule);
		$targetModel       = Ottocrat_RelationListView_Model::getInstance($sourceRecordModel, $relatedModule, $relatedLabel);
        $sql = $targetModel->getRelationQuery();

        $relatedWebserviceObject = OttocratWebserviceObject::fromName($adb,$relatedModule);
        ...
        $sql = sprintf("SELECT DISTINCT concat('%sx',ottocrat_crmentity.crmid) as wsid FROM %s", $relatedModuleWSId, $sqlFromPart);
