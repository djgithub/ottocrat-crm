...
class OttocratCRMObject{
	
	private $moduleName ;
	private $moduleId ;
	private $instance ;
	
	function OttocratCRMObject($moduleCredential, $isId=false){
	...
	private function getObjectTypeId($objectName){
		...
		if($tid === false) {
			...
			$sql = "select * from ottocrat_tab where name=?;";
	...
	function exists($id){
		...
		$exists = false;
		$sql = "select * from ottocrat_crmentity where crmid=? and deleted=0";
