...
include_once 'vtlib/Ottocrat/PDF/models/Model.php';
include_once 'vtlib/Ottocrat/PDF/inventory/HeaderViewer.php';
include_once 'vtlib/Ottocrat/PDF/inventory/FooterViewer.php';
include_once 'vtlib/Ottocrat/PDF/inventory/ContentViewer.php';
include_once 'vtlib/Ottocrat/PDF/inventory/ContentViewer2.php';
include_once 'vtlib/Ottocrat/PDF/viewers/PagerViewer.php';
include_once 'vtlib/Ottocrat/PDF/PDFGenerator.php';
include_once 'data/CRMEntity.php';

class Ottocrat_InventoryPDFController {
	...
	function getPDFGenerator() {
		return new Ottocrat_PDF_Generator();
	}

	function getContentViewer() {
		if($this->focusColumnValue('hdnTaxType') == "individual") {
			$contentViewer = new Ottocrat_PDF_InventoryContentViewer();
		} else {
			$contentViewer = new Ottocrat_PDF_InventoryTaxGroupContentViewer();
		}
	...
	function getHeaderViewer() {
		$headerViewer = new Ottocrat_PDF_InventoryHeaderViewer();
		$headerViewer->setModel($this->buildHeaderModel());
		return $headerViewer;
	}

	function getFooterViewer() {
		$footerViewer = new Ottocrat_PDF_InventoryFooterViewer();
	...
	function getPagerViewer() {
		$pagerViewer = new Ottocrat_PDF_PagerViewer();
	...
	function buildContentModels() {
		...
		foreach($associated_products as $productLineItem) {
			...
			$contentModel = new Ottocrat_PDF_Model();
	...
	function buildContentLabelModel() {
		$labelModel = new Ottocrat_PDF_Model();
	...
	function buildSummaryModel() {
		...
		$summaryModel = new Ottocrat_PDF_Model();
	...
	function buildHeaderModel() {
		$headerModel = new Ottocrat_PDF_Model();
	...
	function buildHeaderModelColumnLeft() {
		...
		// Company information
		$result = $adb->pquery("SELECT * FROM ottocrat_organizationdetails", array());
	...
	function buildFooterModel() {
		$footerModel = new Ottocrat_PDF_Model();
		$footerModel->set(Ottocrat_PDF_InventoryFooterViewer::$DESCRIPTION_DATA_KEY, from_html($this->focusColumnValue('description')));
		$footerModel->set(Ottocrat_PDF_InventoryFooterViewer::$TERMSANDCONDITION_DATA_KEY, from_html($this->focusColumnValue('terms_conditions')));
		return $footerModel;
	}

	function buildFooterLabelModel() {
		$labelModel = new Ottocrat_PDF_Model();
		$labelModel->set(Ottocrat_PDF_InventoryFooterViewer::$DESCRIPTION_LABEL_KEY, getTranslatedString('Description',$this->moduleName));
		$labelModel->set(Ottocrat_PDF_InventoryFooterViewer::$TERMSANDCONDITION_LABEL_KEY, getTranslatedString('Terms & Conditions',$this->moduleName));
		return $labelModel;
	}
	...
	function buildWatermarkModel() {
		$watermarkModel = new Ottocrat_PDF_Model();
	...
	function buildCurrencySymbol() {
		...
		if(!empty($currencyId)) {
			$result = $adb->pquery("SELECT currency_symbol FROM ottocrat_currency_info WHERE id=?", array($currencyId));
