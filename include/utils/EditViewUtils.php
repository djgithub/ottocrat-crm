 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/ottocrat_crm/sugarcrm/include/utils/EditViewUtils.php,v 1.188 2005/04/29 05:5 * 4:39 rank Exp
...
/** This function returns the ottocrat_invoice object populated with the details from sales order object.
...
/** This function returns the detailed list of ottocrat_products associated to a given entity or a record.
...
function getAssociatedProducts($module,$focus,$seid='')
{
	...
	{
		$query="SELECT
					case when ottocrat_products.productid != '' then ottocrat_products.productname else ottocrat_service.servicename end as productname,
 		            case when ottocrat_products.productid != '' then ottocrat_products.product_no else ottocrat_service.service_no end as productcode,
					case when ottocrat_products.productid != '' then ottocrat_products.unit_price else ottocrat_service.unit_price end as unit_price,
 		            case when ottocrat_products.productid != '' then ottocrat_products.qtyinstock else 'NA' end as qtyinstock,
 		            case when ottocrat_products.productid != '' then 'Products' else 'Services' end as entitytype,
 		                        ottocrat_inventoryproductrel.listprice,
 		                        ottocrat_inventoryproductrel.description AS product_description,
 		                        ottocrat_inventoryproductrel.*,ottocrat_crmentity.deleted
 	                            FROM ottocrat_inventoryproductrel
								LEFT JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid=ottocrat_inventoryproductrel.productid
 		                        LEFT JOIN ottocrat_products
 		                                ON ottocrat_products.productid=ottocrat_inventoryproductrel.productid
 		                        LEFT JOIN ottocrat_service
	...
	elseif($module == 'Potentials')
	{
		$query="SELECT
 		                        ...
 		                        ottocrat_products.productcode,
 		                        ottocrat_products.unit_price,
 		                        ottocrat_products.qtyinstock,
 		                        ottocrat_seproductsrel.*,ottocrat_crmentity.deleted,
 		                        ottocrat_crmentity.description AS product_description
 		                        ...
 		                        INNER JOIN ottocrat_seproductsrel
 		                                ON ottocrat_seproductsrel.productid=ottocrat_products.productid
 		                        WHERE ottocrat_seproductsrel.crmid=?";
			$params = array($seid);
	}
	elseif($module == 'Products')
	{
		$query="SELECT
 		                        ottocrat_products.productid,
 		                        ottocrat_products.productcode,
 		                        ottocrat_products.productname,
 		                        ottocrat_products.unit_price,
 		                        ottocrat_products.qtyinstock,ottocrat_crmentity.deleted,
 		                        ottocrat_crmentity.description AS product_description,
	...
	{
		$query="SELECT
 		                        ottocrat_service.serviceid AS productid,
 		                        'NA' AS productcode,
 		                        ottocrat_service.servicename AS productname,
 		                        ottocrat_service.unit_price AS unit_price,
 		                        'NA' AS qtyinstock,ottocrat_crmentity.deleted,
 		                        ottocrat_crmentity.description AS product_description,
 		                       	'Services' AS entitytype
 								FROM ottocrat_service
 		                        INNER JOIN ottocrat_crmentity
 		                                ON ottocrat_crmentity.crmid=ottocrat_service.serviceid
 		                        WHERE ottocrat_crmentity.deleted=0
 		                                AND serviceid=?";
			$params = array($seid);
	}
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		if(empty($focus->mode) && $seid!=''){
			$sub_prod_query = $adb->pquery("SELECT crmid as prod_id from ottocrat_seproductsrel WHERE productid=? AND setype='Products'",array($seid));
		} else {
			$sub_prod_query = $adb->pquery("SELECT productid as prod_id from ottocrat_inventorysubproductrel WHERE id=? AND sequence_no=?",array($focus->id,$i));
		}
		$subprodid_str='';
		$subprodname_str='';
	...
	//if taxtype is group then the tax should be same for all products in ottocrat_inventoryproductrel table
...
/** This function returns the data type of the ottocrat_fields, with ottocrat_field label, which is used for javascript validation.
* Param $validationData - array of ottocrat_fieldnames with datatype
* Return type array
*/
