...
if(!isset($__cache_ottocrat_imagepath)) {
    $__cache_ottocrat_imagepath = Array();
}
function ottocrat_imageurl($imagename, $themename) {
	global $__cache_ottocrat_imagepath;
	if($__cache_ottocrat_imagepath[$imagename]) {
        $imagepath = $__cache_ottocrat_imagepath[$imagename];
    } else {
		$imagepath = false;
        ...
        $__cache_ottocrat_imagepath[$imagename] = $imagepath;
    }
	return $imagepath;
}
...
function vtlib_getModuleNameById($tabid) {
	global $adb;
	$sqlresult = $adb->pquery("SELECT name FROM ottocrat_tab WHERE tabid = ?",array($tabid));
...
function vtlib_prefetchModuleActiveInfo($force = true) {
	...
	if($tabrows === false || $force) {
		global $adb;
		$tabres = $adb->query("SELECT * FROM ottocrat_tab");
		$tabrows = array();
		if($tabres) {
...
function vtlib_isModuleActive($module) {
	...
	$active = false;
	//Fix for http://trac.ottocrat.com/cgi-bin/trac.cgi/ticket/7991
...
function vtlib_RecreateUserPrivilegeFiles() {
	global $adb;
	$userres = $adb->query('SELECT id FROM ottocrat_users WHERE deleted = 0');
...
function vtlib_moduleAlwaysActive() {
	$modules = Array (
		...
		'Utilities', 'uploads', 'Import', 'System', 'com_ottocrat_workflow', 'PickList'
	);
	return $modules;
}
...
function vtlib_toggleModuleAccess($module, $enable_disable) {
	...
	if($enable_disable === true) {
		$enable_disable = 0;
		$event_type = Ottocrat_Module::EVENT_MODULE_ENABLED;
	} else if($enable_disable === false) {
		$enable_disable = 1;
		$event_type = Ottocrat_Module::EVENT_MODULE_DISABLED;
	}

	$adb->pquery("UPDATE ottocrat_tab set presence = ? WHERE name = ?", array($enable_disable,$module));
	...
	// ottocrat 5.1.0 onwards
	global $ottocrat_current_version;
	if(version_compare($ottocrat_current_version, '5.0.4', '>')) {
		vtlib_RecreateUserPrivilegeFiles();
	}

	Ottocrat_Module::fireEvent($module, $event_type);
}
...
function vtlib_getToggleModuleInfo() {
	...
	$sqlresult = $adb->query("SELECT name, presence, customized, isentitytype FROM ottocrat_tab WHERE name NOT IN ('Users','Home') AND presence IN (0,1) ORDER BY name");
...
function vtlib_getToggleLanguageInfo() {
	...
	$langinfo = Array();
	$sqlresult = $adb->query("SELECT * FROM ottocrat_language");
	if($sqlresult) {
...
function vtlib_toggleLanguageAccess($langprefix, $enable_disable) {
	...
	$adb->pquery('UPDATE ottocrat_language set active = ? WHERE prefix = ?', Array($enable_disable, $langprefix));
...
function vtlib_getFieldHelpInfo($module) {
	global $adb;
	$fieldhelpinfo = Array();
	if(in_array('helpinfo', $adb->getColumnNames('ottocrat_field'))) {
		$result = $adb->pquery('SELECT fieldname,helpinfo FROM ottocrat_field WHERE tabid=?', Array(getTabid($module)));
		...
		Array(
			...
			Array(
				...
				'related_tables' => Array(
					'ottocrat_accountbillads' => Array ('accountaddressid', 'ottocrat_account', 'accountid'),
					'ottocrat_accountshipads' => Array ('accountaddressid', 'ottocrat_account', 'accountid'),
					'ottocrat_accountscf' => Array ('accountid', 'ottocrat_account', 'accountid'),
				),
			...
			Array(
				'IsCustomModule'=>false,
				'table_name'  => 'ottocrat_contactdetails',
				'table_index' => 'contactid',
				'related_tables'=> Array( 
					'ottocrat_account' => Array ('accountid' ),
					//REVIEW: Added these tables for displaying the data into relatedlist (based on configurable fields)
					'ottocrat_contactaddress' => Array('contactaddressid', 'ottocrat_contactdetails', 'contactid'),
					'ottocrat_contactsubdetails' => Array('contactsubscriptionid', 'ottocrat_contactdetails', 'contactid'),
					'ottocrat_customerdetails' => Array('customerid', 'ottocrat_contactdetails', 'contactid'),
					'ottocrat_contactscf' => Array('contactid', 'ottocrat_contactdetails', 'contactid')
					),
			...
			Array(
				'IsCustomModule'=>false,
				'table_name'  => 'ottocrat_leaddetails',
				'table_index' => 'leadid',
				'related_tables' => Array (
					'ottocrat_leadsubdetails' => Array ( 'leadsubscriptionid', 'ottocrat_leaddetails', 'leadid' ),
					'ottocrat_leadaddress'    => Array ( 'leadaddressid', 'ottocrat_leaddetails', 'leadid' ),
					'ottocrat_leadscf'    => Array ( 'leadid', 'ottocrat_leaddetails', 'leadid' ),
				),
				'popup_fields'=> Array ('lastname'),
			),
			'Campaigns' =>
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_potential',
				'table_index'=> 'potentialid',
				// NOTE: UIType 10 is being used instead of direct relationship from 5.1.0
				//'related_tables' => Array ('ottocrat_account' => Array('accountid')),
				'popup_fields'=> Array('potentialname'),
				'related_tables' => Array (
					'ottocrat_potentialscf'    => Array ( 'potentialid', 'ottocrat_potential', 'potentialid' ),
				),
			),
			'Quotes' =>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_quotes',
				'table_index'=> 'quoteid',
				'related_tables' => Array ('ottocrat_account' => Array('accountid')),
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_salesorder',
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_purchaseorder',
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_invoice',
				'table_index'=> 'invoiceid',
				'popup_fields'=> Array('subject'),
                'related_tables'=> Array( 
					'ottocrat_invoicecf' => Array('invoiceid', 'ottocrat_invoice', 'invoiceid')
					),
			),
			'HelpDesk'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_troubletickets',
				'table_index'=> 'ticketid',
				'related_tables'=> Array ('ottocrat_ticketcf' => Array('ticketid')),
				'popup_fields'=> Array('ticket_title')
			),
			'Faq'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_faq',
				'table_index'=> 'id',
			),
			'Documents'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_notes',
				'table_index'=> 'notesid',
			),
			'Products'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_products',
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_pricebook',
				'table_index'=> 'pricebookid',
			),
			'Vendors'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_vendor',
			...
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_project',
				'table_index'=> 'projectid',
				'related_tables'=> Array( 
					'ottocrat_projectcf' => Array('projectid', 'ottocrat_project', 'projectid')
					),
			),
            'ProjectMilestone' =>
            Array(
                'IsCustomModule'=>false,
				'table_name' => 'ottocrat_projectmilestone',
				'table_index'=> 'projectmilestoneid',
				'related_tables'=> Array( 
					'ottocrat_projectmilestonecf' => Array('projectmilestoneid', 'ottocrat_projectmilestone', 'projectmilestoneid')
					),
            ),
            'ProjectTask' => 
            Array(
                'IsCustomModule'=>false,
				'table_name' => 'ottocrat_projecttask',
				'table_index'=> 'projecttaskid',
				'related_tables'=> Array( 
					'ottocrat_projecttaskcf' => Array('projecttaskid', 'ottocrat_projecttask', 'projecttaskid')
					),
            ),
			'Services' => 
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_service',
				'table_index'=> 'serviceid',
				'related_tables'=> Array( 
					'ottocrat_servicecf' => Array('serviceid')
					),
			),
			'ServiceContracts' => 
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_servicecontracts',
				'table_index'=> 'servicecontractsid',
				'related_tables'=> Array( 
					'ottocrat_servicecontractscf' => Array('servicecontractsid')
					),
			),
            'Assets' => 
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'ottocrat_assets',
				'table_index'=> 'assetsid',
				'related_tables'=> Array( 
					'ottocrat_assetscf' => Array('assetsid')
					),
			)
		);
...
function vtlib_getPicklistValues_AccessibleToAll($field_columnname) {
	...
	$tablename = "ottocrat_$columnname";

	// Gather all the roles (except H1 which is organization role)
	$roleres = $adb->query("SELECT roleid FROM ottocrat_role WHERE roleid != 'H1'");
	...
	$picklistres = $adb->query(
		...
		INNER JOIN ottocrat_role2picklist ON $tablename.picklist_valueid=ottocrat_role2picklist.picklistvalueid
		WHERE roleid != 'H1'");
...
function vtlib_isDirWriteable($dirpath) {
	if(is_dir($dirpath)) {
		do {
			$tmpfile = 'ottocrat' . time() . '-' . rand(1,1000) . '.tmp';
 ...
 * @param Ottocrat_Link $widgetLinkInfo
...
function vtlib_module_icon($modulename){
	...
	return "modules/Ottocrat/Ottocrat.png";
}
