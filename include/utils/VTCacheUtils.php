...
class VTCacheUtils {
	...
	static function updateFieldInfo($tabid, $fieldname, $fieldid, $fieldlabel,
        ...
        Ottocrat_Cache::set('fieldInfo', $tabid, self::$_fieldinfo_cache[$tabid]);
	}
	static function lookupFieldInfo($tabid, $fieldname) {
        $fieldInfo = Ottocrat_Cache::get('fieldInfo', $tabid);
        ...
        $field = Ottocrat_Cache::get('field-'.$tabid,$fieldname);
        if($field){
            $cacheField = array(
                'tabid' => $tabid,
                'fieldid' => $field->getId(),
	...
	static function lookupFieldInfo_Module($module, $presencein = array('0', '2')) {
        ...
        $fieldInfo = Ottocrat_Cache::get('ModuleFields',$tabid);
        if($fieldInfo){
	...
	static function updateEntityNameInfo($module, $data) {
        ...
        Ottocrat_Cache::set('EntityInfo', $module, self::$_module_entityname_cache[$module]);
	}
	static function lookupEntityNameInfo($module) {
        $entityNames = Ottocrat_Cache::get('EntityInfo', $module);
        if($entityNames){
            return $entityNames;
