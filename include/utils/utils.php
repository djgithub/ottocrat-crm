...
require_once 'vtlib/Ottocrat/Functions.php';
require_once 'vtlib/Ottocrat/Deprecated.php';
...
function get_user_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module=false)
{
	...
	if($user_array == null)
	{
		...
		$temp_result = Array();
		// Including deleted ottocrat_users for now.
		if (empty($status)) {
				$query = "SELECT id, user_name from ottocrat_users";
				$params = array();
		}
		else {
				if($private == 'private')
				{
					...
					$query = "select id as id,user_name as user_name,first_name,last_name from ottocrat_users where id=? and status='Active' union select ottocrat_user2role.userid as id,ottocrat_users.user_name as user_name ,
							  ottocrat_users.first_name as first_name ,ottocrat_users.last_name as last_name
							  from ottocrat_user2role inner join ottocrat_users on ottocrat_users.id=ottocrat_user2role.userid inner join ottocrat_role on ottocrat_role.roleid=ottocrat_user2role.roleid where ottocrat_role.parentrole like ? and status='Active' union
							  select shareduserid as id,ottocrat_users.user_name as user_name ,
							  ottocrat_users.first_name as first_name ,ottocrat_users.last_name as last_name  from ottocrat_tmp_write_user_sharing_per inner join ottocrat_users on ottocrat_users.id=ottocrat_tmp_write_user_sharing_per.shareduserid where status='Active' and ottocrat_tmp_write_user_sharing_per.userid=? and ottocrat_tmp_write_user_sharing_per.tabid=?";
					$params = array($current_user->id, $current_user_parent_role_seq."::%", $current_user->id, getTabid($module));
				}
				else
				{
					$log->debug("Sharing is Public. All ottocrat_users should be listed");
					$query = "SELECT id, user_name,first_name,last_name from ottocrat_users WHERE status=?";
					$params = array($status);
				}
		}
...
function get_group_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module = false)
{
	...
	if($group_array == null)
	{
		...
		// Including deleted ottocrat_users for now.
		$log->debug("Sharing is Public. All ottocrat_users should be listed");
		$query = "SELECT groupid, groupname from ottocrat_groups";
		$params = array();
		...
		if($private == 'private'){
			...
			if(count($current_user_groups) != 0) {
				$query .= " OR ottocrat_groups.groupid in (".generateQuestionMarks($current_user_groups).")";
				array_push($params, $current_user_groups);
			}
			$log->debug("Sharing is Private. Only the current user should be listed");
			$query .= " union select ottocrat_group2role.groupid as groupid,ottocrat_groups.groupname as groupname from ottocrat_group2role inner join ottocrat_groups on ottocrat_groups.groupid=ottocrat_group2role.groupid inner join ottocrat_role on ottocrat_role.roleid=ottocrat_group2role.roleid where ottocrat_role.parentrole like ?";
			array_push($params, $current_user_parent_role_seq."::%");

			if(count($current_user_groups) != 0) {
				$query .= " union select ottocrat_groups.groupid as groupid,ottocrat_groups.groupname as groupname from ottocrat_groups inner join ottocrat_group2rs on ottocrat_groups.groupid=ottocrat_group2rs.groupid where ottocrat_group2rs.roleandsubid in (".generateQuestionMarks($parent_roles).")";
				array_push($params, $parent_roles);
			}

			$query .= " union select sharedgroupid as groupid,ottocrat_groups.groupname as groupname from ottocrat_tmp_write_group_sharing_per inner join ottocrat_groups on ottocrat_groups.groupid=ottocrat_tmp_write_group_sharing_per.sharedgroupid where ottocrat_tmp_write_group_sharing_per.userid=?";
			array_push($params, $current_user->id);

			$query .= " and ottocrat_tmp_write_group_sharing_per.tabid=?";
...
function return_app_list_strings_language($language) {
	return Ottocrat_Deprecated::return_app_list_strings_language($language);
}
...
function return_module_language($language, $module) {
	return Ottocrat_Deprecated::getModuleTranslationStrings($language, $module);
}
...
function return_specified_module_language($language, $module) {
	return Ottocrat_Deprecated::return_app_list_strings_language($language, $module);
}
 ...
 */
function get_themes() {
	return Ottocrat_Theme::getAllSkins();
   }
...
function to_html($string, $encode=true)
{
	...
	if(is_string($string))
	{
		...
		// In ottocrat5 ajax request are treated specially and the data is encoded
		if ($doconvert == true)
		{
			if($inUTF8)
...
function getTabname($tabid)
{
	...
	if (!isset($cache[$tabid])) {
		$sql = "select tablabel from ottocrat_tab where tabid=?";
...
{
	return Ottocrat_Functions::getModuleName($tabid);
}
...
function getColumnFields($module)
{
	...
	if($cachedModuleFields === false) {
		...
		FROM ottocrat_field WHERE tabid in (" . generateQuestionMarks($tabid) . ")";
...
function getUserEmail($userid)
{
	...
	$log->info("in getUserEmail ".$userid);
        ...
        if($userid != '')
        {
                $sql = "select email1 from ottocrat_users where id=?";
...
function getUserId_Ol($username)
{
	...
	global $adb;
	$sql = "select id from ottocrat_users where user_name=?";
...
function getActionid($action)
{
	...
	{
		$query="select * from ottocrat_actionmapping where actionname=?";
...
function getActionname($actionid)
{
	...
	else
	{
		...
		$query="select * from ottocrat_actionmapping where actionid=? and securitycheck=0";
...
function getRecordOwnerId($record)
{
	...
	if($ownerId === false) {
		$query="select smownerid from ottocrat_crmentity where crmid = ?";
	...
	if($ownerId)
	{
		...
		if($count === false) {
			$sql_result = $adb->pquery("select count(*) as count from ottocrat_users where id = ?",array($ownerId));
...
function insertProfile2field($profileid)
{
	...
	$fld_result = $adb->pquery("select * from ottocrat_field where generatedtype=1 and displaytype in (1,2,3) and ottocrat_field.presence in (0,2) and tabid != 29", array());
    ...
    for($i=0; $i<$num_rows; $i++) {
         ...
         $adb->pquery("insert into ottocrat_profile2field values (?,?,?,?,?)", $params);
	}
...
function insert_def_org_field()
{
	...
	$fld_result = $adb->pquery("select * from ottocrat_field where generatedtype=1 and displaytype in (1,2,3) and ottocrat_field.presence in (0,2) and tabid != 29", array());
        ...
        for($i=0; $i<$num_rows; $i++)
        {
                 ...
                 $adb->pquery("insert into ottocrat_def_org_field values (?,?,?,?)", $params);
	}
...
function updateProductQty($product_id, $upd_qty)
{
	...
	global $adb;
	$query= "update ottocrat_products set qtyinstock=? where productid=?";
...
function addToProductStock($productId,$qty)
{
	...
	$sql = "UPDATE ottocrat_products set qtyinstock=? where productid=?";
...
function addToProductDemand($productId,$qty)
{
	...
	$updQty=$qtyInStck + $qty;
	$sql = "UPDATE ottocrat_products set qtyindemand=? where productid=?";
	$adb->pquery($sql, array($updQty, $productId));
...
function getProductQtyInStock($product_id)
{
	...
	global $adb;
        $query1 = "select qtyinstock from ottocrat_products where productid=?";
...
function getProductQtyInDemand($product_id)
{
	...
	global $adb;
        $query1 = "select qtyindemand from ottocrat_products where productid=?";
        $result = $adb->pquery($query1, array($product_id));
...
/**     Function to get the ottocrat_table name from 'field' ottocrat_table for the input ottocrat_field based on the module
 *      @param  : string $module - current module value
 *      @param  : string $fieldname - ottocrat_fieldname to which we want the ottocrat_tablename
 *      @return : string $tablename - ottocrat_tablename in which $fieldname is a column, which is retrieved from 'field' ottocrat_table per $module basis
  */
function getTableNameForField($module,$fieldname)
{
	...
	$sql = "select tablename from ottocrat_field where tabid in (". generateQuestionMarks($tabid) .") and ottocrat_field.presence in (0,2) and columnname like ?";
...
function getPotentialsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query="select related_to from ottocrat_potential where potentialid=?";
...
function getEmailsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query = "select ottocrat_seactivityrel.crmid from ottocrat_seactivityrel inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_seactivityrel.crmid where ottocrat_crmentity.setype='Accounts' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getEmailsRelatedLeads($record_id)
{
	...
	global $adb;
	$query = "select ottocrat_seactivityrel.crmid from ottocrat_seactivityrel inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_seactivityrel.crmid where ottocrat_crmentity.setype='Leads' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getHelpDeskRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select parent_id from ottocrat_troubletickets inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_troubletickets.parent_id where ticketid=? and ottocrat_crmentity.setype='Accounts'";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from ottocrat_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from ottocrat_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getSalesOrderRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from ottocrat_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getSalesOrderRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from ottocrat_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $potid=$adb->query_result($result,0,'potentialid');
...
function getSalesOrderRelatedQuotes($record_id)
{
	...
	global $adb;
        $query="select quoteid from ottocrat_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getInvoiceRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from ottocrat_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getInvoiceRelatedSalesOrder($record_id)
{
	...
	global $adb;
        $query="select salesorderid from ottocrat_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getAccessPickListValues($module)
{
	...
	$id = getTabid($module);
	$query = "select fieldname,columnname,fieldid,fieldlabel,tabid,uitype from ottocrat_field where tabid = ? and uitype in ('15','33','55') and ottocrat_field.presence in (0,2)";
	...
	for($i=0;$i < $adb->num_rows($result);$i++)
{
		...
		if (count($roleids) > 1)
	{
			$mulsel="select distinct $fieldname from ottocrat_$fieldname inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_$fieldname.picklist_valueid where roleid in (\"". implode($roleids,"\",\"") ."\") and picklistid in (select picklistid from ottocrat_$fieldname) order by sortid asc";
	}
	else
	{
			$mulsel="select distinct $fieldname from ottocrat_$fieldname inner join ottocrat_role2picklist on ottocrat_role2picklist.picklistvalueid = ottocrat_$fieldname.picklist_valueid where roleid ='".$roleid."' and picklistid in (select picklistid from ottocrat_$fieldname) order by sortid asc";
	}
...
function transferUserCurrency($old_cur, $new_cur) {
	...
	$sql = "update ottocrat_users set currency_id=? where currency_id=?";
...
function transferProductCurrency($old_cur, $new_cur) {
	...
	$prod_res = $adb->pquery("select productid from ottocrat_products where currency_id = ?", array($old_cur));
	...
	if(count($prod_ids) > 0) {
		...
		for($i=0;$i<count($prod_ids);$i++) {
			...
			$query = "update ottocrat_products set currency_id=?, unit_price=? where productid=?";
...
function transferPriceBookCurrency($old_cur, $new_cur) {
	...
	$pb_res = $adb->pquery("select pricebookid from ottocrat_pricebook where currency_id = ?", array($old_cur));
...
/**
 * this function searches for a given number in ottocrat and returns the callerInfo in an array format
...
function get_use_asterisk($id){
	...
	$sql = "select * from ottocrat_asteriskextensions where userid = ?";
...
function addToCallHistory($userExtension, $callfrom, $callto, $status, $adb, $useCallerInfo){
	$sql = "select * from ottocrat_asteriskextensions where asterisk_extension=?";
	$result = $adb->pquery($sql,array($userExtension));
	$userID = $adb->query_result($result, 0, "userid");
	if(empty($userID)) {
		// we have observed call to extension not configured in Ottocrat will returns NULL
		return;
	}
	if(empty($callfrom)){
		$callfrom = "Unknown";
	...
	if($status == 'outgoing'){
		...
		if(empty($receiver)){
			...
			$receiver = "<a href='".Ottocrat_Request:: encryptLink("index.php?module=".$receiver['module']."&action=DetailView&record=".$receiver['id'])."'>".$receiver['name']."</a>";
		}
	}else{
		...
		if(empty($callerName)){
			...
			$callerName = "<a href='".Ottocrat_Request:: encryptLink("index.php?module=".$callerName['module']."&action=DetailView&record=".$callerName['id'])."'>".decode_html($callerName['name'])."</a>";
}
}

	$crmID = $adb->getUniqueID('ottocrat_crmentity');
	$timeOfCall = date('Y-m-d H:i:s');

	$query = "INSERT INTO ottocrat_crmentity (crmid,smcreatorid,smownerid,modifiedby,setype,description,createdtime,
	...
	$sql = "insert into ottocrat_pbxmanager (pbxmanagerid,callfrom,callto,timeofcall,status)values (?,?,?,?,?)";
...
function getRelationTables($module,$secmodule){
	...
	$ui10_query = $adb->pquery("SELECT ottocrat_field.tabid AS tabid,ottocrat_field.tablename AS tablename, ottocrat_field.columnname AS columnname FROM ottocrat_field INNER JOIN ottocrat_fieldmodulerel ON ottocrat_fieldmodulerel.fieldid = ottocrat_field.fieldid WHERE (ottocrat_fieldmodulerel.module=? AND ottocrat_fieldmodulerel.relmodule=?) OR (ottocrat_fieldmodulerel.module=? AND ottocrat_fieldmodulerel.relmodule=?)",array($module,$secmodule,$secmodule,$module));
	...
	if(is_array($reltables) && !empty($reltables)){
		...
		$rel_array = array("ottocrat_crmentityrel"=>array("crmid","relcrmid"),"".$primary_obj->table_name."" => "".$primary_obj->table_index."");
	}
	return $rel_array;
}
...
function installVtlibModule($packagename, $packagepath, $customized=false) {
	global $log, $Ottocrat_Utils_Log, $_installOrUpdateVtlibModule;
	...
	require_once('vtlib/Ottocrat/Package.php');
	require_once('vtlib/Ottocrat/Module.php');
	$Ottocrat_Utils_Log = defined('INSTALLATION_MODE_DEBUG')? INSTALLATION_MODE_DEBUG : true;
	...
	if($package->isLanguageType()) {
		require_once('vtlib/Ottocrat/Language.php');
		$languagePack = new Ottocrat_Language();
	...
	} else if(Ottocrat_Module::getInstance($module)) {
...
function com_vtGetModules($adb) {
	$sql="select distinct ottocrat_field.tabid, name
		from ottocrat_field
		inner join ottocrat_tab
			on ottocrat_field.tabid=ottocrat_tab.tabid
		where ottocrat_field.tabid not in(9,10,16,15,29) and ottocrat_tab.presence = 0 and ottocrat_tab.isentitytype=1";
...
function isRecordExists($recordId) {
	global $adb;
	$query = "SELECT crmid FROM ottocrat_crmentity where crmid=? AND deleted=0";
...
function getTabInfo($tabId) {
	...
	$tabInfoResult = $adb->pquery('SELECT prefname, prefvalue FROM ottocrat_tab_info WHERE tabid=?', array($tabId));
	$tabInfo = array();
...
function getBlockName($blockid) {
	...
	if(!empty($blockid) && $blockname === false){
		$block_res = $adb->pquery('SELECT blocklabel FROM ottocrat_blocks WHERE blockid = ?',array($blockid));
...
function getActivityRelatedContacts($activityId) {
	...
	$query = 'SELECT * FROM ottocrat_cntactivityrel WHERE activityid=?';
...
function isLeadConverted($leadId) {
	...
	$query = 'SELECT converted FROM ottocrat_leaddetails WHERE converted = 1 AND leadid=?';
	$params = array($leadId);
...
function getSelectAllQuery($input,$module) {
	...
	if($module == "Calendar") {
		...
		if($module == 'Documents') {
			...
			$query .= " AND ottocrat_notes.folderid in (".$folderid.")";
		}
	}
...
function getCampaignAccountIds($id) {
	global $adb;
	$sql="SELECT ottocrat_account.accountid as id FROM ottocrat_account
		INNER JOIN ottocrat_campaignaccountrel ON ottocrat_campaignaccountrel.accountid = ottocrat_account.accountid
		LEFT JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_account.accountid
		WHERE ottocrat_campaignaccountrel.campaignid = ? AND ottocrat_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getCampaignContactIds($id) {
	global $adb;
	$sql="SELECT ottocrat_contactdetails.contactid as id FROM ottocrat_contactdetails
		INNER JOIN ottocrat_campaigncontrel ON ottocrat_campaigncontrel.contactid = ottocrat_contactdetails.contactid
		LEFT JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_contactdetails.contactid
		WHERE ottocrat_campaigncontrel.campaignid = ? AND ottocrat_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getCampaignLeadIds($id) {
	global $adb;
	$sql="SELECT ottocrat_leaddetails.leadid as id FROM ottocrat_leaddetails
		INNER JOIN ottocrat_campaignleadrel ON ottocrat_campaignleadrel.leadid = ottocrat_leaddetails.leadid
		LEFT JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid = ottocrat_leaddetails.leadid
		WHERE ottocrat_campaignleadrel.campaignid = ? AND ottocrat_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
