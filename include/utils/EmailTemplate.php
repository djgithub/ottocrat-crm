...
class EmailTemplate {
	...
	public function process($params) {
		...
		if (count($variableList) > 0) {
			...
			if (count($tableList) > 0 && count($columnList) > 0) {
				...
				if($module == 'Leads' && !in_array('ottocrat_leaddetails', $tableList)){
					$sql .=' INNER JOIN ottocrat_leaddetails ON ottocrat_leaddetails.leadid = ottocrat_crmentity.crmid';
				}
	...
	public function isActive($field, $mod) {
		global $adb;
		$tabid = getTabid($mod);
		$query = 'select * from ottocrat_field where fieldname = ?  and tabid = ? and presence in (0,2)';
