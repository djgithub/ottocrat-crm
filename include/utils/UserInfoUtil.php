...
function getMailServerInfo($user)
{
	...
	global $adb;
        $sql = "select * from ottocrat_mail_accounts where status=1 and user_id=?";
...
/** To get the Role of the specified user
  ...
  * @returns  ottocrat_roleid :: Type String
 */
function fetchUserRole($userid)
{
	...
	global $adb;
	$sql = "select roleid from ottocrat_user2role where userid=?";
...
/** Function to get all the ottocrat_tab utility action permission for the specified ottocrat_profile
...
function getTabsUtilityActionPermission($profileid)
{
	...
	$check = Array();
	$temp_tabid = Array();
	$sql1 = "select * from ottocrat_profile2utility where profileid=? order by(tabid)";
...
function getDefaultSharingEditAction()
{
	...
	$sql= "select * from ottocrat_def_org_share where editstatus=0";
...
function getDefaultSharingAction()
{
	...
	$sql= "select * from ottocrat_def_org_share where editstatus in(0,1)";
	$result = $adb->pquery($sql, array());
	$permissionRow=$adb->fetch_array($result);
	do
	{
...
function getAllDefaultSharingAction()
{
	...
	$copy=Array();
	//retreiving the standard permissions
	$sql= "select * from ottocrat_def_org_share";
	$result = $adb->pquery($sql, array());
...
/** Function to update user to ottocrat_role mapping based on the userid
...
$log->debug("Entering updateUser2RoleMapping(".$roleid.",".$userid.") method ...");
  ...
  $sqlcheck = "select * from ottocrat_user2role where userid=?";
  $resultcheck = $adb->pquery($sqlcheck, array($userid));
  if($adb->num_rows($resultcheck) == 1)
  {
  	$sqldelete = "delete from ottocrat_user2role where userid=?";
	$delparams = array($userid);
  	$result_delete = $adb->pquery($sqldelete, $delparams);
  }
  $sql = "insert into ottocrat_user2role(userid,roleid) values(?,?)";
...
/** Function to get the ottocrat_role name from the ottocrat_roleid
  * @param $roleid -- Role Id:: Type varchar
...
function getRoleName($roleid)
{
	...
	global $adb;
	$sql1 = "select * from ottocrat_role where roleid=?";
...
function isPermitted($module,$actionname,$record_id='')
{
	...
	if(vtlib_isModuleActive($checkModule)){
		...
		//If no actionid, then allow action is ottocrat_tab permission is available
		if($actionid === '')
		{
		...
		//Checking for ottocrat_tab permission
		if($profileTabsPermission[$tabid] !=0)
		{
			$permission = "no";
			$log->debug("Exiting isPermitted method ...");
...
/** Function to get the Profile Global Information for the specified ottocrat_profileid
  * @param $profileid -- Profile Id:: Type integer
...
$log->debug("Entering getProfileGlobalPermission(".$profileid.") method ...");
  global $adb;
  $sql = "select * from ottocrat_profile2globalpermissions where profileid=?" ;
...
/** Function to get the Profile Tab Permissions for the specified ottocrat_profileid
  * @param $profileid -- Profile Id:: Type integer
...
$log->debug("Entering getProfileTabsPermission(".$profileid.") method ...");
  global $adb;
  $sql = "select * from ottocrat_profile2tab where profileid=?" ;
  $result = $adb->pquery($sql, array($profileid));
  $num_rows = $adb->num_rows($result);
...
/** Function to get the Profile Action Permissions for the specified ottocrat_profileid
  * @param $profileid -- Profile Id:: Type integer
...
$log->debug("Entering getProfileActionPermission(".$profileid.") method ...");
	...
	$check = Array();
	$temp_tabid = Array();
	$sql1 = "select * from ottocrat_profile2standardpermissions where profileid=?";
	$result1 = $adb->pquery($sql1, array($profileid));
        $num_rows1 = $adb->num_rows($result1);
        for($i=0; $i<$num_rows1; $i++)
        {
...
/** Function to get the Standard and Utility Profile Action Permissions for the specified ottocrat_profileid
  * @param $profileid -- Profile Id:: Type integer
  * @returns Profile Tabs Action Permission Array in the following format:
  *    $tabActionPermission = Array($tabid1=>Array(actionid1=>permission, actionid2=>permission,...,actionidn=>permission),
  *                        $tabid2=>Array(actionid1=>permission, actionid2=>permission,...,actionidn=>permission),
...
/** Function to get all  the ottocrat_role information
  * @returns $allRoleDetailArray-- Array will contain the details of all the ottocrat_roles. RoleId will be the key:: Type array
 */
...
$log->debug("Entering getAllRoleDetails() method ...");
	global $adb;
	$role_det = Array();
	$query = "select * from ottocrat_role";
	...
	for($i=0; $i<$num_rows;$i++)
	{
		...
		$query1="select * from ottocrat_role where parentrole like ? and depth=?";
...
/** Function to get the ottocrat_role information of the specified ottocrat_role
...
function getRoleInformation($roleid)
{
	...
	global $adb;
	$query = "select * from ottocrat_role where roleid=?";
...
/** Function to get the ottocrat_role related ottocrat_users
  * @param $roleid -- RoleId :: Type varchar
...
function getRoleUsers($roleId)
{
	...
	global $adb;
	$query = "select ottocrat_user2role.*,ottocrat_users.* from ottocrat_user2role inner join ottocrat_users on ottocrat_users.id=ottocrat_user2role.userid where roleid=?";
	$result = $adb->pquery($query, array($roleId));
	$num_rows=$adb->num_rows($result);
...
/** Function to get the ottocrat_role related user ids
  * @param $roleid -- RoleId :: Type varchar
...
function getRoleUserIds($roleId)
{
	...
	global $adb;
	$query = "select ottocrat_user2role.*,ottocrat_users.user_name from ottocrat_user2role inner join ottocrat_users on ottocrat_users.id=ottocrat_user2role.userid where roleid=?";
	$result = $adb->pquery($query, array($roleId));
	$num_rows=$adb->num_rows($result);
	$roleRelatedUsers=Array();
	for($i=0; $i<$num_rows; $i++)
...
/** Function to get the ottocrat_role and subordinate ottocrat_users
  * @param $roleid -- RoleId :: Type varchar
...
function getRoleAndSubordinateUsers($roleId)
{
	...
	$query = "select ottocrat_user2role.*,ottocrat_users.user_name from ottocrat_user2role inner join ottocrat_users on ottocrat_users.id=ottocrat_user2role.userid inner join ottocrat_role on ottocrat_role.roleid=ottocrat_user2role.roleid where ottocrat_role.parentrole like ?";
...
/** Function to get the ottocrat_role and subordinate Information for the specified ottocrat_roleId
  * @param $roleid -- RoleId :: Type varchar
...
function getRoleAndSubordinatesInformation($roleId)
{
	...
	$query="select * from ottocrat_role where parentrole like ? order by parentrole asc";
...
/** Function to get the ottocrat_role and subordinate ottocrat_role ids
  * @param $roleid -- RoleId :: Type varchar
...
/** Function to delete the ottocrat_role related sharing rules
  * @param $roleid -- RoleId :: Type varchar
 */
function deleteRoleRelatedSharingRules($roleId)
{
	...
	$log->debug("Entering deleteRoleRelatedSharingRules(".$roleId.") method ...");
        global $adb;
        $dataShareTableColArr=Array('ottocrat_datashare_grp2role'=>'to_roleid',
                                    'ottocrat_datashare_grp2rs'=>'to_roleandsubid',
                                    'ottocrat_datashare_role2group'=>'share_roleid',
                                    'ottocrat_datashare_role2role'=>'share_roleid::to_roleid',
                                    'ottocrat_datashare_role2rs'=>'share_roleid::to_roleandsubid',
                                    'ottocrat_datashare_rs2grp'=>'share_roleandsubid',
                                    'ottocrat_datashare_rs2role'=>'share_roleandsubid::to_roleid',
                                    'ottocrat_datashare_rs2rs'=>'share_roleandsubid::to_roleandsubid');
...
function deleteGroupRelatedSharingRules($grpId)
{
	...
	$log->debug("Entering deleteGroupRelatedSharingRules(".$grpId.") method ...");
        ...
        global $adb;
        $dataShareTableColArr=Array('ottocrat_datashare_grp2grp'=>'share_groupid::to_groupid',
                                    'ottocrat_datashare_grp2role'=>'share_groupid',
                                    'ottocrat_datashare_grp2rs'=>'share_groupid',
                                    'ottocrat_datashare_role2group'=>'to_groupid',
                                    'ottocrat_datashare_rs2grp'=>'to_groupid');
...
/** Function to get userid and username of all ottocrat_users
...
function getAllUserName()
{
	...
	global $adb;
	$query="select * from ottocrat_users where deleted=0";
	$result = $adb->pquery($query, array());
	$num_rows=$adb->num_rows($result);
	$user_details=Array();
	for($i=0;$i<$num_rows;$i++)
...
/** Function to get groupid and groupname of all ottocrat_groups
...
function getAllGroupName()
{
	...
	global $adb;
	$query="select * from ottocrat_groups";
	$result = $adb->pquery($query, array());
	$num_rows=$adb->num_rows($result);
	$group_details=Array();
	for($i=0;$i<$num_rows;$i++)
...
function deleteSharingRule($shareid)
{
	...
	global $adb;
	$query2="select * from ottocrat_datashare_module_rel where shareid=?";
	...
	$query4="delete from ottocrat_datashare_module_rel where shareid=?";
	$adb->pquery($query4, array($shareid));

	//deleting the releated module sharing permission
	$query5="delete from ottocrat_datashare_relatedmodule_permission where shareid=?";
...
function getDataShareTableName()
{
	...
	$dataShareTableColArr=Array('GRP::GRP'=>'ottocrat_datashare_grp2grp',
				    'GRP::ROLE'=>'ottocrat_datashare_grp2role',
				    'GRP::RS'=>'ottocrat_datashare_grp2rs',
				    'ROLE::GRP'=>'ottocrat_datashare_role2group',
				    'ROLE::ROLE'=>'ottocrat_datashare_role2role',
				    'ROLE::RS'=>'ottocrat_datashare_role2rs',
				    'RS::GRP'=>'ottocrat_datashare_rs2grp',
				    'RS::ROLE'=>'ottocrat_datashare_rs2role',
				    'RS::RS'=>'ottocrat_datashare_rs2rs');
...
/** This function is to retreive the ottocrat_profiles associated with the  the specified user
  * It takes the following input parameters:
  *     $userid -- The User Id:: Type Integer
  *This function will return the ottocrat_profiles associated to the specified ottocrat_users in an Array in the following format:
...
function getUserProfile($userId)
{
	...
	$profArr=Array();
	$sql1 = "select profileid from ottocrat_role2profile where roleid=?";
...
/** To retreive the global permission of the specifed user from the various ottocrat_profiles associated with the user
  * @param $userid -- The User Id:: Type Integer
...
/** To retreive the ottocrat_tab permissions of the specifed user from the various ottocrat_profiles associated with the user
  * @param $userid -- The User Id:: Type Integer
  * @returns  user global permission  array in the following format:
...
/** To retreive the ottocrat_tab acion permissions of the specifed user from the various ottocrat_profiles associated with the user
  * @param $userid -- The User Id:: Type Integer
  * @returns  user global permission  array in the following format:
...
/** To retreive the parent ottocrat_role of the specified ottocrat_role
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  parent ottocrat_role array in the following format:
...
/** To retreive the subordinate ottocrat_roles of the specified parent ottocrat_role
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  subordinate ottocrat_role array in the following format:
...
/** To retreive the subordinate ottocrat_roles and ottocrat_users of the specified parent ottocrat_role
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  subordinate ottocrat_role array in the following format:
  *     $subordinateRoleUserArray=(roleid1=>Array(userid1,userid2,userid3),
                               ottocrat_roleid2=>Array(userid1,userid2,userid3)
				                |
						|
			       ottocrat_roleidn=>Array(userid1,userid2,userid3));
 */
...
function getWriteSharingGroupsList($module)
{
	...
	$grp_array=Array();
	$tabid=getTabid($module);
	$query = "select sharedgroupid from ottocrat_tmp_write_group_sharing_per where userid=? and tabid=?";
...
function getListViewSecurityParameter($module)
{
	...
	{
		$sec_query .= " and (
						ottocrat_crmentity.smownerid in($current_user->id)
						or ottocrat_crmentity.smownerid in(select ottocrat_user2role.userid from ottocrat_user2role inner join ottocrat_users on ottocrat_users.id=ottocrat_user2role.userid inner join ottocrat_role on ottocrat_role.roleid=ottocrat_user2role.roleid where ottocrat_role.parentrole like '".$current_user_parent_role_seq."::%')
						or ottocrat_crmentity.smownerid in(select shareduserid from ottocrat_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.")
						or (";
                        ...
                        if(sizeof($current_user_groups) > 0)
                        {
                         ...
                         $sec_query .= " ottocrat_groups.groupid in(select ottocrat_tmp_read_group_sharing_per.sharedgroupid from ottocrat_tmp_read_group_sharing_per where userid=".$current_user->id." and tabid=".$tabid."))) ";
	}
	elseif($module == 'Accounts')
	{
		$sec_query .= " and (ottocrat_crmentity.smownerid in($current_user->id) " .
				"or ottocrat_crmentity.smownerid in(select ottocrat_user2role.userid from ottocrat_user2role inner join ottocrat_users on ottocrat_users.id=ottocrat_user2role.userid inner join ottocrat_role on ottocrat_role.roleid=ottocrat_user2role.roleid where ottocrat_role.parentrole like '".$current_user_parent_role_seq."::%') " .
				"or ottocrat_crmentity.smownerid in(select shareduserid from ottocrat_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or (";
	...
	{
		$sec_query .= " and (ottocrat_crmentity.smownerid in($current_user->id) or ottocrat_crmentity.smownerid in(select ottocrat_user2role.userid from ottocrat_user2role inner join ottocrat_users on ottocrat_users.id=ottocrat_user2role.userid inner join ottocrat_role on ottocrat_role.roleid=ottocrat_user2role.roleid where ottocrat_role.parentrole like '".$current_user_parent_role_seq."::%') or ottocrat_crmentity.smownerid in(select shareduserid from ottocrat_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") ";
	...
	{
		$sec_query .= " and ottocrat_crmentity.smownerid=".$current_user->id." ";
	...
	elseif($module == 'Calendar')
	{
		...
		if(isset($shared_ids) && $shared_ids != '')
			$condition = " or (ottocrat_crmentity.smownerid in($shared_ids) and ottocrat_activity.visibility = 'Public')";
		else
			$condition = null;
		$sec_query .= " and (ottocrat_crmentity.smownerid in($current_user->id) $condition or ottocrat_crmentity.smownerid in(select ottocrat_user2role.userid from ottocrat_user2role inner join ottocrat_users on ottocrat_users.id=ottocrat_user2role.userid inner join ottocrat_role on ottocrat_role.roleid=ottocrat_user2role.roleid where ottocrat_role.parentrole like '".$current_user_parent_role_seq."::%')";

		if(sizeof($current_user_groups) > 0)
		{
			$sec_query .= " or ((ottocrat_groups.groupid in (". implode(",", $current_user_groups) .")))";
		}
		$sec_query .= ")";
	}
...
function get_current_user_access_groups($module)
{
	...
	$query ="select groupname,groupid from ottocrat_groups";
	$params = array();
...
function getGrpId($groupname)
{
	...
	global $adb;
    $groupid = Ottocrat_Cache::get('group',$groupname);
    if(!$groupid && $groupid !== 0){
        $result = $adb->pquery("select groupid from ottocrat_groups where groupname=?", array($groupname));
        $groupid = ($adb->num_rows($result) > 0) ? $adb->query_result($result,0,'groupid') : 0;
        Ottocrat_Cache::set('group',$groupname,$groupid);
    }
	$log->debug("Exiting getGrpId method ...");
	return $groupid;
}

/** Function to check permission to access a ottocrat_field for a given user
...
function getFieldVisibilityPermission($fld_module, $userid, $fieldname, $accessmode='readonly')
{
	...
	{
        //get ottocrat_profile list using userid
        ...
        $tabid = getTabid($fld_module);
			...
			if($accessmode == 'readonly') {
				$query="SELECT ottocrat_profile2field.visible FROM ottocrat_field INNER JOIN ottocrat_profile2field ON ottocrat_profile2field.fieldid=ottocrat_field.fieldid INNER JOIN ottocrat_def_org_field ON ottocrat_def_org_field.fieldid=ottocrat_field.fieldid WHERE ottocrat_field.tabid=? AND ottocrat_profile2field.visible=0 AND ottocrat_def_org_field.visible=0  AND ottocrat_profile2field.profileid in (". generateQuestionMarks($profilelist) .") AND ottocrat_field.fieldname= ? and ottocrat_field.presence in (0,2) GROUP BY ottocrat_field.fieldid";
                } else {
				$query="SELECT ottocrat_profile2field.visible FROM ottocrat_field INNER JOIN ottocrat_profile2field ON ottocrat_profile2field.fieldid=ottocrat_field.fieldid INNER JOIN ottocrat_def_org_field ON ottocrat_def_org_field.fieldid=ottocrat_field.fieldid WHERE ottocrat_field.tabid=? AND ottocrat_profile2field.visible=0 AND ottocrat_profile2field.readonly=0 AND ottocrat_def_org_field.visible=0  AND ottocrat_profile2field.profileid in (". generateQuestionMarks($profilelist) .") AND ottocrat_field.fieldname= ? and ottocrat_field.presence in (0,2) GROUP BY ottocrat_field.fieldid";
                }
                $params = array($tabid, $profilelist, $fieldname);

            } else {
			if($accessmode == 'readonly') {
				$query="SELECT ottocrat_profile2field.visible FROM ottocrat_field INNER JOIN ottocrat_profile2field ON ottocrat_profile2field.fieldid=ottocrat_field.fieldid INNER JOIN ottocrat_def_org_field ON ottocrat_def_org_field.fieldid=ottocrat_field.fieldid WHERE ottocrat_field.tabid=? AND ottocrat_profile2field.visible=0 AND ottocrat_def_org_field.visible=0  AND ottocrat_field.fieldname= ? and ottocrat_field.presence in (0,2) GROUP BY ottocrat_field.fieldid";
                } else {
				$query="SELECT ottocrat_profile2field.visible FROM ottocrat_field INNER JOIN ottocrat_profile2field ON ottocrat_profile2field.fieldid=ottocrat_field.fieldid INNER JOIN ottocrat_def_org_field ON ottocrat_def_org_field.fieldid=ottocrat_field.fieldid WHERE ottocrat_field.tabid=? AND ottocrat_profile2field.visible=0 AND ottocrat_profile2field.readonly=0 AND ottocrat_def_org_field.visible=0  AND ottocrat_field.fieldname= ? and ottocrat_field.presence in (0,2) GROUP BY ottocrat_field.fieldid";
                }
...
function getColumnVisibilityPermission($userid, $columnname, $module, $accessmode='readonly')
{
	...
	if($cacheFieldInfo === false) {
		$res = $adb->pquery("select fieldname from ottocrat_field where tabid=? and columnname=? and ottocrat_field.presence in (0,2)", array($tabid, $columnname));
...
/** Function to recalculate the Sharing Rules for all the ottocrat_users
  * This function will recalculate all the sharing rules for all the ottocrat_users in the Organization and will write them in flat ottocrat_files
  *
 */
function RecalculateSharingRules()
{
	...
	$query="select id from ottocrat_users where deleted=0";
	$result=$adb->pquery($query, array());
	$num_rows=$adb->num_rows($result);
	for($i=0;$i<$num_rows;$i++)
	{
...
function getSharingModuleList($eliminateModules=false)
{
	...
	$query = "SELECT name FROM ottocrat_tab WHERE presence=0 AND ownedby = 0 AND isentitytype = 1";
...
function isCalendarPermittedBySharing($recordId)
{
	...
	$permission = 'no';
	$query = "SELECT ottocrat_sharedcalendar.sharedid, ottocrat_users.calendarsharedtype FROM ottocrat_sharedcalendar RIGHT JOIN ottocrat_users ON ottocrat_sharedcalendar.userid=ottocrat_users.id and status='Active'
				WHERE ottocrat_users.id IN(SELECT smownerid FROM ottocrat_activity INNER JOIN ottocrat_crmentity ON ottocrat_crmentity.crmid=ottocrat_activity.activityid
