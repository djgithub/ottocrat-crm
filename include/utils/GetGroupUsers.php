...
/** Class to retreive all the ottocrat_users present in a group 
 *
 */
...
class GetGroupUsers { 
	...
	/** to get all the ottocrat_users and ottocrat_groups of the specified group
	 * @params $groupId --> Group Id :: Type Integer
         * @returns the ottocrat_users present in the group in the variable $parent_groups of the class
         * @returns the sub ottocrat_groups present in the group in the variable $group_subgroups of the class
         */
	function getAllUsersInGroup($groupid)
	{
		...
		$query="select * from ottocrat_users2group where groupid=?";
		...
		//Retreiving from the ottocrat_group2role
		$query="select * from ottocrat_group2role where groupid=?";
                $result = $adb->pquery($query, array($groupid));
                $num_rows=$adb->num_rows($result);
                for($i=0;$i<$num_rows;$i++)
                {
		...
		//Retreiving from the ottocrat_group2rs
		$query="select * from ottocrat_group2rs where groupid=?";
                $result = $adb->pquery($query, array($groupid));
                $num_rows=$adb->num_rows($result);
                for($i=0;$i<$num_rows;$i++)
                {
		...
		$query="select * from ottocrat_group2grouprel where groupid=?";
                $result = $adb->pquery($query, array($groupid));
                $num_rows=$adb->num_rows($result);
                for($i=0;$i<$num_rows;$i++)
                {
