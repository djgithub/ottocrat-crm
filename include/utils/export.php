...
if(isset($_SESSION['authenticated_user_id']))
{
	...
	{
		session_destroy();
		header("Location:".Ottocrat_Request:: encryptLink( 'index.php?action=Login&module=Users'));
		exit;
	}
...
?>
	<script type='text/javascript'>
		...
		window.location="<?php Ottocrat_Request:: encryptLink("index.php?module=". vtlib_purify($_REQUEST['module'])."&action=index")?>";
	</script>
...
function export($type){
	...
	if(($search_type == 'withoutsearch' || $search_type == 'includesearch') && $export_data == 'selecteddata'){
		...
		if($type == 'Accounts' && count($idstring) > 0) {
			$query .= ' and ottocrat_account.accountid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Contacts' && count($idstring) > 0) {
			$query .= ' and ottocrat_contactdetails.contactid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Potentials' && count($idstring) > 0) {
			$query .= ' and ottocrat_potential.potentialid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Leads' && count($idstring) > 0) {
			$query .= ' and ottocrat_leaddetails.leadid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Products' && count($idstring) > 0) {
			$query .= ' and ottocrat_products.productid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Documents' && count($idstring) > 0) {
			$query .= ' and ottocrat_notes.notesid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'HelpDesk' && count($idstring) > 0) {
			$query .= ' and ottocrat_troubletickets.ticketid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Vendors' && count($idstring) > 0) {
			$query .= ' and ottocrat_vendor.vendorid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Invoice' && count($idstring) > 0) {
			$query .= ' and ottocrat_invoice.invoiceid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'Quotes' && count($idstring) > 0) {
			$query .= ' and ottocrat_quotes.quoteid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'SalesOrder' && count($idstring) > 0) {
			$query .= ' and ottocrat_salesorder.salesorderid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		} elseif($type == 'PurchaseOrder' && count($idstring) > 0) {
			$query .= ' and ottocrat_purchaseorder.purchaseorderid in ('. generateQuestionMarks($idstring) .')';
			array_push($params, $idstring);
		}
	...
	if(isset($order_by) && $order_by != ''){
		if($order_by == 'smownerid'){
			...
			$query .= ' ORDER BY ottocrat_contactdetails.lastname  '. $sorder;
		}elseif($order_by == 'crmid' && $type == 'HelpDesk'){
			$query .= ' ORDER BY ottocrat_troubletickets.ticketid  '. $sorder;
		}else{
...
class ExportUtils{
	...
	function getInformationArray($module){
		...
		$result = $adb->pquery("select * from ottocrat_field where tabid=?", array($tabid));
