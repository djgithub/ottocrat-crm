 ...
 * Param $ext_prod_arr - existing ottocrat_products
...
function sendPrdStckMail($product_id,$upd_qty,$prod_name,$qtyinstk,$qty,$module)
{
	...
	{	
		...
		$query = "select * from ottocrat_inventorynotification where notificationname=?";
...
function getPrdReOrderLevel($product_id)
{
	...
	global $adb;
	$query1 = "SELECT reorderlevel FROM ottocrat_products WHERE productid = ?";
	$result=$adb->pquery($query1, array($product_id));
 ...
 *	return int   $taxid - taxid corresponding to the Tax type from ottocrat_inventorytaxinfo ottocrat_table
 */
function getTaxId($type)
{
	...
	$res = $adb->pquery("SELECT taxid FROM ottocrat_inventorytaxinfo WHERE taxname=?", array($type));
 ...
 *	@param string $type       - tax type (VAT or Sales or Service)
 *	return int $taxpercentage - taxpercentage corresponding to the Tax type from ottocrat_inventorytaxinfo ottocrat_table
 */
function getTaxPercentage($type)
{
	...
	$res = $adb->pquery("SELECT percentage FROM ottocrat_inventorytaxinfo WHERE taxname = ?", array($type));
...
function getProductTaxPercentage($type,$productid,$default='')
{
	...
	$res = $adb->pquery("SELECT taxpercentage
			FROM ottocrat_inventorytaxinfo
			INNER JOIN ottocrat_producttaxrel
				ON ottocrat_inventorytaxinfo.taxid = ottocrat_producttaxrel.taxid
			WHERE ottocrat_producttaxrel.productid = ?
			AND ottocrat_inventorytaxinfo.taxname = ?", array($productid, $type));
...
function addInventoryHistory($module, $id, $relatedname, $total, $history_fldval)
{
	...
	$history_table_array = Array(
					"PurchaseOrder"=>"ottocrat_postatushistory",
					"SalesOrder"=>"ottocrat_sostatushistory",
					"Quotes"=>"ottocrat_quotestagehistory",
					"Invoice"=>"ottocrat_invoicestatushistory"
				    );
...
function getAllTaxes($available='all', $sh='',$mode='',$id='')
{
	...
	$taxtypes = Array();
	if($sh != '' && $sh == 'sh') {
		$tablename = 'ottocrat_shippingtaxinfo';
		$value_table='ottocrat_inventoryshippingrel';
		if($mode == 'edit' && id != '') {
		...
		$tablename = 'ottocrat_inventorytaxinfo';
		$value_table='ottocrat_inventoryproductrel';
		if($mode == 'edit' && $id != '' ) {
...
function getTaxDetailsForProduct($productid, $available='all')
{
	...
	if($productid != '')
	{
		...
		if($available != 'all' && $available == 'available')
		{
			$where = ' and ottocrat_inventorytaxinfo.deleted=0';
		}
		if($available != 'all' && $available == 'available_associated')
		{
			$query = "SELECT ottocrat_producttaxrel.*, ottocrat_inventorytaxinfo.* FROM ottocrat_inventorytaxinfo left JOIN ottocrat_producttaxrel ON ottocrat_inventorytaxinfo.taxid = ottocrat_producttaxrel.taxid WHERE ottocrat_producttaxrel.productid = ? or ottocrat_inventorytaxinfo.deleted=0 GROUP BY ottocrat_inventorytaxinfo.taxid";
		}
		else
		{
			$query = "SELECT ottocrat_producttaxrel.*, ottocrat_inventorytaxinfo.* FROM ottocrat_inventorytaxinfo INNER JOIN ottocrat_producttaxrel ON ottocrat_inventorytaxinfo.taxid = ottocrat_producttaxrel.taxid WHERE ottocrat_producttaxrel.productid = ? $where";
		}
...
function deleteInventoryProductDetails($focus)
{
	...
	$product_info = $adb->pquery("SELECT productid, quantity, sequence_no, incrementondel from ottocrat_inventoryproductrel WHERE id=?",array($focus->id));
	...
	for($index = 0;$index <$numrows;$index++){
		...
		if($incrementondel){
			...
			$sub_prod_query = $adb->pquery("SELECT productid from ottocrat_inventorysubproductrel WHERE id=? AND sequence_no=?",array($focus->id,$sequence_no));
    ...
    $adb->pquery("delete from ottocrat_inventoryproductrel where id=?", array($focus->id));
    $adb->pquery("delete from ottocrat_inventorysubproductrel where id=?", array($focus->id));
    $adb->pquery("delete from ottocrat_inventoryshippingrel where id=?", array($focus->id));
...
function updateInventoryProductRel($entity) {
	...
	if($statusChanged) {
		if($recordDetails[$statusFieldName] == $statusFieldValue) {
			$adb->pquery("UPDATE ottocrat_inventoryproductrel SET incrementondel=0 WHERE id=?",array($entity_id));
	...
	if($updateInventoryProductRel_deduct_stock) {
		$adb->pquery("UPDATE ottocrat_inventoryproductrel SET incrementondel=1 WHERE id=?",array($entity_id));

		$product_info = $adb->pquery("SELECT productid,sequence_no, quantity from ottocrat_inventoryproductrel WHERE id=?",array($entity_id));
		$numrows = $adb->num_rows($product_info);
		for($index = 0;$index <$numrows;$index++) {
			$productid = $adb->query_result($product_info,$index,'productid');
			$qty = $adb->query_result($product_info,$index,'quantity');
			...
			$sub_prod_query = $adb->pquery("SELECT productid from ottocrat_inventorysubproductrel WHERE id=? AND sequence_no=?",array($entity_id,$sequence_no));
			if($adb->num_rows($sub_prod_query)>0) {
				for($j=0;$j<$adb->num_rows($sub_prod_query);$j++) {
					$sub_prod_id = $adb->query_result($sub_prod_query,$j,"productid");
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		/*else{
			$desc_duery = "select ottocrat_crmentity.description AS product_description from ottocrat_crmentity where ottocrat_crmentity.crmid=?";
		...
		$query ="insert into ottocrat_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description) values(?,?,?,?,?,?,?)";
		...
		if (!empty($sub_prod_str)) {
			...
			for($j=0;$j<count($sub_prod);$j++){
				$query ="insert into ottocrat_inventorysubproductrel(id, sequence_no, productid) values(?,?,?)";
		...
		$updatequery = "update ottocrat_inventoryproductrel set ";
		$updateparams = array();
	...
	//to save the S&H tax details in ottocrat_inventoryshippingrel table
	...
	$sh_query = "insert into ottocrat_inventoryshippingrel($sh_query_fields) values($sh_query_values)";
...
function getInventoryTaxType($module, $id)
{
	...
	$inv_table_array = Array('PurchaseOrder'=>'ottocrat_purchaseorder','SalesOrder'=>'ottocrat_salesorder','Quotes'=>'ottocrat_quotes','Invoice'=>'ottocrat_invoice');
...
function getInventoryCurrencyInfo($module, $id)
{
	...
	$res = $adb->pquery("select currency_id, $inventory_table.conversion_rate as conv_rate, ottocrat_currency_info.* from $inventory_table
						inner join ottocrat_currency_info on $inventory_table.currency_id = ottocrat_currency_info.id
...
function getInventoryProductTaxValue($id, $productid, $taxname)
{
	...
	$res = $adb->pquery("select $taxname from ottocrat_inventoryproductrel where id = ? and productid = ?", array($id, $productid));
...
function getInventorySHTaxPercent($id, $taxname)
{
	...
	$res = $adb->pquery("select $taxname from ottocrat_inventoryshippingrel where id= ?", array($id));
...
function getAllCurrencies($available='available') {
	...
	$sql = "select * from ottocrat_currency_info";
	if ($available != 'all') {
...
function getPriceDetailsForProduct($productid, $unit_price, $available='available', $itemtype='Products')
{
	...
	if($productid != '')
	{
		...
		if ($available == 'available_associated') {
			$query = "select ottocrat_currency_info.*, ottocrat_productcurrencyrel.converted_price, ottocrat_productcurrencyrel.actual_price
					from ottocrat_currency_info
					inner join ottocrat_productcurrencyrel on ottocrat_currency_info.id = ottocrat_productcurrencyrel.currencyid
					where ottocrat_currency_info.currency_status = 'Active' and ottocrat_currency_info.deleted=0
					and ottocrat_productcurrencyrel.productid = ? and ottocrat_currency_info.id != ?";
			$params = array($productid, $product_currency_id);
		} else { // Edit View
			$query = "select ottocrat_currency_info.*, ottocrat_productcurrencyrel.converted_price, ottocrat_productcurrencyrel.actual_price
					from ottocrat_currency_info
					left join ottocrat_productcurrencyrel
					on ottocrat_currency_info.id = ottocrat_productcurrencyrel.currencyid and ottocrat_productcurrencyrel.productid = ?
					where ottocrat_currency_info.currency_status = 'Active' and ottocrat_currency_info.deleted=0";
			$params = array($productid);
		}
	...
	else
	{
		if($available == 'available') { // Create View
			...
			$query = "select ottocrat_currency_info.* from ottocrat_currency_info
					where ottocrat_currency_info.currency_status = 'Active' and ottocrat_currency_info.deleted=0";
			$params = array();
...
function getProductBaseCurrency($productid,$module='Products') {
	global $adb, $log;
	if ($module == 'Services') {
		$sql = "select currency_id from ottocrat_service where serviceid=?";
	} else {
		$sql = "select currency_id from ottocrat_products where productid=?";
	}
	$params = array($productid);
...
function getBaseConversionRateForProduct($productid, $mode='edit', $module='Products') {
	...
	if ($mode == 'edit') {
		if ($module == 'Services') {
			$sql = "select conversion_rate from ottocrat_service inner join ottocrat_currency_info
					on ottocrat_service.currency_id = ottocrat_currency_info.id where ottocrat_service.serviceid=?";
		} else {
			$sql = "select conversion_rate from ottocrat_products inner join ottocrat_currency_info
					on ottocrat_products.currency_id = ottocrat_currency_info.id where ottocrat_products.productid=?";
		}
		$params = array($productid);
	} else {
		$sql = "select conversion_rate from ottocrat_currency_info where id=?";
...
function getPricesForProducts($currencyid, $product_ids, $module='Products') {
	...
	if (count($product_ids) > 0) {
		if ($module == 'Services') {
			$query = "SELECT ottocrat_currency_info.id, ottocrat_currency_info.conversion_rate, " .
					"ottocrat_service.serviceid AS productid, ottocrat_service.unit_price, " .
					"ottocrat_productcurrencyrel.actual_price " .
					"FROM (ottocrat_currency_info, ottocrat_service) " .
					"left join ottocrat_productcurrencyrel on ottocrat_service.serviceid = ottocrat_productcurrencyrel.productid " .
					"and ottocrat_currency_info.id = ottocrat_productcurrencyrel.currencyid " .
					"where ottocrat_service.serviceid in (". generateQuestionMarks($product_ids) .") and ottocrat_currency_info.id = ?";
		} else {
			$query = "SELECT ottocrat_currency_info.id, ottocrat_currency_info.conversion_rate, " .
					"ottocrat_products.productid, ottocrat_products.unit_price, " .
					"ottocrat_productcurrencyrel.actual_price " .
					"FROM (ottocrat_currency_info, ottocrat_products) " .
					"left join ottocrat_productcurrencyrel on ottocrat_products.productid = ottocrat_productcurrencyrel.productid " .
					"and ottocrat_currency_info.id = ottocrat_productcurrencyrel.currencyid " .
					"where ottocrat_products.productid in (". generateQuestionMarks($product_ids) .") and ottocrat_currency_info.id = ?";
		}
...
function getPriceBookCurrency($pricebook_id) {
	global $adb;
	$result = $adb->pquery("select currency_id from ottocrat_pricebook where pricebookid=?", array($pricebook_id));
...
function deductProductsFromStock($recordId) {
	global $adb;
	$adb->pquery("UPDATE ottocrat_inventoryproductrel SET incrementondel=1 WHERE id=?",array($recordId));

	$product_info = $adb->pquery("SELECT productid,sequence_no, quantity from ottocrat_inventoryproductrel WHERE id=?",array($recordId));
	$numrows = $adb->num_rows($product_info);
	for($index = 0;$index <$numrows;$index++) {
		$productid = $adb->query_result($product_info,$index,'productid');
		$qty = $adb->query_result($product_info,$index,'quantity');
		...
		$upd_qty = $qtyinstk-$qty;
		updateProductQty($productid, $upd_qty);
		$sub_prod_query = $adb->pquery("SELECT productid from ottocrat_inventorysubproductrel WHERE id=? AND sequence_no=?",array($recordId,$sequence_no));
		if($adb->num_rows($sub_prod_query)>0) {
			for($j=0;$j<$adb->num_rows($sub_prod_query);$j++) {
				$sub_prod_id = $adb->query_result($sub_prod_query,$j,"productid");
				$sqtyinstk= getPrdQtyInStck($sub_prod_id);
		...
		for ($j = 0; $j < $count; ++$j) {
			...
			if ($subjectRow['productid'] == '' || $subjectRow['quantity'] == '' || $subjectRow['listprice'] == '') {
				...
				foreach ($fieldMapping as $fieldName => $index) {
					if($moduleFields[$fieldName]->getTableName() == 'ottocrat_inventoryproductrel') {
...
function importRecord($obj, $inventoryFieldData, $lineItemDetails) {
	...
	$webserviceObject = OttocratWebserviceObject::fromName($adb, $moduleName);
	$inventoryOperation = new OttocratInventoryOperation($webserviceObject, $obj->user, $adb, $log);
...
function getInventoryFieldsForExport($tableName) {
	...
	$sql .= 'ottocrat_currency_info.currency_name as "Currency" ';
...
function getCurrencyId($fieldValue) {
	...
	$sql = 'SELECT id FROM ottocrat_currency_info WHERE currency_name = ? AND deleted = 0';
...
function getLineItemFields(){
	...
	$sql = 'SELECT DISTINCT columnname FROM ottocrat_field WHERE tablename=?';
	$result = $adb->pquery($sql, array('ottocrat_inventoryproductrel'));
