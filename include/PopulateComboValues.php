...
class PopulateComboValues
{
	...
	/** 
	 * To populate the default combo values for the combo ottocrat_tables
	...
	function insertComboValues($values, $tableName,$picklistid)
	{
		...
		global $adb;
		//inserting the value in the ottocrat_picklistvalues_seq for the getting uniqueID for each picklist values...
		$i=0;
		foreach ($values as $val => $cal)
		{
			$picklist_valueid = getUniquePicklistID();
			$id = $adb->getUniqueID('ottocrat_'.$tableName);
			if($val != '')
			{
				$params = array($id, $val, 1, $picklist_valueid, $i);
				$adb->pquery("insert into ottocrat_$tableName values(?,?,?,?,?)", $params);
			}
			else
			{
			...
			$sql="select roleid from ottocrat_role";
			...
			for($k=0; $k < $numrow; $k ++)
			{
				...
				$adb->pquery("insert into ottocrat_role2picklist values(?,?,?,?)", $params);
			}
	...
	/** 
	 * To populate the combo ottocrat_tables at startup time
	 */
	...
	function create_tables () 
	{
		...
		$comboRes = $adb->query("SELECT distinct fieldname FROM ottocrat_field WHERE uitype IN ('15') OR fieldname = 'salutationtype' and ottocrat_field.presence in (0,2)");
		$noOfCombos = $adb->num_rows($comboRes);
		for($i=0; $i<$noOfCombos; $i++)
		{
			$comTab = $adb->query_result($comboRes, $i, 'fieldname');
			$picklistid = $adb->getUniqueID("ottocrat_picklist");
			$params = array($picklistid, $comTab);
			$picklist_qry = "insert into ottocrat_picklist values(?,?)";
		...
		{
			$adb->pquery("update ottocrat_".$picklistname." set PRESENCE=0", array());
		}
		foreach($noneditable_values as $picklistname => $value)
		{
			$adb->pquery("update ottocrat_$value set PRESENCE=0 where $value=?", array($picklistname));
		}
	...
	function create_nonpicklist_tables ()
	{
		...
		$comboRes = $adb->query("SELECT distinct fieldname FROM ottocrat_field WHERE uitype IN ('16','115','83') AND fieldname NOT IN ('hdnTaxType','email_flag') and ottocrat_field.presence in (0,2)");
		$noOfCombos = $adb->num_rows($comboRes);
		for($i=0; $i<$noOfCombos; $i++)
		{
			$comTab = $adb->query_result($comboRes, $i, 'fieldname');
