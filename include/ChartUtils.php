...
Class ChartUtils {
	...
	public static function generateChartDataFromReports($queryResult, $groupbyField, $fieldDetails='', $reportid='') {
		...
		if ($fieldDetails != '') {
			...
			if ($single == 'D') {
				$dateField = true;
				$query = "SELECT * FROM ottocrat_reportgroupbycolumn WHERE reportid=? ORDER BY sortid";
			...
			if (!empty($groupFieldValue)) {
				...
				if ($dateField) {
					...
					if ($fieldname == 'currency_id' && in_array($module, $inventorymodules)) {
						$tablename = 'ottocrat_currency_info';
					} elseif ($refenceModule == 'DocumentFolders' && $fieldname == 'folderid') {
						$tablename = 'ottocrat_attachmentsfolder';
						$colname = 'foldername';
					} else {
				...
				if ($fieldDetails != '') {
					if ($dateField) {
						...
						if ($module == 'Calendar') {
							$link_val = Ottocrat_Request:: encryptLink("index.php?module=" . $module . "&query=true&action=ListView&" . $advanceSearchCondition);
						}else
							$link_val = Ottocrat_Request:: encryptLink("index.php?module=" . $module . "&query=true&action=index&" . $advanceSearchCondition);
					}
					else {
						...
						if ($module == 'Calendar') {
							$link_val =Ottocrat_Request:: encryptLink( "index.php?module=" . $module . "&action=ListView&search_text=" . $esc_search_str . "&search_field=" . $fieldname . "&searchtype=BasicSearch&query=true&operator=e&viewname=" . $cvid);
						} else {
							$link_val = Ottocrat_Request:: encryptLink("index.php?module=" . $module . "&action=index&search_text=" . $esc_search_str . "&search_field=" . $fieldname . "&searchtype=BasicSearch&query=true&operator=e&viewname=" . $cvid);
						}
					}
