	...
	function handleEvent($eventName, $entityData) {
		...
		if($eventName == 'ottocrat.entity.beforesave') {			
			...
			if ($moduleName == 'HelpDesk') {
				...
				$oldStatus = '';
				if(!empty($ticketId)) {
					$tktResult = $adb->pquery('SELECT status FROM ottocrat_troubletickets WHERE ticketid = ?', array($ticketId));
			...
			if ($moduleName == 'ServiceContracts') {				
				...
				$oldTrackingUnit = '';
				if(!empty($contractId)) {
					$contractResult = $adb->pquery('SELECT tracking_unit FROM ottocrat_servicecontracts WHERE servicecontractsid = ?', array($contractId));
			...
			if ($moduleName == 'HelpDesk' && $_REQUEST['return_module'] != 'ServiceContracts') {
				...
				if($data['ticketstatus'] != $entityData->oldStatus) {
					if(strtolower($data['ticketstatus']) == 'closed' || strtolower($entityData->oldStatus) == 'closed') {
						...
						$contract_tktresult = $adb->pquery("SELECT crmid FROM ottocrat_crmentityrel
																WHERE module = 'ServiceContracts'
																AND relmodule = 'HelpDesk' AND relcrmid = ?
															UNION
																SELECT relcrmid FROM ottocrat_crmentityrel
