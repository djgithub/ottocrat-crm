...
require_once 'modules/com_ottocrat_workflow/VTEventHandler.inc';
...
function CustomerCommentFromPortal($entityData) {
	...
	if($moduleName == 'HelpDesk' && !empty($customerWSId)) {
		...
		$result = $adb->pquery("SELECT email FROM ottocrat_contactdetails WHERE contactid=?", array($customerId[0]));
...
function TicketOwnerComments($entityData) {
	...
	if($moduleName == 'HelpDesk') {
		...
		if(!empty($contactId)) {
			$result = $adb->pquery('SELECT email, emailoptout FROM ottocrat_contactdetails WHERE contactid=?',
										array($contactId));
