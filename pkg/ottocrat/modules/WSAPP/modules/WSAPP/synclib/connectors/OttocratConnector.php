...
require_once 'modules/WSAPP/synclib/models/OttocratModel.php';
...
class WSAPP_OttocratConnector extends WSAPP_BaseConnector {
	...
	public function getSyncState() {
		...
		if($this->getSynchronizeController()->getSyncType() == "app"){
			$result = $this->db->pquery("SELECT * FROM ottocrat_wsapp_sync_state WHERE name=?", array($this->getName()));
		} else {
			$result = $this->db->pquery("SELECT * FROM ottocrat_wsapp_sync_state WHERE name=? and userid=?", array($this->getName(), $this->getSynchronizeController()->user->id));//$this->getSYnchronizeController()->getSyncType();
		}
	...
	function updateSyncState(WSAPP_SyncStateModel $syncStateModel) {
		...
		$query = 'INSERT INTO ottocrat_wsapp_sync_state(stateencodedvalues,name,userid) VALUES (?,?,?)';
		$parameters = array($encodedValues, $this->getName(), $this->getSynchronizeController()->user->id);
		if ($this->isSyncStateExists()) {
			$query		= '';
			$parameters = array();
			if($this->getSynchronizeController()->getSyncType() == "app"){
				$query = 'UPDATE ottocrat_wsapp_sync_state SET stateencodedvalues=? where name=?';
				$parameters = array($encodedValues, $this->getName());
			}else {
				$query = 'UPDATE ottocrat_wsapp_sync_state SET stateencodedvalues=? where name=? and userid=?';
				$parameters = array($encodedValues, $this->getName(), $this->getSynchronizeController()->user->id);
			}
		}
	...
	function isSyncStateExists() {
		$result = null;
		if($this->getSynchronizeController()->getSyncType() == "app"){
			$result = $this->db->pquery('SELECT 1 FROM ottocrat_wsapp_sync_state where name=?', array($this->getName()));
		} else {
			$result = $this->db->pquery('SELECT 1 FROM ottocrat_wsapp_sync_state where name=? and userid=?', array($this->getName(), $this->getSynchronizeController()->user->id));
		}
