...
class SyncServer {
	...
	function appid_with_key($key) {
		...
		$appidresult = $db->pquery("SELECT appid FROM ottocrat_wsapp WHERE appkey=?", array($key));
	...
	function idmap_get_clientmap($appid, $serverids) {
		...
		$result = $db->pquery(sprintf(
			"SELECT serverid, clientid,clientmodifiedtime,servermodifiedtime,id FROM ottocrat_wsapp_recordmapping WHERE appid=? AND serverid IN ('%s')",
    ...
    function idmap_get_clientservermap($appid,$clientids){
        ...
        $result = $db->pquery(sprintf(
        "SELECT serverid, clientid FROM ottocrat_wsapp_recordmapping WHERE appid=? AND clientid IN ('%s')",
    ...
    function idmap_storeRecordsInQueue($syncServerId,$recordDetails,$flag,$appid){
        ...
        $params[] = $flag;
        $params[] = $appid;
        $db->pquery("INSERT INTO ottocrat_wsapp_queuerecords(syncserverid,details,flag,appid) VALUES(?,?,?,?)",array($params));
    }
	
	function checkIdExistInQueue($syncServerId){
		$db = PearDatabase::getInstance();
		$checkQuery = "SELECT syncserverid FROM ottocrat_wsapp_queuerecords WHERE syncserverid=?";
	...
	function markRecordAsDeleteForAllCleints($recordValues){
		...
		$db = PearDatabase::getInstance();
		$query = "SELECT * FROM ottocrat_wsapp_recordmapping WHERE serverid=? and servermodifiedtime < ?";
	...
	function getSyncServerId($clientId,$serverId,$clientAppId){
		$db = PearDatabase::getInstance();
		$syncServerId = NULL;
		$query = "SELECT id FROM ottocrat_wsapp_recordmapping WHERE clientid=? and serverid=? and appid=?";
	...
	function deleteQueueRecords($syncServerIdList){
		$db= PearDatabase::getInstance();
		$deleteQuery = "DELETE FROM ottocrat_wsapp_queuerecords WHERE syncserverid IN (".generateQuestionMarks($syncServerIdList).")";
	...
	function idmap_put($appid, $serverid, $clientid,$clientModifiedTime,$serverModifiedTime,$serverAppId,$mode="save") {
        ...
        if($mode == $this->create)
            ...
            $result = $db->pquery("SELECT * FROM ottocrat_wsapp_recordmapping WHERE appid=? and serverid=? and clientid=?",array($appid,$serverid,$clientid));
            if($db->num_rows($result)<=0)
                $this->idmap_create($appid, $serverid, $clientid, $clientModifiedTime,$serverModifiedTime,$serverAppId);
            else
                $this->idmap_update($appid, $serverid, $clientid, $clientModifiedTime,$serverModifiedTime,$serverAppId);
    ...
    function idmap_create($appid, $serverid, $clientid,$clientModifiedTime,$serverModifiedTime,$serverAppId){
        $db = PearDatabase::getInstance();
        $db->pquery("INSERT INTO ottocrat_wsapp_recordmapping (appid, serverid, clientid,clientmodifiedtime,servermodifiedtime,serverappid) VALUES (?,?,?,?,?,?)",
    ...
    function idmap_update($appid, $serverid, $clientid,$clientModifiedTime,$serverModifiedTime,$serverAppId){
        $db = PearDatabase::getInstance();
        $db->pquery("UPDATE ottocrat_wsapp_recordmapping SET clientmodifiedtime=?,servermodifiedtime=? WHERE appid=? and serverid=? and clientid=? and serverappid=?",
    ...
    function idmap_delete($appid, $serverid, $clientid,$serverAppId){
        $db = PearDatabase::getInstance();
        $db->pquery("DELETE FROM ottocrat_wsapp_recordmapping WHERE appid=? and serverid=? and clientid=? and serverappid=?",
    ...
    function idmap_updateMapDetails($appid,$clientid,$clientModifiedTime,$serverModifiedTime){
        $db = PearDatabase::getInstance();
        $db->pquery("UPDATE ottocrat_wsapp_recordmapping SET clientmodifiedtime=?,servermodifiedtime=? WHERE appid=? and clientid=?",
                            array($clientModifiedTime,$serverModifiedTime,$appid, $clientid));
    }

	function getDestinationHandleDetails(){
		return wsapp_getHandler('ottocratCRM');
	}
	...
	function register($name,$type) {
		...
		$db = PearDatabase::getInstance();
		$uid = uniqid();
        $db->pquery("INSERT INTO ottocrat_wsapp (name, appkey,type) VALUES(?,?,?)", array($name, $uid,$type));
	...
	function deregister($name, $key, $user) {
		if (!empty($name) && !empty($key)) {
            $db = PearDatabase::getInstance();
			$uid = uniqid();
			$db->pquery("DELETE FROM ottocrat_wsapp_recordmapping WHERE appid=(SELECT appid FROM ottocrat_wsapp WHERE name=? AND appkey=?)",
				array($name, $key));
			$db->pquery("DELETE FROM ottocrat_wsapp WHERE name=? AND appkey=?", array($name, $key));
		}
	...
	function put($key, $element, $user) {
        ...
        //hardcoded since the destination handler will be ottocratCRM
        $serverKey = wsapp_getAppKey("ottocratCRM");
        ...
        foreach ($records as $record) {
			...
			$lookupResult = $db->pquery("SELECT serverid,clientmodifiedtime FROM ottocrat_wsapp_recordmapping WHERE appid=? AND clientid=?", array($appid, $clientRecordId));
    ...
    function getQueueDeleteRecord($appId){
        $db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT * FROM ottocrat_wsapp_queuerecords
                                INNER JOIN ottocrat_wsapp_recordmapping ON (ottocrat_wsapp_recordmapping.id=ottocrat_wsapp_queuerecords.syncserverid)
                                WHERE ottocrat_wsapp_recordmapping.appid=? ",array($appId));
        $serverIds = array();
  ...
  */
    function idmap_get_serverId($clientid,$appId){
        ...
        $result = $db->pquery("SELECT serverid, clientid FROM ottocrat_wsapp_recordmapping WHERE  clientid = ? and appid=?", array($clientid,$appId));
        $mapping = array();
        if($db->num_rows($result)){
            while($row = $db->fetch_array($result)){
                return $row['serverid'];
  ...
  */
    function idmap_get_clientId($serverid,$appId){
        ...
        $result = $db->pquery("SELECT serverid, clientid FROM ottocrat_wsapp_recordmapping WHERE  serverid = ? and appid=?", array($serverid,$appId));
        $mapping = array();
        if($db->num_rows($result)){
            while($row = $db->fetch_array($result)){
                return $row['clientid'];
