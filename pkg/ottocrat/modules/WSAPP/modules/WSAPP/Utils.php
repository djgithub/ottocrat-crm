...
function wsapp_getHandler($appType){
    ...
    $result = $db->pquery("SELECT * FROM ottocrat_wsapp_handlerdetails WHERE type=?",array($appType));
...
function wsapp_getApplicationName($key){
    ...
    $result = $db->pquery("SELECT name from ottocrat_wsapp WHERE appkey=?",array($key));
    $name = false;
...
function wsapp_getRecordEntityNameIds($entityNames,$modules,$user){
    ...
    foreach($modules as $moduleName){
        ...
        $query = "SELECT ".$meta->getObectIndexColumn()." as id,$nameFields as entityname FROM ".$meta->getEntityBaseTable()." as moduleentity INNER JOIN ottocrat_crmentity as crmentity WHERE $nameFields IN(".generateQuestionMarks($entityNames).") AND crmentity.deleted=0 AND crmentity.crmid = moduleentity.".$meta->getObectIndexColumn()."";
...
function wsapp_checkIfRecordsAssignToUser($recordsIds,$userIds){
    ...
    $db = PearDatabase::getInstance();
    $query = "SELECT * FROM ottocrat_crmentity where crmid IN (".generateQuestionMarks($recordsIds).") and smownerid in (".generateQuestionMarks($userIds).")";
    $params = array();
...
function wsapp_getAppKey($appName){
    $db = PearDatabase::getInstance();
    $query = "SELECT * FROM ottocrat_wsapp WHERE name=?";
...
function wsapp_getAppSyncType($appKey){
	$db = PearDatabase::getInstance();
    $query = "SELECT type FROM ottocrat_wsapp WHERE appkey=?";
    $params = array($appKey);
    $result = $db->pquery($query,$params);
    $syncType="";
    if($db->num_rows($result)>0){
...
function wsapp_RegisterHandler($type,$handlerClass,$handlerPath){
	$db = PearDatabase::getInstance();
	$query = "SELECT 1 FROM ottocrat_wsapp_handlerdetails where type=?";
	$result = $db->pquery($query,array($type));
	if($db->num_rows($result)>0){
		$saveQuery = "UPDATE ottocrat_wsapp_handlerdetails SET handlerclass=?,handlerpath=? WHERE type=?";
		$parameters = array($handlerClass,$handlerPath,$type);
	} else{
		$saveQuery = "INSERT INTO ottocrat_wsapp_handlerdetails VALUES(?,?,?)";
