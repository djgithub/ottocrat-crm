...
class OutlookSyncServer extends SyncServer{
    ...
    function getDestinationHandleDetails(){
        return array('handlerclass' => 'OutlookOttocratCRMHandler',
            'handlerpath' => 'modules/WSAPP/Handlers/OutlookOttocratCRMHandler.php');
	}
     ...
     function put($key, $element, $user) {
        ...
        foreach ($records as $record) {
            ...
            if (!empty($record['crmid'])) {
                ...
                $lookupResult = $db->pquery("SELECT crmid,modifiedtime FROM ottocrat_crmentity WHERE crmid=?", array($crmid[1]));
