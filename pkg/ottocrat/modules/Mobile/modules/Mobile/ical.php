...
class Mobile_ICAL {
	...
	function generate() {
		...
		$properties = array();
		$properties['prodid'] = '-//ottocrat/Mobile/NONSGML 1.0//EN';
		...
		$query = "SELECT " . implode(',', $fieldnames) . " FROM ottocrat_activity
			INNER JOIN ottocrat_crmentity ON
			(ottocrat_activity.activityid=ottocrat_crmentity.crmid 	AND ottocrat_crmentity.deleted = 0 AND ottocrat_crmentity.smownerid = ?)
			LEFT JOIN ottocrat_activity_reminder ON ottocrat_activity_reminder.activity_id=ottocrat_activity.activityid
