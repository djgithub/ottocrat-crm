...
class Mobile_WS_Utils {
	...
	static function getOttocratVersion() {
	...
	static function getVersion() {
		global $adb;
		$versionResult = $adb->pquery("SELECT version FROM ottocrat_tab WHERE name='Mobile'", array());
	...
	static function getEntityModuleWSId($moduleName) {
		...
		if (!isset(self::$moduleWSIdCache[$moduleName])) {
			global $adb;
			$result = $adb->pquery("SELECT id FROM ottocrat_ws_entity WHERE name=?", array($moduleName));
	...
	static function getEntityModuleWSIds($ignoreNonModule = true) {
		...
		$result = false;
		if($ignoreNonModule) {
			$result = $adb->pquery("SELECT id, name FROM ottocrat_ws_entity WHERE ismodule=1", array());
		} else {
			$result = $adb->pquery("SELECT id, name FROM ottocrat_ws_entity", array());
		}
	...
	static function getEntityFieldnames($module) {
		global $adb;
		$result = $adb->pquery("SELECT fieldname FROM ottocrat_entityname WHERE modulename=?", array($module));
		$fieldnames = array();
		if($result && $adb->num_rows($result)) {
	...
	static function getModuleColumnTableByFieldNames($module, $fieldnames) {
		global $adb;
		$result = $adb->pquery("SELECT fieldname,columnname,tablename FROM ottocrat_field WHERE tabid=? AND fieldname IN (".
	...
	static function detectModulenameFromRecordId($wsrecordid) {
		...
		$result = $adb->pquery("SELECT name FROM ottocrat_ws_entity WHERE id=?", array($idComponents[0]));
		if($result && $adb->num_rows($result)) {
	...
	static function detectFieldnamesToResolve($module) {
		...
		$result = $adb->pquery(
			"SELECT DISTINCT fieldname FROM ottocrat_field WHERE uitype IN(". 
	...
	static function gatherModuleFieldGroupInfo($module) {
		...
		$result = $adb->pquery(
			"SELECT fieldname, fieldlabel, blocklabel, uitype FROM ottocrat_field INNER JOIN
			ottocrat_blocks ON ottocrat_blocks.tabid=ottocrat_field.tabid AND ottocrat_blocks.blockid=ottocrat_field.block 
			WHERE ottocrat_field.tabid=? AND ottocrat_field.presence != 1 ORDER BY ottocrat_blocks.sequence, ottocrat_field.sequence", array(getTabid($module))
		);
	...
	static function documentFoldersInfo() {
		global $adb;
		$folders = $adb->pquery("SELECT folderid, foldername FROM ottocrat_attachmentsfolder", array());
		$folderOptions = array();
	...
	static function getRelatedFunctionHandler($sourceModule, $targetModule) {
		global $adb;
		$relationResult = $adb->pquery("SELECT name FROM ottocrat_relatedlists WHERE tabid=? and related_tabid=? and presence=0", array(getTabid($sourceModule), getTabid($targetModule)));
		$functionName = false;
	...
	static function querySecurityFromSuffix($module, $current_user) {
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 
			&& $defaultOrgSharingPermission[$tabid] == 3) {
				...
				$querySuffix .= " AND (ottocrat_crmentity.smownerid in($current_user->id) OR ottocrat_crmentity.smownerid IN 
					(
					...
					if(sizeof($current_user_groups) > 0) {
						$querySuffix .= " ottocrat_groups.groupid IN (". implode(",", $current_user_groups) .") OR ";
					}
					$querySuffix .= " ottocrat_groups.groupid IN 
						(
