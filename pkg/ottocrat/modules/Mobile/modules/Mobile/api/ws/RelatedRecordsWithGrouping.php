...
class Mobile_WS_RelatedRecordsWithGrouping extends Mobile_WS_QueryWithGrouping {
	...
	function process(Mobile_API_Request $request) {
		...
		if ($functionHandler) {
			...
			$querySEtype = "ottocrat_crmentity.setype as setype";
			if ($relatedmodule == 'Calendar') {
				$querySEtype = "ottocrat_activity.activitytype as setype";
			}
			
			$query = sprintf("SELECT ottocrat_crmentity.crmid, $querySEtype %s", substr($query, stripos($query, 'FROM')));
