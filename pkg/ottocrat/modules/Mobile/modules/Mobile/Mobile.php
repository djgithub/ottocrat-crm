...
class Mobile {
	...
	static function alert_lookup($handlerPath, $handlerClass) {
		global $adb;
		$check = $adb->pquery("SELECT id FROM ottocrat_mobile_alerts WHERE handler_path=? and handler_class=?", array($handlerPath, $handlerClass));
	...
	static function alert_register($handlerPath, $handlerClass) {
		...
		if (self::alert_lookup($handlerPath, $handlerClass) === false) {
			Ottocrat_Utils::Log("Registered alert {$handlerClass} [$handlerPath]");
			$adb->pquery("INSERT INTO ottocrat_mobile_alerts (handler_path, handler_class, deleted) VALUES(?,?,?)", array($handlerPath, $handlerClass, 0));
		}
	}
	static function alert_deregister($handlerPath, $handlerClass) {
		global $adb;
		Ottocrat_Utils::Log("De-registered alert {$handlerClass} [$handlerPath]");
		$adb->pquery("DELETE FROM ottocrat_mobile_alerts WHERE handler_path=? AND handler_class=?", array($handlerPath, $handlerClass));
	}
	static function alert_markdeleted($handlerPath, $handlerClass, $flag) {
		global $adb;
		$adb->pquery("UPDATE ottocrat_mobile_alerts SET deleted=? WHERE handler_path=? AND handler_class=?", array($flag, $handlerPath, $handlerClass));
	}
...
class Mobile_WS_Operation {
	...
	function register() {
		global $adb;
		$checkresult = $adb->pquery("SELECT 1 FROM ottocrat_ws_operation WHERE name = ?", array($this->opName));
		if($adb->num_rows($checkresult)) {
			return;
		}
		
		Ottocrat_Utils::Log("Enabling webservice operation {$this->opName}", true);
