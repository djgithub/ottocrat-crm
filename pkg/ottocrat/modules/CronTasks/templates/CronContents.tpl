...
<table width="100%" cellpadding="5" cellspacing="0" class="listTable" >
	...
	{if $smarty.foreach.cronlist.first neq true}
		<td  align="center" class="listTableRow"><a href="javascript:move_module('{$elements.id}','Up');" ><img src="{'arrow_up.png'|@ottocrat_imageurl:$THEME}" style="width:16px;height:16px;" border="0" /></a>
	{/if}
	{if $smarty.foreach.cronlist.last eq true}
		<img src="{'blank.gif'|@ottocrat_imageurl:$THEME}" style="width:16px;height:16px;" border="0" />
	{/if}	
	{if $smarty.foreach.cronlist.first eq true}
		<td align="center" class="listTableRow"><img src="{'blank.gif'|@ottocrat_imageurl:$THEME}" style="width:16px;height:16px;" border="0" />
			<a href="javascript:move_module('{$elements.id}','Down');" ><img src="{'arrow_down.png'|@ottocrat_imageurl:$THEME}" style="width:16px;height:16px;" border="0" /></a></td>
	{/if}
	
	{if $smarty.foreach.cronlist.last neq true && $smarty.foreach.cronlist.first neq true}
		<a href="javascript:move_module('{$elements.id}','Down');" ><img src="{'arrow_down.png'|@ottocrat_imageurl:$THEME}" style="width:16px;height:16px;" border="0" /></a></td>
	{/if}
        <td class="listTableRow small" align="center" ><img onClick="fnvshobj(this,'editdiv');fetchEditCron('{$elements.id}');" style="cursor:pointer;" src="{'editfield.gif'|@ottocrat_imageurl:$THEME}" title="{$APP.LBL_EDIT}"></td>
        </tr>
	{/foreach}
	</table>
