...
class ModTracker_Record_Model extends Ottocrat_Record_Model {
	...
	public static function getUpdates($parentRecordId, $pagingModel) {
		...
		$listQuery = "SELECT * FROM ottocrat_modtracker_basic WHERE crmid = ? ".
	...
	function setParent($id, $moduleName) {
		$this->parent = Ottocrat_Record_Model::getInstanceById($id, $moduleName);
	}
	...
	function getFieldInstances() {
		...
		if($this->isCreate() || $this->isUpdate()) {
			$result = $db->pquery('SELECT * FROM ottocrat_modtracker_detail WHERE id = ?', array($id));
			$rows = $db->num_rows($result);
			for($i=0; $i<$rows; $i++) {
				...
				$fieldModel = Ottocrat_Field_Model::getInstance($row['fieldname'], $this->getParent()->getModule());
	...
	function getRelationInstance() {
		...
		if($this->isRelationLink() || $this->isRelationUnLink()) {
			$result = $db->pquery('SELECT * FROM ottocrat_modtracker_relations WHERE id = ?', array($id));
	...
	public function getTotalRecordCount($recordId) {
    	$db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT COUNT(*) AS count FROM ottocrat_modtracker_basic WHERE crmid = ?", array($recordId));
