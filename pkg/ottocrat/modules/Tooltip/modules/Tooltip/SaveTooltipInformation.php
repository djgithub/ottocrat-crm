...
function SaveTooltipInformation($fieldid, $sequence, $view=1){
	...
	if(empty($fieldid)){
		...
		if(!empty($_REQUEST['checkedFields'])){
			...
			//add to ottocrat_quickview table
			foreach($checkedFields as $checkedField){
				$query = "insert into ottocrat_quickview (fieldid,related_fieldid,sequence,currentview) values (?,?,?,?)";
...
function deleteOldInfo($fieldid, $view=1){
	...
	$query = "delete from ottocrat_quickview where fieldid = ? and currentview = ?";
...
function getFieldLabels($checkedFields){
	...
	$sql = "select * from ottocrat_field where fieldid in (".generateQuestionMarks($checkedFields).") and ottocrat_field.presence in (0,2)";
