...
function getFieldList($module_name, $field_name = "") {
	...
	$query = "select * from ottocrat_field where tabid = ?";
	$params = array($tabid);
	if (!empty ($field_name)) {
		$query .= " and fieldname not like ?";
		$params = array($tabid,$field_name);
	}
	$query.= " and columnname not like 'imagename' and uitype not in (61, 122) and ottocrat_field.presence in (0,2)";
...
function moduleList() {
	global $adb;
	$sql = "select distinct ottocrat_field.tabid,name from ottocrat_field inner join ottocrat_tab on ottocrat_field.tabid=ottocrat_tab.tabid where name not in ('Calendar','Events','Users')";
...
function tooltip_exists($fieldid, $related_fieldid) {
	global $adb;
	$query = "select * from ottocrat_quickview where fieldid=? and related_fieldid=?";
...
function getToolTipText($view,$fieldname,$module,$value){
	...
	$quickview = 'select fieldname,fieldlabel from ottocrat_quickview inner join ottocrat_field on ottocrat_quickview.related_fieldid=ottocrat_field.fieldid where ottocrat_quickview.fieldid = ? and currentview= ? and ottocrat_field.presence in (0,2) order by ottocrat_quickview.sequence';
...
function ToolTipExists($fieldname,$tabid){
	if(empty($fieldname) || empty($tabid)){
		...
		global $adb;
		$sql = "select fieldid from ottocrat_field where tabid = ? and fieldname = ? and ottocrat_field.presence in (0,2)";
		$result = $adb->pquery($sql,array($tabid,$fieldname));
		$count = $adb->num_rows($result);
		if($count > 0){
			$fieldid = $adb->query_result($result,0,'fieldid');
		
			$sql = "select * from ottocrat_quickview where fieldid = ?";
...
function QuickViewFieldList($module){
	...
	$query = "select * from ottocrat_field where tabid = ? and columnname not like 'imagename' and uitype not in (61, 122) and ottocrat_field.presence in (0,2)";
