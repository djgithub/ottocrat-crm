...
class Google_Oauth2_Connector {
    ...
    protected $table_name = 'ottocrat_google_oauth2';
    ...
    public function __construct($module,$userId=false) {
        ...
        $this->redirect_uri = rtrim($site_URL, '/') . '/'.Ottocrat_Request:: encryptLink('index.php?module=Google&view=List&operation=sync&sourcemodule=' . $this->source_module . '&service=' . $this->service_name);
    ...
    public function getState($source) {
        global $site_URL;
        $callbackUri = rtrim($site_URL, '/') . '/'.Ottocrat_Request:: encryptLink('index.php?module=Google&view=List&operation=sync&sourcemodule=' . $this->source_module . '&service=' . $source);
    ...
    protected function decryptAuthCode($cipherText) {
        $publicKey = OttocratConfig::getOD('OAUTHREDIR_PUBK');
