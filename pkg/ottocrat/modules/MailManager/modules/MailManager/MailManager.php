...
class MailManager {
	...
	static function updateMailAssociation($mailuid, $emailid, $crmid) {
		global $adb;
		$adb->pquery("INSERT INTO ottocrat_mailmanager_mailrel (mailuid, emailid, crmid) VALUES (?,?,?)", array($mailuid, $emailid, $crmid));
	}

	static function lookupMailInOttocrat($searchTerm, $user) {
		...
		foreach($referenceModules as $referenceModule) {
			...
			if(!empty($searchFieldList) && !empty($referenceModuleEmailFields)) {
                ...
                if($referenceModule == 'Users' && !is_admin($user)){
                    ...
                    global $adb;
                    $where .= " AND ottocrat_users.status='Active'";
                    $query = "select $searchFieldListString,id from ottocrat_users $where";
	...
	static function lookupMailAssociation($mailuid) {
		...
		$result = $adb->pquery(
			"SELECT ottocrat_mailmanager_mailrel.* FROM ottocrat_mailmanager_mailrel INNER JOIN
			ottocrat_crmentity ON ottocrat_crmentity.crmid=ottocrat_mailmanager_mailrel.crmid AND ottocrat_crmentity.deleted=0
			AND ottocrat_mailmanager_mailrel.mailuid=? LIMIT 1", array(decode_html($mailuid)));
