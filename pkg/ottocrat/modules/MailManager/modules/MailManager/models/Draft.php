...
class MailManager_Draft_Model {
		...
		for($i=0; $i<count($draftMails); $i++) {
			foreach($draftMails[$i] as $fieldname=>$fieldvalue) {
				if($fieldname == "saved_toid" || $fieldname == "ccmail" || $fieldname == "bccmail") {
                    ...
                    if(!empty($fieldvalue)) {
						$value = Ottocrat_Date_UIType::getDisplayDateValue($fieldvalue);
						$draftMails[$i][$fieldname] = $value;
					}
                } elseif($fieldname == "id") {
	...
	public function getAttachmentDetails($crmid) {
		...
		$documentRes = $db->pquery("SELECT * FROM ottocrat_senotesrel
									...
									WHERE ottocrat_crmentity.deleted = 0", array($crmid));
	...
	public function saveEmailParentRel($emailId, $parentIds) {
		...
		if(!empty($emailId)) {
			$db->pquery("delete from ottocrat_seactivityrel where activityid=?",array($emailId)); //remove all previous relation
		}
		for ($i=0; $i<(count($myids)); $i++) {
			$realid = explode("@",$myids[$i]);
			if(!empty($realid[0]) && !empty($emailId)) {
				// this is needed as we might save the mail in draft mode earlier
				$result = $db->pquery("SELECT * FROM ottocrat_seactivityrel WHERE crmid=? AND activityid=?",array($realid[0], $emailId));
				if(!$db->num_rows($result)) {
					$db->pquery('INSERT INTO ottocrat_seactivityrel(crmid, activityid) VALUES(?,?)',array($realid[0], $emailId));
				}
			}
		}
	}
	...
	public function getFromEmailAddress() {
		...
		$fromEmail = false;
		if (Ottocrat_Version::check('5.2.0', '>=')) {
			$smtpFromResult = $db->pquery('SELECT from_email_field FROM ottocrat_systems WHERE server_type=?', array('email'));
	...
	public function getParentFromEmails($to_string) {
		...
		if (!empty($to_string)) {
			...
			foreach($toArray as $to) {
				$relatedtos = MailManager::lookupMailInOttocrat(trim($to), $currentUserModel);
	...
	public function saveEmailDocumentRel($emailId, $documentId) {
		...
		if(!empty($emailId) && !empty($documentId)) {
			$db->pquery("INSERT INTO ottocrat_senotesrel(crmid, notesid) VALUES(?,?)",
	...
	public function saveAttachmentRel($crmid, $attachId) {
		...
		if(!empty($crmid) && !empty($attachId)) {
			$db->pquery("INSERT INTO ottocrat_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)",
	...
	public function deleteAttachment($request) {
		...
		if(!empty($docid) && !empty($emailid)) {
			$db->pquery("DELETE FROM ottocrat_senotesrel WHERE crmid = ? AND notesid = ?", array($emailid, $docid));
			return true;
		}
		return false;
	}
