...
class MailManager_Relation_View extends MailManager_Abstract_View {
	...
	public function process(Ottocrat_Request $request) {
		...
		if ('find' == $this->getOperationArg($request)) {
			...
			if (!class_exists($quickCreateviewClassName)) {
				$quickCreateviewClassName = 'Ottocrat_QuickCreateAjax_View';
			}
			...
			$recordModel = Ottocrat_Record_Model::getCleanInstance($linkModule);
			...
			switch ($linkModule) {
				case 'Calendar' :   $activityType = $recordModel->get('activitytype');
									...
									$startTime = Ottocrat_Time_UIType::getTimeValueWithSeconds($request->get('time_start'));
									$startDateTime = Ottocrat_Datetime_UIType::getDBDateTimeValue($request->get('date_start')." ".$startTime);
									...
									$endDate = Ottocrat_Date_UIType::getDBInsertedValue($request->get('due_date'));
									if ($activityType != 'Task') {
										$endTime = Ottocrat_Time_UIType::getTimeValueWithSeconds($request->get('time_end'));
										$endDateTime = Ottocrat_Datetime_UIType::getDBDateTimeValue($request->get('due_date')." ".$endTime);
			...
			try {
				...
				if ($linkModule === 'Calendar') {
					...
					//Start Date and Time values
					$startTime = Ottocrat_Time_UIType::getTimeValueWithSeconds($request->get('followup_time_start'));
					$startDateTime = Ottocrat_Datetime_UIType::getDBDateTimeValue($request->get('followup_date_start') . " " . $startTime);
					list($startDate, $startTime) = explode(' ', $startDateTime);
	...
	public function setParentForHelpDesk($parent, $from) {
		...
		if(empty($parent)) {
			if(!empty($from)) {
				$parentInfo = MailManager::lookupMailInOttocrat($from[0], $currentUserModel);
