...
<form name="{$RECORD_MODEL->getName()}" action="{$ACTION_PATH}"
  method="post" accept-charset="utf-8" enctype="multipart/form-data">
	...
	<input type="hidden" name="name" value="{$RECORD_MODEL->getName()}" />
        <input type="hidden" name="OTTOCRAT_RECAPTCHA_PUBLIC_KEY" value="{$OTTOCRAT_RECAPTCHA_PUBLIC_KEY}" />
...
<option value="{$PICKLIST_VALUE}" {if in_array(Ottocrat_Util_Helper::toSafeHTML($PICKLIST_VALUE), $FIELD_VALUE_LIST)} selected {/if}>{vtranslate($PICKLIST_VALUE, $MODULE)}</option>
                                                {/foreach}
</select>
