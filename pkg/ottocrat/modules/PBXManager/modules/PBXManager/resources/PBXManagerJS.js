...
var Ottocrat_PBXManager_Js = {
    /**
	 * Function registers PBX for popups
	 */
    registerPBXCall : function() {
        Ottocrat_PBXManager_Js.requestPBXgetCalls();
    },
    ...
    registerPBXOutboundCall : function(number,record) {
        Ottocrat_PBXManager_Js.makeOutboundCall(number,record);
    },
    /**
            ...
            if(data.success && data.result) {
                for(i=0; i< data.result.length; i++) {
                    ...
                    if(jQuery('#pbxcall_'+record.pbxmanagerid+'').size()== 0 )
                        Ottocrat_PBXManager_Js.showPBXIncomingCallPopup(record);
                    else
                        Ottocrat_PBXManager_Js.updatePBXIncomingCallPopup(record);
                    }
                }
        });
        Ottocrat_PBXManager_Js.removeCompletedCallPopup();
    },
    ...
    showPBXIncomingCallPopup : function(record) {
            ...
            if(record.user) {
                if(record.user != record.current_user_id) {
                    Ottocrat_PBXManager_Js.removeCallPopup(record.pbxmanagerid);
                }
            }
                
            // To check if it is new or existing contact
            Ottocrat_PBXManager_Js.checkIfRelatedModuleRecordExist(record);
            ...
            jQuery('#pbxcontactsave_'+record.pbxmanagerid+'').bind('click', function(e) {
              ...
              Ottocrat_PBXManager_Js.createRecord(e, record);
            ...
            AppConnector.request(url).then(function(data){
                if(data.result){
                    if(data.result!='in-progress' && data.result!='ringing'){
                        Ottocrat_PBXManager_Js.removeCallPopup(callid);
                    }
                }
            });    
        }
    ...
    makeOutboundCall : function(number, record){
     ...
     registerEvents : function(){
        ...
        AppConnector.request(url).then(function(data){
            if(data.result) {
                Ottocrat_PBXManager_Js.registerPBXCall();
                setInterval("Ottocrat_PBXManager_Js.registerPBXCall()", 3000);
            }
        });
    }
...
jQuery(document).ready(function() {
    Ottocrat_PBXManager_Js.registerEvents();
});
