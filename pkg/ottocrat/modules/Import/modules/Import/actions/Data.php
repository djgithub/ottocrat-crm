...
require_once 'vtlib/Ottocrat/Utils.php';
require_once 'modules/Ottocrat/CRMEntity.php';
require_once 'include/QueryGenerator/QueryGenerator.php';
require_once 'vtlib/Ottocrat/Mailer.php';
require_once 'include/events/include.inc';


class Import_Data_Action extends Ottocrat_Action_Controller {
	...
	public function getDefaultFieldValues($moduleMeta) {
		...
		if ($className != 'OttocratLineItemMeta') {
	...
	public function importData() {
		...
		$moduleModel = Ottocrat_Module_Model::getInstance($this->module);
        // pre fetch the fields and premmisions of module
        Ottocrat_Field_Model::getAllForModule($moduleModel);
        if($this->user->is_admin == 'off'){
            Ottocrat_Field_Model::preFetchModuleFieldPermission($moduleModel->getId());
        }
	...
	public function createRecords() {
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
            ...
            if($createRecord || $mergeType == Import_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == Import_Utils_Helper::$AUTO_MERGE_OVERWRITE){
                ...
                $label = trim($label);
                $adb->pquery('UPDATE ottocrat_crmentity SET label=? WHERE crmid=?', array($label, $recordId));
            }
        ...
        if($this->entityData) {
            ...
            $entity->triggerEvent('ottocrat.batchevent.save',$this->entityData);
        }
	...
	public function transformForImport($fieldData, $moduleMeta, $fillDefault=true, $checkMandatoryFieldValues=true) {
		...
		foreach ($fieldData as $fieldName => $fieldValue) {
			...
			if ($fieldInstance->getFieldDataType() == 'owner') {
				...
				$olderCacheEnable = Ottocrat_Cache::$cacheEnable;
				Ottocrat_Cache::$cacheEnable = false;
				...
				if (!in_array($picklistValueInLowerCase, $allPicklistValuesInLowerCase)) {
					$moduleObject = Ottocrat_Module::getInstance($moduleMeta->getEntityName());
					$fieldObject = Ottocrat_Field::getInstance($fieldName, $moduleObject);
				...
				Ottocrat_Cache::$cacheEnable = $olderCacheEnable;
	...
	public static function runScheduledImport() {
		...
		foreach ($scheduledImports as $scheduledId => $importDataController) {
			...
			$emailSubject = 'ottocrat CRM - Scheduled Import Report for '.$importDataController->module;
			...
			$emailData = 'ottocrat CRM has just completed your import process. <br/><br/>' .
							$importResult . '<br/><br/>'.
			...
			$ottocratMailer->to = array( array($userEmail, $userName));
			$ottocratMailer->Subject = $emailSubject;
			$ottocratMailer->Body    = $emailData;
			$ottocratMailer->Send();

			$importDataController->finishImport();
		}
		Ottocrat_Mailer::dispatchQueue(null);
	}
    ...
    public static function getImportDetails($user){
        ...
        if($result) {
			...
			for($i=0; $i<$noOfRows; ++$i) {
                ...
                $record= new Ottocrat_Base_Model();
