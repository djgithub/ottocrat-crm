...
:initiate
rem if true means , ottocrat crm mysql is being used
...
:StartApacheService
echo ""
echo "making an attempt to kill any existing ottocratcrm service"
echo ""
bin\apache -k stop -n ottocratcrm504
...
echo ""
echo ""
echo "Installing  ottocratcrm504 apache service after sleeping for 10 seconds"
echo ""
%SLEEP_STR% -n 10 127.0.0.1>nul
bin\apache -k install -n ottocratcrm504 -f conf\httpd.conf
echo ""
echo "Starting  ottocratcrm504 apache service"
echo ""
bin\apache -n ottocratcrm504 -k start
IF ERRORLEVEL 1 goto stopservice
goto checkmysql
...
echo ""
echo ""
echo "Apache in the location %apache_dir% is not running. Start Apache and then start ottocrat crm"
echo ""
echo ""
...
:checkdatabase
echo ""
echo "check to see if ottocratcrm504 database already exists"
echo ""
mysql --port=%mysql_port% --user=%mysql_username% --password=%mysql_password% -e "show databases like 'ottocratcrm504'" | "%WINDIR%\system32\find.exe" "ottocratcrm504" > NUL
IF ERRORLEVEL 1 goto dbnotexists
echo ""
ECHO  "ottocratcrm504 database exists"
echo ""
goto end
...
:dbnotexists
echo ""
ECHO "ottocratcrm504 database does not exist"
echo ""
echo %cd%
echo ""
echo "Proceeding to create database ottocratcrm504 and populate the same"
echo ""
mysql --user=%mysql_username% --password=%mysql_password% --port=%mysql_port% -e "create database if not exists ottocratcrm504"
echo ""
echo "ottocratcrm504 database created"
echo ""
goto end
