...
if [ $apache_bundled == 'false' ];then
	...
	if [ $apache_stat -ne 0 ];then
		echo ""
		echo "**************************************************************************************"
		echo "Apache Server is not running. Start the Apache server and then start the ottocratCRM application"
		echo "**************************************************************************************"
		echo ""
		exit
	else
...
if [ $exit_status -ne 0 ]; then
	if [ $mysql_bundled == 'false' ];then
		echo ""
		echo "**************************************************************************************"
		echo "Mysql server in the directory $mysql_dir is not running at port $mysql_port. Start the mysql server and then start ottocratCRM application"
		echo "**************************************************************************************"
		echo ""
...
echo ""
echo "Checking if the ottocratcrm540 database already exists"
echo ""
echo "select 1" | ./bin/mysql --user=$mysql_username --password=$mysql_password  --port=$mysql_port --socket=$mysql_socket -D ottocratcrm540 >/dev/null
if [ $? -ne 0 ]; then
	echo ""
	echo "Database ottocratcrm540 does not exist. Creating database ottocratcrm540"
	echo ""
	./bin/mysql --user=$mysql_username --password=$mysql_password  --port=$mysql_port --socket=$mysql_socket -e "create database if not exists ottocratcrm540"
fi

host=`hostname`
echo "*****************************************************************************************************"
if [ $apache_bundled == 'false' ];then
	echo "Please access the product at http://${host}:<apache port>/ottocratCRM5/ottocratCRM"
else
