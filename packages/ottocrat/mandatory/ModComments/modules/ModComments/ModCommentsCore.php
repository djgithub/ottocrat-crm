...
class ModCommentsCore extends CRMEntity {
	...
	var $table_name = 'ottocrat_modcomments';
	var $table_index= 'modcommentsid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('ottocrat_modcommentscf', 'modcommentsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('ottocrat_crmentity', 'ottocrat_modcomments', 'ottocrat_modcommentscf');
	...
	var $tab_name_index = Array(
		...
		'ottocrat_modcomments' => 'modcommentsid',
		'ottocrat_modcommentscf'=>'modcommentsid');
	...
	function generateReportsSecQuery($module,$secmodule,$queryplanner){
		...
		$matrix->setDependency('ottocrat_crmentityModComments',array('ottocrat_groupsModComments','ottocrat_usersModComments', 'ottocrat_contactdetailsRelModComments', 'ottocrat_modcommentsRelModComments'));
		$matrix->setDependency('ottocrat_modcomments', array('ottocrat_crmentityModComments'));

		if (!$queryplanner->requireTable("ottocrat_modcomments",$matrix)){
			return '';
		}

		$query = $this->getRelationQuery($module,$secmodule,"ottocrat_modcomments","modcommentsid", $queryplanner);

		if ($queryplanner->requireTable("ottocrat_crmentityModComments",$matrix)){
			$query .= " left join ottocrat_crmentity as ottocrat_crmentityModComments on ottocrat_crmentityModComments.crmid=ottocrat_modcomments.modcommentsid and ottocrat_crmentityModComments.deleted=0";
		}
		if ($queryplanner->requireTable("ottocrat_groupsModComments")){
			$query .= " left join ottocrat_groups ottocrat_groupsModComments on ottocrat_groupsModComments.groupid = ottocrat_crmentityModComments.smownerid";
		}
		if ($queryplanner->requireTable("ottocrat_usersModComments")){
			$query .= " left join ottocrat_users as ottocrat_usersModComments on ottocrat_usersModComments.id = ottocrat_crmentityModComments.smownerid";
		}
		if ($queryplanner->requireTable("ottocrat_contactdetailsRelModComments")){
			$query .= " left join ottocrat_contactdetails as ottocrat_contactdetailsRelModComments on ottocrat_contactdetailsRelModComments.contactid = ottocrat_crmentityModComments.crmid";
		}
		if ($queryplanner->requireTable("ottocrat_modcommentsRelModComments")){
			$query .= " left join ottocrat_modcomments as ottocrat_modcommentsRelModComments on ottocrat_modcommentsRelModComments.modcommentsid = ottocrat_crmentityModComments.crmid";
		}
		return $query;
	}
