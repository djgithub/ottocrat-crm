...
$query="select ottocrat_contactdetails.contactid,ottocrat_contactdetails.email,ottocrat_contactdetails.firstname,ottocrat_contactdetails.lastname,contactid  from ottocrat_customerdetails inner join ottocrat_crmentity on ottocrat_crmentity.crmid=ottocrat_customerdetails.customerid inner join ottocrat_contactdetails on ottocrat_contactdetails.contactid=ottocrat_customerdetails.customerid  where ottocrat_crmentity.deleted=0 and support_end_date=DATE_ADD(now(), INTERVAL 1 WEEK)";
...
{
	global $adb;
	$query='select ottocrat_emailtemplates.subject,ottocrat_emailtemplates.body from ottocrat_notificationscheduler inner join ottocrat_emailtemplates on ottocrat_emailtemplates.templateid=ottocrat_notificationscheduler.notificationbody where schedulednotificationid=7';
	$result = $adb->pquery($query, array());
...
{
	global $adb;
	$query='select ottocrat_emailtemplates.subject,ottocrat_emailtemplates.body from ottocrat_notificationscheduler inner join ottocrat_emailtemplates on ottocrat_emailtemplates.templateid=ottocrat_notificationscheduler.notificationbody where schedulednotificationid=6';
	$result = $adb->pquery($query, array());
	$body=$adb->query_result($result,0,'body');
	$body=getMergedDescription($body,$id,"Contacts");
	$body=getMergedDescription($body,$id,"Users");
